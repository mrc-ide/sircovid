...
public:
  ...
  struct shared_type {
    ...
    int dim_hfr_age_vacc_class;
    int dim_hfr_age_vacc_class_1;
    int dim_hfr_age_vacc_class_2;
    int dim_hfr_strain;
    int dim_ifr_age_vacc_class;
    int dim_ifr_age_vacc_class_1;
    int dim_ifr_age_vacc_class_2;
    int dim_ifr_strain;
    int dim_ihr_age_vacc_class;
    int dim_ihr_age_vacc_class_1;
    int dim_ihr_age_vacc_class_2;
    int dim_ihr_strain;
    int dim_index_dose;
    ...
    int offset_variable_hfr_age_vacc_class;
    int offset_variable_hfr_strain;
    int offset_variable_ifr_age_vacc_class;
    int offset_variable_ifr_strain;
    int offset_variable_ihr_age_vacc_class;
  ...
  std::vector<real_type> initial(size_t step) {
    ...
    std::copy(shared->initial_cum_infections_per_strain.begin(), shared->initial_cum_infections_per_strain.end(), state.begin() + 87);
    ...
    std::copy(shared->initial_ifr_age_vacc_class.begin(), shared->initial_ifr_age_vacc_class.end(), state.begin() + shared->offset_variable_ifr_age_vacc_class);
    std::copy(shared->initial_ihr_age_vacc_class.begin(), shared->initial_ihr_age_vacc_class.end(), state.begin() + shared->offset_variable_ihr_age_vacc_class);
    std::copy(shared->initial_hfr_age_vacc_class.begin(), shared->initial_hfr_age_vacc_class.end(), state.begin() + shared->offset_variable_hfr_age_vacc_class);
  ...
  void update(size_t step, const real_type * state, rng_state_type& rng_state, real_type * state_next) {
    ...
    const real_type * cum_infections_per_strain = state + 87;
    ...
    for (int i = 1; i <= shared->dim_cum_infections_per_strain; ++i) {
      state_next[87 + i - 1] = cum_infections_per_strain[i - 1] + internal.delta_infections_per_strain[i - 1];
    }
...
dust::pars_type<lancelot> dust_pars<lancelot>(cpp11::list user) {
  ...
  shared->dim_hfr_age_vacc_class_1 = shared->n_groups;
  shared->dim_hfr_age_vacc_class_2 = shared->n_vacc_classes;
  shared->dim_ifr_age_vacc_class_1 = shared->n_groups;
  shared->dim_ifr_age_vacc_class_2 = shared->n_vacc_classes;
  shared->dim_ihr_age_vacc_class_1 = shared->n_groups;
  shared->dim_ihr_age_vacc_class_2 = shared->n_vacc_classes;
  ...
  shared->dim_hfr_age_vacc_class = shared->dim_hfr_age_vacc_class_1 * shared->dim_hfr_age_vacc_class_2;
  shared->dim_hfr_strain = shared->n_real_strains;
  shared->dim_ifr_age_vacc_class = shared->dim_ifr_age_vacc_class_1 * shared->dim_ifr_age_vacc_class_2;
  shared->dim_ifr_strain = shared->n_real_strains;
  shared->dim_ihr_age_vacc_class = shared->dim_ihr_age_vacc_class_1 * shared->dim_ihr_age_vacc_class_2;
  ...
  shared->offset_variable_D_hosp = shared->dim_cum_infections_per_strain + shared->dim_infections_inc_per_strain + 87;
  shared->offset_variable_D_non_hosp = shared->dim_D_hosp + shared->dim_cum_infections_per_strain + shared->dim_infections_inc_per_strain + 87;
  shared->offset_variable_N_tot = shared->dim_D_hosp + shared->dim_D_non_hosp + shared->dim_cum_admit_by_age + shared->dim_cum_infections_per_strain + shared->dim_infections_inc_per_strain + 87;
  shared->offset_variable_cum_admit_by_age = shared->dim_D_hosp + shared->dim_D_non_hosp + shared->dim_cum_infections_per_strain + shared->dim_infections_inc_per_strain + 87;
  shared->offset_variable_infections_inc_per_strain = shared->dim_cum_infections_per_strain + 87;
  shared->offset_variable_prob_strain = shared->dim_D_hosp + shared->dim_D_non_hosp + shared->dim_N_tot + shared->dim_cum_admit_by_age + shared->dim_cum_infections_per_strain + shared->dim_infections_inc_per_strain + 87;
  ...
  shared->initial_hfr_age_vacc_class = std::vector<real_type>(shared->dim_hfr_age_vacc_class);
  shared->initial_hfr_strain = std::vector<real_type>(shared->dim_hfr_strain);
  shared->initial_ifr_age_vacc_class = std::vector<real_type>(shared->dim_ifr_age_vacc_class);
  shared->initial_ifr_strain = std::vector<real_type>(shared->dim_ifr_strain);
  shared->initial_ihr_age_vacc_class = std::vector<real_type>(shared->dim_ihr_age_vacc_class);
  ...
  for (int i = 1; i <= shared->dim_hfr_age_vacc_class_1; ++i) {
    for (int j = 1; j <= shared->dim_hfr_age_vacc_class_2; ++j) {
      shared->initial_hfr_age_vacc_class[i - 1 + shared->dim_hfr_age_vacc_class_1 * (j - 1)] = 0;
    }
  }
  for (int i = 1; i <= shared->dim_hfr_strain; ++i) {
  ...
  for (int i = 1; i <= shared->dim_ifr_age_vacc_class_1; ++i) {
    for (int j = 1; j <= shared->dim_ifr_age_vacc_class_2; ++j) {
      shared->initial_ifr_age_vacc_class[i - 1 + shared->dim_ifr_age_vacc_class_1 * (j - 1)] = 0;
    }
  }
  for (int i = 1; i <= shared->dim_ifr_strain; ++i) {
  ...
  for (int i = 1; i <= shared->dim_ihr_age_vacc_class_1; ++i) {
    for (int j = 1; j <= shared->dim_ihr_age_vacc_class_2; ++j) {
      shared->initial_ihr_age_vacc_class[i - 1 + shared->dim_ihr_age_vacc_class_1 * (j - 1)] = 0;
    }
  }
  for (int i = 1; i <= shared->dim_ihr_strain; ++i) {
  ...
  shared->offset_variable_effective_susceptible = shared->dim_D_hosp + shared->dim_D_non_hosp + shared->dim_N_tot + shared->dim_cum_admit_by_age + shared->dim_cum_infections_per_strain + shared->dim_infections_inc_per_strain + shared->dim_prob_strain + 87;
  shared->offset_variable_hfr_age_vacc_class = shared->dim_D + shared->dim_D_hosp + shared->dim_D_non_hosp + shared->dim_N_tot + shared->dim_S + shared->dim_cum_admit_by_age + shared->dim_cum_infections_disag + shared->dim_cum_infections_per_strain + shared->dim_cum_n_E_vacc_skip + shared->dim_cum_n_E_vaccinated + shared->dim_cum_n_I_A_vacc_skip + shared->dim_cum_n_I_A_vaccinated + shared->dim_cum_n_I_P_vacc_skip + shared->dim_cum_n_I_P_vaccinated + shared->dim_cum_n_R_vacc_skip + shared->dim_cum_n_R_vaccinated + shared->dim_cum_n_S_vacc_skip + shared->dim_cum_n_S_vaccinated + shared->dim_cum_n_vacc_skip + shared->dim_cum_n_vaccinated + shared->dim_diagnoses_admitted + shared->dim_effective_susceptible + shared->dim_hfr_strain + shared->dim_ifr_age_vacc_class + shared->dim_ifr_strain + shared->dim_ihr_age_vacc_class + shared->dim_ihr_strain + shared->dim_infections_inc_per_strain + shared->dim_prob_strain + shared->dim_tmp_vaccine_n_candidates + shared->dim_tmp_vaccine_probability + shared->dim_vaccine_missed_doses + 87;
  shared->offset_variable_hfr_strain = shared->dim_D_hosp + shared->dim_D_non_hosp + shared->dim_N_tot + shared->dim_cum_admit_by_age + shared->dim_cum_infections_per_strain + shared->dim_effective_susceptible + shared->dim_ifr_strain + shared->dim_ihr_strain + shared->dim_infections_inc_per_strain + shared->dim_prob_strain + 87;
  shared->offset_variable_ifr_age_vacc_class = shared->dim_D + shared->dim_D_hosp + shared->dim_D_non_hosp + shared->dim_N_tot + shared->dim_S + shared->dim_cum_admit_by_age + shared->dim_cum_infections_disag + shared->dim_cum_infections_per_strain + shared->dim_cum_n_E_vacc_skip + shared->dim_cum_n_E_vaccinated + shared->dim_cum_n_I_A_vacc_skip + shared->dim_cum_n_I_A_vaccinated + shared->dim_cum_n_I_P_vacc_skip + shared->dim_cum_n_I_P_vaccinated + shared->dim_cum_n_R_vacc_skip + shared->dim_cum_n_R_vaccinated + shared->dim_cum_n_S_vacc_skip + shared->dim_cum_n_S_vaccinated + shared->dim_cum_n_vacc_skip + shared->dim_cum_n_vaccinated + shared->dim_diagnoses_admitted + shared->dim_effective_susceptible + shared->dim_hfr_strain + shared->dim_ifr_strain + shared->dim_ihr_strain + shared->dim_infections_inc_per_strain + shared->dim_prob_strain + shared->dim_tmp_vaccine_n_candidates + shared->dim_tmp_vaccine_probability + shared->dim_vaccine_missed_doses + 87;
  shared->offset_variable_ifr_strain = shared->dim_D_hosp + shared->dim_D_non_hosp + shared->dim_N_tot + shared->dim_cum_admit_by_age + shared->dim_cum_infections_per_strain + shared->dim_effective_susceptible + shared->dim_infections_inc_per_strain + shared->dim_prob_strain + 87;
  shared->offset_variable_ihr_age_vacc_class = shared->dim_D + shared->dim_D_hosp + shared->dim_D_non_hosp + shared->dim_N_tot + shared->dim_S + shared->dim_cum_admit_by_age + shared->dim_cum_infections_disag + shared->dim_cum_infections_per_strain + shared->dim_cum_n_E_vacc_skip + shared->dim_cum_n_E_vaccinated + shared->dim_cum_n_I_A_vacc_skip + shared->dim_cum_n_I_A_vaccinated + shared->dim_cum_n_I_P_vacc_skip + shared->dim_cum_n_I_P_vaccinated + shared->dim_cum_n_R_vacc_skip + shared->dim_cum_n_R_vaccinated + shared->dim_cum_n_S_vacc_skip + shared->dim_cum_n_S_vaccinated + shared->dim_cum_n_vacc_skip + shared->dim_cum_n_vaccinated + shared->dim_diagnoses_admitted + shared->dim_effective_susceptible + shared->dim_hfr_strain + shared->dim_ifr_age_vacc_class + shared->dim_ifr_strain + shared->dim_ihr_strain + shared->dim_infections_inc_per_strain + shared->dim_prob_strain + shared->dim_tmp_vaccine_n_candidates + shared->dim_tmp_vaccine_probability + shared->dim_vaccine_missed_doses + 87;
  shared->offset_variable_ihr_strain = shared->dim_D_hosp + shared->dim_D_non_hosp + shared->dim_N_tot + shared->dim_cum_admit_by_age + shared->dim_cum_infections_per_strain + shared->dim_effective_susceptible + shared->dim_ifr_strain + shared->dim_infections_inc_per_strain + shared->dim_prob_strain + 87;
...
cpp11::sexp dust_info<lancelot>(const dust::pars_type<lancelot>& pars) {
  ...
  dim[87] = cpp11::writable::integers({shared->dim_cum_infections_per_strain});
  dim[88] = cpp11::writable::integers({shared->dim_infections_inc_per_strain});
  dim[89] = cpp11::writable::integers({shared->dim_D_hosp});
  dim[90] = cpp11::writable::integers({shared->dim_D_non_hosp});
  dim[91] = cpp11::writable::integers({shared->dim_cum_admit_by_age});
  dim[92] = cpp11::writable::integers({shared->dim_N_tot});
  dim[93] = cpp11::writable::integers({shared->dim_prob_strain});
  dim[94] = cpp11::writable::integers({shared->dim_effective_susceptible});
  dim[95] = cpp11::writable::integers({shared->dim_ifr_strain});
  dim[96] = cpp11::writable::integers({shared->dim_ihr_strain});
  dim[97] = cpp11::writable::integers({shared->dim_hfr_strain});
  dim[98] = cpp11::writable::integers({shared->dim_cum_n_S_vaccinated_1, shared->dim_cum_n_S_vaccinated_2});
  dim[99] = cpp11::writable::integers({shared->dim_cum_n_E_vaccinated_1, shared->dim_cum_n_E_vaccinated_2});
  dim[100] = cpp11::writable::integers({shared->dim_cum_n_I_A_vaccinated_1, shared->dim_cum_n_I_A_vaccinated_2});
  dim[101] = cpp11::writable::integers({shared->dim_cum_n_I_P_vaccinated_1, shared->dim_cum_n_I_P_vaccinated_2});
  dim[102] = cpp11::writable::integers({shared->dim_cum_n_R_vaccinated_1, shared->dim_cum_n_R_vaccinated_2});
  dim[103] = cpp11::writable::integers({shared->dim_cum_n_vaccinated_1, shared->dim_cum_n_vaccinated_2});
  dim[104] = cpp11::writable::integers({shared->dim_cum_n_S_vacc_skip_1, shared->dim_cum_n_S_vacc_skip_2});
  dim[105] = cpp11::writable::integers({shared->dim_cum_n_E_vacc_skip_1, shared->dim_cum_n_E_vacc_skip_2});
  dim[106] = cpp11::writable::integers({shared->dim_cum_n_I_A_vacc_skip_1, shared->dim_cum_n_I_A_vacc_skip_2});
  dim[107] = cpp11::writable::integers({shared->dim_cum_n_I_P_vacc_skip_1, shared->dim_cum_n_I_P_vacc_skip_2});
  dim[108] = cpp11::writable::integers({shared->dim_cum_n_R_vacc_skip_1, shared->dim_cum_n_R_vacc_skip_2});
  dim[109] = cpp11::writable::integers({shared->dim_cum_n_vacc_skip_1, shared->dim_cum_n_vacc_skip_2});
  dim[110] = cpp11::writable::integers({shared->dim_diagnoses_admitted_1, shared->dim_diagnoses_admitted_2});
  dim[111] = cpp11::writable::integers({shared->dim_cum_infections_disag_1, shared->dim_cum_infections_disag_2});
  dim[112] = cpp11::writable::integers({shared->dim_D_1, shared->dim_D_2});
  dim[113] = cpp11::writable::integers({shared->dim_S_1, shared->dim_S_2});
  dim[114] = cpp11::writable::integers({shared->dim_vaccine_missed_doses_1, shared->dim_vaccine_missed_doses_2});
  dim[115] = cpp11::writable::integers({shared->dim_tmp_vaccine_n_candidates_1, shared->dim_tmp_vaccine_n_candidates_2});
  dim[116] = cpp11::writable::integers({shared->dim_tmp_vaccine_probability_1, shared->dim_tmp_vaccine_probability_2});
  dim[117] = cpp11::writable::integers({shared->dim_ifr_age_vacc_class_1, shared->dim_ifr_age_vacc_class_2});
  dim[118] = cpp11::writable::integers({shared->dim_ihr_age_vacc_class_1, shared->dim_ihr_age_vacc_class_2});
  dim[119] = cpp11::writable::integers({shared->dim_hfr_age_vacc_class_1, shared->dim_hfr_age_vacc_class_2});
  ...
  index[87] = integer_sequence(88, shared->dim_cum_infections_per_strain);
  index[88] = integer_sequence(shared->offset_variable_infections_inc_per_strain + 1, shared->dim_infections_inc_per_strain);
  index[89] = integer_sequence(shared->offset_variable_D_hosp + 1, shared->dim_D_hosp);
  index[90] = integer_sequence(shared->offset_variable_D_non_hosp + 1, shared->dim_D_non_hosp);
  index[91] = integer_sequence(shared->offset_variable_cum_admit_by_age + 1, shared->dim_cum_admit_by_age);
  index[92] = integer_sequence(shared->offset_variable_N_tot + 1, shared->dim_N_tot);
  index[93] = integer_sequence(shared->offset_variable_prob_strain + 1, shared->dim_prob_strain);
  index[94] = integer_sequence(shared->offset_variable_effective_susceptible + 1, shared->dim_effective_susceptible);
  index[95] = integer_sequence(shared->offset_variable_ifr_strain + 1, shared->dim_ifr_strain);
  index[96] = integer_sequence(shared->offset_variable_ihr_strain + 1, shared->dim_ihr_strain);
  index[97] = integer_sequence(shared->offset_variable_hfr_strain + 1, shared->dim_hfr_strain);
  index[98] = integer_sequence(shared->offset_variable_cum_n_S_vaccinated + 1, shared->dim_cum_n_S_vaccinated);
  index[99] = integer_sequence(shared->offset_variable_cum_n_E_vaccinated + 1, shared->dim_cum_n_E_vaccinated);
  index[100] = integer_sequence(shared->offset_variable_cum_n_I_A_vaccinated + 1, shared->dim_cum_n_I_A_vaccinated);
  index[101] = integer_sequence(shared->offset_variable_cum_n_I_P_vaccinated + 1, shared->dim_cum_n_I_P_vaccinated);
  index[102] = integer_sequence(shared->offset_variable_cum_n_R_vaccinated + 1, shared->dim_cum_n_R_vaccinated);
  index[103] = integer_sequence(shared->offset_variable_cum_n_vaccinated + 1, shared->dim_cum_n_vaccinated);
  index[104] = integer_sequence(shared->offset_variable_cum_n_S_vacc_skip + 1, shared->dim_cum_n_S_vacc_skip);
  index[105] = integer_sequence(shared->offset_variable_cum_n_E_vacc_skip + 1, shared->dim_cum_n_E_vacc_skip);
  index[106] = integer_sequence(shared->offset_variable_cum_n_I_A_vacc_skip + 1, shared->dim_cum_n_I_A_vacc_skip);
  index[107] = integer_sequence(shared->offset_variable_cum_n_I_P_vacc_skip + 1, shared->dim_cum_n_I_P_vacc_skip);
  index[108] = integer_sequence(shared->offset_variable_cum_n_R_vacc_skip + 1, shared->dim_cum_n_R_vacc_skip);
  index[109] = integer_sequence(shared->offset_variable_cum_n_vacc_skip + 1, shared->dim_cum_n_vacc_skip);
  index[110] = integer_sequence(shared->offset_variable_diagnoses_admitted + 1, shared->dim_diagnoses_admitted);
  index[111] = integer_sequence(shared->offset_variable_cum_infections_disag + 1, shared->dim_cum_infections_disag);
  index[112] = integer_sequence(shared->offset_variable_D + 1, shared->dim_D);
  index[113] = integer_sequence(shared->offset_variable_S + 1, shared->dim_S);
  index[114] = integer_sequence(shared->offset_variable_vaccine_missed_doses + 1, shared->dim_vaccine_missed_doses);
  index[115] = integer_sequence(shared->offset_variable_tmp_vaccine_n_candidates + 1, shared->dim_tmp_vaccine_n_candidates);
  index[116] = integer_sequence(shared->offset_variable_tmp_vaccine_probability + 1, shared->dim_tmp_vaccine_probability);
  index[117] = integer_sequence(shared->offset_variable_ifr_age_vacc_class + 1, shared->dim_ifr_age_vacc_class);
  index[118] = integer_sequence(shared->offset_variable_ihr_age_vacc_class + 1, shared->dim_ihr_age_vacc_class);
  index[119] = integer_sequence(shared->offset_variable_hfr_age_vacc_class + 1, shared->dim_hfr_age_vacc_class);
