...
template <typename real_type, typename container>
HOSTDEVICE real_type odin_sum4(const container x, int from_i, int to_i, int from_j, int to_j, int from_k, int to_k, int from_l, int to_l, int dim_x_1, int dim_x_12, int dim_x_123);
...
compare(const typename T::real_type * state,
  ...
  const real_type model_sero_pos_1 = state[21];
  const real_type model_sero_pos_2 = state[22];
  const real_type model_sympt_cases = state[33];
  const real_type model_sympt_cases_over25 = state[35];
  const real_type model_sympt_cases_under15 = state[37];
  const real_type model_sympt_cases_15_24 = state[38];
  const real_type model_sympt_cases_25_49 = state[39];
  const real_type model_sympt_cases_50_64 = state[40];
  const real_type model_sympt_cases_65_79 = state[41];
  const real_type model_sympt_cases_80_plus = state[42];
  const real_type model_sympt_cases_non_variant =
    state[34];
  const real_type model_sympt_cases_non_variant_over25 =
    state[36];
  const real_type model_react_pos = state[43];
...
public:
  ...
  HOST void update(size_t step, const real_type * state, rng_state_type& rng_state, real_type * state_next) {
    ...
    const real_type * cum_infections_per_strain = state + 44;
    ...
    const real_type cum_sympt_cases = state[23];
    const real_type cum_sympt_cases_non_variant = state[24];
    const real_type cum_sympt_cases_over25 = state[25];
    const real_type cum_sympt_cases_non_variant_over25 = state[26];
    const real_type cum_sympt_cases_under15 = state[27];
    const real_type cum_sympt_cases_15_24 = state[28];
    const real_type cum_sympt_cases_25_49 = state[29];
    const real_type cum_sympt_cases_50_64 = state[30];
    const real_type cum_sympt_cases_65_79 = state[31];
    const real_type cum_sympt_cases_80_plus = state[32];
    const real_type sympt_cases_inc = state[33];
    const real_type sympt_cases_non_variant_inc = state[34];
    const real_type sympt_cases_over25_inc = state[35];
    const real_type sympt_cases_non_variant_over25_inc = state[36];
    const real_type sympt_cases_under15_inc = state[37];
    const real_type sympt_cases_15_24_inc = state[38];
    const real_type sympt_cases_25_49_inc = state[39];
    const real_type sympt_cases_50_64_inc = state[40];
    const real_type sympt_cases_65_79_inc = state[41];
    const real_type sympt_cases_80_plus_inc = state[42];
    ...
    state_next[21] = odin_sum4<real_type>(internal.new_T_sero_pos_1.data(), 3, 13, 0, shared->dim_new_T_sero_pos_1_2, 0, shared->dim_new_T_sero_pos_1_3, 0, shared->dim_new_T_sero_pos_1_4, shared->dim_new_T_sero_pos_1_1, shared->dim_new_T_sero_pos_1_12, shared->dim_new_T_sero_pos_1_123);
    state_next[22] = odin_sum4<real_type>(internal.new_T_sero_pos_2.data(), 3, 13, 0, shared->dim_new_T_sero_pos_2_2, 0, shared->dim_new_T_sero_pos_2_3, 0, shared->dim_new_T_sero_pos_2_4, shared->dim_new_T_sero_pos_2_1, shared->dim_new_T_sero_pos_2_12, shared->dim_new_T_sero_pos_2_123);
    ...
    state_next[38] = ((fmodr<real_type>(step, shared->steps_per_day) == 0 ? new_sympt_cases_15_24 : sympt_cases_15_24_inc + new_sympt_cases_15_24));
    state_next[39] = ((fmodr<real_type>(step, shared->steps_per_day) == 0 ? new_sympt_cases_25_49 : sympt_cases_25_49_inc + new_sympt_cases_25_49));
    state_next[40] = ((fmodr<real_type>(step, shared->steps_per_day) == 0 ? new_sympt_cases_50_64 : sympt_cases_50_64_inc + new_sympt_cases_50_64));
    state_next[41] = ((fmodr<real_type>(step, shared->steps_per_day) == 0 ? new_sympt_cases_65_79 : sympt_cases_65_79_inc + new_sympt_cases_65_79));
    state_next[42] = ((fmodr<real_type>(step, shared->steps_per_day) == 0 ? new_sympt_cases_80_plus : sympt_cases_80_plus_inc + new_sympt_cases_80_plus));
    state_next[33] = ((fmodr<real_type>(step, shared->steps_per_day) == 0 ? new_sympt_cases : sympt_cases_inc + new_sympt_cases));
    state_next[34] = ((fmodr<real_type>(step, shared->steps_per_day) == 0 ? new_sympt_cases_non_variant : sympt_cases_non_variant_inc + new_sympt_cases_non_variant));
    state_next[36] = ((fmodr<real_type>(step, shared->steps_per_day) == 0 ? new_sympt_cases_non_variant_over25 : sympt_cases_non_variant_over25_inc + new_sympt_cases_non_variant_over25));
    state_next[35] = ((fmodr<real_type>(step, shared->steps_per_day) == 0 ? new_sympt_cases_over25 : sympt_cases_over25_inc + new_sympt_cases_over25));
    state_next[37] = ((fmodr<real_type>(step, shared->steps_per_day) == 0 ? new_sympt_cases_under15 : sympt_cases_under15_inc + new_sympt_cases_under15));
    ...
    state_next[43] = odin_sum4<real_type>(internal.new_T_PCR_pos.data(), 1, 18, 0, shared->dim_new_T_PCR_pos_2, 0, shared->dim_new_T_PCR_pos_3, 0, shared->dim_new_T_PCR_pos_4, shared->dim_new_T_PCR_pos_1, shared->dim_new_T_PCR_pos_12, shared->dim_new_T_PCR_pos_123);
...
cpp11::sexp dust_lancelot_device_info() {
  return dust::cuda::device_info<lancelot::real_type>();
}
