...
[[cpp11::register]]
SEXP dust_cpu_lancelot_alloc(cpp11::list r_pars, bool pars_multi, size_t step,
...
[[cpp11::register]]
SEXP dust_cpu_lancelot_update_state(SEXP ptr, SEXP r_pars, SEXP r_state,
...
[[cpp11::register]]
SEXP dust_cpu_lancelot_filter(SEXP ptr, SEXP step_end,
...
typename T::real_type
  compare(const typename T::real_type * state,
    ...
    const real_type model_icu = state[47];
    const real_type model_general = state[48];
    const real_type model_hosp = model_icu + model_general;
    const real_type model_deaths_carehomes = state[62];
    const real_type model_deaths_comm = state[60];
    const real_type model_deaths_hosp = state[65];
    const real_type model_deaths_hosp_0_49 = state[66];
    const real_type model_deaths_hosp_50_54 = state[67];
    const real_type model_deaths_hosp_55_59 = state[68];
    const real_type model_deaths_hosp_60_64 = state[69];
    const real_type model_deaths_hosp_65_69 = state[70];
    const real_type model_deaths_hosp_70_74 = state[71];
    ...
    const real_type model_sympt_cases_over25 = state[88];
    const real_type model_sympt_cases_under15 = state[90];
    const real_type model_sympt_cases_15_24 = state[91];
    const real_type model_sympt_cases_25_49 = state[92];
    const real_type model_sympt_cases_50_64 = state[93];
    const real_type model_sympt_cases_65_79 = state[94];
    const real_type model_sympt_cases_80_plus = state[95];
    const real_type model_sympt_cases_non_variant =
      state[87];
    const real_type model_sympt_cases_non_variant_over25 =
      state[89];
    const real_type model_react_pos = state[96];
...
public:
  ...
  struct shared_type {
    ...
    real_type initial_admit_0_9_conf_inc;
    real_type initial_admit_10_19_conf_inc;
    real_type initial_admit_20_29_conf_inc;
    real_type initial_admit_30_39_conf_inc;
    real_type initial_admit_40_49_conf_inc;
    real_type initial_admit_50_59_conf_inc;
    real_type initial_admit_60_69_conf_inc;
    real_type initial_admit_70_79_conf_inc;
    real_type initial_admit_80_plus_conf_inc;
    real_type initial_admit_conf_inc;
    real_type initial_beta_out;
    real_type initial_cum_admit_0_9_conf;
    real_type initial_cum_admit_10_19_conf;
    real_type initial_cum_admit_20_29_conf;
    real_type initial_cum_admit_30_39_conf;
    real_type initial_cum_admit_40_49_conf;
    real_type initial_cum_admit_50_59_conf;
    real_type initial_cum_admit_60_69_conf;
    real_type initial_cum_admit_70_79_conf;
    real_type initial_cum_admit_80_plus_conf;
    ...
    std::vector<real_type> initial_cum_infections_per_strain;
    ...
    real_type initial_cum_new_0_9_conf;
    real_type initial_cum_new_10_19_conf;
    real_type initial_cum_new_20_29_conf;
    real_type initial_cum_new_30_39_conf;
    real_type initial_cum_new_40_49_conf;
    real_type initial_cum_new_50_59_conf;
    real_type initial_cum_new_60_69_conf;
    real_type initial_cum_new_70_79_conf;
    real_type initial_cum_new_80_plus_conf;
    ...
    real_type initial_new_0_9_conf_inc;
    real_type initial_new_10_19_conf_inc;
    real_type initial_new_20_29_conf_inc;
    real_type initial_new_30_39_conf_inc;
    real_type initial_new_40_49_conf_inc;
    real_type initial_new_50_59_conf_inc;
    real_type initial_new_60_69_conf_inc;
    real_type initial_new_70_79_conf_inc;
    real_type initial_new_80_plus_conf_inc;
    ...
    int offset_variable_cum_infections_per_strain;
  ...
  struct internal_type {
    ...
    std::vector<real_type> aux_E;
    std::vector<real_type> aux_G_D;
  ...
  size_t size() {
    return shared->dim_D + shared->dim_D_hosp + shared->dim_D_non_hosp + shared->dim_E + shared->dim_G_D + shared->dim_H_D_conf + shared->dim_H_D_unconf + shared->dim_H_R_conf + shared->dim_H_R_unconf + shared->dim_ICU_D_conf + shared->dim_ICU_D_unconf + shared->dim_ICU_W_D_conf + shared->dim_ICU_W_D_unconf + shared->dim_ICU_W_R_conf + shared->dim_ICU_W_R_unconf + shared->dim_ICU_pre_conf + shared->dim_ICU_pre_unconf + shared->dim_I_A + shared->dim_I_C_1 + shared->dim_I_C_2 + shared->dim_I_P + shared->dim_I_weighted + shared->dim_N_tot + shared->dim_R + shared->dim_S + shared->dim_T_PCR_neg + shared->dim_T_PCR_pos + shared->dim_T_PCR_pre + shared->dim_T_sero_neg_1 + shared->dim_T_sero_neg_2 + shared->dim_T_sero_pos_1 + shared->dim_T_sero_pos_2 + shared->dim_T_sero_pre_1 + shared->dim_T_sero_pre_2 + shared->dim_W_D_conf + shared->dim_W_D_unconf + shared->dim_W_R_conf + shared->dim_W_R_unconf + shared->dim_cum_admit_by_age + shared->dim_cum_infections_disag + shared->dim_cum_infections_per_strain + shared->dim_cum_n_E_vacc_skip + shared->dim_cum_n_E_vaccinated + shared->dim_cum_n_I_A_vacc_skip + shared->dim_cum_n_I_A_vaccinated + shared->dim_cum_n_I_P_vacc_skip + shared->dim_cum_n_I_P_vaccinated + shared->dim_cum_n_R_vacc_skip + shared->dim_cum_n_R_vaccinated + shared->dim_cum_n_S_vacc_skip + shared->dim_cum_n_S_vaccinated + shared->dim_cum_n_vacc_skip + shared->dim_cum_n_vaccinated + shared->dim_diagnoses_admitted + shared->dim_prob_strain + shared->dim_tmp_vaccine_n_candidates + shared->dim_tmp_vaccine_probability + shared->dim_vaccine_missed_doses + 97;
  }
  std::vector<real_type> initial(size_t step) {
    std::vector<real_type> state(shared->dim_D + shared->dim_D_hosp + shared->dim_D_non_hosp + shared->dim_E + shared->dim_G_D + shared->dim_H_D_conf + shared->dim_H_D_unconf + shared->dim_H_R_conf + shared->dim_H_R_unconf + shared->dim_ICU_D_conf + shared->dim_ICU_D_unconf + shared->dim_ICU_W_D_conf + shared->dim_ICU_W_D_unconf + shared->dim_ICU_W_R_conf + shared->dim_ICU_W_R_unconf + shared->dim_ICU_pre_conf + shared->dim_ICU_pre_unconf + shared->dim_I_A + shared->dim_I_C_1 + shared->dim_I_C_2 + shared->dim_I_P + shared->dim_I_weighted + shared->dim_N_tot + shared->dim_R + shared->dim_S + shared->dim_T_PCR_neg + shared->dim_T_PCR_pos + shared->dim_T_PCR_pre + shared->dim_T_sero_neg_1 + shared->dim_T_sero_neg_2 + shared->dim_T_sero_pos_1 + shared->dim_T_sero_pos_2 + shared->dim_T_sero_pre_1 + shared->dim_T_sero_pre_2 + shared->dim_W_D_conf + shared->dim_W_D_unconf + shared->dim_W_R_conf + shared->dim_W_R_unconf + shared->dim_cum_admit_by_age + shared->dim_cum_infections_disag + shared->dim_cum_infections_per_strain + shared->dim_cum_n_E_vacc_skip + shared->dim_cum_n_E_vaccinated + shared->dim_cum_n_I_A_vacc_skip + shared->dim_cum_n_I_A_vaccinated + shared->dim_cum_n_I_P_vacc_skip + shared->dim_cum_n_I_P_vaccinated + shared->dim_cum_n_R_vacc_skip + shared->dim_cum_n_R_vaccinated + shared->dim_cum_n_S_vacc_skip + shared->dim_cum_n_S_vaccinated + shared->dim_cum_n_vacc_skip + shared->dim_cum_n_vaccinated + shared->dim_diagnoses_admitted + shared->dim_prob_strain + shared->dim_tmp_vaccine_n_candidates + shared->dim_tmp_vaccine_probability + shared->dim_vaccine_missed_doses + 97);
    state[0] = shared->initial_time;
    state[1] = shared->initial_admit_conf_inc;
    state[2] = shared->initial_admit_0_9_conf_inc;
    state[3] = shared->initial_admit_10_19_conf_inc;
    state[4] = shared->initial_admit_20_29_conf_inc;
    state[5] = shared->initial_admit_30_39_conf_inc;
    state[6] = shared->initial_admit_40_49_conf_inc;
    state[7] = shared->initial_admit_50_59_conf_inc;
    state[8] = shared->initial_admit_60_69_conf_inc;
    state[9] = shared->initial_admit_70_79_conf_inc;
    state[10] = shared->initial_admit_80_plus_conf_inc;
    state[11] = shared->initial_new_conf_inc;
    state[12] = shared->initial_new_0_9_conf_inc;
    state[13] = shared->initial_new_10_19_conf_inc;
    state[14] = shared->initial_new_20_29_conf_inc;
    state[15] = shared->initial_new_30_39_conf_inc;
    state[16] = shared->initial_new_40_49_conf_inc;
    state[17] = shared->initial_new_50_59_conf_inc;
    state[18] = shared->initial_new_60_69_conf_inc;
    state[19] = shared->initial_new_70_79_conf_inc;
    state[20] = shared->initial_new_80_plus_conf_inc;
    state[21] = shared->initial_cum_infections;
    state[22] = shared->initial_infections_inc;
    state[23] = shared->initial_cum_admit_conf;
    state[24] = shared->initial_cum_new_conf;
    state[25] = shared->initial_cum_admit_0_9_conf;
    state[26] = shared->initial_cum_admit_10_19_conf;
    state[27] = shared->initial_cum_admit_20_29_conf;
    state[28] = shared->initial_cum_admit_30_39_conf;
    state[29] = shared->initial_cum_admit_40_49_conf;
    state[30] = shared->initial_cum_admit_50_59_conf;
    state[31] = shared->initial_cum_admit_60_69_conf;
    state[32] = shared->initial_cum_admit_70_79_conf;
    state[33] = shared->initial_cum_admit_80_plus_conf;
    state[34] = shared->initial_cum_new_0_9_conf;
    state[35] = shared->initial_cum_new_10_19_conf;
    state[36] = shared->initial_cum_new_20_29_conf;
    state[37] = shared->initial_cum_new_30_39_conf;
    state[38] = shared->initial_cum_new_40_49_conf;
    state[39] = shared->initial_cum_new_50_59_conf;
    state[40] = shared->initial_cum_new_60_69_conf;
    state[41] = shared->initial_cum_new_70_79_conf;
    state[42] = shared->initial_cum_new_80_plus_conf;
    state[43] = shared->initial_beta_out;
    state[44] = shared->initial_N_tot_sero_1;
    state[45] = shared->initial_N_tot_sero_2;
    state[46] = shared->initial_N_tot_PCR;
    state[47] = shared->initial_ICU_tot;
    state[48] = shared->initial_general_tot;
    state[49] = shared->initial_hosp_tot;
    state[50] = shared->initial_D_hosp_tot;
    state[51] = shared->initial_D_hosp_0_49_tot;
    state[52] = shared->initial_D_hosp_50_54_tot;
    state[53] = shared->initial_D_hosp_55_59_tot;
    state[54] = shared->initial_D_hosp_60_64_tot;
    state[55] = shared->initial_D_hosp_65_69_tot;
    state[56] = shared->initial_D_hosp_70_74_tot;
    state[57] = shared->initial_D_hosp_75_79_tot;
    state[58] = shared->initial_D_hosp_80_plus_tot;
    state[59] = shared->initial_D_comm_tot;
    state[60] = shared->initial_D_comm_inc;
    state[61] = shared->initial_D_carehomes_tot;
    state[62] = shared->initial_D_carehomes_inc;
    state[63] = shared->initial_D_tot;
    state[64] = shared->initial_D_inc;
    state[65] = shared->initial_D_hosp_inc;
    state[66] = shared->initial_D_hosp_0_49_inc;
    state[67] = shared->initial_D_hosp_50_54_inc;
    state[68] = shared->initial_D_hosp_55_59_inc;
    state[69] = shared->initial_D_hosp_60_64_inc;
    state[70] = shared->initial_D_hosp_65_69_inc;
    state[71] = shared->initial_D_hosp_70_74_inc;
    state[72] = shared->initial_D_hosp_75_79_inc;
    state[73] = shared->initial_D_hosp_80_plus_inc;
    state[74] = shared->initial_sero_pos_1;
    state[75] = shared->initial_sero_pos_2;
    state[76] = shared->initial_cum_sympt_cases;
    state[77] = shared->initial_cum_sympt_cases_non_variant;
    state[78] = shared->initial_cum_sympt_cases_over25;
    state[79] = shared->initial_cum_sympt_cases_non_variant_over25;
    state[80] = shared->initial_cum_sympt_cases_under15;
    state[81] = shared->initial_cum_sympt_cases_15_24;
    state[82] = shared->initial_cum_sympt_cases_25_49;
    state[83] = shared->initial_cum_sympt_cases_50_64;
    state[84] = shared->initial_cum_sympt_cases_65_79;
    state[85] = shared->initial_cum_sympt_cases_80_plus;
    state[86] = shared->initial_sympt_cases_inc;
    state[87] = shared->initial_sympt_cases_non_variant_inc;
    state[88] = shared->initial_sympt_cases_over25_inc;
    state[89] = shared->initial_sympt_cases_non_variant_over25_inc;
    state[90] = shared->initial_sympt_cases_under15_inc;
    state[91] = shared->initial_sympt_cases_15_24_inc;
    state[92] = shared->initial_sympt_cases_25_49_inc;
    state[93] = shared->initial_sympt_cases_50_64_inc;
    state[94] = shared->initial_sympt_cases_65_79_inc;
    state[95] = shared->initial_sympt_cases_80_plus_inc;
    state[96] = shared->initial_react_pos;
    std::copy(shared->initial_cum_n_S_vacc_skip.begin(), shared->initial_cum_n_S_vacc_skip.end(), state.begin() + 97);
    ...
    std::copy(shared->initial_cum_infections_per_strain.begin(), shared->initial_cum_infections_per_strain.end(), state.begin() + shared->offset_variable_cum_infections_per_strain);
  ...
  void update(size_t step, const real_type * state, rng_state_type& rng_state, real_type * state_next) {
    ...
    const real_type * cum_n_S_vacc_skip = state + 97;
    ...
    const real_type cum_admit_conf = state[23];
    const real_type cum_admit_0_9_conf = state[25];
    const real_type cum_admit_10_19_conf = state[26];
    const real_type cum_admit_20_29_conf = state[27];
    const real_type cum_admit_30_39_conf = state[28];
    const real_type cum_admit_40_49_conf = state[29];
    const real_type cum_admit_50_59_conf = state[30];
    const real_type cum_admit_60_69_conf = state[31];
    const real_type cum_admit_70_79_conf = state[32];
    const real_type cum_admit_80_plus_conf = state[33];
    const real_type cum_new_conf = state[24];
    const real_type cum_new_0_9_conf = state[34];
    const real_type cum_new_10_19_conf = state[35];
    const real_type cum_new_20_29_conf = state[36];
    const real_type cum_new_30_39_conf = state[37];
    const real_type cum_new_40_49_conf = state[38];
    const real_type cum_new_50_59_conf = state[39];
    const real_type cum_new_60_69_conf = state[40];
    const real_type cum_new_70_79_conf = state[41];
    const real_type cum_new_80_plus_conf = state[42];
    ...
    const real_type admit_0_9_conf_inc = state[2];
    const real_type admit_10_19_conf_inc = state[3];
    const real_type admit_20_29_conf_inc = state[4];
    const real_type admit_30_39_conf_inc = state[5];
    const real_type admit_40_49_conf_inc = state[6];
    const real_type admit_50_59_conf_inc = state[7];
    const real_type admit_60_69_conf_inc = state[8];
    const real_type admit_70_79_conf_inc = state[9];
    const real_type admit_80_plus_conf_inc = state[10];
    const real_type new_conf_inc = state[11];
    const real_type new_0_9_conf_inc = state[12];
    const real_type new_10_19_conf_inc = state[13];
    const real_type new_20_29_conf_inc = state[14];
    const real_type new_30_39_conf_inc = state[15];
    const real_type new_40_49_conf_inc = state[16];
    const real_type new_50_59_conf_inc = state[17];
    const real_type new_60_69_conf_inc = state[18];
    const real_type new_70_79_conf_inc = state[19];
    const real_type new_80_plus_conf_inc = state[20];
    const real_type * cum_admit_by_age = state + shared->offset_variable_cum_admit_by_age;
    const real_type cum_infections = state[21];
    const real_type infections_inc = state[22];
    const real_type * cum_infections_per_strain = state + shared->offset_variable_cum_infections_per_strain;
    const real_type * D = state + shared->offset_variable_D;
    const real_type D_hosp_tot = state[50];
    const real_type D_hosp_0_49_tot = state[51];
    const real_type D_hosp_50_54_tot = state[52];
    const real_type D_hosp_55_59_tot = state[53];
    const real_type D_hosp_60_64_tot = state[54];
    const real_type D_hosp_65_69_tot = state[55];
    const real_type D_hosp_70_74_tot = state[56];
    const real_type D_hosp_75_79_tot = state[57];
    const real_type D_hosp_80_plus_tot = state[58];
    const real_type D_comm_tot = state[59];
    const real_type D_comm_inc = state[60];
    const real_type D_carehomes_tot = state[61];
    const real_type D_carehomes_inc = state[62];
    const real_type D_tot = state[63];
    const real_type D_inc = state[64];
    const real_type D_hosp_inc = state[65];
    const real_type D_hosp_0_49_inc = state[66];
    const real_type D_hosp_50_54_inc = state[67];
    const real_type D_hosp_55_59_inc = state[68];
    const real_type D_hosp_60_64_inc = state[69];
    const real_type D_hosp_65_69_inc = state[70];
    const real_type D_hosp_70_74_inc = state[71];
    const real_type D_hosp_75_79_inc = state[72];
    const real_type D_hosp_80_plus_inc = state[73];
    const real_type cum_sympt_cases = state[76];
    const real_type cum_sympt_cases_non_variant = state[77];
    const real_type cum_sympt_cases_over25 = state[78];
    const real_type cum_sympt_cases_non_variant_over25 = state[79];
    const real_type cum_sympt_cases_under15 = state[80];
    const real_type cum_sympt_cases_15_24 = state[81];
    const real_type cum_sympt_cases_25_49 = state[82];
    const real_type cum_sympt_cases_50_64 = state[83];
    const real_type cum_sympt_cases_65_79 = state[84];
    const real_type cum_sympt_cases_80_plus = state[85];
    const real_type sympt_cases_inc = state[86];
    const real_type sympt_cases_non_variant_inc = state[87];
    const real_type sympt_cases_over25_inc = state[88];
    const real_type sympt_cases_non_variant_over25_inc = state[89];
    const real_type sympt_cases_under15_inc = state[90];
    const real_type sympt_cases_15_24_inc = state[91];
    const real_type sympt_cases_25_49_inc = state[92];
    const real_type sympt_cases_50_64_inc = state[93];
    const real_type sympt_cases_65_79_inc = state[94];
    const real_type sympt_cases_80_plus_inc = state[95];
    const real_type * vaccine_missed_doses = state + shared->offset_variable_vaccine_missed_doses;
    state_next[46] = odin_sum1<real_type>(S, 0, shared->dim_S) + odin_sum1<real_type>(T_PCR_pre, 0, shared->dim_T_PCR_pre) + odin_sum1<real_type>(T_PCR_pos, 0, shared->dim_T_PCR_pos) + odin_sum1<real_type>(T_PCR_neg, 0, shared->dim_T_PCR_neg);
    state_next[44] = odin_sum1<real_type>(S, 0, shared->dim_S) + odin_sum1<real_type>(T_sero_pre_1, 0, shared->dim_T_sero_pre_1) + odin_sum1<real_type>(T_sero_pos_1, 0, shared->dim_T_sero_pos_1) + odin_sum1<real_type>(T_sero_neg_1, 0, shared->dim_T_sero_neg_1) + odin_sum1<real_type>(E, 0, shared->dim_E);
    state_next[45] = odin_sum1<real_type>(S, 0, shared->dim_S) + odin_sum1<real_type>(T_sero_pre_2, 0, shared->dim_T_sero_pre_2) + odin_sum1<real_type>(T_sero_pos_2, 0, shared->dim_T_sero_pos_2) + odin_sum1<real_type>(T_sero_neg_2, 0, shared->dim_T_sero_neg_2) + odin_sum1<real_type>(E, 0, shared->dim_E);
    ...
    for (int i = 1; i <= shared->dim_vaccine_attempted_doses_1; ++i) {
      for (int j = 1; j <= shared->dim_vaccine_attempted_doses_2; ++j) {
        internal.vaccine_attempted_doses[i - 1 + shared->dim_vaccine_attempted_doses_1 * (j - 1)] = ((j == shared->vacc_skip_dose ? ((internal.vaccine_n_candidates[shared->dim_vaccine_n_candidates_1 * (j - 1) + i - 1] == 0 ? 0 : std::min(internal.vaccine_n_candidates[shared->dim_vaccine_n_candidates_1 * (j - 1) + i - 1] / (real_type) (internal.vaccine_n_candidates[shared->dim_vaccine_n_candidates_1 * (j - 1) + i - 1] + shared->vacc_skip_weight * internal.vacc_skip_n_candidates[i - 1]) * internal.total_attempted_doses[shared->dim_total_attempted_doses_1 * (j - 1) + i - 1], internal.vaccine_n_candidates[shared->dim_vaccine_n_candidates_1 * (j - 1) + i - 1]))) : internal.total_attempted_doses[shared->dim_total_attempted_doses_1 * (j - 1) + i - 1]));
      }
    }
    ...
    for (int i = 1; i <= shared->dim_vacc_skip_attempted_doses; ++i) {
      internal.vacc_skip_attempted_doses[i - 1] = ((shared->vacc_skip_weight > 0 ? ((shared->vacc_skip_dose > 0 ? internal.total_attempted_doses[shared->dim_total_attempted_doses_1 * (shared->vacc_skip_dose - 1) + i - 1] - internal.vaccine_attempted_doses[shared->dim_vaccine_attempted_doses_1 * (shared->vacc_skip_dose - 1) + i - 1] : 0)) : 0));
    }
    ...
    state_next[74] = odin_sum4<real_type>(internal.new_T_sero_pos_1.data(), 3, 13, 0, shared->dim_new_T_sero_pos_1_2, 0, shared->dim_new_T_sero_pos_1_3, 0, shared->dim_new_T_sero_pos_1_4, shared->dim_new_T_sero_pos_1_1, shared->dim_new_T_sero_pos_1_12, shared->dim_new_T_sero_pos_1_123);
    state_next[75] = odin_sum4<real_type>(internal.new_T_sero_pos_2.data(), 3, 13, 0, shared->dim_new_T_sero_pos_2_2, 0, shared->dim_new_T_sero_pos_2_3, 0, shared->dim_new_T_sero_pos_2_4, shared->dim_new_T_sero_pos_2_1, shared->dim_new_T_sero_pos_2_12, shared->dim_new_T_sero_pos_2_123);
    ...
    real_type delta_D_hosp_65_69_tot = internal.delta_D_hosp[13] + internal.delta_D_hosp[18] * 0.050000000000000003;
    real_type delta_D_hosp_70_74_tot = internal.delta_D_hosp[14] + internal.delta_D_hosp[18] * 0.050000000000000003;
    real_type delta_D_hosp_75_79_tot = internal.delta_D_hosp[15] + internal.delta_D_hosp[18] * 0.14999999999999999;
    real_type delta_D_hosp_80_plus_tot = internal.delta_D_hosp[16] + internal.delta_D_hosp[18] * 0.75;
    ...
    real_type new_sympt_cases_65_79 = odin_sum3<real_type>(internal.n_EI_P.data(), 13, 16, 0, shared->dim_n_EI_P_2, 0, shared->dim_n_EI_P_3, shared->dim_n_EI_P_1, shared->dim_n_EI_P_12) + (odin_sum3<real_type>(internal.n_EI_P.data(), 18, 19, 0, shared->dim_n_EI_P_2, 0, shared->dim_n_EI_P_3, shared->dim_n_EI_P_1, shared->dim_n_EI_P_12) * 0.25);
    real_type new_sympt_cases_80_plus = odin_sum3<real_type>(internal.n_EI_P.data(), 16, 17, 0, shared->dim_n_EI_P_2, 0, shared->dim_n_EI_P_3, shared->dim_n_EI_P_1, shared->dim_n_EI_P_12) + (odin_sum3<real_type>(internal.n_EI_P.data(), 18, 19, 0, shared->dim_n_EI_P_2, 0, shared->dim_n_EI_P_3, shared->dim_n_EI_P_1, shared->dim_n_EI_P_12) * 0.75);
    ...
    state_next[62] = (fmodr<real_type>(step, shared->steps_per_day) == 0 ? delta_D_carehomes_tot : D_carehomes_inc + delta_D_carehomes_tot);
    state_next[61] = D_carehomes_tot + delta_D_carehomes_tot;
    state_next[60] = (fmodr<real_type>(step, shared->steps_per_day) == 0 ? delta_D_comm_tot : D_comm_inc + delta_D_comm_tot);
    state_next[59] = D_comm_tot + delta_D_comm_tot;
    state_next[66] = (fmodr<real_type>(step, shared->steps_per_day) == 0 ? delta_D_hosp_0_49_tot : D_hosp_0_49_inc + delta_D_hosp_0_49_tot);
    state_next[51] = D_hosp_0_49_tot + delta_D_hosp_0_49_tot;
    state_next[67] = (fmodr<real_type>(step, shared->steps_per_day) == 0 ? delta_D_hosp_50_54_tot : D_hosp_50_54_inc + delta_D_hosp_50_54_tot);
    state_next[52] = D_hosp_50_54_tot + delta_D_hosp_50_54_tot;
    state_next[68] = (fmodr<real_type>(step, shared->steps_per_day) == 0 ? delta_D_hosp_55_59_tot : D_hosp_55_59_inc + delta_D_hosp_55_59_tot);
    state_next[53] = D_hosp_55_59_tot + delta_D_hosp_55_59_tot;
    state_next[69] = (fmodr<real_type>(step, shared->steps_per_day) == 0 ? delta_D_hosp_60_64_tot : D_hosp_60_64_inc + delta_D_hosp_60_64_tot);
    state_next[54] = D_hosp_60_64_tot + delta_D_hosp_60_64_tot;
    state_next[70] = (fmodr<real_type>(step, shared->steps_per_day) == 0 ? delta_D_hosp_65_69_tot : D_hosp_65_69_inc + delta_D_hosp_65_69_tot);
    state_next[55] = D_hosp_65_69_tot + delta_D_hosp_65_69_tot;
    state_next[71] = (fmodr<real_type>(step, shared->steps_per_day) == 0 ? delta_D_hosp_70_74_tot : D_hosp_70_74_inc + delta_D_hosp_70_74_tot);
    state_next[56] = D_hosp_70_74_tot + delta_D_hosp_70_74_tot;
    state_next[72] = (fmodr<real_type>(step, shared->steps_per_day) == 0 ? delta_D_hosp_75_79_tot : D_hosp_75_79_inc + delta_D_hosp_75_79_tot);
    state_next[57] = D_hosp_75_79_tot + delta_D_hosp_75_79_tot;
    state_next[73] = (fmodr<real_type>(step, shared->steps_per_day) == 0 ? delta_D_hosp_80_plus_tot : D_hosp_80_plus_inc + delta_D_hosp_80_plus_tot);
    state_next[58] = D_hosp_80_plus_tot + delta_D_hosp_80_plus_tot;
    state_next[65] = (fmodr<real_type>(step, shared->steps_per_day) == 0 ? delta_D_hosp_tot : D_hosp_inc + delta_D_hosp_tot);
    state_next[50] = D_hosp_tot + delta_D_hosp_tot;
    ...
    state_next[76] = cum_sympt_cases + new_sympt_cases;
    state_next[81] = cum_sympt_cases_15_24 + new_sympt_cases_15_24;
    state_next[82] = cum_sympt_cases_25_49 + new_sympt_cases_25_49;
    state_next[83] = cum_sympt_cases_50_64 + new_sympt_cases_50_64;
    state_next[84] = cum_sympt_cases_65_79 + new_sympt_cases_65_79;
    state_next[85] = cum_sympt_cases_80_plus + new_sympt_cases_80_plus;
    state_next[77] = cum_sympt_cases_non_variant + new_sympt_cases_non_variant;
    state_next[79] = cum_sympt_cases_non_variant_over25 + new_sympt_cases_non_variant_over25;
    state_next[78] = cum_sympt_cases_over25 + new_sympt_cases_over25;
    state_next[80] = cum_sympt_cases_under15 + new_sympt_cases_under15;
    state_next[91] = ((fmodr<real_type>(step, shared->steps_per_day) == 0 ? new_sympt_cases_15_24 : sympt_cases_15_24_inc + new_sympt_cases_15_24));
    state_next[92] = ((fmodr<real_type>(step, shared->steps_per_day) == 0 ? new_sympt_cases_25_49 : sympt_cases_25_49_inc + new_sympt_cases_25_49));
    state_next[93] = ((fmodr<real_type>(step, shared->steps_per_day) == 0 ? new_sympt_cases_50_64 : sympt_cases_50_64_inc + new_sympt_cases_50_64));
    state_next[94] = ((fmodr<real_type>(step, shared->steps_per_day) == 0 ? new_sympt_cases_65_79 : sympt_cases_65_79_inc + new_sympt_cases_65_79));
    state_next[95] = ((fmodr<real_type>(step, shared->steps_per_day) == 0 ? new_sympt_cases_80_plus : sympt_cases_80_plus_inc + new_sympt_cases_80_plus));
    state_next[86] = ((fmodr<real_type>(step, shared->steps_per_day) == 0 ? new_sympt_cases : sympt_cases_inc + new_sympt_cases));
    state_next[87] = ((fmodr<real_type>(step, shared->steps_per_day) == 0 ? new_sympt_cases_non_variant : sympt_cases_non_variant_inc + new_sympt_cases_non_variant));
    state_next[89] = ((fmodr<real_type>(step, shared->steps_per_day) == 0 ? new_sympt_cases_non_variant_over25 : sympt_cases_non_variant_over25_inc + new_sympt_cases_non_variant_over25));
    state_next[88] = ((fmodr<real_type>(step, shared->steps_per_day) == 0 ? new_sympt_cases_over25 : sympt_cases_over25_inc + new_sympt_cases_over25));
    state_next[90] = ((fmodr<real_type>(step, shared->steps_per_day) == 0 ? new_sympt_cases_under15 : sympt_cases_under15_inc + new_sympt_cases_under15));
    ...
    state_next[64] = (fmodr<real_type>(step, shared->steps_per_day) == 0 ? delta_D_tot : D_inc + delta_D_tot);
    state_next[63] = D_tot + delta_D_tot;
      ...
      for (int j = 1; j <= shared->dim_aux_I_A_2; ++j) {
        for (int k = 1; k <= shared->dim_aux_I_A_3; ++k) {
          for (int l = 1; l <= shared->dim_aux_I_A_4; ++l) {
            internal.aux_I_A[i - 1 + shared->dim_aux_I_A_1 * (j - 1) + shared->dim_aux_I_A_12 * (k - 1) + shared->dim_aux_I_A_123 * (l - 1)] = ((k == 1 ? internal.n_EI_A[shared->dim_n_EI_A_12 * (l - 1) + shared->dim_n_EI_A_1 * (j - 1) + i - 1] : internal.n_I_A_progress[shared->dim_n_I_A_progress_123 * (l - 1) + shared->dim_n_I_A_progress_12 * (k - 1 - 1) + shared->dim_n_I_A_progress_1 * (j - 1) + i - 1])) - internal.n_I_A_progress[shared->dim_n_I_A_progress_123 * (l - 1) + shared->dim_n_I_A_progress_12 * (k - 1) + shared->dim_n_I_A_progress_1 * (j - 1) + i - 1] - internal.n_I_A_next_vacc_class[shared->dim_n_I_A_next_vacc_class_123 * (l - 1) + shared->dim_n_I_A_next_vacc_class_12 * (k - 1) + shared->dim_n_I_A_next_vacc_class_1 * (j - 1) + i - 1] + ((l == 1 ? internal.n_I_A_next_vacc_class[shared->dim_n_I_A_next_vacc_class_123 * (shared->n_vacc_classes - 1) + shared->dim_n_I_A_next_vacc_class_12 * (k - 1) + shared->dim_n_I_A_next_vacc_class_1 * (j - 1) + i - 1] : internal.n_I_A_next_vacc_class[shared->dim_n_I_A_next_vacc_class_123 * (l - 1 - 1) + shared->dim_n_I_A_next_vacc_class_12 * (k - 1) + shared->dim_n_I_A_next_vacc_class_1 * (j - 1) + i - 1])) - ((l == shared->vacc_skip_from ? internal.n_I_A_vacc_skip[shared->dim_n_I_A_vacc_skip_12 * (k - 1) + shared->dim_n_I_A_vacc_skip_1 * (j - 1) + i - 1] : 0)) + ((l == shared->vacc_skip_to ? internal.n_I_A_vacc_skip[shared->dim_n_I_A_vacc_skip_12 * (k - 1) + shared->dim_n_I_A_vacc_skip_1 * (j - 1) + i - 1] : 0));
          }
        }
      }
    }
      ...
      for (int j = 1; j <= shared->dim_aux_I_P_2; ++j) {
        for (int k = 1; k <= shared->dim_aux_I_P_3; ++k) {
          for (int l = 1; l <= shared->dim_aux_I_P_4; ++l) {
            internal.aux_I_P[i - 1 + shared->dim_aux_I_P_1 * (j - 1) + shared->dim_aux_I_P_12 * (k - 1) + shared->dim_aux_I_P_123 * (l - 1)] = ((k == 1 ? internal.n_EI_P[shared->dim_n_EI_P_12 * (l - 1) + shared->dim_n_EI_P_1 * (j - 1) + i - 1] : internal.n_I_P_progress[shared->dim_n_I_P_progress_123 * (l - 1) + shared->dim_n_I_P_progress_12 * (k - 1 - 1) + shared->dim_n_I_P_progress_1 * (j - 1) + i - 1])) - internal.n_I_P_progress[shared->dim_n_I_P_progress_123 * (l - 1) + shared->dim_n_I_P_progress_12 * (k - 1) + shared->dim_n_I_P_progress_1 * (j - 1) + i - 1] - internal.n_I_P_next_vacc_class[shared->dim_n_I_P_next_vacc_class_123 * (l - 1) + shared->dim_n_I_P_next_vacc_class_12 * (k - 1) + shared->dim_n_I_P_next_vacc_class_1 * (j - 1) + i - 1] + ((l == 1 ? internal.n_I_P_next_vacc_class[shared->dim_n_I_P_next_vacc_class_123 * (shared->n_vacc_classes - 1) + shared->dim_n_I_P_next_vacc_class_12 * (k - 1) + shared->dim_n_I_P_next_vacc_class_1 * (j - 1) + i - 1] : internal.n_I_P_next_vacc_class[shared->dim_n_I_P_next_vacc_class_123 * (l - 1 - 1) + shared->dim_n_I_P_next_vacc_class_12 * (k - 1) + shared->dim_n_I_P_next_vacc_class_1 * (j - 1) + i - 1])) - ((l == shared->vacc_skip_from ? internal.n_I_P_vacc_skip[shared->dim_n_I_P_vacc_skip_12 * (k - 1) + shared->dim_n_I_P_vacc_skip_1 * (j - 1) + i - 1] : 0)) + ((l == shared->vacc_skip_to ? internal.n_I_P_vacc_skip[shared->dim_n_I_P_vacc_skip_12 * (k - 1) + shared->dim_n_I_P_vacc_skip_1 * (j - 1) + i - 1] : 0));
          }
        }
      }
    }
    real_type delta_new_0_9_conf = odin_sum4<real_type>(internal.n_H_D_unconf_to_conf.data(), 0, 2, 0, shared->dim_n_H_D_unconf_to_conf_2, 0, shared->dim_n_H_D_unconf_to_conf_3, 0, shared->dim_n_H_D_unconf_to_conf_4, shared->dim_n_H_D_unconf_to_conf_1, shared->dim_n_H_D_unconf_to_conf_12, shared->dim_n_H_D_unconf_to_conf_123) + odin_sum4<real_type>(internal.n_H_R_unconf_to_conf.data(), 0, 2, 0, shared->dim_n_H_R_unconf_to_conf_2, 0, shared->dim_n_H_R_unconf_to_conf_3, 0, shared->dim_n_H_R_unconf_to_conf_4, shared->dim_n_H_R_unconf_to_conf_1, shared->dim_n_H_R_unconf_to_conf_12, shared->dim_n_H_R_unconf_to_conf_123) + odin_sum4<real_type>(internal.n_ICU_pre_unconf_to_conf.data(), 0, 2, 0, shared->dim_n_ICU_pre_unconf_to_conf_2, 0, shared->dim_n_ICU_pre_unconf_to_conf_3, 0, shared->dim_n_ICU_pre_unconf_to_conf_4, shared->dim_n_ICU_pre_unconf_to_conf_1, shared->dim_n_ICU_pre_unconf_to_conf_12, shared->dim_n_ICU_pre_unconf_to_conf_123) + odin_sum4<real_type>(internal.n_ICU_D_unconf_to_conf.data(), 0, 2, 0, shared->dim_n_ICU_D_unconf_to_conf_2, 0, shared->dim_n_ICU_D_unconf_to_conf_3, 0, shared->dim_n_ICU_D_unconf_to_conf_4, shared->dim_n_ICU_D_unconf_to_conf_1, shared->dim_n_ICU_D_unconf_to_conf_12, shared->dim_n_ICU_D_unconf_to_conf_123) + odin_sum4<real_type>(internal.n_ICU_W_R_unconf_to_conf.data(), 0, 2, 0, shared->dim_n_ICU_W_R_unconf_to_conf_2, 0, shared->dim_n_ICU_W_R_unconf_to_conf_3, 0, shared->dim_n_ICU_W_R_unconf_to_conf_4, shared->dim_n_ICU_W_R_unconf_to_conf_1, shared->dim_n_ICU_W_R_unconf_to_conf_12, shared->dim_n_ICU_W_R_unconf_to_conf_123) + odin_sum4<real_type>(internal.n_ICU_W_D_unconf_to_conf.data(), 0, 2, 0, shared->dim_n_ICU_W_D_unconf_to_conf_2, 0, shared->dim_n_ICU_W_D_unconf_to_conf_3, 0, shared->dim_n_ICU_W_D_unconf_to_conf_4, shared->dim_n_ICU_W_D_unconf_to_conf_1, shared->dim_n_ICU_W_D_unconf_to_conf_12, shared->dim_n_ICU_W_D_unconf_to_conf_123) + odin_sum4<real_type>(internal.n_W_R_unconf_to_conf.data(), 0, 2, 0, shared->dim_n_W_R_unconf_to_conf_2, 0, shared->dim_n_W_R_unconf_to_conf_3, 0, shared->dim_n_W_R_unconf_to_conf_4, shared->dim_n_W_R_unconf_to_conf_1, shared->dim_n_W_R_unconf_to_conf_12, shared->dim_n_W_R_unconf_to_conf_123) + odin_sum4<real_type>(internal.n_W_D_unconf_to_conf.data(), 0, 2, 0, shared->dim_n_W_D_unconf_to_conf_2, 0, shared->dim_n_W_D_unconf_to_conf_3, 0, shared->dim_n_W_D_unconf_to_conf_4, shared->dim_n_W_D_unconf_to_conf_1, shared->dim_n_W_D_unconf_to_conf_12, shared->dim_n_W_D_unconf_to_conf_123);
    real_type delta_new_10_19_conf = odin_sum4<real_type>(internal.n_H_D_unconf_to_conf.data(), 2, 4, 0, shared->dim_n_H_D_unconf_to_conf_2, 0, shared->dim_n_H_D_unconf_to_conf_3, 0, shared->dim_n_H_D_unconf_to_conf_4, shared->dim_n_H_D_unconf_to_conf_1, shared->dim_n_H_D_unconf_to_conf_12, shared->dim_n_H_D_unconf_to_conf_123) + odin_sum4<real_type>(internal.n_H_R_unconf_to_conf.data(), 2, 4, 0, shared->dim_n_H_R_unconf_to_conf_2, 0, shared->dim_n_H_R_unconf_to_conf_3, 0, shared->dim_n_H_R_unconf_to_conf_4, shared->dim_n_H_R_unconf_to_conf_1, shared->dim_n_H_R_unconf_to_conf_12, shared->dim_n_H_R_unconf_to_conf_123) + odin_sum4<real_type>(internal.n_ICU_pre_unconf_to_conf.data(), 2, 4, 0, shared->dim_n_ICU_pre_unconf_to_conf_2, 0, shared->dim_n_ICU_pre_unconf_to_conf_3, 0, shared->dim_n_ICU_pre_unconf_to_conf_4, shared->dim_n_ICU_pre_unconf_to_conf_1, shared->dim_n_ICU_pre_unconf_to_conf_12, shared->dim_n_ICU_pre_unconf_to_conf_123) + odin_sum4<real_type>(internal.n_ICU_D_unconf_to_conf.data(), 2, 4, 0, shared->dim_n_ICU_D_unconf_to_conf_2, 0, shared->dim_n_ICU_D_unconf_to_conf_3, 0, shared->dim_n_ICU_D_unconf_to_conf_4, shared->dim_n_ICU_D_unconf_to_conf_1, shared->dim_n_ICU_D_unconf_to_conf_12, shared->dim_n_ICU_D_unconf_to_conf_123) + odin_sum4<real_type>(internal.n_ICU_W_R_unconf_to_conf.data(), 2, 4, 0, shared->dim_n_ICU_W_R_unconf_to_conf_2, 0, shared->dim_n_ICU_W_R_unconf_to_conf_3, 0, shared->dim_n_ICU_W_R_unconf_to_conf_4, shared->dim_n_ICU_W_R_unconf_to_conf_1, shared->dim_n_ICU_W_R_unconf_to_conf_12, shared->dim_n_ICU_W_R_unconf_to_conf_123) + odin_sum4<real_type>(internal.n_ICU_W_D_unconf_to_conf.data(), 2, 4, 0, shared->dim_n_ICU_W_D_unconf_to_conf_2, 0, shared->dim_n_ICU_W_D_unconf_to_conf_3, 0, shared->dim_n_ICU_W_D_unconf_to_conf_4, shared->dim_n_ICU_W_D_unconf_to_conf_1, shared->dim_n_ICU_W_D_unconf_to_conf_12, shared->dim_n_ICU_W_D_unconf_to_conf_123) + odin_sum4<real_type>(internal.n_W_R_unconf_to_conf.data(), 2, 4, 0, shared->dim_n_W_R_unconf_to_conf_2, 0, shared->dim_n_W_R_unconf_to_conf_3, 0, shared->dim_n_W_R_unconf_to_conf_4, shared->dim_n_W_R_unconf_to_conf_1, shared->dim_n_W_R_unconf_to_conf_12, shared->dim_n_W_R_unconf_to_conf_123) + odin_sum4<real_type>(internal.n_W_D_unconf_to_conf.data(), 2, 4, 0, shared->dim_n_W_D_unconf_to_conf_2, 0, shared->dim_n_W_D_unconf_to_conf_3, 0, shared->dim_n_W_D_unconf_to_conf_4, shared->dim_n_W_D_unconf_to_conf_1, shared->dim_n_W_D_unconf_to_conf_12, shared->dim_n_W_D_unconf_to_conf_123);
    real_type delta_new_20_29_conf = odin_sum4<real_type>(internal.n_H_D_unconf_to_conf.data(), 4, 6, 0, shared->dim_n_H_D_unconf_to_conf_2, 0, shared->dim_n_H_D_unconf_to_conf_3, 0, shared->dim_n_H_D_unconf_to_conf_4, shared->dim_n_H_D_unconf_to_conf_1, shared->dim_n_H_D_unconf_to_conf_12, shared->dim_n_H_D_unconf_to_conf_123) + odin_sum4<real_type>(internal.n_H_D_unconf_to_conf.data(), 17, 18, 0, shared->dim_n_H_D_unconf_to_conf_2, 0, shared->dim_n_H_D_unconf_to_conf_3, 0, shared->dim_n_H_D_unconf_to_conf_4, shared->dim_n_H_D_unconf_to_conf_1, shared->dim_n_H_D_unconf_to_conf_12, shared->dim_n_H_D_unconf_to_conf_123) * 1 / (real_type) 8 + odin_sum4<real_type>(internal.n_H_R_unconf_to_conf.data(), 4, 6, 0, shared->dim_n_H_R_unconf_to_conf_2, 0, shared->dim_n_H_R_unconf_to_conf_3, 0, shared->dim_n_H_R_unconf_to_conf_4, shared->dim_n_H_R_unconf_to_conf_1, shared->dim_n_H_R_unconf_to_conf_12, shared->dim_n_H_R_unconf_to_conf_123) + odin_sum4<real_type>(internal.n_H_R_unconf_to_conf.data(), 17, 18, 0, shared->dim_n_H_R_unconf_to_conf_2, 0, shared->dim_n_H_R_unconf_to_conf_3, 0, shared->dim_n_H_R_unconf_to_conf_4, shared->dim_n_H_R_unconf_to_conf_1, shared->dim_n_H_R_unconf_to_conf_12, shared->dim_n_H_R_unconf_to_conf_123) * 1 / (real_type) 8 + odin_sum4<real_type>(internal.n_ICU_pre_unconf_to_conf.data(), 4, 6, 0, shared->dim_n_ICU_pre_unconf_to_conf_2, 0, shared->dim_n_ICU_pre_unconf_to_conf_3, 0, shared->dim_n_ICU_pre_unconf_to_conf_4, shared->dim_n_ICU_pre_unconf_to_conf_1, shared->dim_n_ICU_pre_unconf_to_conf_12, shared->dim_n_ICU_pre_unconf_to_conf_123) + odin_sum4<real_type>(internal.n_ICU_pre_unconf_to_conf.data(), 17, 18, 0, shared->dim_n_ICU_pre_unconf_to_conf_2, 0, shared->dim_n_ICU_pre_unconf_to_conf_3, 0, shared->dim_n_ICU_pre_unconf_to_conf_4, shared->dim_n_ICU_pre_unconf_to_conf_1, shared->dim_n_ICU_pre_unconf_to_conf_12, shared->dim_n_ICU_pre_unconf_to_conf_123) * 1 / (real_type) 8 + odin_sum4<real_type>(internal.n_ICU_D_unconf_to_conf.data(), 4, 6, 0, shared->dim_n_ICU_D_unconf_to_conf_2, 0, shared->dim_n_ICU_D_unconf_to_conf_3, 0, shared->dim_n_ICU_D_unconf_to_conf_4, shared->dim_n_ICU_D_unconf_to_conf_1, shared->dim_n_ICU_D_unconf_to_conf_12, shared->dim_n_ICU_D_unconf_to_conf_123) + odin_sum4<real_type>(internal.n_ICU_D_unconf_to_conf.data(), 17, 18, 0, shared->dim_n_ICU_D_unconf_to_conf_2, 0, shared->dim_n_ICU_D_unconf_to_conf_3, 0, shared->dim_n_ICU_D_unconf_to_conf_4, shared->dim_n_ICU_D_unconf_to_conf_1, shared->dim_n_ICU_D_unconf_to_conf_12, shared->dim_n_ICU_D_unconf_to_conf_123) * 1 / (real_type) 8 + odin_sum4<real_type>(internal.n_ICU_W_R_unconf_to_conf.data(), 4, 6, 0, shared->dim_n_ICU_W_R_unconf_to_conf_2, 0, shared->dim_n_ICU_W_R_unconf_to_conf_3, 0, shared->dim_n_ICU_W_R_unconf_to_conf_4, shared->dim_n_ICU_W_R_unconf_to_conf_1, shared->dim_n_ICU_W_R_unconf_to_conf_12, shared->dim_n_ICU_W_R_unconf_to_conf_123) + odin_sum4<real_type>(internal.n_ICU_W_R_unconf_to_conf.data(), 17, 18, 0, shared->dim_n_ICU_W_R_unconf_to_conf_2, 0, shared->dim_n_ICU_W_R_unconf_to_conf_3, 0, shared->dim_n_ICU_W_R_unconf_to_conf_4, shared->dim_n_ICU_W_R_unconf_to_conf_1, shared->dim_n_ICU_W_R_unconf_to_conf_12, shared->dim_n_ICU_W_R_unconf_to_conf_123) * 1 / (real_type) 8 + odin_sum4<real_type>(internal.n_ICU_W_D_unconf_to_conf.data(), 4, 6, 0, shared->dim_n_ICU_W_D_unconf_to_conf_2, 0, shared->dim_n_ICU_W_D_unconf_to_conf_3, 0, shared->dim_n_ICU_W_D_unconf_to_conf_4, shared->dim_n_ICU_W_D_unconf_to_conf_1, shared->dim_n_ICU_W_D_unconf_to_conf_12, shared->dim_n_ICU_W_D_unconf_to_conf_123) + odin_sum4<real_type>(internal.n_ICU_W_D_unconf_to_conf.data(), 17, 18, 0, shared->dim_n_ICU_W_D_unconf_to_conf_2, 0, shared->dim_n_ICU_W_D_unconf_to_conf_3, 0, shared->dim_n_ICU_W_D_unconf_to_conf_4, shared->dim_n_ICU_W_D_unconf_to_conf_1, shared->dim_n_ICU_W_D_unconf_to_conf_12, shared->dim_n_ICU_W_D_unconf_to_conf_123) * 1 / (real_type) 8 + odin_sum4<real_type>(internal.n_W_R_unconf_to_conf.data(), 4, 6, 0, shared->dim_n_W_R_unconf_to_conf_2, 0, shared->dim_n_W_R_unconf_to_conf_3, 0, shared->dim_n_W_R_unconf_to_conf_4, shared->dim_n_W_R_unconf_to_conf_1, shared->dim_n_W_R_unconf_to_conf_12, shared->dim_n_W_R_unconf_to_conf_123) + odin_sum4<real_type>(internal.n_W_R_unconf_to_conf.data(), 17, 18, 0, shared->dim_n_W_R_unconf_to_conf_2, 0, shared->dim_n_W_R_unconf_to_conf_3, 0, shared->dim_n_W_R_unconf_to_conf_4, shared->dim_n_W_R_unconf_to_conf_1, shared->dim_n_W_R_unconf_to_conf_12, shared->dim_n_W_R_unconf_to_conf_123) * 1 / (real_type) 8 + odin_sum4<real_type>(internal.n_W_D_unconf_to_conf.data(), 4, 6, 0, shared->dim_n_W_D_unconf_to_conf_2, 0, shared->dim_n_W_D_unconf_to_conf_3, 0, shared->dim_n_W_D_unconf_to_conf_4, shared->dim_n_W_D_unconf_to_conf_1, shared->dim_n_W_D_unconf_to_conf_12, shared->dim_n_W_D_unconf_to_conf_123) + odin_sum4<real_type>(internal.n_W_D_unconf_to_conf.data(), 17, 18, 0, shared->dim_n_W_D_unconf_to_conf_2, 0, shared->dim_n_W_D_unconf_to_conf_3, 0, shared->dim_n_W_D_unconf_to_conf_4, shared->dim_n_W_D_unconf_to_conf_1, shared->dim_n_W_D_unconf_to_conf_12, shared->dim_n_W_D_unconf_to_conf_123) * 1 / (real_type) 8;
    real_type delta_new_30_39_conf = odin_sum4<real_type>(internal.n_H_D_unconf_to_conf.data(), 6, 8, 0, shared->dim_n_H_D_unconf_to_conf_2, 0, shared->dim_n_H_D_unconf_to_conf_3, 0, shared->dim_n_H_D_unconf_to_conf_4, shared->dim_n_H_D_unconf_to_conf_1, shared->dim_n_H_D_unconf_to_conf_12, shared->dim_n_H_D_unconf_to_conf_123) + odin_sum4<real_type>(internal.n_H_D_unconf_to_conf.data(), 17, 18, 0, shared->dim_n_H_D_unconf_to_conf_2, 0, shared->dim_n_H_D_unconf_to_conf_3, 0, shared->dim_n_H_D_unconf_to_conf_4, shared->dim_n_H_D_unconf_to_conf_1, shared->dim_n_H_D_unconf_to_conf_12, shared->dim_n_H_D_unconf_to_conf_123) * 1 / (real_type) 8 + odin_sum4<real_type>(internal.n_H_R_unconf_to_conf.data(), 6, 8, 0, shared->dim_n_H_R_unconf_to_conf_2, 0, shared->dim_n_H_R_unconf_to_conf_3, 0, shared->dim_n_H_R_unconf_to_conf_4, shared->dim_n_H_R_unconf_to_conf_1, shared->dim_n_H_R_unconf_to_conf_12, shared->dim_n_H_R_unconf_to_conf_123) + odin_sum4<real_type>(internal.n_H_R_unconf_to_conf.data(), 17, 18, 0, shared->dim_n_H_R_unconf_to_conf_2, 0, shared->dim_n_H_R_unconf_to_conf_3, 0, shared->dim_n_H_R_unconf_to_conf_4, shared->dim_n_H_R_unconf_to_conf_1, shared->dim_n_H_R_unconf_to_conf_12, shared->dim_n_H_R_unconf_to_conf_123) * 1 / (real_type) 8 + odin_sum4<real_type>(internal.n_ICU_pre_unconf_to_conf.data(), 6, 8, 0, shared->dim_n_ICU_pre_unconf_to_conf_2, 0, shared->dim_n_ICU_pre_unconf_to_conf_3, 0, shared->dim_n_ICU_pre_unconf_to_conf_4, shared->dim_n_ICU_pre_unconf_to_conf_1, shared->dim_n_ICU_pre_unconf_to_conf_12, shared->dim_n_ICU_pre_unconf_to_conf_123) + odin_sum4<real_type>(internal.n_ICU_pre_unconf_to_conf.data(), 17, 18, 0, shared->dim_n_ICU_pre_unconf_to_conf_2, 0, shared->dim_n_ICU_pre_unconf_to_conf_3, 0, shared->dim_n_ICU_pre_unconf_to_conf_4, shared->dim_n_ICU_pre_unconf_to_conf_1, shared->dim_n_ICU_pre_unconf_to_conf_12, shared->dim_n_ICU_pre_unconf_to_conf_123) * 1 / (real_type) 8 + odin_sum4<real_type>(internal.n_ICU_D_unconf_to_conf.data(), 6, 8, 0, shared->dim_n_ICU_D_unconf_to_conf_2, 0, shared->dim_n_ICU_D_unconf_to_conf_3, 0, shared->dim_n_ICU_D_unconf_to_conf_4, shared->dim_n_ICU_D_unconf_to_conf_1, shared->dim_n_ICU_D_unconf_to_conf_12, shared->dim_n_ICU_D_unconf_to_conf_123) + odin_sum4<real_type>(internal.n_ICU_D_unconf_to_conf.data(), 17, 18, 0, shared->dim_n_ICU_D_unconf_to_conf_2, 0, shared->dim_n_ICU_D_unconf_to_conf_3, 0, shared->dim_n_ICU_D_unconf_to_conf_4, shared->dim_n_ICU_D_unconf_to_conf_1, shared->dim_n_ICU_D_unconf_to_conf_12, shared->dim_n_ICU_D_unconf_to_conf_123) * 1 / (real_type) 8 + odin_sum4<real_type>(internal.n_ICU_W_R_unconf_to_conf.data(), 6, 8, 0, shared->dim_n_ICU_W_R_unconf_to_conf_2, 0, shared->dim_n_ICU_W_R_unconf_to_conf_3, 0, shared->dim_n_ICU_W_R_unconf_to_conf_4, shared->dim_n_ICU_W_R_unconf_to_conf_1, shared->dim_n_ICU_W_R_unconf_to_conf_12, shared->dim_n_ICU_W_R_unconf_to_conf_123) + odin_sum4<real_type>(internal.n_ICU_W_R_unconf_to_conf.data(), 17, 18, 0, shared->dim_n_ICU_W_R_unconf_to_conf_2, 0, shared->dim_n_ICU_W_R_unconf_to_conf_3, 0, shared->dim_n_ICU_W_R_unconf_to_conf_4, shared->dim_n_ICU_W_R_unconf_to_conf_1, shared->dim_n_ICU_W_R_unconf_to_conf_12, shared->dim_n_ICU_W_R_unconf_to_conf_123) * 1 / (real_type) 8 + odin_sum4<real_type>(internal.n_ICU_W_D_unconf_to_conf.data(), 6, 8, 0, shared->dim_n_ICU_W_D_unconf_to_conf_2, 0, shared->dim_n_ICU_W_D_unconf_to_conf_3, 0, shared->dim_n_ICU_W_D_unconf_to_conf_4, shared->dim_n_ICU_W_D_unconf_to_conf_1, shared->dim_n_ICU_W_D_unconf_to_conf_12, shared->dim_n_ICU_W_D_unconf_to_conf_123) + odin_sum4<real_type>(internal.n_ICU_W_D_unconf_to_conf.data(), 17, 18, 0, shared->dim_n_ICU_W_D_unconf_to_conf_2, 0, shared->dim_n_ICU_W_D_unconf_to_conf_3, 0, shared->dim_n_ICU_W_D_unconf_to_conf_4, shared->dim_n_ICU_W_D_unconf_to_conf_1, shared->dim_n_ICU_W_D_unconf_to_conf_12, shared->dim_n_ICU_W_D_unconf_to_conf_123) * 1 / (real_type) 8 + odin_sum4<real_type>(internal.n_W_R_unconf_to_conf.data(), 6, 8, 0, shared->dim_n_W_R_unconf_to_conf_2, 0, shared->dim_n_W_R_unconf_to_conf_3, 0, shared->dim_n_W_R_unconf_to_conf_4, shared->dim_n_W_R_unconf_to_conf_1, shared->dim_n_W_R_unconf_to_conf_12, shared->dim_n_W_R_unconf_to_conf_123) + odin_sum4<real_type>(internal.n_W_R_unconf_to_conf.data(), 17, 18, 0, shared->dim_n_W_R_unconf_to_conf_2, 0, shared->dim_n_W_R_unconf_to_conf_3, 0, shared->dim_n_W_R_unconf_to_conf_4, shared->dim_n_W_R_unconf_to_conf_1, shared->dim_n_W_R_unconf_to_conf_12, shared->dim_n_W_R_unconf_to_conf_123) * 1 / (real_type) 8 + odin_sum4<real_type>(internal.n_W_D_unconf_to_conf.data(), 6, 8, 0, shared->dim_n_W_D_unconf_to_conf_2, 0, shared->dim_n_W_D_unconf_to_conf_3, 0, shared->dim_n_W_D_unconf_to_conf_4, shared->dim_n_W_D_unconf_to_conf_1, shared->dim_n_W_D_unconf_to_conf_12, shared->dim_n_W_D_unconf_to_conf_123) + odin_sum4<real_type>(internal.n_W_D_unconf_to_conf.data(), 17, 18, 0, shared->dim_n_W_D_unconf_to_conf_2, 0, shared->dim_n_W_D_unconf_to_conf_3, 0, shared->dim_n_W_D_unconf_to_conf_4, shared->dim_n_W_D_unconf_to_conf_1, shared->dim_n_W_D_unconf_to_conf_12, shared->dim_n_W_D_unconf_to_conf_123) * 1 / (real_type) 8;
    real_type delta_new_40_49_conf = odin_sum4<real_type>(internal.n_H_D_unconf_to_conf.data(), 8, 10, 0, shared->dim_n_H_D_unconf_to_conf_2, 0, shared->dim_n_H_D_unconf_to_conf_3, 0, shared->dim_n_H_D_unconf_to_conf_4, shared->dim_n_H_D_unconf_to_conf_1, shared->dim_n_H_D_unconf_to_conf_12, shared->dim_n_H_D_unconf_to_conf_123) + odin_sum4<real_type>(internal.n_H_D_unconf_to_conf.data(), 17, 18, 0, shared->dim_n_H_D_unconf_to_conf_2, 0, shared->dim_n_H_D_unconf_to_conf_3, 0, shared->dim_n_H_D_unconf_to_conf_4, shared->dim_n_H_D_unconf_to_conf_1, shared->dim_n_H_D_unconf_to_conf_12, shared->dim_n_H_D_unconf_to_conf_123) * 2 / (real_type) 8 + odin_sum4<real_type>(internal.n_H_R_unconf_to_conf.data(), 8, 10, 0, shared->dim_n_H_R_unconf_to_conf_2, 0, shared->dim_n_H_R_unconf_to_conf_3, 0, shared->dim_n_H_R_unconf_to_conf_4, shared->dim_n_H_R_unconf_to_conf_1, shared->dim_n_H_R_unconf_to_conf_12, shared->dim_n_H_R_unconf_to_conf_123) + odin_sum4<real_type>(internal.n_H_R_unconf_to_conf.data(), 17, 18, 0, shared->dim_n_H_R_unconf_to_conf_2, 0, shared->dim_n_H_R_unconf_to_conf_3, 0, shared->dim_n_H_R_unconf_to_conf_4, shared->dim_n_H_R_unconf_to_conf_1, shared->dim_n_H_R_unconf_to_conf_12, shared->dim_n_H_R_unconf_to_conf_123) * 2 / (real_type) 8 + odin_sum4<real_type>(internal.n_ICU_pre_unconf_to_conf.data(), 8, 10, 0, shared->dim_n_ICU_pre_unconf_to_conf_2, 0, shared->dim_n_ICU_pre_unconf_to_conf_3, 0, shared->dim_n_ICU_pre_unconf_to_conf_4, shared->dim_n_ICU_pre_unconf_to_conf_1, shared->dim_n_ICU_pre_unconf_to_conf_12, shared->dim_n_ICU_pre_unconf_to_conf_123) + odin_sum4<real_type>(internal.n_ICU_pre_unconf_to_conf.data(), 17, 18, 0, shared->dim_n_ICU_pre_unconf_to_conf_2, 0, shared->dim_n_ICU_pre_unconf_to_conf_3, 0, shared->dim_n_ICU_pre_unconf_to_conf_4, shared->dim_n_ICU_pre_unconf_to_conf_1, shared->dim_n_ICU_pre_unconf_to_conf_12, shared->dim_n_ICU_pre_unconf_to_conf_123) * 2 / (real_type) 8 + odin_sum4<real_type>(internal.n_ICU_D_unconf_to_conf.data(), 8, 10, 0, shared->dim_n_ICU_D_unconf_to_conf_2, 0, shared->dim_n_ICU_D_unconf_to_conf_3, 0, shared->dim_n_ICU_D_unconf_to_conf_4, shared->dim_n_ICU_D_unconf_to_conf_1, shared->dim_n_ICU_D_unconf_to_conf_12, shared->dim_n_ICU_D_unconf_to_conf_123) + odin_sum4<real_type>(internal.n_ICU_D_unconf_to_conf.data(), 17, 18, 0, shared->dim_n_ICU_D_unconf_to_conf_2, 0, shared->dim_n_ICU_D_unconf_to_conf_3, 0, shared->dim_n_ICU_D_unconf_to_conf_4, shared->dim_n_ICU_D_unconf_to_conf_1, shared->dim_n_ICU_D_unconf_to_conf_12, shared->dim_n_ICU_D_unconf_to_conf_123) * 2 / (real_type) 8 + odin_sum4<real_type>(internal.n_ICU_W_R_unconf_to_conf.data(), 8, 10, 0, shared->dim_n_ICU_W_R_unconf_to_conf_2, 0, shared->dim_n_ICU_W_R_unconf_to_conf_3, 0, shared->dim_n_ICU_W_R_unconf_to_conf_4, shared->dim_n_ICU_W_R_unconf_to_conf_1, shared->dim_n_ICU_W_R_unconf_to_conf_12, shared->dim_n_ICU_W_R_unconf_to_conf_123) + odin_sum4<real_type>(internal.n_ICU_W_R_unconf_to_conf.data(), 17, 18, 0, shared->dim_n_ICU_W_R_unconf_to_conf_2, 0, shared->dim_n_ICU_W_R_unconf_to_conf_3, 0, shared->dim_n_ICU_W_R_unconf_to_conf_4, shared->dim_n_ICU_W_R_unconf_to_conf_1, shared->dim_n_ICU_W_R_unconf_to_conf_12, shared->dim_n_ICU_W_R_unconf_to_conf_123) * 2 / (real_type) 8 + odin_sum4<real_type>(internal.n_ICU_W_D_unconf_to_conf.data(), 8, 10, 0, shared->dim_n_ICU_W_D_unconf_to_conf_2, 0, shared->dim_n_ICU_W_D_unconf_to_conf_3, 0, shared->dim_n_ICU_W_D_unconf_to_conf_4, shared->dim_n_ICU_W_D_unconf_to_conf_1, shared->dim_n_ICU_W_D_unconf_to_conf_12, shared->dim_n_ICU_W_D_unconf_to_conf_123) + odin_sum4<real_type>(internal.n_ICU_W_D_unconf_to_conf.data(), 17, 18, 0, shared->dim_n_ICU_W_D_unconf_to_conf_2, 0, shared->dim_n_ICU_W_D_unconf_to_conf_3, 0, shared->dim_n_ICU_W_D_unconf_to_conf_4, shared->dim_n_ICU_W_D_unconf_to_conf_1, shared->dim_n_ICU_W_D_unconf_to_conf_12, shared->dim_n_ICU_W_D_unconf_to_conf_123) * 2 / (real_type) 8 + odin_sum4<real_type>(internal.n_W_R_unconf_to_conf.data(), 8, 10, 0, shared->dim_n_W_R_unconf_to_conf_2, 0, shared->dim_n_W_R_unconf_to_conf_3, 0, shared->dim_n_W_R_unconf_to_conf_4, shared->dim_n_W_R_unconf_to_conf_1, shared->dim_n_W_R_unconf_to_conf_12, shared->dim_n_W_R_unconf_to_conf_123) + odin_sum4<real_type>(internal.n_W_R_unconf_to_conf.data(), 17, 18, 0, shared->dim_n_W_R_unconf_to_conf_2, 0, shared->dim_n_W_R_unconf_to_conf_3, 0, shared->dim_n_W_R_unconf_to_conf_4, shared->dim_n_W_R_unconf_to_conf_1, shared->dim_n_W_R_unconf_to_conf_12, shared->dim_n_W_R_unconf_to_conf_123) * 2 / (real_type) 8 + odin_sum4<real_type>(internal.n_W_D_unconf_to_conf.data(), 8, 10, 0, shared->dim_n_W_D_unconf_to_conf_2, 0, shared->dim_n_W_D_unconf_to_conf_3, 0, shared->dim_n_W_D_unconf_to_conf_4, shared->dim_n_W_D_unconf_to_conf_1, shared->dim_n_W_D_unconf_to_conf_12, shared->dim_n_W_D_unconf_to_conf_123) + odin_sum4<real_type>(internal.n_W_D_unconf_to_conf.data(), 17, 18, 0, shared->dim_n_W_D_unconf_to_conf_2, 0, shared->dim_n_W_D_unconf_to_conf_3, 0, shared->dim_n_W_D_unconf_to_conf_4, shared->dim_n_W_D_unconf_to_conf_1, shared->dim_n_W_D_unconf_to_conf_12, shared->dim_n_W_D_unconf_to_conf_123) * 2 / (real_type) 8;
    real_type delta_new_50_59_conf = odin_sum4<real_type>(internal.n_H_D_unconf_to_conf.data(), 10, 12, 0, shared->dim_n_H_D_unconf_to_conf_2, 0, shared->dim_n_H_D_unconf_to_conf_3, 0, shared->dim_n_H_D_unconf_to_conf_4, shared->dim_n_H_D_unconf_to_conf_1, shared->dim_n_H_D_unconf_to_conf_12, shared->dim_n_H_D_unconf_to_conf_123) + odin_sum4<real_type>(internal.n_H_D_unconf_to_conf.data(), 17, 18, 0, shared->dim_n_H_D_unconf_to_conf_2, 0, shared->dim_n_H_D_unconf_to_conf_3, 0, shared->dim_n_H_D_unconf_to_conf_4, shared->dim_n_H_D_unconf_to_conf_1, shared->dim_n_H_D_unconf_to_conf_12, shared->dim_n_H_D_unconf_to_conf_123) * 3 / (real_type) 8 + odin_sum4<real_type>(internal.n_H_R_unconf_to_conf.data(), 10, 12, 0, shared->dim_n_H_R_unconf_to_conf_2, 0, shared->dim_n_H_R_unconf_to_conf_3, 0, shared->dim_n_H_R_unconf_to_conf_4, shared->dim_n_H_R_unconf_to_conf_1, shared->dim_n_H_R_unconf_to_conf_12, shared->dim_n_H_R_unconf_to_conf_123) + odin_sum4<real_type>(internal.n_H_R_unconf_to_conf.data(), 17, 18, 0, shared->dim_n_H_R_unconf_to_conf_2, 0, shared->dim_n_H_R_unconf_to_conf_3, 0, shared->dim_n_H_R_unconf_to_conf_4, shared->dim_n_H_R_unconf_to_conf_1, shared->dim_n_H_R_unconf_to_conf_12, shared->dim_n_H_R_unconf_to_conf_123) * 3 / (real_type) 8 + odin_sum4<real_type>(internal.n_ICU_pre_unconf_to_conf.data(), 10, 12, 0, shared->dim_n_ICU_pre_unconf_to_conf_2, 0, shared->dim_n_ICU_pre_unconf_to_conf_3, 0, shared->dim_n_ICU_pre_unconf_to_conf_4, shared->dim_n_ICU_pre_unconf_to_conf_1, shared->dim_n_ICU_pre_unconf_to_conf_12, shared->dim_n_ICU_pre_unconf_to_conf_123) + odin_sum4<real_type>(internal.n_ICU_pre_unconf_to_conf.data(), 17, 18, 0, shared->dim_n_ICU_pre_unconf_to_conf_2, 0, shared->dim_n_ICU_pre_unconf_to_conf_3, 0, shared->dim_n_ICU_pre_unconf_to_conf_4, shared->dim_n_ICU_pre_unconf_to_conf_1, shared->dim_n_ICU_pre_unconf_to_conf_12, shared->dim_n_ICU_pre_unconf_to_conf_123) * 3 / (real_type) 8 + odin_sum4<real_type>(internal.n_ICU_D_unconf_to_conf.data(), 10, 12, 0, shared->dim_n_ICU_D_unconf_to_conf_2, 0, shared->dim_n_ICU_D_unconf_to_conf_3, 0, shared->dim_n_ICU_D_unconf_to_conf_4, shared->dim_n_ICU_D_unconf_to_conf_1, shared->dim_n_ICU_D_unconf_to_conf_12, shared->dim_n_ICU_D_unconf_to_conf_123) + odin_sum4<real_type>(internal.n_ICU_D_unconf_to_conf.data(), 17, 18, 0, shared->dim_n_ICU_D_unconf_to_conf_2, 0, shared->dim_n_ICU_D_unconf_to_conf_3, 0, shared->dim_n_ICU_D_unconf_to_conf_4, shared->dim_n_ICU_D_unconf_to_conf_1, shared->dim_n_ICU_D_unconf_to_conf_12, shared->dim_n_ICU_D_unconf_to_conf_123) * 3 / (real_type) 8 + odin_sum4<real_type>(internal.n_ICU_W_R_unconf_to_conf.data(), 10, 12, 0, shared->dim_n_ICU_W_R_unconf_to_conf_2, 0, shared->dim_n_ICU_W_R_unconf_to_conf_3, 0, shared->dim_n_ICU_W_R_unconf_to_conf_4, shared->dim_n_ICU_W_R_unconf_to_conf_1, shared->dim_n_ICU_W_R_unconf_to_conf_12, shared->dim_n_ICU_W_R_unconf_to_conf_123) + odin_sum4<real_type>(internal.n_ICU_W_R_unconf_to_conf.data(), 17, 18, 0, shared->dim_n_ICU_W_R_unconf_to_conf_2, 0, shared->dim_n_ICU_W_R_unconf_to_conf_3, 0, shared->dim_n_ICU_W_R_unconf_to_conf_4, shared->dim_n_ICU_W_R_unconf_to_conf_1, shared->dim_n_ICU_W_R_unconf_to_conf_12, shared->dim_n_ICU_W_R_unconf_to_conf_123) * 3 / (real_type) 8 + odin_sum4<real_type>(internal.n_ICU_W_D_unconf_to_conf.data(), 10, 12, 0, shared->dim_n_ICU_W_D_unconf_to_conf_2, 0, shared->dim_n_ICU_W_D_unconf_to_conf_3, 0, shared->dim_n_ICU_W_D_unconf_to_conf_4, shared->dim_n_ICU_W_D_unconf_to_conf_1, shared->dim_n_ICU_W_D_unconf_to_conf_12, shared->dim_n_ICU_W_D_unconf_to_conf_123) + odin_sum4<real_type>(internal.n_ICU_W_D_unconf_to_conf.data(), 17, 18, 0, shared->dim_n_ICU_W_D_unconf_to_conf_2, 0, shared->dim_n_ICU_W_D_unconf_to_conf_3, 0, shared->dim_n_ICU_W_D_unconf_to_conf_4, shared->dim_n_ICU_W_D_unconf_to_conf_1, shared->dim_n_ICU_W_D_unconf_to_conf_12, shared->dim_n_ICU_W_D_unconf_to_conf_123) * 3 / (real_type) 8 + odin_sum4<real_type>(internal.n_W_R_unconf_to_conf.data(), 10, 12, 0, shared->dim_n_W_R_unconf_to_conf_2, 0, shared->dim_n_W_R_unconf_to_conf_3, 0, shared->dim_n_W_R_unconf_to_conf_4, shared->dim_n_W_R_unconf_to_conf_1, shared->dim_n_W_R_unconf_to_conf_12, shared->dim_n_W_R_unconf_to_conf_123) + odin_sum4<real_type>(internal.n_W_R_unconf_to_conf.data(), 17, 18, 0, shared->dim_n_W_R_unconf_to_conf_2, 0, shared->dim_n_W_R_unconf_to_conf_3, 0, shared->dim_n_W_R_unconf_to_conf_4, shared->dim_n_W_R_unconf_to_conf_1, shared->dim_n_W_R_unconf_to_conf_12, shared->dim_n_W_R_unconf_to_conf_123) * 3 / (real_type) 8 + odin_sum4<real_type>(internal.n_W_D_unconf_to_conf.data(), 10, 12, 0, shared->dim_n_W_D_unconf_to_conf_2, 0, shared->dim_n_W_D_unconf_to_conf_3, 0, shared->dim_n_W_D_unconf_to_conf_4, shared->dim_n_W_D_unconf_to_conf_1, shared->dim_n_W_D_unconf_to_conf_12, shared->dim_n_W_D_unconf_to_conf_123) + odin_sum4<real_type>(internal.n_W_D_unconf_to_conf.data(), 17, 18, 0, shared->dim_n_W_D_unconf_to_conf_2, 0, shared->dim_n_W_D_unconf_to_conf_3, 0, shared->dim_n_W_D_unconf_to_conf_4, shared->dim_n_W_D_unconf_to_conf_1, shared->dim_n_W_D_unconf_to_conf_12, shared->dim_n_W_D_unconf_to_conf_123) * 3 / (real_type) 8;
    real_type delta_new_60_69_conf = odin_sum4<real_type>(internal.n_H_D_unconf_to_conf.data(), 12, 14, 0, shared->dim_n_H_D_unconf_to_conf_2, 0, shared->dim_n_H_D_unconf_to_conf_3, 0, shared->dim_n_H_D_unconf_to_conf_4, shared->dim_n_H_D_unconf_to_conf_1, shared->dim_n_H_D_unconf_to_conf_12, shared->dim_n_H_D_unconf_to_conf_123) + odin_sum4<real_type>(internal.n_H_D_unconf_to_conf.data(), 17, 18, 0, shared->dim_n_H_D_unconf_to_conf_2, 0, shared->dim_n_H_D_unconf_to_conf_3, 0, shared->dim_n_H_D_unconf_to_conf_4, shared->dim_n_H_D_unconf_to_conf_1, shared->dim_n_H_D_unconf_to_conf_12, shared->dim_n_H_D_unconf_to_conf_123) * 1 / (real_type) 8 + odin_sum4<real_type>(internal.n_H_D_unconf_to_conf.data(), 18, 19, 0, shared->dim_n_H_D_unconf_to_conf_2, 0, shared->dim_n_H_D_unconf_to_conf_3, 0, shared->dim_n_H_D_unconf_to_conf_4, shared->dim_n_H_D_unconf_to_conf_1, shared->dim_n_H_D_unconf_to_conf_12, shared->dim_n_H_D_unconf_to_conf_123) * 0.050000000000000003 + odin_sum4<real_type>(internal.n_H_R_unconf_to_conf.data(), 12, 14, 0, shared->dim_n_H_R_unconf_to_conf_2, 0, shared->dim_n_H_R_unconf_to_conf_3, 0, shared->dim_n_H_R_unconf_to_conf_4, shared->dim_n_H_R_unconf_to_conf_1, shared->dim_n_H_R_unconf_to_conf_12, shared->dim_n_H_R_unconf_to_conf_123) + odin_sum4<real_type>(internal.n_H_R_unconf_to_conf.data(), 17, 18, 0, shared->dim_n_H_R_unconf_to_conf_2, 0, shared->dim_n_H_R_unconf_to_conf_3, 0, shared->dim_n_H_R_unconf_to_conf_4, shared->dim_n_H_R_unconf_to_conf_1, shared->dim_n_H_R_unconf_to_conf_12, shared->dim_n_H_R_unconf_to_conf_123) * 1 / (real_type) 8 + odin_sum4<real_type>(internal.n_H_R_unconf_to_conf.data(), 18, 19, 0, shared->dim_n_H_R_unconf_to_conf_2, 0, shared->dim_n_H_R_unconf_to_conf_3, 0, shared->dim_n_H_R_unconf_to_conf_4, shared->dim_n_H_R_unconf_to_conf_1, shared->dim_n_H_R_unconf_to_conf_12, shared->dim_n_H_R_unconf_to_conf_123) * 0.050000000000000003 + odin_sum4<real_type>(internal.n_ICU_pre_unconf_to_conf.data(), 12, 14, 0, shared->dim_n_ICU_pre_unconf_to_conf_2, 0, shared->dim_n_ICU_pre_unconf_to_conf_3, 0, shared->dim_n_ICU_pre_unconf_to_conf_4, shared->dim_n_ICU_pre_unconf_to_conf_1, shared->dim_n_ICU_pre_unconf_to_conf_12, shared->dim_n_ICU_pre_unconf_to_conf_123) + odin_sum4<real_type>(internal.n_ICU_pre_unconf_to_conf.data(), 17, 18, 0, shared->dim_n_ICU_pre_unconf_to_conf_2, 0, shared->dim_n_ICU_pre_unconf_to_conf_3, 0, shared->dim_n_ICU_pre_unconf_to_conf_4, shared->dim_n_ICU_pre_unconf_to_conf_1, shared->dim_n_ICU_pre_unconf_to_conf_12, shared->dim_n_ICU_pre_unconf_to_conf_123) * 1 / (real_type) 8 + odin_sum4<real_type>(internal.n_ICU_pre_unconf_to_conf.data(), 18, 19, 0, shared->dim_n_ICU_pre_unconf_to_conf_2, 0, shared->dim_n_ICU_pre_unconf_to_conf_3, 0, shared->dim_n_ICU_pre_unconf_to_conf_4, shared->dim_n_ICU_pre_unconf_to_conf_1, shared->dim_n_ICU_pre_unconf_to_conf_12, shared->dim_n_ICU_pre_unconf_to_conf_123) * 0.050000000000000003 + odin_sum4<real_type>(internal.n_ICU_D_unconf_to_conf.data(), 12, 14, 0, shared->dim_n_ICU_D_unconf_to_conf_2, 0, shared->dim_n_ICU_D_unconf_to_conf_3, 0, shared->dim_n_ICU_D_unconf_to_conf_4, shared->dim_n_ICU_D_unconf_to_conf_1, shared->dim_n_ICU_D_unconf_to_conf_12, shared->dim_n_ICU_D_unconf_to_conf_123) + odin_sum4<real_type>(internal.n_ICU_D_unconf_to_conf.data(), 17, 18, 0, shared->dim_n_ICU_D_unconf_to_conf_2, 0, shared->dim_n_ICU_D_unconf_to_conf_3, 0, shared->dim_n_ICU_D_unconf_to_conf_4, shared->dim_n_ICU_D_unconf_to_conf_1, shared->dim_n_ICU_D_unconf_to_conf_12, shared->dim_n_ICU_D_unconf_to_conf_123) * 1 / (real_type) 8 + odin_sum4<real_type>(internal.n_ICU_D_unconf_to_conf.data(), 18, 19, 0, shared->dim_n_ICU_D_unconf_to_conf_2, 0, shared->dim_n_ICU_D_unconf_to_conf_3, 0, shared->dim_n_ICU_D_unconf_to_conf_4, shared->dim_n_ICU_D_unconf_to_conf_1, shared->dim_n_ICU_D_unconf_to_conf_12, shared->dim_n_ICU_D_unconf_to_conf_123) * 0.050000000000000003 + odin_sum4<real_type>(internal.n_ICU_W_R_unconf_to_conf.data(), 12, 14, 0, shared->dim_n_ICU_W_R_unconf_to_conf_2, 0, shared->dim_n_ICU_W_R_unconf_to_conf_3, 0, shared->dim_n_ICU_W_R_unconf_to_conf_4, shared->dim_n_ICU_W_R_unconf_to_conf_1, shared->dim_n_ICU_W_R_unconf_to_conf_12, shared->dim_n_ICU_W_R_unconf_to_conf_123) + odin_sum4<real_type>(internal.n_ICU_W_R_unconf_to_conf.data(), 17, 18, 0, shared->dim_n_ICU_W_R_unconf_to_conf_2, 0, shared->dim_n_ICU_W_R_unconf_to_conf_3, 0, shared->dim_n_ICU_W_R_unconf_to_conf_4, shared->dim_n_ICU_W_R_unconf_to_conf_1, shared->dim_n_ICU_W_R_unconf_to_conf_12, shared->dim_n_ICU_W_R_unconf_to_conf_123) * 1 / (real_type) 8 + odin_sum4<real_type>(internal.n_ICU_W_R_unconf_to_conf.data(), 18, 19, 0, shared->dim_n_ICU_W_R_unconf_to_conf_2, 0, shared->dim_n_ICU_W_R_unconf_to_conf_3, 0, shared->dim_n_ICU_W_R_unconf_to_conf_4, shared->dim_n_ICU_W_R_unconf_to_conf_1, shared->dim_n_ICU_W_R_unconf_to_conf_12, shared->dim_n_ICU_W_R_unconf_to_conf_123) * 0.050000000000000003 + odin_sum4<real_type>(internal.n_ICU_W_D_unconf_to_conf.data(), 12, 14, 0, shared->dim_n_ICU_W_D_unconf_to_conf_2, 0, shared->dim_n_ICU_W_D_unconf_to_conf_3, 0, shared->dim_n_ICU_W_D_unconf_to_conf_4, shared->dim_n_ICU_W_D_unconf_to_conf_1, shared->dim_n_ICU_W_D_unconf_to_conf_12, shared->dim_n_ICU_W_D_unconf_to_conf_123) + odin_sum4<real_type>(internal.n_ICU_W_D_unconf_to_conf.data(), 17, 18, 0, shared->dim_n_ICU_W_D_unconf_to_conf_2, 0, shared->dim_n_ICU_W_D_unconf_to_conf_3, 0, shared->dim_n_ICU_W_D_unconf_to_conf_4, shared->dim_n_ICU_W_D_unconf_to_conf_1, shared->dim_n_ICU_W_D_unconf_to_conf_12, shared->dim_n_ICU_W_D_unconf_to_conf_123) * 1 / (real_type) 8 + odin_sum4<real_type>(internal.n_ICU_W_D_unconf_to_conf.data(), 18, 19, 0, shared->dim_n_ICU_W_D_unconf_to_conf_2, 0, shared->dim_n_ICU_W_D_unconf_to_conf_3, 0, shared->dim_n_ICU_W_D_unconf_to_conf_4, shared->dim_n_ICU_W_D_unconf_to_conf_1, shared->dim_n_ICU_W_D_unconf_to_conf_12, shared->dim_n_ICU_W_D_unconf_to_conf_123) * 0.050000000000000003 + odin_sum4<real_type>(internal.n_W_R_unconf_to_conf.data(), 12, 14, 0, shared->dim_n_W_R_unconf_to_conf_2, 0, shared->dim_n_W_R_unconf_to_conf_3, 0, shared->dim_n_W_R_unconf_to_conf_4, shared->dim_n_W_R_unconf_to_conf_1, shared->dim_n_W_R_unconf_to_conf_12, shared->dim_n_W_R_unconf_to_conf_123) + odin_sum4<real_type>(internal.n_W_R_unconf_to_conf.data(), 17, 18, 0, shared->dim_n_W_R_unconf_to_conf_2, 0, shared->dim_n_W_R_unconf_to_conf_3, 0, shared->dim_n_W_R_unconf_to_conf_4, shared->dim_n_W_R_unconf_to_conf_1, shared->dim_n_W_R_unconf_to_conf_12, shared->dim_n_W_R_unconf_to_conf_123) * 1 / (real_type) 8 + odin_sum4<real_type>(internal.n_W_R_unconf_to_conf.data(), 18, 19, 0, shared->dim_n_W_R_unconf_to_conf_2, 0, shared->dim_n_W_R_unconf_to_conf_3, 0, shared->dim_n_W_R_unconf_to_conf_4, shared->dim_n_W_R_unconf_to_conf_1, shared->dim_n_W_R_unconf_to_conf_12, shared->dim_n_W_R_unconf_to_conf_123) * 0.050000000000000003 + odin_sum4<real_type>(internal.n_W_D_unconf_to_conf.data(), 12, 14, 0, shared->dim_n_W_D_unconf_to_conf_2, 0, shared->dim_n_W_D_unconf_to_conf_3, 0, shared->dim_n_W_D_unconf_to_conf_4, shared->dim_n_W_D_unconf_to_conf_1, shared->dim_n_W_D_unconf_to_conf_12, shared->dim_n_W_D_unconf_to_conf_123) + odin_sum4<real_type>(internal.n_W_D_unconf_to_conf.data(), 17, 18, 0, shared->dim_n_W_D_unconf_to_conf_2, 0, shared->dim_n_W_D_unconf_to_conf_3, 0, shared->dim_n_W_D_unconf_to_conf_4, shared->dim_n_W_D_unconf_to_conf_1, shared->dim_n_W_D_unconf_to_conf_12, shared->dim_n_W_D_unconf_to_conf_123) * 1 / (real_type) 8 + odin_sum4<real_type>(internal.n_W_D_unconf_to_conf.data(), 18, 19, 0, shared->dim_n_W_D_unconf_to_conf_2, 0, shared->dim_n_W_D_unconf_to_conf_3, 0, shared->dim_n_W_D_unconf_to_conf_4, shared->dim_n_W_D_unconf_to_conf_1, shared->dim_n_W_D_unconf_to_conf_12, shared->dim_n_W_D_unconf_to_conf_123) * 0.050000000000000003;
    real_type delta_new_70_79_conf = odin_sum4<real_type>(internal.n_H_D_unconf_to_conf.data(), 14, 16, 0, shared->dim_n_H_D_unconf_to_conf_2, 0, shared->dim_n_H_D_unconf_to_conf_3, 0, shared->dim_n_H_D_unconf_to_conf_4, shared->dim_n_H_D_unconf_to_conf_1, shared->dim_n_H_D_unconf_to_conf_12, shared->dim_n_H_D_unconf_to_conf_123) + odin_sum4<real_type>(internal.n_H_D_unconf_to_conf.data(), 18, 19, 0, shared->dim_n_H_D_unconf_to_conf_2, 0, shared->dim_n_H_D_unconf_to_conf_3, 0, shared->dim_n_H_D_unconf_to_conf_4, shared->dim_n_H_D_unconf_to_conf_1, shared->dim_n_H_D_unconf_to_conf_12, shared->dim_n_H_D_unconf_to_conf_123) * 0.20000000000000001 + odin_sum4<real_type>(internal.n_H_R_unconf_to_conf.data(), 14, 16, 0, shared->dim_n_H_R_unconf_to_conf_2, 0, shared->dim_n_H_R_unconf_to_conf_3, 0, shared->dim_n_H_R_unconf_to_conf_4, shared->dim_n_H_R_unconf_to_conf_1, shared->dim_n_H_R_unconf_to_conf_12, shared->dim_n_H_R_unconf_to_conf_123) + odin_sum4<real_type>(internal.n_H_R_unconf_to_conf.data(), 18, 19, 0, shared->dim_n_H_R_unconf_to_conf_2, 0, shared->dim_n_H_R_unconf_to_conf_3, 0, shared->dim_n_H_R_unconf_to_conf_4, shared->dim_n_H_R_unconf_to_conf_1, shared->dim_n_H_R_unconf_to_conf_12, shared->dim_n_H_R_unconf_to_conf_123) * 0.20000000000000001 + odin_sum4<real_type>(internal.n_ICU_pre_unconf_to_conf.data(), 14, 16, 0, shared->dim_n_ICU_pre_unconf_to_conf_2, 0, shared->dim_n_ICU_pre_unconf_to_conf_3, 0, shared->dim_n_ICU_pre_unconf_to_conf_4, shared->dim_n_ICU_pre_unconf_to_conf_1, shared->dim_n_ICU_pre_unconf_to_conf_12, shared->dim_n_ICU_pre_unconf_to_conf_123) + odin_sum4<real_type>(internal.n_ICU_pre_unconf_to_conf.data(), 18, 19, 0, shared->dim_n_ICU_pre_unconf_to_conf_2, 0, shared->dim_n_ICU_pre_unconf_to_conf_3, 0, shared->dim_n_ICU_pre_unconf_to_conf_4, shared->dim_n_ICU_pre_unconf_to_conf_1, shared->dim_n_ICU_pre_unconf_to_conf_12, shared->dim_n_ICU_pre_unconf_to_conf_123) * 0.20000000000000001 + odin_sum4<real_type>(internal.n_ICU_D_unconf_to_conf.data(), 14, 16, 0, shared->dim_n_ICU_D_unconf_to_conf_2, 0, shared->dim_n_ICU_D_unconf_to_conf_3, 0, shared->dim_n_ICU_D_unconf_to_conf_4, shared->dim_n_ICU_D_unconf_to_conf_1, shared->dim_n_ICU_D_unconf_to_conf_12, shared->dim_n_ICU_D_unconf_to_conf_123) + odin_sum4<real_type>(internal.n_ICU_D_unconf_to_conf.data(), 18, 19, 0, shared->dim_n_ICU_D_unconf_to_conf_2, 0, shared->dim_n_ICU_D_unconf_to_conf_3, 0, shared->dim_n_ICU_D_unconf_to_conf_4, shared->dim_n_ICU_D_unconf_to_conf_1, shared->dim_n_ICU_D_unconf_to_conf_12, shared->dim_n_ICU_D_unconf_to_conf_123) * 0.20000000000000001 + odin_sum4<real_type>(internal.n_ICU_W_R_unconf_to_conf.data(), 14, 16, 0, shared->dim_n_ICU_W_R_unconf_to_conf_2, 0, shared->dim_n_ICU_W_R_unconf_to_conf_3, 0, shared->dim_n_ICU_W_R_unconf_to_conf_4, shared->dim_n_ICU_W_R_unconf_to_conf_1, shared->dim_n_ICU_W_R_unconf_to_conf_12, shared->dim_n_ICU_W_R_unconf_to_conf_123) + odin_sum4<real_type>(internal.n_ICU_W_R_unconf_to_conf.data(), 18, 19, 0, shared->dim_n_ICU_W_R_unconf_to_conf_2, 0, shared->dim_n_ICU_W_R_unconf_to_conf_3, 0, shared->dim_n_ICU_W_R_unconf_to_conf_4, shared->dim_n_ICU_W_R_unconf_to_conf_1, shared->dim_n_ICU_W_R_unconf_to_conf_12, shared->dim_n_ICU_W_R_unconf_to_conf_123) * 0.20000000000000001 + odin_sum4<real_type>(internal.n_ICU_W_D_unconf_to_conf.data(), 14, 16, 0, shared->dim_n_ICU_W_D_unconf_to_conf_2, 0, shared->dim_n_ICU_W_D_unconf_to_conf_3, 0, shared->dim_n_ICU_W_D_unconf_to_conf_4, shared->dim_n_ICU_W_D_unconf_to_conf_1, shared->dim_n_ICU_W_D_unconf_to_conf_12, shared->dim_n_ICU_W_D_unconf_to_conf_123) + odin_sum4<real_type>(internal.n_ICU_W_D_unconf_to_conf.data(), 18, 19, 0, shared->dim_n_ICU_W_D_unconf_to_conf_2, 0, shared->dim_n_ICU_W_D_unconf_to_conf_3, 0, shared->dim_n_ICU_W_D_unconf_to_conf_4, shared->dim_n_ICU_W_D_unconf_to_conf_1, shared->dim_n_ICU_W_D_unconf_to_conf_12, shared->dim_n_ICU_W_D_unconf_to_conf_123) * 0.20000000000000001 + odin_sum4<real_type>(internal.n_W_R_unconf_to_conf.data(), 14, 16, 0, shared->dim_n_W_R_unconf_to_conf_2, 0, shared->dim_n_W_R_unconf_to_conf_3, 0, shared->dim_n_W_R_unconf_to_conf_4, shared->dim_n_W_R_unconf_to_conf_1, shared->dim_n_W_R_unconf_to_conf_12, shared->dim_n_W_R_unconf_to_conf_123) + odin_sum4<real_type>(internal.n_W_R_unconf_to_conf.data(), 18, 19, 0, shared->dim_n_W_R_unconf_to_conf_2, 0, shared->dim_n_W_R_unconf_to_conf_3, 0, shared->dim_n_W_R_unconf_to_conf_4, shared->dim_n_W_R_unconf_to_conf_1, shared->dim_n_W_R_unconf_to_conf_12, shared->dim_n_W_R_unconf_to_conf_123) * 0.20000000000000001 + odin_sum4<real_type>(internal.n_W_D_unconf_to_conf.data(), 14, 16, 0, shared->dim_n_W_D_unconf_to_conf_2, 0, shared->dim_n_W_D_unconf_to_conf_3, 0, shared->dim_n_W_D_unconf_to_conf_4, shared->dim_n_W_D_unconf_to_conf_1, shared->dim_n_W_D_unconf_to_conf_12, shared->dim_n_W_D_unconf_to_conf_123) + odin_sum4<real_type>(internal.n_W_D_unconf_to_conf.data(), 18, 19, 0, shared->dim_n_W_D_unconf_to_conf_2, 0, shared->dim_n_W_D_unconf_to_conf_3, 0, shared->dim_n_W_D_unconf_to_conf_4, shared->dim_n_W_D_unconf_to_conf_1, shared->dim_n_W_D_unconf_to_conf_12, shared->dim_n_W_D_unconf_to_conf_123) * 0.20000000000000001;
    real_type delta_new_80_plus_conf = odin_sum4<real_type>(internal.n_H_D_unconf_to_conf.data(), 16, 17, 0, shared->dim_n_H_D_unconf_to_conf_2, 0, shared->dim_n_H_D_unconf_to_conf_3, 0, shared->dim_n_H_D_unconf_to_conf_4, shared->dim_n_H_D_unconf_to_conf_1, shared->dim_n_H_D_unconf_to_conf_12, shared->dim_n_H_D_unconf_to_conf_123) + odin_sum4<real_type>(internal.n_H_D_unconf_to_conf.data(), 18, 19, 0, shared->dim_n_H_D_unconf_to_conf_2, 0, shared->dim_n_H_D_unconf_to_conf_3, 0, shared->dim_n_H_D_unconf_to_conf_4, shared->dim_n_H_D_unconf_to_conf_1, shared->dim_n_H_D_unconf_to_conf_12, shared->dim_n_H_D_unconf_to_conf_123) * 0.75 + odin_sum4<real_type>(internal.n_H_R_unconf_to_conf.data(), 16, 17, 0, shared->dim_n_H_R_unconf_to_conf_2, 0, shared->dim_n_H_R_unconf_to_conf_3, 0, shared->dim_n_H_R_unconf_to_conf_4, shared->dim_n_H_R_unconf_to_conf_1, shared->dim_n_H_R_unconf_to_conf_12, shared->dim_n_H_R_unconf_to_conf_123) + odin_sum4<real_type>(internal.n_H_R_unconf_to_conf.data(), 18, 19, 0, shared->dim_n_H_R_unconf_to_conf_2, 0, shared->dim_n_H_R_unconf_to_conf_3, 0, shared->dim_n_H_R_unconf_to_conf_4, shared->dim_n_H_R_unconf_to_conf_1, shared->dim_n_H_R_unconf_to_conf_12, shared->dim_n_H_R_unconf_to_conf_123) * 0.75 + odin_sum4<real_type>(internal.n_ICU_pre_unconf_to_conf.data(), 16, 17, 0, shared->dim_n_ICU_pre_unconf_to_conf_2, 0, shared->dim_n_ICU_pre_unconf_to_conf_3, 0, shared->dim_n_ICU_pre_unconf_to_conf_4, shared->dim_n_ICU_pre_unconf_to_conf_1, shared->dim_n_ICU_pre_unconf_to_conf_12, shared->dim_n_ICU_pre_unconf_to_conf_123) + odin_sum4<real_type>(internal.n_ICU_pre_unconf_to_conf.data(), 18, 19, 0, shared->dim_n_ICU_pre_unconf_to_conf_2, 0, shared->dim_n_ICU_pre_unconf_to_conf_3, 0, shared->dim_n_ICU_pre_unconf_to_conf_4, shared->dim_n_ICU_pre_unconf_to_conf_1, shared->dim_n_ICU_pre_unconf_to_conf_12, shared->dim_n_ICU_pre_unconf_to_conf_123) * 0.75 + odin_sum4<real_type>(internal.n_ICU_D_unconf_to_conf.data(), 16, 17, 0, shared->dim_n_ICU_D_unconf_to_conf_2, 0, shared->dim_n_ICU_D_unconf_to_conf_3, 0, shared->dim_n_ICU_D_unconf_to_conf_4, shared->dim_n_ICU_D_unconf_to_conf_1, shared->dim_n_ICU_D_unconf_to_conf_12, shared->dim_n_ICU_D_unconf_to_conf_123) + odin_sum4<real_type>(internal.n_ICU_D_unconf_to_conf.data(), 18, 19, 0, shared->dim_n_ICU_D_unconf_to_conf_2, 0, shared->dim_n_ICU_D_unconf_to_conf_3, 0, shared->dim_n_ICU_D_unconf_to_conf_4, shared->dim_n_ICU_D_unconf_to_conf_1, shared->dim_n_ICU_D_unconf_to_conf_12, shared->dim_n_ICU_D_unconf_to_conf_123) * 0.75 + odin_sum4<real_type>(internal.n_ICU_W_R_unconf_to_conf.data(), 16, 17, 0, shared->dim_n_ICU_W_R_unconf_to_conf_2, 0, shared->dim_n_ICU_W_R_unconf_to_conf_3, 0, shared->dim_n_ICU_W_R_unconf_to_conf_4, shared->dim_n_ICU_W_R_unconf_to_conf_1, shared->dim_n_ICU_W_R_unconf_to_conf_12, shared->dim_n_ICU_W_R_unconf_to_conf_123) + odin_sum4<real_type>(internal.n_ICU_W_R_unconf_to_conf.data(), 18, 19, 0, shared->dim_n_ICU_W_R_unconf_to_conf_2, 0, shared->dim_n_ICU_W_R_unconf_to_conf_3, 0, shared->dim_n_ICU_W_R_unconf_to_conf_4, shared->dim_n_ICU_W_R_unconf_to_conf_1, shared->dim_n_ICU_W_R_unconf_to_conf_12, shared->dim_n_ICU_W_R_unconf_to_conf_123) * 0.75 + odin_sum4<real_type>(internal.n_ICU_W_D_unconf_to_conf.data(), 16, 17, 0, shared->dim_n_ICU_W_D_unconf_to_conf_2, 0, shared->dim_n_ICU_W_D_unconf_to_conf_3, 0, shared->dim_n_ICU_W_D_unconf_to_conf_4, shared->dim_n_ICU_W_D_unconf_to_conf_1, shared->dim_n_ICU_W_D_unconf_to_conf_12, shared->dim_n_ICU_W_D_unconf_to_conf_123) + odin_sum4<real_type>(internal.n_ICU_W_D_unconf_to_conf.data(), 18, 19, 0, shared->dim_n_ICU_W_D_unconf_to_conf_2, 0, shared->dim_n_ICU_W_D_unconf_to_conf_3, 0, shared->dim_n_ICU_W_D_unconf_to_conf_4, shared->dim_n_ICU_W_D_unconf_to_conf_1, shared->dim_n_ICU_W_D_unconf_to_conf_12, shared->dim_n_ICU_W_D_unconf_to_conf_123) * 0.75 + odin_sum4<real_type>(internal.n_W_R_unconf_to_conf.data(), 16, 17, 0, shared->dim_n_W_R_unconf_to_conf_2, 0, shared->dim_n_W_R_unconf_to_conf_3, 0, shared->dim_n_W_R_unconf_to_conf_4, shared->dim_n_W_R_unconf_to_conf_1, shared->dim_n_W_R_unconf_to_conf_12, shared->dim_n_W_R_unconf_to_conf_123) + odin_sum4<real_type>(internal.n_W_R_unconf_to_conf.data(), 18, 19, 0, shared->dim_n_W_R_unconf_to_conf_2, 0, shared->dim_n_W_R_unconf_to_conf_3, 0, shared->dim_n_W_R_unconf_to_conf_4, shared->dim_n_W_R_unconf_to_conf_1, shared->dim_n_W_R_unconf_to_conf_12, shared->dim_n_W_R_unconf_to_conf_123) * 0.75 + odin_sum4<real_type>(internal.n_W_D_unconf_to_conf.data(), 16, 17, 0, shared->dim_n_W_D_unconf_to_conf_2, 0, shared->dim_n_W_D_unconf_to_conf_3, 0, shared->dim_n_W_D_unconf_to_conf_4, shared->dim_n_W_D_unconf_to_conf_1, shared->dim_n_W_D_unconf_to_conf_12, shared->dim_n_W_D_unconf_to_conf_123) + odin_sum4<real_type>(internal.n_W_D_unconf_to_conf.data(), 18, 19, 0, shared->dim_n_W_D_unconf_to_conf_2, 0, shared->dim_n_W_D_unconf_to_conf_3, 0, shared->dim_n_W_D_unconf_to_conf_4, shared->dim_n_W_D_unconf_to_conf_1, shared->dim_n_W_D_unconf_to_conf_12, shared->dim_n_W_D_unconf_to_conf_123) * 0.75;
    ...
    for (int i = 1; i <= shared->dim_n_E_vaccinated_1; ++i) {
      for (int j = 1; j <= shared->dim_n_E_vaccinated_2; ++j) {
        internal.n_E_vaccinated[i - 1 + shared->dim_n_E_vaccinated_1 * (j - 1)] = odin_sum4<real_type>(internal.n_E_next_vacc_class.data(), i - 1, i, 0, shared->dim_n_E_next_vacc_class_2, 0, shared->dim_n_E_next_vacc_class_3, j - 1, j, shared->dim_n_E_next_vacc_class_1, shared->dim_n_E_next_vacc_class_12, shared->dim_n_E_next_vacc_class_123) + ((j >= shared->vacc_skip_from && j < shared->vacc_skip_to ? odin_sum3<real_type>(internal.n_E_vacc_skip.data(), i - 1, i, 0, shared->dim_n_E_vacc_skip_2, 0, shared->dim_n_E_vacc_skip_3, shared->dim_n_E_vacc_skip_1, shared->dim_n_E_vacc_skip_12) : 0));
      }
    }
    for (int i = 1; i <= shared->dim_n_I_A_vaccinated_1; ++i) {
      for (int j = 1; j <= shared->dim_n_I_A_vaccinated_2; ++j) {
        internal.n_I_A_vaccinated[i - 1 + shared->dim_n_I_A_vaccinated_1 * (j - 1)] = odin_sum4<real_type>(internal.n_I_A_next_vacc_class.data(), i - 1, i, 0, shared->dim_n_I_A_next_vacc_class_2, 0, shared->dim_n_I_A_next_vacc_class_3, j - 1, j, shared->dim_n_I_A_next_vacc_class_1, shared->dim_n_I_A_next_vacc_class_12, shared->dim_n_I_A_next_vacc_class_123) + ((j >= shared->vacc_skip_from && j < shared->vacc_skip_to ? odin_sum3<real_type>(internal.n_I_A_vacc_skip.data(), i - 1, i, 0, shared->dim_n_I_A_vacc_skip_2, 0, shared->dim_n_I_A_vacc_skip_3, shared->dim_n_I_A_vacc_skip_1, shared->dim_n_I_A_vacc_skip_12) : 0));
      }
    }
    ...
    for (int i = 1; i <= shared->dim_n_I_P_vaccinated_1; ++i) {
      for (int j = 1; j <= shared->dim_n_I_P_vaccinated_2; ++j) {
        internal.n_I_P_vaccinated[i - 1 + shared->dim_n_I_P_vaccinated_1 * (j - 1)] = odin_sum4<real_type>(internal.n_I_P_next_vacc_class.data(), i - 1, i, 0, shared->dim_n_I_P_next_vacc_class_2, 0, shared->dim_n_I_P_next_vacc_class_3, j - 1, j, shared->dim_n_I_P_next_vacc_class_1, shared->dim_n_I_P_next_vacc_class_12, shared->dim_n_I_P_next_vacc_class_123) + ((j >= shared->vacc_skip_from && j < shared->vacc_skip_to ? odin_sum3<real_type>(internal.n_I_P_vacc_skip.data(), i - 1, i, 0, shared->dim_n_I_P_vacc_skip_2, 0, shared->dim_n_I_P_vacc_skip_3, shared->dim_n_I_P_vacc_skip_1, shared->dim_n_I_P_vacc_skip_12) : 0));
      }
    }
    ...
    for (int i = 1; i <= shared->dim_cum_n_E_vacc_skip; ++i) {
      state_next[shared->offset_variable_cum_n_E_vacc_skip + i - 1] = cum_n_E_vacc_skip[i - 1] + odin_sum3<real_type>(internal.n_E_vacc_skip.data(), i - 1, i, 0, shared->dim_n_E_vacc_skip_2, 0, shared->dim_n_E_vacc_skip_3, shared->dim_n_E_vacc_skip_1, shared->dim_n_E_vacc_skip_12);
    }
    for (int i = 1; i <= shared->dim_cum_n_I_A_vacc_skip; ++i) {
      state_next[shared->offset_variable_cum_n_I_A_vacc_skip + i - 1] = cum_n_I_A_vacc_skip[i - 1] + odin_sum3<real_type>(internal.n_I_A_vacc_skip.data(), i - 1, i, 0, shared->dim_n_I_A_vacc_skip_2, 0, shared->dim_n_I_A_vacc_skip_3, shared->dim_n_I_A_vacc_skip_1, shared->dim_n_I_A_vacc_skip_12);
    }
    for (int i = 1; i <= shared->dim_cum_n_I_P_vacc_skip; ++i) {
      state_next[shared->offset_variable_cum_n_I_P_vacc_skip + i - 1] = cum_n_I_P_vacc_skip[i - 1] + odin_sum3<real_type>(internal.n_I_P_vacc_skip.data(), i - 1, i, 0, shared->dim_n_I_P_vacc_skip_2, 0, shared->dim_n_I_P_vacc_skip_3, shared->dim_n_I_P_vacc_skip_1, shared->dim_n_I_P_vacc_skip_12);
    }
    ...
    state_next[34] = cum_new_0_9_conf + delta_new_0_9_conf;
    state_next[35] = cum_new_10_19_conf + delta_new_10_19_conf;
    state_next[36] = cum_new_20_29_conf + delta_new_20_29_conf;
    state_next[37] = cum_new_30_39_conf + delta_new_30_39_conf;
    state_next[38] = cum_new_40_49_conf + delta_new_40_49_conf;
    state_next[39] = cum_new_50_59_conf + delta_new_50_59_conf;
    state_next[40] = cum_new_60_69_conf + delta_new_60_69_conf;
    state_next[41] = cum_new_70_79_conf + delta_new_70_79_conf;
    state_next[42] = cum_new_80_plus_conf + delta_new_80_plus_conf;
    state_next[24] = cum_new_conf + delta_new_conf;
    state_next[12] = (fmodr<real_type>(step, shared->steps_per_day) == 0 ? delta_new_0_9_conf : new_0_9_conf_inc + delta_new_0_9_conf);
    state_next[13] = (fmodr<real_type>(step, shared->steps_per_day) == 0 ? delta_new_10_19_conf : new_10_19_conf_inc + delta_new_10_19_conf);
    state_next[14] = (fmodr<real_type>(step, shared->steps_per_day) == 0 ? delta_new_20_29_conf : new_20_29_conf_inc + delta_new_20_29_conf);
    state_next[15] = (fmodr<real_type>(step, shared->steps_per_day) == 0 ? delta_new_30_39_conf : new_30_39_conf_inc + delta_new_30_39_conf);
    state_next[16] = (fmodr<real_type>(step, shared->steps_per_day) == 0 ? delta_new_40_49_conf : new_40_49_conf_inc + delta_new_40_49_conf);
    state_next[17] = (fmodr<real_type>(step, shared->steps_per_day) == 0 ? delta_new_50_59_conf : new_50_59_conf_inc + delta_new_50_59_conf);
    state_next[18] = (fmodr<real_type>(step, shared->steps_per_day) == 0 ? delta_new_60_69_conf : new_60_69_conf_inc + delta_new_60_69_conf);
    state_next[19] = (fmodr<real_type>(step, shared->steps_per_day) == 0 ? delta_new_70_79_conf : new_70_79_conf_inc + delta_new_70_79_conf);
    state_next[20] = (fmodr<real_type>(step, shared->steps_per_day) == 0 ? delta_new_80_plus_conf : new_80_plus_conf_inc + delta_new_80_plus_conf);
    state_next[11] = (fmodr<real_type>(step, shared->steps_per_day) == 0 ? delta_new_conf : new_conf_inc + delta_new_conf);
    ...
    for (int i = 1; i <= shared->dim_n_S_vaccinated_1; ++i) {
      for (int j = 1; j <= shared->dim_n_S_vaccinated_2; ++j) {
        internal.n_S_vaccinated[i - 1 + shared->dim_n_S_vaccinated_1 * (j - 1)] = internal.n_S_next_vacc_class[shared->dim_n_S_next_vacc_class_1 * (j - 1) + i - 1] + ((j >= shared->vacc_skip_from && j < shared->vacc_skip_to ? internal.n_S_vacc_skip[i - 1] : 0));
      }
    }
    ...
    for (int i = 1; i <= shared->dim_cum_infections_disag_1; ++i) {
      for (int j = 1; j <= shared->dim_cum_infections_disag_2; ++j) {
        state_next[shared->offset_variable_cum_infections_disag + i - 1 + shared->dim_cum_infections_disag_1 * (j - 1)] = cum_infections_disag[shared->dim_cum_infections_disag_1 * (j - 1) + i - 1] + odin_sum3<real_type>(internal.n_S_progress.data(), i - 1, i, 0, shared->dim_n_S_progress_2, j - 1, j, shared->dim_n_S_progress_1, shared->dim_n_S_progress_12) + odin_sum3<real_type>(internal.n_RE.data(), i - 1, i, 0, shared->dim_n_RE_2, j - 1, j, shared->dim_n_RE_1, shared->dim_n_RE_12);
      }
    }
    for (int i = 1; i <= shared->dim_cum_infections_per_strain; ++i) {
      state_next[shared->offset_variable_cum_infections_per_strain + i - 1] = cum_infections_per_strain[i - 1] + odin_sum3<real_type>(internal.n_S_progress.data(), 0, shared->dim_n_S_progress_1, i - 1, i, 0, shared->dim_n_S_progress_3, shared->dim_n_S_progress_1, shared->dim_n_S_progress_12) + ((i > 2 ? (odin_sum3<real_type>(internal.n_RE.data(), 0, shared->dim_n_RE_1, i - 2 - 1, i - 2, 0, shared->dim_n_RE_3, shared->dim_n_RE_1, shared->dim_n_RE_12)) : 0));
    }
    for (int i = 1; i <= shared->dim_cum_n_S_vacc_skip; ++i) {
      state_next[97 + i - 1] = cum_n_S_vacc_skip[i - 1] + internal.n_S_vacc_skip[i - 1];
    }
    real_type delta_admit_0_9_conf = odin_sum3<real_type>(internal.n_I_C_2_to_H_D_conf.data(), 0, 2, 0, shared->dim_n_I_C_2_to_H_D_conf_2, 0, shared->dim_n_I_C_2_to_H_D_conf_3, shared->dim_n_I_C_2_to_H_D_conf_1, shared->dim_n_I_C_2_to_H_D_conf_12) + odin_sum3<real_type>(internal.n_I_C_2_to_H_R_conf.data(), 0, 2, 0, shared->dim_n_I_C_2_to_H_R_conf_2, 0, shared->dim_n_I_C_2_to_H_R_conf_3, shared->dim_n_I_C_2_to_H_R_conf_1, shared->dim_n_I_C_2_to_H_R_conf_12) + odin_sum3<real_type>(internal.n_I_C_2_to_ICU_pre_conf.data(), 0, 2, 0, shared->dim_n_I_C_2_to_ICU_pre_conf_2, 0, shared->dim_n_I_C_2_to_ICU_pre_conf_3, shared->dim_n_I_C_2_to_ICU_pre_conf_1, shared->dim_n_I_C_2_to_ICU_pre_conf_12);
    real_type delta_admit_10_19_conf = odin_sum3<real_type>(internal.n_I_C_2_to_H_D_conf.data(), 2, 4, 0, shared->dim_n_I_C_2_to_H_D_conf_2, 0, shared->dim_n_I_C_2_to_H_D_conf_3, shared->dim_n_I_C_2_to_H_D_conf_1, shared->dim_n_I_C_2_to_H_D_conf_12) + odin_sum3<real_type>(internal.n_I_C_2_to_H_R_conf.data(), 2, 4, 0, shared->dim_n_I_C_2_to_H_R_conf_2, 0, shared->dim_n_I_C_2_to_H_R_conf_3, shared->dim_n_I_C_2_to_H_R_conf_1, shared->dim_n_I_C_2_to_H_R_conf_12) + odin_sum3<real_type>(internal.n_I_C_2_to_ICU_pre_conf.data(), 2, 4, 0, shared->dim_n_I_C_2_to_ICU_pre_conf_2, 0, shared->dim_n_I_C_2_to_ICU_pre_conf_3, shared->dim_n_I_C_2_to_ICU_pre_conf_1, shared->dim_n_I_C_2_to_ICU_pre_conf_12);
    real_type delta_admit_20_29_conf = odin_sum3<real_type>(internal.n_I_C_2_to_H_D_conf.data(), 4, 6, 0, shared->dim_n_I_C_2_to_H_D_conf_2, 0, shared->dim_n_I_C_2_to_H_D_conf_3, shared->dim_n_I_C_2_to_H_D_conf_1, shared->dim_n_I_C_2_to_H_D_conf_12) + odin_sum3<real_type>(internal.n_I_C_2_to_H_D_conf.data(), 17, 18, 0, shared->dim_n_I_C_2_to_H_D_conf_2, 0, shared->dim_n_I_C_2_to_H_D_conf_3, shared->dim_n_I_C_2_to_H_D_conf_1, shared->dim_n_I_C_2_to_H_D_conf_12) * 1 / (real_type) 8 + odin_sum3<real_type>(internal.n_I_C_2_to_H_R_conf.data(), 4, 6, 0, shared->dim_n_I_C_2_to_H_R_conf_2, 0, shared->dim_n_I_C_2_to_H_R_conf_3, shared->dim_n_I_C_2_to_H_R_conf_1, shared->dim_n_I_C_2_to_H_R_conf_12) + odin_sum3<real_type>(internal.n_I_C_2_to_H_R_conf.data(), 17, 18, 0, shared->dim_n_I_C_2_to_H_R_conf_2, 0, shared->dim_n_I_C_2_to_H_R_conf_3, shared->dim_n_I_C_2_to_H_R_conf_1, shared->dim_n_I_C_2_to_H_R_conf_12) * 1 / (real_type) 8 + odin_sum3<real_type>(internal.n_I_C_2_to_ICU_pre_conf.data(), 4, 6, 0, shared->dim_n_I_C_2_to_ICU_pre_conf_2, 0, shared->dim_n_I_C_2_to_ICU_pre_conf_3, shared->dim_n_I_C_2_to_ICU_pre_conf_1, shared->dim_n_I_C_2_to_ICU_pre_conf_12) + odin_sum3<real_type>(internal.n_I_C_2_to_ICU_pre_conf.data(), 17, 18, 0, shared->dim_n_I_C_2_to_ICU_pre_conf_2, 0, shared->dim_n_I_C_2_to_ICU_pre_conf_3, shared->dim_n_I_C_2_to_ICU_pre_conf_1, shared->dim_n_I_C_2_to_ICU_pre_conf_12) * 1 / (real_type) 8;
    real_type delta_admit_30_39_conf = odin_sum3<real_type>(internal.n_I_C_2_to_H_D_conf.data(), 6, 8, 0, shared->dim_n_I_C_2_to_H_D_conf_2, 0, shared->dim_n_I_C_2_to_H_D_conf_3, shared->dim_n_I_C_2_to_H_D_conf_1, shared->dim_n_I_C_2_to_H_D_conf_12) + odin_sum3<real_type>(internal.n_I_C_2_to_H_D_conf.data(), 17, 18, 0, shared->dim_n_I_C_2_to_H_D_conf_2, 0, shared->dim_n_I_C_2_to_H_D_conf_3, shared->dim_n_I_C_2_to_H_D_conf_1, shared->dim_n_I_C_2_to_H_D_conf_12) * 1 / (real_type) 8 + odin_sum3<real_type>(internal.n_I_C_2_to_H_R_conf.data(), 6, 8, 0, shared->dim_n_I_C_2_to_H_R_conf_2, 0, shared->dim_n_I_C_2_to_H_R_conf_3, shared->dim_n_I_C_2_to_H_R_conf_1, shared->dim_n_I_C_2_to_H_R_conf_12) + odin_sum3<real_type>(internal.n_I_C_2_to_H_R_conf.data(), 17, 18, 0, shared->dim_n_I_C_2_to_H_R_conf_2, 0, shared->dim_n_I_C_2_to_H_R_conf_3, shared->dim_n_I_C_2_to_H_R_conf_1, shared->dim_n_I_C_2_to_H_R_conf_12) * 1 / (real_type) 8 + odin_sum3<real_type>(internal.n_I_C_2_to_ICU_pre_conf.data(), 6, 8, 0, shared->dim_n_I_C_2_to_ICU_pre_conf_2, 0, shared->dim_n_I_C_2_to_ICU_pre_conf_3, shared->dim_n_I_C_2_to_ICU_pre_conf_1, shared->dim_n_I_C_2_to_ICU_pre_conf_12) + odin_sum3<real_type>(internal.n_I_C_2_to_ICU_pre_conf.data(), 17, 18, 0, shared->dim_n_I_C_2_to_ICU_pre_conf_2, 0, shared->dim_n_I_C_2_to_ICU_pre_conf_3, shared->dim_n_I_C_2_to_ICU_pre_conf_1, shared->dim_n_I_C_2_to_ICU_pre_conf_12) * 1 / (real_type) 8;
    real_type delta_admit_40_49_conf = odin_sum3<real_type>(internal.n_I_C_2_to_H_D_conf.data(), 8, 10, 0, shared->dim_n_I_C_2_to_H_D_conf_2, 0, shared->dim_n_I_C_2_to_H_D_conf_3, shared->dim_n_I_C_2_to_H_D_conf_1, shared->dim_n_I_C_2_to_H_D_conf_12) + odin_sum3<real_type>(internal.n_I_C_2_to_H_D_conf.data(), 17, 18, 0, shared->dim_n_I_C_2_to_H_D_conf_2, 0, shared->dim_n_I_C_2_to_H_D_conf_3, shared->dim_n_I_C_2_to_H_D_conf_1, shared->dim_n_I_C_2_to_H_D_conf_12) * 2 / (real_type) 8 + odin_sum3<real_type>(internal.n_I_C_2_to_H_R_conf.data(), 8, 10, 0, shared->dim_n_I_C_2_to_H_R_conf_2, 0, shared->dim_n_I_C_2_to_H_R_conf_3, shared->dim_n_I_C_2_to_H_R_conf_1, shared->dim_n_I_C_2_to_H_R_conf_12) + odin_sum3<real_type>(internal.n_I_C_2_to_H_R_conf.data(), 17, 18, 0, shared->dim_n_I_C_2_to_H_R_conf_2, 0, shared->dim_n_I_C_2_to_H_R_conf_3, shared->dim_n_I_C_2_to_H_R_conf_1, shared->dim_n_I_C_2_to_H_R_conf_12) * 2 / (real_type) 8 + odin_sum3<real_type>(internal.n_I_C_2_to_ICU_pre_conf.data(), 8, 10, 0, shared->dim_n_I_C_2_to_ICU_pre_conf_2, 0, shared->dim_n_I_C_2_to_ICU_pre_conf_3, shared->dim_n_I_C_2_to_ICU_pre_conf_1, shared->dim_n_I_C_2_to_ICU_pre_conf_12) + odin_sum3<real_type>(internal.n_I_C_2_to_ICU_pre_conf.data(), 17, 18, 0, shared->dim_n_I_C_2_to_ICU_pre_conf_2, 0, shared->dim_n_I_C_2_to_ICU_pre_conf_3, shared->dim_n_I_C_2_to_ICU_pre_conf_1, shared->dim_n_I_C_2_to_ICU_pre_conf_12) * 2 / (real_type) 8;
    real_type delta_admit_50_59_conf = odin_sum3<real_type>(internal.n_I_C_2_to_H_D_conf.data(), 10, 12, 0, shared->dim_n_I_C_2_to_H_D_conf_2, 0, shared->dim_n_I_C_2_to_H_D_conf_3, shared->dim_n_I_C_2_to_H_D_conf_1, shared->dim_n_I_C_2_to_H_D_conf_12) + odin_sum3<real_type>(internal.n_I_C_2_to_H_D_conf.data(), 17, 18, 0, shared->dim_n_I_C_2_to_H_D_conf_2, 0, shared->dim_n_I_C_2_to_H_D_conf_3, shared->dim_n_I_C_2_to_H_D_conf_1, shared->dim_n_I_C_2_to_H_D_conf_12) * 3 / (real_type) 8 + odin_sum3<real_type>(internal.n_I_C_2_to_H_R_conf.data(), 10, 12, 0, shared->dim_n_I_C_2_to_H_R_conf_2, 0, shared->dim_n_I_C_2_to_H_R_conf_3, shared->dim_n_I_C_2_to_H_R_conf_1, shared->dim_n_I_C_2_to_H_R_conf_12) + odin_sum3<real_type>(internal.n_I_C_2_to_H_R_conf.data(), 17, 18, 0, shared->dim_n_I_C_2_to_H_R_conf_2, 0, shared->dim_n_I_C_2_to_H_R_conf_3, shared->dim_n_I_C_2_to_H_R_conf_1, shared->dim_n_I_C_2_to_H_R_conf_12) * 3 / (real_type) 8 + odin_sum3<real_type>(internal.n_I_C_2_to_ICU_pre_conf.data(), 10, 12, 0, shared->dim_n_I_C_2_to_ICU_pre_conf_2, 0, shared->dim_n_I_C_2_to_ICU_pre_conf_3, shared->dim_n_I_C_2_to_ICU_pre_conf_1, shared->dim_n_I_C_2_to_ICU_pre_conf_12) + odin_sum3<real_type>(internal.n_I_C_2_to_ICU_pre_conf.data(), 17, 18, 0, shared->dim_n_I_C_2_to_ICU_pre_conf_2, 0, shared->dim_n_I_C_2_to_ICU_pre_conf_3, shared->dim_n_I_C_2_to_ICU_pre_conf_1, shared->dim_n_I_C_2_to_ICU_pre_conf_12) * 3 / (real_type) 8;
    real_type delta_admit_60_69_conf = odin_sum3<real_type>(internal.n_I_C_2_to_H_D_conf.data(), 12, 14, 0, shared->dim_n_I_C_2_to_H_D_conf_2, 0, shared->dim_n_I_C_2_to_H_D_conf_3, shared->dim_n_I_C_2_to_H_D_conf_1, shared->dim_n_I_C_2_to_H_D_conf_12) + odin_sum3<real_type>(internal.n_I_C_2_to_H_D_conf.data(), 17, 18, 0, shared->dim_n_I_C_2_to_H_D_conf_2, 0, shared->dim_n_I_C_2_to_H_D_conf_3, shared->dim_n_I_C_2_to_H_D_conf_1, shared->dim_n_I_C_2_to_H_D_conf_12) * 1 / (real_type) 8 + odin_sum3<real_type>(internal.n_I_C_2_to_H_D_conf.data(), 18, 19, 0, shared->dim_n_I_C_2_to_H_D_conf_2, 0, shared->dim_n_I_C_2_to_H_D_conf_3, shared->dim_n_I_C_2_to_H_D_conf_1, shared->dim_n_I_C_2_to_H_D_conf_12) * 0.050000000000000003 + odin_sum3<real_type>(internal.n_I_C_2_to_H_R_conf.data(), 12, 14, 0, shared->dim_n_I_C_2_to_H_R_conf_2, 0, shared->dim_n_I_C_2_to_H_R_conf_3, shared->dim_n_I_C_2_to_H_R_conf_1, shared->dim_n_I_C_2_to_H_R_conf_12) + odin_sum3<real_type>(internal.n_I_C_2_to_H_R_conf.data(), 17, 18, 0, shared->dim_n_I_C_2_to_H_R_conf_2, 0, shared->dim_n_I_C_2_to_H_R_conf_3, shared->dim_n_I_C_2_to_H_R_conf_1, shared->dim_n_I_C_2_to_H_R_conf_12) * 1 / (real_type) 8 + odin_sum3<real_type>(internal.n_I_C_2_to_H_R_conf.data(), 18, 19, 0, shared->dim_n_I_C_2_to_H_R_conf_2, 0, shared->dim_n_I_C_2_to_H_R_conf_3, shared->dim_n_I_C_2_to_H_R_conf_1, shared->dim_n_I_C_2_to_H_R_conf_12) * 0.050000000000000003 + odin_sum3<real_type>(internal.n_I_C_2_to_ICU_pre_conf.data(), 12, 14, 0, shared->dim_n_I_C_2_to_ICU_pre_conf_2, 0, shared->dim_n_I_C_2_to_ICU_pre_conf_3, shared->dim_n_I_C_2_to_ICU_pre_conf_1, shared->dim_n_I_C_2_to_ICU_pre_conf_12) + odin_sum3<real_type>(internal.n_I_C_2_to_ICU_pre_conf.data(), 17, 18, 0, shared->dim_n_I_C_2_to_ICU_pre_conf_2, 0, shared->dim_n_I_C_2_to_ICU_pre_conf_3, shared->dim_n_I_C_2_to_ICU_pre_conf_1, shared->dim_n_I_C_2_to_ICU_pre_conf_12) * 1 / (real_type) 8 + odin_sum3<real_type>(internal.n_I_C_2_to_ICU_pre_conf.data(), 18, 19, 0, shared->dim_n_I_C_2_to_ICU_pre_conf_2, 0, shared->dim_n_I_C_2_to_ICU_pre_conf_3, shared->dim_n_I_C_2_to_ICU_pre_conf_1, shared->dim_n_I_C_2_to_ICU_pre_conf_12) * 0.050000000000000003;
    real_type delta_admit_70_79_conf = odin_sum3<real_type>(internal.n_I_C_2_to_H_D_conf.data(), 14, 16, 0, shared->dim_n_I_C_2_to_H_D_conf_2, 0, shared->dim_n_I_C_2_to_H_D_conf_3, shared->dim_n_I_C_2_to_H_D_conf_1, shared->dim_n_I_C_2_to_H_D_conf_12) + odin_sum3<real_type>(internal.n_I_C_2_to_H_D_conf.data(), 18, 19, 0, shared->dim_n_I_C_2_to_H_D_conf_2, 0, shared->dim_n_I_C_2_to_H_D_conf_3, shared->dim_n_I_C_2_to_H_D_conf_1, shared->dim_n_I_C_2_to_H_D_conf_12) * 0.20000000000000001 + odin_sum3<real_type>(internal.n_I_C_2_to_H_R_conf.data(), 14, 16, 0, shared->dim_n_I_C_2_to_H_R_conf_2, 0, shared->dim_n_I_C_2_to_H_R_conf_3, shared->dim_n_I_C_2_to_H_R_conf_1, shared->dim_n_I_C_2_to_H_R_conf_12) + odin_sum3<real_type>(internal.n_I_C_2_to_H_R_conf.data(), 18, 19, 0, shared->dim_n_I_C_2_to_H_R_conf_2, 0, shared->dim_n_I_C_2_to_H_R_conf_3, shared->dim_n_I_C_2_to_H_R_conf_1, shared->dim_n_I_C_2_to_H_R_conf_12) * 0.20000000000000001 + odin_sum3<real_type>(internal.n_I_C_2_to_ICU_pre_conf.data(), 14, 16, 0, shared->dim_n_I_C_2_to_ICU_pre_conf_2, 0, shared->dim_n_I_C_2_to_ICU_pre_conf_3, shared->dim_n_I_C_2_to_ICU_pre_conf_1, shared->dim_n_I_C_2_to_ICU_pre_conf_12) + odin_sum3<real_type>(internal.n_I_C_2_to_ICU_pre_conf.data(), 18, 19, 0, shared->dim_n_I_C_2_to_ICU_pre_conf_2, 0, shared->dim_n_I_C_2_to_ICU_pre_conf_3, shared->dim_n_I_C_2_to_ICU_pre_conf_1, shared->dim_n_I_C_2_to_ICU_pre_conf_12) * 0.20000000000000001;
    real_type delta_admit_80_plus_conf = odin_sum3<real_type>(internal.n_I_C_2_to_H_D_conf.data(), 16, 17, 0, shared->dim_n_I_C_2_to_H_D_conf_2, 0, shared->dim_n_I_C_2_to_H_D_conf_3, shared->dim_n_I_C_2_to_H_D_conf_1, shared->dim_n_I_C_2_to_H_D_conf_12) + odin_sum3<real_type>(internal.n_I_C_2_to_H_D_conf.data(), 18, 19, 0, shared->dim_n_I_C_2_to_H_D_conf_2, 0, shared->dim_n_I_C_2_to_H_D_conf_3, shared->dim_n_I_C_2_to_H_D_conf_1, shared->dim_n_I_C_2_to_H_D_conf_12) * 0.75 + odin_sum3<real_type>(internal.n_I_C_2_to_H_R_conf.data(), 16, 17, 0, shared->dim_n_I_C_2_to_H_R_conf_2, 0, shared->dim_n_I_C_2_to_H_R_conf_3, shared->dim_n_I_C_2_to_H_R_conf_1, shared->dim_n_I_C_2_to_H_R_conf_12) + odin_sum3<real_type>(internal.n_I_C_2_to_H_R_conf.data(), 18, 19, 0, shared->dim_n_I_C_2_to_H_R_conf_2, 0, shared->dim_n_I_C_2_to_H_R_conf_3, shared->dim_n_I_C_2_to_H_R_conf_1, shared->dim_n_I_C_2_to_H_R_conf_12) * 0.75 + odin_sum3<real_type>(internal.n_I_C_2_to_ICU_pre_conf.data(), 16, 17, 0, shared->dim_n_I_C_2_to_ICU_pre_conf_2, 0, shared->dim_n_I_C_2_to_ICU_pre_conf_3, shared->dim_n_I_C_2_to_ICU_pre_conf_1, shared->dim_n_I_C_2_to_ICU_pre_conf_12) + odin_sum3<real_type>(internal.n_I_C_2_to_ICU_pre_conf.data(), 18, 19, 0, shared->dim_n_I_C_2_to_ICU_pre_conf_2, 0, shared->dim_n_I_C_2_to_ICU_pre_conf_3, shared->dim_n_I_C_2_to_ICU_pre_conf_1, shared->dim_n_I_C_2_to_ICU_pre_conf_12) * 0.75;
    ...
    for (int i = 1; i <= shared->dim_n_R_vaccinated_1; ++i) {
      for (int j = 1; j <= shared->dim_n_R_vaccinated_2; ++j) {
        internal.n_R_vaccinated[i - 1 + shared->dim_n_R_vaccinated_1 * (j - 1)] = odin_sum3<real_type>(internal.n_R_next_vacc_class.data(), i - 1, i, 0, shared->dim_n_R_next_vacc_class_2, j - 1, j, shared->dim_n_R_next_vacc_class_1, shared->dim_n_R_next_vacc_class_12) + ((j >= shared->vacc_skip_from && j < shared->vacc_skip_to ? odin_sum2<real_type>(internal.n_R_vacc_skip.data(), i - 1, i, 0, shared->dim_n_R_vacc_skip_2, shared->dim_n_R_vacc_skip_1) : 0));
      }
    }
    for (int i = 1; i <= shared->dim_n_vacc_skip; ++i) {
      internal.n_vacc_skip[i - 1] = internal.n_S_vacc_skip[i - 1] + odin_sum3<real_type>(internal.n_E_vacc_skip.data(), i - 1, i, 0, shared->dim_n_E_vacc_skip_2, 0, shared->dim_n_E_vacc_skip_3, shared->dim_n_E_vacc_skip_1, shared->dim_n_E_vacc_skip_12) + odin_sum3<real_type>(internal.n_I_A_vacc_skip.data(), i - 1, i, 0, shared->dim_n_I_A_vacc_skip_2, 0, shared->dim_n_I_A_vacc_skip_3, shared->dim_n_I_A_vacc_skip_1, shared->dim_n_I_A_vacc_skip_12) + odin_sum3<real_type>(internal.n_I_P_vacc_skip.data(), i - 1, i, 0, shared->dim_n_I_P_vacc_skip_2, 0, shared->dim_n_I_P_vacc_skip_3, shared->dim_n_I_P_vacc_skip_1, shared->dim_n_I_P_vacc_skip_12) + odin_sum2<real_type>(internal.n_R_vacc_skip.data(), i - 1, i, 0, shared->dim_n_R_vacc_skip_2, shared->dim_n_R_vacc_skip_1);
    }
    ...
    for (int i = 1; i <= shared->dim_new_R_1; ++i) {
      for (int j = 1; j <= shared->dim_new_R_2; ++j) {
        for (int k = 1; k <= shared->dim_new_R_3; ++k) {
          internal.new_R[i - 1 + shared->dim_new_R_1 * (j - 1) + shared->dim_new_R_12 * (k - 1)] = R[shared->dim_R_12 * (k - 1) + shared->dim_R_1 * (j - 1) + i - 1] - internal.n_R_progress[shared->dim_n_R_progress_12 * (k - 1) + shared->dim_n_R_progress_1 * (j - 1) + i - 1] - internal.n_R_next_vacc_class[shared->dim_n_R_next_vacc_class_12 * (k - 1) + shared->dim_n_R_next_vacc_class_1 * (j - 1) + i - 1] + internal.n_infected_to_R[shared->dim_n_infected_to_R_12 * (k - 1) + shared->dim_n_infected_to_R_1 * (j - 1) + i - 1] + ((k == 1 ? internal.n_R_next_vacc_class[shared->dim_n_R_next_vacc_class_12 * (shared->n_vacc_classes - 1) + shared->dim_n_R_next_vacc_class_1 * (j - 1) + i - 1] : internal.n_R_next_vacc_class[shared->dim_n_R_next_vacc_class_12 * (k - 1 - 1) + shared->dim_n_R_next_vacc_class_1 * (j - 1) + i - 1])) - ((k == shared->vacc_skip_from ? internal.n_R_vacc_skip[shared->dim_n_R_vacc_skip_1 * (j - 1) + i - 1] : 0)) + ((k == shared->vacc_skip_to ? internal.n_R_vacc_skip[shared->dim_n_R_vacc_skip_1 * (j - 1) + i - 1] : 0));
        }
      }
    }
    ...
    state_next[21] = cum_infections + delta_infections;
    for (int i = 1; i <= shared->dim_cum_n_R_vacc_skip; ++i) {
      state_next[shared->offset_variable_cum_n_R_vacc_skip + i - 1] = cum_n_R_vacc_skip[i - 1] + odin_sum2<real_type>(internal.n_R_vacc_skip.data(), i - 1, i, 0, shared->dim_n_R_vacc_skip_2, shared->dim_n_R_vacc_skip_1);
    }
    ...
    state_next[22] = (fmodr<real_type>(step, shared->steps_per_day) == 0 ? delta_infections : infections_inc + delta_infections);
    state_next[96] = odin_sum4<real_type>(internal.new_T_PCR_pos.data(), 1, 18, 0, shared->dim_new_T_PCR_pos_2, 0, shared->dim_new_T_PCR_pos_3, 0, shared->dim_new_T_PCR_pos_4, shared->dim_new_T_PCR_pos_1, shared->dim_new_T_PCR_pos_12, shared->dim_new_T_PCR_pos_123);
    ...
    state_next[2] = (fmodr<real_type>(step, shared->steps_per_day) == 0 ? delta_admit_0_9_conf : admit_0_9_conf_inc + delta_admit_0_9_conf);
    state_next[3] = (fmodr<real_type>(step, shared->steps_per_day) == 0 ? delta_admit_10_19_conf : admit_10_19_conf_inc + delta_admit_10_19_conf);
    state_next[4] = (fmodr<real_type>(step, shared->steps_per_day) == 0 ? delta_admit_20_29_conf : admit_20_29_conf_inc + delta_admit_20_29_conf);
    state_next[5] = (fmodr<real_type>(step, shared->steps_per_day) == 0 ? delta_admit_30_39_conf : admit_30_39_conf_inc + delta_admit_30_39_conf);
    state_next[6] = (fmodr<real_type>(step, shared->steps_per_day) == 0 ? delta_admit_40_49_conf : admit_40_49_conf_inc + delta_admit_40_49_conf);
    state_next[7] = (fmodr<real_type>(step, shared->steps_per_day) == 0 ? delta_admit_50_59_conf : admit_50_59_conf_inc + delta_admit_50_59_conf);
    state_next[8] = (fmodr<real_type>(step, shared->steps_per_day) == 0 ? delta_admit_60_69_conf : admit_60_69_conf_inc + delta_admit_60_69_conf);
    state_next[9] = (fmodr<real_type>(step, shared->steps_per_day) == 0 ? delta_admit_70_79_conf : admit_70_79_conf_inc + delta_admit_70_79_conf);
    state_next[10] = (fmodr<real_type>(step, shared->steps_per_day) == 0 ? delta_admit_80_plus_conf : admit_80_plus_conf_inc + delta_admit_80_plus_conf);
    state_next[1] = (fmodr<real_type>(step, shared->steps_per_day) == 0 ? delta_admit_conf : admit_conf_inc + delta_admit_conf);
    state_next[25] = cum_admit_0_9_conf + delta_admit_0_9_conf;
    state_next[26] = cum_admit_10_19_conf + delta_admit_10_19_conf;
    state_next[27] = cum_admit_20_29_conf + delta_admit_20_29_conf;
    state_next[28] = cum_admit_30_39_conf + delta_admit_30_39_conf;
    state_next[29] = cum_admit_40_49_conf + delta_admit_40_49_conf;
    state_next[30] = cum_admit_50_59_conf + delta_admit_50_59_conf;
    state_next[31] = cum_admit_60_69_conf + delta_admit_60_69_conf;
    state_next[32] = cum_admit_70_79_conf + delta_admit_70_79_conf;
    state_next[33] = cum_admit_80_plus_conf + delta_admit_80_plus_conf;
    state_next[23] = cum_admit_conf + delta_admit_conf;
    ...
    for (int i = 1; i <= shared->dim_cum_n_vacc_skip; ++i) {
    ...
    state_next[48] = new_general_tot;
    state_next[49] = new_ICU_tot + new_general_tot;
...
dust::pars_type<lancelot> dust_pars<lancelot>(cpp11::list user) {
  ...
  shared->initial_admit_0_9_conf_inc = 0;
  shared->initial_admit_10_19_conf_inc = 0;
  shared->initial_admit_20_29_conf_inc = 0;
  shared->initial_admit_30_39_conf_inc = 0;
  shared->initial_admit_40_49_conf_inc = 0;
  shared->initial_admit_50_59_conf_inc = 0;
  shared->initial_admit_60_69_conf_inc = 0;
  shared->initial_admit_70_79_conf_inc = 0;
  shared->initial_admit_80_plus_conf_inc = 0;
  shared->initial_admit_conf_inc = 0;
  shared->initial_cum_admit_0_9_conf = 0;
  shared->initial_cum_admit_10_19_conf = 0;
  shared->initial_cum_admit_20_29_conf = 0;
  shared->initial_cum_admit_30_39_conf = 0;
  shared->initial_cum_admit_40_49_conf = 0;
  shared->initial_cum_admit_50_59_conf = 0;
  shared->initial_cum_admit_60_69_conf = 0;
  shared->initial_cum_admit_70_79_conf = 0;
  shared->initial_cum_admit_80_plus_conf = 0;
  shared->initial_cum_admit_conf = 0;
  shared->initial_cum_infections = 0;
  shared->initial_cum_new_0_9_conf = 0;
  shared->initial_cum_new_10_19_conf = 0;
  shared->initial_cum_new_20_29_conf = 0;
  shared->initial_cum_new_30_39_conf = 0;
  shared->initial_cum_new_40_49_conf = 0;
  shared->initial_cum_new_50_59_conf = 0;
  shared->initial_cum_new_60_69_conf = 0;
  shared->initial_cum_new_70_79_conf = 0;
  shared->initial_cum_new_80_plus_conf = 0;
  ...
  shared->initial_new_0_9_conf_inc = 0;
  shared->initial_new_10_19_conf_inc = 0;
  shared->initial_new_20_29_conf_inc = 0;
  shared->initial_new_30_39_conf_inc = 0;
  shared->initial_new_40_49_conf_inc = 0;
  shared->initial_new_50_59_conf_inc = 0;
  shared->initial_new_60_69_conf_inc = 0;
  shared->initial_new_70_79_conf_inc = 0;
  shared->initial_new_80_plus_conf_inc = 0;
  ...
  shared->N_tot_over25 = user_get_scalar<real_type>(user, "N_tot_over25", shared->N_tot_over25, NA_REAL, NA_REAL);
  ...
  shared->n_gamma_U_steps = user_get_scalar<int>(user, "n_gamma_U_steps", shared->n_gamma_U_steps, NA_REAL, NA_REAL);
  ...
  shared->dim_n_R_vacc_skip_2 = shared->n_strains;
  ...
  shared->dim_n_S_vacc_skip = shared->n_groups;
  ...
  shared->dim_n_vacc_skip = shared->n_groups;
  ...
  shared->dim_p_R_vacc_skip_2 = shared->n_strains;
  ...
  shared->dim_p_S_vacc_skip = shared->n_groups;
  ...
  shared->initial_cum_infections_per_strain = std::vector<real_type>(shared->dim_cum_infections_per_strain);
  ...
  shared->dim_n_E_vacc_skip = shared->dim_n_E_vacc_skip_1 * shared->dim_n_E_vacc_skip_2 * shared->dim_n_E_vacc_skip_3;
  ...
  shared->dim_n_I_A_vacc_skip = shared->dim_n_I_A_vacc_skip_1 * shared->dim_n_I_A_vacc_skip_2 * shared->dim_n_I_A_vacc_skip_3;
  ...
  shared->dim_n_I_P_vacc_skip = shared->dim_n_I_P_vacc_skip_1 * shared->dim_n_I_P_vacc_skip_2 * shared->dim_n_I_P_vacc_skip_3;
  ...
  shared->dim_n_R_vacc_skip = shared->dim_n_R_vacc_skip_1 * shared->dim_n_R_vacc_skip_2;
  ...
  shared->dim_p_E_vacc_skip = shared->dim_p_E_vacc_skip_1 * shared->dim_p_E_vacc_skip_2 * shared->dim_p_E_vacc_skip_3;
  ...
  shared->dim_p_I_A_vacc_skip = shared->dim_p_I_A_vacc_skip_1 * shared->dim_p_I_A_vacc_skip_2 * shared->dim_p_I_A_vacc_skip_3;
  ...
  shared->dim_p_I_P_vacc_skip = shared->dim_p_I_P_vacc_skip_1 * shared->dim_p_I_P_vacc_skip_2 * shared->dim_p_I_P_vacc_skip_3;
  ...
  shared->dim_p_R_vacc_skip = shared->dim_p_R_vacc_skip_1 * shared->dim_p_R_vacc_skip_2;
  ...
  for (int i = 1; i <= shared->dim_cum_n_E_vacc_skip; ++i) {
    shared->initial_cum_n_E_vacc_skip[i - 1] = 0;
  }
  for (int i = 1; i <= shared->dim_cum_n_I_A_vacc_skip; ++i) {
    shared->initial_cum_n_I_A_vacc_skip[i - 1] = 0;
  }
  for (int i = 1; i <= shared->dim_cum_n_I_P_vacc_skip; ++i) {
    shared->initial_cum_n_I_P_vacc_skip[i - 1] = 0;
  }
  for (int i = 1; i <= shared->dim_cum_n_R_vacc_skip; ++i) {
    shared->initial_cum_n_R_vacc_skip[i - 1] = 0;
  }
  for (int i = 1; i <= shared->dim_cum_n_S_vacc_skip; ++i) {
    shared->initial_cum_n_S_vacc_skip[i - 1] = 0;
  }
  for (int i = 1; i <= shared->dim_cum_n_vacc_skip; ++i) {
    shared->initial_cum_n_vacc_skip[i - 1] = 0;
  }
  shared->offset_variable_D_hosp = shared->dim_cum_infections_per_strain + shared->dim_cum_n_E_vacc_skip + shared->dim_cum_n_I_A_vacc_skip + shared->dim_cum_n_I_P_vacc_skip + shared->dim_cum_n_R_vacc_skip + shared->dim_cum_n_S_vacc_skip + shared->dim_cum_n_vacc_skip + 97;
  shared->offset_variable_D_non_hosp = shared->dim_D_hosp + shared->dim_cum_infections_per_strain + shared->dim_cum_n_E_vacc_skip + shared->dim_cum_n_I_A_vacc_skip + shared->dim_cum_n_I_P_vacc_skip + shared->dim_cum_n_R_vacc_skip + shared->dim_cum_n_S_vacc_skip + shared->dim_cum_n_vacc_skip + 97;
  shared->offset_variable_N_tot = shared->dim_D_hosp + shared->dim_D_non_hosp + shared->dim_cum_admit_by_age + shared->dim_cum_infections_per_strain + shared->dim_cum_n_E_vacc_skip + shared->dim_cum_n_I_A_vacc_skip + shared->dim_cum_n_I_P_vacc_skip + shared->dim_cum_n_R_vacc_skip + shared->dim_cum_n_S_vacc_skip + shared->dim_cum_n_vacc_skip + 97;
  shared->offset_variable_cum_admit_by_age = shared->dim_D_hosp + shared->dim_D_non_hosp + shared->dim_cum_infections_per_strain + shared->dim_cum_n_E_vacc_skip + shared->dim_cum_n_I_A_vacc_skip + shared->dim_cum_n_I_P_vacc_skip + shared->dim_cum_n_R_vacc_skip + shared->dim_cum_n_S_vacc_skip + shared->dim_cum_n_vacc_skip + 97;
  shared->offset_variable_cum_infections_per_strain = shared->dim_cum_n_E_vacc_skip + shared->dim_cum_n_I_A_vacc_skip + shared->dim_cum_n_I_P_vacc_skip + shared->dim_cum_n_R_vacc_skip + shared->dim_cum_n_S_vacc_skip + shared->dim_cum_n_vacc_skip + 97;
  shared->offset_variable_cum_n_E_vacc_skip = shared->dim_cum_n_S_vacc_skip + 97;
  shared->offset_variable_cum_n_I_A_vacc_skip = shared->dim_cum_n_E_vacc_skip + shared->dim_cum_n_S_vacc_skip + 97;
  shared->offset_variable_cum_n_I_P_vacc_skip = shared->dim_cum_n_E_vacc_skip + shared->dim_cum_n_I_A_vacc_skip + shared->dim_cum_n_S_vacc_skip + 97;
  shared->offset_variable_cum_n_R_vacc_skip = shared->dim_cum_n_E_vacc_skip + shared->dim_cum_n_I_A_vacc_skip + shared->dim_cum_n_I_P_vacc_skip + shared->dim_cum_n_S_vacc_skip + 97;
  shared->offset_variable_cum_n_vacc_skip = shared->dim_cum_n_E_vacc_skip + shared->dim_cum_n_I_A_vacc_skip + shared->dim_cum_n_I_P_vacc_skip + shared->dim_cum_n_R_vacc_skip + shared->dim_cum_n_S_vacc_skip + 97;
  shared->offset_variable_prob_strain = shared->dim_D_hosp + shared->dim_D_non_hosp + shared->dim_N_tot + shared->dim_cum_admit_by_age + shared->dim_cum_infections_per_strain + shared->dim_cum_n_E_vacc_skip + shared->dim_cum_n_I_A_vacc_skip + shared->dim_cum_n_I_P_vacc_skip + shared->dim_cum_n_R_vacc_skip + shared->dim_cum_n_S_vacc_skip + shared->dim_cum_n_vacc_skip + 97;
  ...
  internal.aux_E = std::vector<real_type>(shared->dim_aux_E);
  internal.aux_G_D = std::vector<real_type>(shared->dim_aux_G_D);
  ...
  shared->offset_variable_cum_n_E_vaccinated = shared->dim_D_hosp + shared->dim_D_non_hosp + shared->dim_N_tot + shared->dim_cum_admit_by_age + shared->dim_cum_infections_per_strain + shared->dim_cum_n_E_vacc_skip + shared->dim_cum_n_I_A_vacc_skip + shared->dim_cum_n_I_P_vacc_skip + shared->dim_cum_n_R_vacc_skip + shared->dim_cum_n_S_vacc_skip + shared->dim_cum_n_S_vaccinated + shared->dim_cum_n_vacc_skip + shared->dim_prob_strain + 97;
  shared->offset_variable_cum_n_I_A_vaccinated = shared->dim_D_hosp + shared->dim_D_non_hosp + shared->dim_N_tot + shared->dim_cum_admit_by_age + shared->dim_cum_infections_per_strain + shared->dim_cum_n_E_vacc_skip + shared->dim_cum_n_E_vaccinated + shared->dim_cum_n_I_A_vacc_skip + shared->dim_cum_n_I_P_vacc_skip + shared->dim_cum_n_R_vacc_skip + shared->dim_cum_n_S_vacc_skip + shared->dim_cum_n_S_vaccinated + shared->dim_cum_n_vacc_skip + shared->dim_prob_strain + 97;
  shared->offset_variable_cum_n_I_P_vaccinated = shared->dim_D_hosp + shared->dim_D_non_hosp + shared->dim_N_tot + shared->dim_cum_admit_by_age + shared->dim_cum_infections_per_strain + shared->dim_cum_n_E_vacc_skip + shared->dim_cum_n_E_vaccinated + shared->dim_cum_n_I_A_vacc_skip + shared->dim_cum_n_I_A_vaccinated + shared->dim_cum_n_I_P_vacc_skip + shared->dim_cum_n_R_vacc_skip + shared->dim_cum_n_S_vacc_skip + shared->dim_cum_n_S_vaccinated + shared->dim_cum_n_vacc_skip + shared->dim_prob_strain + 97;
...
cpp11::sexp dust_info<lancelot>(const dust::pars_type<lancelot>& pars) {
  const lancelot::internal_type internal = pars.internal;
  const std::shared_ptr<const lancelot::shared_type> shared = pars.shared;
  cpp11::writable::strings nms({"time", "admit_conf_inc", "admit_0_9_conf_inc", "admit_10_19_conf_inc", "admit_20_29_conf_inc", "admit_30_39_conf_inc", "admit_40_49_conf_inc", "admit_50_59_conf_inc", "admit_60_69_conf_inc", "admit_70_79_conf_inc", "admit_80_plus_conf_inc", "new_conf_inc", "new_0_9_conf_inc", "new_10_19_conf_inc", "new_20_29_conf_inc", "new_30_39_conf_inc", "new_40_49_conf_inc", "new_50_59_conf_inc", "new_60_69_conf_inc", "new_70_79_conf_inc", "new_80_plus_conf_inc", "cum_infections", "infections_inc", "cum_admit_conf", "cum_new_conf", "cum_admit_0_9_conf", "cum_admit_10_19_conf", "cum_admit_20_29_conf", "cum_admit_30_39_conf", "cum_admit_40_49_conf", "cum_admit_50_59_conf", "cum_admit_60_69_conf", "cum_admit_70_79_conf", "cum_admit_80_plus_conf", "cum_new_0_9_conf", "cum_new_10_19_conf", "cum_new_20_29_conf", "cum_new_30_39_conf", "cum_new_40_49_conf", "cum_new_50_59_conf", "cum_new_60_69_conf", "cum_new_70_79_conf", "cum_new_80_plus_conf", "beta_out", "N_tot_sero_1", "N_tot_sero_2", "N_tot_PCR", "ICU_tot", "general_tot", "hosp_tot", "D_hosp_tot", "D_hosp_0_49_tot", "D_hosp_50_54_tot", "D_hosp_55_59_tot", "D_hosp_60_64_tot", "D_hosp_65_69_tot", "D_hosp_70_74_tot", "D_hosp_75_79_tot", "D_hosp_80_plus_tot", "D_comm_tot", "D_comm_inc", "D_carehomes_tot", "D_carehomes_inc", "D_tot", "D_inc", "D_hosp_inc", "D_hosp_0_49_inc", "D_hosp_50_54_inc", "D_hosp_55_59_inc", "D_hosp_60_64_inc", "D_hosp_65_69_inc", "D_hosp_70_74_inc", "D_hosp_75_79_inc", "D_hosp_80_plus_inc", "sero_pos_1", "sero_pos_2", "cum_sympt_cases", "cum_sympt_cases_non_variant", "cum_sympt_cases_over25", "cum_sympt_cases_non_variant_over25", "cum_sympt_cases_under15", "cum_sympt_cases_15_24", "cum_sympt_cases_25_49", "cum_sympt_cases_50_64", "cum_sympt_cases_65_79", "cum_sympt_cases_80_plus", "sympt_cases_inc", "sympt_cases_non_variant_inc", "sympt_cases_over25_inc", "sympt_cases_non_variant_over25_inc", "sympt_cases_under15_inc", "sympt_cases_15_24_inc", "sympt_cases_25_49_inc", "sympt_cases_50_64_inc", "sympt_cases_65_79_inc", "sympt_cases_80_plus_inc", "react_pos", "cum_n_S_vacc_skip", "cum_n_E_vacc_skip", "cum_n_I_A_vacc_skip", "cum_n_I_P_vacc_skip", "cum_n_R_vacc_skip", "cum_n_vacc_skip", "cum_infections_per_strain", "D_hosp", "D_non_hosp", "cum_admit_by_age", "N_tot", "prob_strain", "cum_n_S_vaccinated", "cum_n_E_vaccinated", "cum_n_I_A_vaccinated", "cum_n_I_P_vaccinated", "cum_n_R_vaccinated", "cum_n_vaccinated", "diagnoses_admitted", "cum_infections_disag", "D", "S", "vaccine_missed_doses", "tmp_vaccine_n_candidates", "tmp_vaccine_probability", "T_sero_neg_1", "T_sero_neg_2", "R", "T_PCR_neg", "I_weighted", "E", "I_A", "I_P", "I_C_1", "I_C_2", "G_D", "ICU_pre_unconf", "ICU_pre_conf", "H_R_unconf", "H_R_conf", "H_D_unconf", "H_D_conf", "ICU_W_R_unconf", "ICU_W_R_conf", "ICU_W_D_unconf", "ICU_W_D_conf", "ICU_D_unconf", "ICU_D_conf", "W_R_unconf", "W_R_conf", "W_D_unconf", "W_D_conf", "T_sero_pre_1", "T_sero_pos_1", "T_sero_pre_2", "T_sero_pos_2", "T_PCR_pre", "T_PCR_pos"});
  cpp11::writable::list dim(155);
  ...
  dim[90] = cpp11::writable::integers({1});
  dim[91] = cpp11::writable::integers({1});
  dim[92] = cpp11::writable::integers({1});
  dim[93] = cpp11::writable::integers({1});
  dim[94] = cpp11::writable::integers({1});
  dim[95] = cpp11::writable::integers({1});
  dim[96] = cpp11::writable::integers({1});
  dim[97] = cpp11::writable::integers({shared->dim_cum_n_S_vacc_skip});
  dim[98] = cpp11::writable::integers({shared->dim_cum_n_E_vacc_skip});
  dim[99] = cpp11::writable::integers({shared->dim_cum_n_I_A_vacc_skip});
  dim[100] = cpp11::writable::integers({shared->dim_cum_n_I_P_vacc_skip});
  dim[101] = cpp11::writable::integers({shared->dim_cum_n_R_vacc_skip});
  dim[102] = cpp11::writable::integers({shared->dim_cum_n_vacc_skip});
  dim[103] = cpp11::writable::integers({shared->dim_cum_infections_per_strain});
  dim[104] = cpp11::writable::integers({shared->dim_D_hosp});
  dim[105] = cpp11::writable::integers({shared->dim_D_non_hosp});
  dim[106] = cpp11::writable::integers({shared->dim_cum_admit_by_age});
  dim[107] = cpp11::writable::integers({shared->dim_N_tot});
  dim[108] = cpp11::writable::integers({shared->dim_prob_strain});
  dim[109] = cpp11::writable::integers({shared->dim_cum_n_S_vaccinated_1, shared->dim_cum_n_S_vaccinated_2});
  dim[110] = cpp11::writable::integers({shared->dim_cum_n_E_vaccinated_1, shared->dim_cum_n_E_vaccinated_2});
  dim[111] = cpp11::writable::integers({shared->dim_cum_n_I_A_vaccinated_1, shared->dim_cum_n_I_A_vaccinated_2});
  dim[112] = cpp11::writable::integers({shared->dim_cum_n_I_P_vaccinated_1, shared->dim_cum_n_I_P_vaccinated_2});
  dim[113] = cpp11::writable::integers({shared->dim_cum_n_R_vaccinated_1, shared->dim_cum_n_R_vaccinated_2});
  dim[114] = cpp11::writable::integers({shared->dim_cum_n_vaccinated_1, shared->dim_cum_n_vaccinated_2});
  dim[115] = cpp11::writable::integers({shared->dim_diagnoses_admitted_1, shared->dim_diagnoses_admitted_2});
  dim[116] = cpp11::writable::integers({shared->dim_cum_infections_disag_1, shared->dim_cum_infections_disag_2});
  dim[117] = cpp11::writable::integers({shared->dim_D_1, shared->dim_D_2});
  dim[118] = cpp11::writable::integers({shared->dim_S_1, shared->dim_S_2});
  dim[119] = cpp11::writable::integers({shared->dim_vaccine_missed_doses_1, shared->dim_vaccine_missed_doses_2});
  dim[120] = cpp11::writable::integers({shared->dim_tmp_vaccine_n_candidates_1, shared->dim_tmp_vaccine_n_candidates_2});
  dim[121] = cpp11::writable::integers({shared->dim_tmp_vaccine_probability_1, shared->dim_tmp_vaccine_probability_2});
  dim[122] = cpp11::writable::integers({shared->dim_T_sero_neg_1_1, shared->dim_T_sero_neg_1_2, shared->dim_T_sero_neg_1_3});
  dim[123] = cpp11::writable::integers({shared->dim_T_sero_neg_2_1, shared->dim_T_sero_neg_2_2, shared->dim_T_sero_neg_2_3});
  dim[124] = cpp11::writable::integers({shared->dim_R_1, shared->dim_R_2, shared->dim_R_3});
  dim[125] = cpp11::writable::integers({shared->dim_T_PCR_neg_1, shared->dim_T_PCR_neg_2, shared->dim_T_PCR_neg_3});
  dim[126] = cpp11::writable::integers({shared->dim_I_weighted_1, shared->dim_I_weighted_2, shared->dim_I_weighted_3});
  dim[127] = cpp11::writable::integers({shared->dim_E_1, shared->dim_E_2, shared->dim_E_3, shared->dim_E_4});
  dim[128] = cpp11::writable::integers({shared->dim_I_A_1, shared->dim_I_A_2, shared->dim_I_A_3, shared->dim_I_A_4});
  dim[129] = cpp11::writable::integers({shared->dim_I_P_1, shared->dim_I_P_2, shared->dim_I_P_3, shared->dim_I_P_4});
  dim[130] = cpp11::writable::integers({shared->dim_I_C_1_1, shared->dim_I_C_1_2, shared->dim_I_C_1_3, shared->dim_I_C_1_4});
  dim[131] = cpp11::writable::integers({shared->dim_I_C_2_1, shared->dim_I_C_2_2, shared->dim_I_C_2_3, shared->dim_I_C_2_4});
  dim[132] = cpp11::writable::integers({shared->dim_G_D_1, shared->dim_G_D_2, shared->dim_G_D_3, shared->dim_G_D_4});
  dim[133] = cpp11::writable::integers({shared->dim_ICU_pre_unconf_1, shared->dim_ICU_pre_unconf_2, shared->dim_ICU_pre_unconf_3, shared->dim_ICU_pre_unconf_4});
  dim[134] = cpp11::writable::integers({shared->dim_ICU_pre_conf_1, shared->dim_ICU_pre_conf_2, shared->dim_ICU_pre_conf_3, shared->dim_ICU_pre_conf_4});
  dim[135] = cpp11::writable::integers({shared->dim_H_R_unconf_1, shared->dim_H_R_unconf_2, shared->dim_H_R_unconf_3, shared->dim_H_R_unconf_4});
  dim[136] = cpp11::writable::integers({shared->dim_H_R_conf_1, shared->dim_H_R_conf_2, shared->dim_H_R_conf_3, shared->dim_H_R_conf_4});
  dim[137] = cpp11::writable::integers({shared->dim_H_D_unconf_1, shared->dim_H_D_unconf_2, shared->dim_H_D_unconf_3, shared->dim_H_D_unconf_4});
  dim[138] = cpp11::writable::integers({shared->dim_H_D_conf_1, shared->dim_H_D_conf_2, shared->dim_H_D_conf_3, shared->dim_H_D_conf_4});
  dim[139] = cpp11::writable::integers({shared->dim_ICU_W_R_unconf_1, shared->dim_ICU_W_R_unconf_2, shared->dim_ICU_W_R_unconf_3, shared->dim_ICU_W_R_unconf_4});
  dim[140] = cpp11::writable::integers({shared->dim_ICU_W_R_conf_1, shared->dim_ICU_W_R_conf_2, shared->dim_ICU_W_R_conf_3, shared->dim_ICU_W_R_conf_4});
  dim[141] = cpp11::writable::integers({shared->dim_ICU_W_D_unconf_1, shared->dim_ICU_W_D_unconf_2, shared->dim_ICU_W_D_unconf_3, shared->dim_ICU_W_D_unconf_4});
  dim[142] = cpp11::writable::integers({shared->dim_ICU_W_D_conf_1, shared->dim_ICU_W_D_conf_2, shared->dim_ICU_W_D_conf_3, shared->dim_ICU_W_D_conf_4});
  dim[143] = cpp11::writable::integers({shared->dim_ICU_D_unconf_1, shared->dim_ICU_D_unconf_2, shared->dim_ICU_D_unconf_3, shared->dim_ICU_D_unconf_4});
  dim[144] = cpp11::writable::integers({shared->dim_ICU_D_conf_1, shared->dim_ICU_D_conf_2, shared->dim_ICU_D_conf_3, shared->dim_ICU_D_conf_4});
  dim[145] = cpp11::writable::integers({shared->dim_W_R_unconf_1, shared->dim_W_R_unconf_2, shared->dim_W_R_unconf_3, shared->dim_W_R_unconf_4});
  dim[146] = cpp11::writable::integers({shared->dim_W_R_conf_1, shared->dim_W_R_conf_2, shared->dim_W_R_conf_3, shared->dim_W_R_conf_4});
  dim[147] = cpp11::writable::integers({shared->dim_W_D_unconf_1, shared->dim_W_D_unconf_2, shared->dim_W_D_unconf_3, shared->dim_W_D_unconf_4});
  dim[148] = cpp11::writable::integers({shared->dim_W_D_conf_1, shared->dim_W_D_conf_2, shared->dim_W_D_conf_3, shared->dim_W_D_conf_4});
  dim[149] = cpp11::writable::integers({shared->dim_T_sero_pre_1_1, shared->dim_T_sero_pre_1_2, shared->dim_T_sero_pre_1_3, shared->dim_T_sero_pre_1_4});
  dim[150] = cpp11::writable::integers({shared->dim_T_sero_pos_1_1, shared->dim_T_sero_pos_1_2, shared->dim_T_sero_pos_1_3, shared->dim_T_sero_pos_1_4});
  dim[151] = cpp11::writable::integers({shared->dim_T_sero_pre_2_1, shared->dim_T_sero_pre_2_2, shared->dim_T_sero_pre_2_3, shared->dim_T_sero_pre_2_4});
  dim[152] = cpp11::writable::integers({shared->dim_T_sero_pos_2_1, shared->dim_T_sero_pos_2_2, shared->dim_T_sero_pos_2_3, shared->dim_T_sero_pos_2_4});
  dim[153] = cpp11::writable::integers({shared->dim_T_PCR_pre_1, shared->dim_T_PCR_pre_2, shared->dim_T_PCR_pre_3, shared->dim_T_PCR_pre_4});
  dim[154] = cpp11::writable::integers({shared->dim_T_PCR_pos_1, shared->dim_T_PCR_pos_2, shared->dim_T_PCR_pos_3, shared->dim_T_PCR_pos_4});
  dim.names() = nms;
  cpp11::writable::list index(155);
  ...
  index[90] = cpp11::writable::integers({91});
  index[91] = cpp11::writable::integers({92});
  index[92] = cpp11::writable::integers({93});
  index[93] = cpp11::writable::integers({94});
  index[94] = cpp11::writable::integers({95});
  index[95] = cpp11::writable::integers({96});
  index[96] = cpp11::writable::integers({97});
  index[97] = integer_sequence(98, shared->dim_cum_n_S_vacc_skip);
  index[98] = integer_sequence(shared->offset_variable_cum_n_E_vacc_skip + 1, shared->dim_cum_n_E_vacc_skip);
  index[99] = integer_sequence(shared->offset_variable_cum_n_I_A_vacc_skip + 1, shared->dim_cum_n_I_A_vacc_skip);
  index[100] = integer_sequence(shared->offset_variable_cum_n_I_P_vacc_skip + 1, shared->dim_cum_n_I_P_vacc_skip);
  index[101] = integer_sequence(shared->offset_variable_cum_n_R_vacc_skip + 1, shared->dim_cum_n_R_vacc_skip);
  index[102] = integer_sequence(shared->offset_variable_cum_n_vacc_skip + 1, shared->dim_cum_n_vacc_skip);
  index[103] = integer_sequence(shared->offset_variable_cum_infections_per_strain + 1, shared->dim_cum_infections_per_strain);
  index[104] = integer_sequence(shared->offset_variable_D_hosp + 1, shared->dim_D_hosp);
  index[105] = integer_sequence(shared->offset_variable_D_non_hosp + 1, shared->dim_D_non_hosp);
  index[106] = integer_sequence(shared->offset_variable_cum_admit_by_age + 1, shared->dim_cum_admit_by_age);
  index[107] = integer_sequence(shared->offset_variable_N_tot + 1, shared->dim_N_tot);
  index[108] = integer_sequence(shared->offset_variable_prob_strain + 1, shared->dim_prob_strain);
  index[109] = integer_sequence(shared->offset_variable_cum_n_S_vaccinated + 1, shared->dim_cum_n_S_vaccinated);
  index[110] = integer_sequence(shared->offset_variable_cum_n_E_vaccinated + 1, shared->dim_cum_n_E_vaccinated);
  index[111] = integer_sequence(shared->offset_variable_cum_n_I_A_vaccinated + 1, shared->dim_cum_n_I_A_vaccinated);
  index[112] = integer_sequence(shared->offset_variable_cum_n_I_P_vaccinated + 1, shared->dim_cum_n_I_P_vaccinated);
  index[113] = integer_sequence(shared->offset_variable_cum_n_R_vaccinated + 1, shared->dim_cum_n_R_vaccinated);
  index[114] = integer_sequence(shared->offset_variable_cum_n_vaccinated + 1, shared->dim_cum_n_vaccinated);
  index[115] = integer_sequence(shared->offset_variable_diagnoses_admitted + 1, shared->dim_diagnoses_admitted);
  index[116] = integer_sequence(shared->offset_variable_cum_infections_disag + 1, shared->dim_cum_infections_disag);
  index[117] = integer_sequence(shared->offset_variable_D + 1, shared->dim_D);
  index[118] = integer_sequence(shared->offset_variable_S + 1, shared->dim_S);
  index[119] = integer_sequence(shared->offset_variable_vaccine_missed_doses + 1, shared->dim_vaccine_missed_doses);
  index[120] = integer_sequence(shared->offset_variable_tmp_vaccine_n_candidates + 1, shared->dim_tmp_vaccine_n_candidates);
  index[121] = integer_sequence(shared->offset_variable_tmp_vaccine_probability + 1, shared->dim_tmp_vaccine_probability);
  index[122] = integer_sequence(shared->offset_variable_T_sero_neg_1 + 1, shared->dim_T_sero_neg_1);
  index[123] = integer_sequence(shared->offset_variable_T_sero_neg_2 + 1, shared->dim_T_sero_neg_2);
  index[124] = integer_sequence(shared->offset_variable_R + 1, shared->dim_R);
  index[125] = integer_sequence(shared->offset_variable_T_PCR_neg + 1, shared->dim_T_PCR_neg);
  index[126] = integer_sequence(shared->offset_variable_I_weighted + 1, shared->dim_I_weighted);
  index[127] = integer_sequence(shared->offset_variable_E + 1, shared->dim_E);
  index[128] = integer_sequence(shared->offset_variable_I_A + 1, shared->dim_I_A);
  index[129] = integer_sequence(shared->offset_variable_I_P + 1, shared->dim_I_P);
  index[130] = integer_sequence(shared->offset_variable_I_C_1 + 1, shared->dim_I_C_1);
  index[131] = integer_sequence(shared->offset_variable_I_C_2 + 1, shared->dim_I_C_2);
  index[132] = integer_sequence(shared->offset_variable_G_D + 1, shared->dim_G_D);
  index[133] = integer_sequence(shared->offset_variable_ICU_pre_unconf + 1, shared->dim_ICU_pre_unconf);
  index[134] = integer_sequence(shared->offset_variable_ICU_pre_conf + 1, shared->dim_ICU_pre_conf);
  index[135] = integer_sequence(shared->offset_variable_H_R_unconf + 1, shared->dim_H_R_unconf);
  index[136] = integer_sequence(shared->offset_variable_H_R_conf + 1, shared->dim_H_R_conf);
  index[137] = integer_sequence(shared->offset_variable_H_D_unconf + 1, shared->dim_H_D_unconf);
  index[138] = integer_sequence(shared->offset_variable_H_D_conf + 1, shared->dim_H_D_conf);
  index[139] = integer_sequence(shared->offset_variable_ICU_W_R_unconf + 1, shared->dim_ICU_W_R_unconf);
  index[140] = integer_sequence(shared->offset_variable_ICU_W_R_conf + 1, shared->dim_ICU_W_R_conf);
  index[141] = integer_sequence(shared->offset_variable_ICU_W_D_unconf + 1, shared->dim_ICU_W_D_unconf);
  index[142] = integer_sequence(shared->offset_variable_ICU_W_D_conf + 1, shared->dim_ICU_W_D_conf);
  index[143] = integer_sequence(shared->offset_variable_ICU_D_unconf + 1, shared->dim_ICU_D_unconf);
  index[144] = integer_sequence(shared->offset_variable_ICU_D_conf + 1, shared->dim_ICU_D_conf);
  index[145] = integer_sequence(shared->offset_variable_W_R_unconf + 1, shared->dim_W_R_unconf);
  index[146] = integer_sequence(shared->offset_variable_W_R_conf + 1, shared->dim_W_R_conf);
  index[147] = integer_sequence(shared->offset_variable_W_D_unconf + 1, shared->dim_W_D_unconf);
  index[148] = integer_sequence(shared->offset_variable_W_D_conf + 1, shared->dim_W_D_conf);
  index[149] = integer_sequence(shared->offset_variable_T_sero_pre_1 + 1, shared->dim_T_sero_pre_1);
  index[150] = integer_sequence(shared->offset_variable_T_sero_pos_1 + 1, shared->dim_T_sero_pos_1);
  index[151] = integer_sequence(shared->offset_variable_T_sero_pre_2 + 1, shared->dim_T_sero_pre_2);
  index[152] = integer_sequence(shared->offset_variable_T_sero_pos_2 + 1, shared->dim_T_sero_pos_2);
  index[153] = integer_sequence(shared->offset_variable_T_PCR_pre + 1, shared->dim_T_PCR_pre);
  index[154] = integer_sequence(shared->offset_variable_T_PCR_pos + 1, shared->dim_T_PCR_pos);
  index.names() = nms;
...
using model_cpu = dust::dust_cpu<lancelot>;

SEXP dust_cpu_lancelot_alloc(cpp11::list r_pars, bool pars_multi, size_t step,
                             cpp11::sexp r_n_particles, size_t n_threads,
                             cpp11::sexp r_seed, bool deterministic,
                             cpp11::sexp gpu_config) {
  return dust::r::dust_cpu_alloc<lancelot>(r_pars, pars_multi, step, r_n_particles,
