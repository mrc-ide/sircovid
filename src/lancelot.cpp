...
public:
  ...
  struct shared_type {
    ...
    int dim_delta_hospitalisations_by_age;
    int dim_delta_hospitalisations_by_strain;
    ...
    int dim_delta_infections_by_age;
    ...
    int dim_hfr_disag_2;
    int dim_hfr_strain;
    int dim_hospitalisations_inc_by_age;
    int dim_hospitalisations_inc_by_strain;
    int dim_ifr_age;
    int dim_ifr_disag;
    int dim_ifr_disag_1;
    int dim_ifr_disag_2;
    ...
    real_type initial_HF_expected_inc;
    ...
    real_type initial_IF_expected_inc;
    real_type initial_IH_expected_inc;
    ...
    std::vector<real_type> initial_hospitalisations_inc_by_age;
    std::vector<real_type> initial_hospitalisations_inc_by_strain;
    ...
    std::vector<real_type> initial_infections_inc_by_age;
    ...
    int offset_variable_hospitalisations_inc_by_age;
    int offset_variable_hospitalisations_inc_by_strain;
    ...
    int offset_variable_infections_inc_by_age;
  ...
  struct internal_type {
    ...
    std::vector<real_type> delta_hospitalisations_by_age;
    std::vector<real_type> delta_hospitalisations_by_strain;
    std::vector<real_type> delta_infections;
    std::vector<real_type> delta_infections_by_age;
  ...
  std::vector<real_type> initial(size_t step) {
    ...
    state[86] = shared->initial_IH_expected_inc;
    state[87] = shared->initial_ihr;
    state[88] = shared->initial_HF_expected_inc;
    state[89] = shared->initial_hfr;
    state[90] = shared->initial_IF_expected_inc;
    state[91] = shared->initial_ifr;
    std::copy(shared->initial_cum_infections_per_strain.begin(), shared->initial_cum_infections_per_strain.end(), state.begin() + 92);
    ...
    std::copy(shared->initial_infections_inc_by_age.begin(), shared->initial_infections_inc_by_age.end(), state.begin() + shared->offset_variable_infections_inc_by_age);
    std::copy(shared->initial_hospitalisations_inc_by_strain.begin(), shared->initial_hospitalisations_inc_by_strain.end(), state.begin() + shared->offset_variable_hospitalisations_inc_by_strain);
    std::copy(shared->initial_hospitalisations_inc_by_age.begin(), shared->initial_hospitalisations_inc_by_age.end(), state.begin() + shared->offset_variable_hospitalisations_inc_by_age);
  ...
  void update(size_t step, const real_type * state, rng_state_type& rng_state, real_type * state_next) {
    ...
    const real_type * cum_infections_per_strain = state + 92;
    const real_type * infections_inc_per_strain = state + shared->offset_variable_infections_inc_per_strain;
    const real_type * infections_inc_by_age = state + shared->offset_variable_infections_inc_by_age;
    const real_type hospitalisations_inc = state[14];
    const real_type * hospitalisations_inc_by_strain = state + shared->offset_variable_hospitalisations_inc_by_strain;
    const real_type * hospitalisations_inc_by_age = state + shared->offset_variable_hospitalisations_inc_by_age;
    ...
    const real_type IH_expected_inc = state[86];
    const real_type HF_expected_inc = state[88];
    const real_type IF_expected_inc = state[90];
    ...
    real_type new_HF_expected_inc = (fmodr<real_type>(step, shared->steps_per_day) == 0 ? odin_sum1<real_type>(internal.HFR_disag_weighted.data(), 0, shared->dim_HFR_disag_weighted) : HF_expected_inc + odin_sum1<real_type>(internal.HFR_disag_weighted.data(), 0, shared->dim_HFR_disag_weighted));
    ...
    state_next[88] = new_HF_expected_inc;
    ...
    state_next[89] = new_HF_expected_inc / (real_type) new_hospitalisations_inc;
    ...
    for (int i = 1; i <= shared->dim_delta_infections_by_age; ++i) {
      internal.delta_infections_by_age[i - 1] = odin_sum3<real_type>(internal.delta_infections.data(), i - 1, i, 0, shared->dim_delta_infections_2, 0, shared->dim_delta_infections_3, shared->dim_delta_infections_1, shared->dim_delta_infections_12);
    }
    for (int i = 1; i <= shared->dim_delta_infections_per_strain; ++i) {
      internal.delta_infections_per_strain[i - 1] = odin_sum3<real_type>(internal.delta_infections.data(), 0, shared->dim_delta_infections_1, i - 1, i, 0, shared->dim_delta_infections_3, shared->dim_delta_infections_1, shared->dim_delta_infections_12);
    }
    ...
    real_type new_IF_expected_inc = (fmodr<real_type>(step, shared->steps_per_day) == 0 ? odin_sum1<real_type>(internal.IFR_disag_weighted.data(), 0, shared->dim_IFR_disag_weighted) : IF_expected_inc + odin_sum1<real_type>(internal.IFR_disag_weighted.data(), 0, shared->dim_IFR_disag_weighted));
    real_type new_IH_expected_inc = (fmodr<real_type>(step, shared->steps_per_day) == 0 ? odin_sum1<real_type>(internal.IHR_disag_weighted.data(), 0, shared->dim_IHR_disag_weighted) : IH_expected_inc + odin_sum1<real_type>(internal.IHR_disag_weighted.data(), 0, shared->dim_IHR_disag_weighted));
    ...
    for (int i = 1; i <= shared->dim_cum_infections_per_strain; ++i) {
      state_next[92 + i - 1] = cum_infections_per_strain[i - 1] + internal.delta_infections_per_strain[i - 1];
    }
    ...
    state_next[90] = new_IF_expected_inc;
    state_next[86] = new_IH_expected_inc;
    state_next[91] = new_IF_expected_inc / (real_type) new_infections_inc;
    state_next[87] = new_IH_expected_inc / (real_type) new_infections_inc;
...
dust::pars_type<lancelot> dust_pars<lancelot>(cpp11::list user) {
  ...
  shared->initial_HF_expected_inc = 0;
  shared->initial_ICU_tot = 0;
  shared->initial_IF_expected_inc = 0;
  shared->initial_IH_expected_inc = 0;
  ...
  shared->dim_delta_hospitalisations_by_age = shared->n_groups;
  shared->dim_delta_hospitalisations_by_strain = shared->n_strains;
  ...
  shared->dim_delta_infections_by_age = shared->n_groups;
  ...
  shared->dim_hospitalisations_inc_by_age = shared->n_groups;
  shared->dim_hospitalisations_inc_by_strain = shared->n_strains;
  ...
  shared->dim_infections_inc_by_age = shared->n_groups;
  ...
  internal.delta_hospitalisations_by_age = std::vector<real_type>(shared->dim_delta_hospitalisations_by_age);
  internal.delta_hospitalisations_by_strain = std::vector<real_type>(shared->dim_delta_hospitalisations_by_strain);
  internal.delta_infections_by_age = std::vector<real_type>(shared->dim_delta_infections_by_age);
  ...
  shared->initial_hospitalisations_inc_by_age = std::vector<real_type>(shared->dim_hospitalisations_inc_by_age);
  shared->initial_hospitalisations_inc_by_strain = std::vector<real_type>(shared->dim_hospitalisations_inc_by_strain);
  ...
  for (int i = 1; i <= shared->dim_hospitalisations_inc_by_age; ++i) {
    shared->initial_hospitalisations_inc_by_age[i - 1] = 0;
  }
  for (int i = 1; i <= shared->dim_hospitalisations_inc_by_strain; ++i) {
    shared->initial_hospitalisations_inc_by_strain[i - 1] = 0;
  }
  for (int i = 1; i <= shared->dim_ifr_age; ++i) {
  ...
  for (int i = 1; i <= shared->dim_infections_inc_by_age; ++i) {
    shared->initial_infections_inc_by_age[i - 1] = 0;
  }
  for (int i = 1; i <= shared->dim_infections_inc_per_strain; ++i) {
    shared->initial_infections_inc_per_strain[i - 1] = 0;
  }
  shared->offset_variable_D_non_hosp = shared->dim_cum_infections_per_strain + shared->dim_hospitalisations_inc_by_age + shared->dim_hospitalisations_inc_by_strain + shared->dim_infections_inc_by_age + shared->dim_infections_inc_per_strain + 92;
  shared->offset_variable_N_tot = shared->dim_D_non_hosp + shared->dim_cum_admit_by_age + shared->dim_cum_infections_per_strain + shared->dim_hospitalisations_inc_by_age + shared->dim_hospitalisations_inc_by_strain + shared->dim_infections_inc_by_age + shared->dim_infections_inc_per_strain + 92;
  shared->offset_variable_cum_admit_by_age = shared->dim_D_non_hosp + shared->dim_cum_infections_per_strain + shared->dim_hospitalisations_inc_by_age + shared->dim_hospitalisations_inc_by_strain + shared->dim_infections_inc_by_age + shared->dim_infections_inc_per_strain + 92;
  shared->offset_variable_hospitalisations_inc_by_age = shared->dim_cum_infections_per_strain + shared->dim_hospitalisations_inc_by_strain + shared->dim_infections_inc_by_age + shared->dim_infections_inc_per_strain + 92;
  shared->offset_variable_hospitalisations_inc_by_strain = shared->dim_cum_infections_per_strain + shared->dim_infections_inc_by_age + shared->dim_infections_inc_per_strain + 92;
  shared->offset_variable_infections_inc_by_age = shared->dim_cum_infections_per_strain + shared->dim_infections_inc_per_strain + 92;
  shared->offset_variable_infections_inc_per_strain = shared->dim_cum_infections_per_strain + 92;
  shared->offset_variable_prob_strain = shared->dim_D_non_hosp + shared->dim_N_tot + shared->dim_cum_admit_by_age + shared->dim_cum_infections_per_strain + shared->dim_hospitalisations_inc_by_age + shared->dim_hospitalisations_inc_by_strain + shared->dim_infections_inc_by_age + shared->dim_infections_inc_per_strain + 92;
  ...
  shared->offset_variable_D = shared->dim_D_non_hosp + shared->dim_N_tot + shared->dim_cum_admit_by_age + shared->dim_cum_infections_disag + shared->dim_cum_infections_per_strain + shared->dim_cum_n_E_vacc_skip + shared->dim_cum_n_E_vaccinated + shared->dim_cum_n_I_A_vacc_skip + shared->dim_cum_n_I_A_vaccinated + shared->dim_cum_n_I_P_vacc_skip + shared->dim_cum_n_I_P_vaccinated + shared->dim_cum_n_R_vacc_skip + shared->dim_cum_n_R_vaccinated + shared->dim_cum_n_S_vacc_skip + shared->dim_cum_n_S_vaccinated + shared->dim_cum_n_vacc_skip + shared->dim_cum_n_vaccinated + shared->dim_diagnoses_admitted + shared->dim_effective_susceptible + shared->dim_hfr_age + shared->dim_hfr_strain + shared->dim_hospitalisations_inc_by_age + shared->dim_hospitalisations_inc_by_strain + shared->dim_ifr_age + shared->dim_ifr_strain + shared->dim_ihr_age + shared->dim_ihr_strain + shared->dim_infections_inc_by_age + shared->dim_infections_inc_per_strain + shared->dim_prob_strain + 92;
  ...
  shared->offset_variable_hfr_strain = shared->dim_D_non_hosp + shared->dim_N_tot + shared->dim_cum_admit_by_age + shared->dim_cum_infections_per_strain + shared->dim_effective_susceptible + shared->dim_hospitalisations_inc_by_age + shared->dim_hospitalisations_inc_by_strain + shared->dim_ifr_strain + shared->dim_ihr_strain + shared->dim_infections_inc_by_age + shared->dim_infections_inc_per_strain + shared->dim_prob_strain + 92;
...
cpp11::sexp dust_info<lancelot>(const dust::pars_type<lancelot>& pars) {
  ...
  dim[92] = cpp11::writable::integers({shared->dim_cum_infections_per_strain});
  dim[93] = cpp11::writable::integers({shared->dim_infections_inc_per_strain});
  dim[94] = cpp11::writable::integers({shared->dim_infections_inc_by_age});
  dim[95] = cpp11::writable::integers({shared->dim_hospitalisations_inc_by_strain});
  dim[96] = cpp11::writable::integers({shared->dim_hospitalisations_inc_by_age});
  dim[97] = cpp11::writable::integers({shared->dim_D_non_hosp});
  dim[98] = cpp11::writable::integers({shared->dim_cum_admit_by_age});
  dim[99] = cpp11::writable::integers({shared->dim_N_tot});
  dim[100] = cpp11::writable::integers({shared->dim_prob_strain});
  dim[101] = cpp11::writable::integers({shared->dim_effective_susceptible});
  dim[102] = cpp11::writable::integers({shared->dim_ifr_strain});
  dim[103] = cpp11::writable::integers({shared->dim_ihr_strain});
  dim[104] = cpp11::writable::integers({shared->dim_hfr_strain});
  dim[105] = cpp11::writable::integers({shared->dim_ifr_age});
  dim[106] = cpp11::writable::integers({shared->dim_ihr_age});
  dim[107] = cpp11::writable::integers({shared->dim_hfr_age});
  dim[108] = cpp11::writable::integers({shared->dim_cum_n_S_vaccinated_1, shared->dim_cum_n_S_vaccinated_2});
  dim[109] = cpp11::writable::integers({shared->dim_cum_n_E_vaccinated_1, shared->dim_cum_n_E_vaccinated_2});
  dim[110] = cpp11::writable::integers({shared->dim_cum_n_I_A_vaccinated_1, shared->dim_cum_n_I_A_vaccinated_2});
  dim[111] = cpp11::writable::integers({shared->dim_cum_n_I_P_vaccinated_1, shared->dim_cum_n_I_P_vaccinated_2});
  dim[112] = cpp11::writable::integers({shared->dim_cum_n_R_vaccinated_1, shared->dim_cum_n_R_vaccinated_2});
  dim[113] = cpp11::writable::integers({shared->dim_cum_n_vaccinated_1, shared->dim_cum_n_vaccinated_2});
  dim[114] = cpp11::writable::integers({shared->dim_cum_n_S_vacc_skip_1, shared->dim_cum_n_S_vacc_skip_2});
  dim[115] = cpp11::writable::integers({shared->dim_cum_n_E_vacc_skip_1, shared->dim_cum_n_E_vacc_skip_2});
  dim[116] = cpp11::writable::integers({shared->dim_cum_n_I_A_vacc_skip_1, shared->dim_cum_n_I_A_vacc_skip_2});
  dim[117] = cpp11::writable::integers({shared->dim_cum_n_I_P_vacc_skip_1, shared->dim_cum_n_I_P_vacc_skip_2});
  dim[118] = cpp11::writable::integers({shared->dim_cum_n_R_vacc_skip_1, shared->dim_cum_n_R_vacc_skip_2});
  dim[119] = cpp11::writable::integers({shared->dim_cum_n_vacc_skip_1, shared->dim_cum_n_vacc_skip_2});
  dim[120] = cpp11::writable::integers({shared->dim_diagnoses_admitted_1, shared->dim_diagnoses_admitted_2});
  dim[121] = cpp11::writable::integers({shared->dim_cum_infections_disag_1, shared->dim_cum_infections_disag_2});
  dim[122] = cpp11::writable::integers({shared->dim_D_1, shared->dim_D_2});
  dim[123] = cpp11::writable::integers({shared->dim_S_1, shared->dim_S_2});
  dim[124] = cpp11::writable::integers({shared->dim_D_hosp_1, shared->dim_D_hosp_2});
  dim[125] = cpp11::writable::integers({shared->dim_vaccine_missed_doses_1, shared->dim_vaccine_missed_doses_2});
  dim[126] = cpp11::writable::integers({shared->dim_tmp_vaccine_n_candidates_1, shared->dim_tmp_vaccine_n_candidates_2});
  dim[127] = cpp11::writable::integers({shared->dim_tmp_vaccine_probability_1, shared->dim_tmp_vaccine_probability_2});
  dim[128] = cpp11::writable::integers({shared->dim_ifr_disag_1, shared->dim_ifr_disag_2});
  dim[129] = cpp11::writable::integers({shared->dim_ihr_disag_1, shared->dim_ihr_disag_2});
  dim[130] = cpp11::writable::integers({shared->dim_hfr_disag_1, shared->dim_hfr_disag_2});
  dim[131] = cpp11::writable::integers({shared->dim_T_sero_neg_1_1, shared->dim_T_sero_neg_1_2, shared->dim_T_sero_neg_1_3});
  dim[132] = cpp11::writable::integers({shared->dim_T_sero_neg_2_1, shared->dim_T_sero_neg_2_2, shared->dim_T_sero_neg_2_3});
  dim[133] = cpp11::writable::integers({shared->dim_R_1, shared->dim_R_2, shared->dim_R_3});
  dim[134] = cpp11::writable::integers({shared->dim_T_PCR_neg_1, shared->dim_T_PCR_neg_2, shared->dim_T_PCR_neg_3});
  dim[135] = cpp11::writable::integers({shared->dim_I_weighted_1, shared->dim_I_weighted_2, shared->dim_I_weighted_3});
  dim[136] = cpp11::writable::integers({shared->dim_E_1, shared->dim_E_2, shared->dim_E_3, shared->dim_E_4});
  dim[137] = cpp11::writable::integers({shared->dim_I_A_1, shared->dim_I_A_2, shared->dim_I_A_3, shared->dim_I_A_4});
  dim[138] = cpp11::writable::integers({shared->dim_I_P_1, shared->dim_I_P_2, shared->dim_I_P_3, shared->dim_I_P_4});
  dim[139] = cpp11::writable::integers({shared->dim_I_C_1_1, shared->dim_I_C_1_2, shared->dim_I_C_1_3, shared->dim_I_C_1_4});
  dim[140] = cpp11::writable::integers({shared->dim_I_C_2_1, shared->dim_I_C_2_2, shared->dim_I_C_2_3, shared->dim_I_C_2_4});
  dim[141] = cpp11::writable::integers({shared->dim_G_D_1, shared->dim_G_D_2, shared->dim_G_D_3, shared->dim_G_D_4});
  dim[142] = cpp11::writable::integers({shared->dim_ICU_pre_unconf_1, shared->dim_ICU_pre_unconf_2, shared->dim_ICU_pre_unconf_3, shared->dim_ICU_pre_unconf_4});
  dim[143] = cpp11::writable::integers({shared->dim_ICU_pre_conf_1, shared->dim_ICU_pre_conf_2, shared->dim_ICU_pre_conf_3, shared->dim_ICU_pre_conf_4});
  dim[144] = cpp11::writable::integers({shared->dim_H_R_unconf_1, shared->dim_H_R_unconf_2, shared->dim_H_R_unconf_3, shared->dim_H_R_unconf_4});
  dim[145] = cpp11::writable::integers({shared->dim_H_R_conf_1, shared->dim_H_R_conf_2, shared->dim_H_R_conf_3, shared->dim_H_R_conf_4});
  dim[146] = cpp11::writable::integers({shared->dim_H_D_unconf_1, shared->dim_H_D_unconf_2, shared->dim_H_D_unconf_3, shared->dim_H_D_unconf_4});
  dim[147] = cpp11::writable::integers({shared->dim_H_D_conf_1, shared->dim_H_D_conf_2, shared->dim_H_D_conf_3, shared->dim_H_D_conf_4});
  dim[148] = cpp11::writable::integers({shared->dim_ICU_W_R_unconf_1, shared->dim_ICU_W_R_unconf_2, shared->dim_ICU_W_R_unconf_3, shared->dim_ICU_W_R_unconf_4});
  dim[149] = cpp11::writable::integers({shared->dim_ICU_W_R_conf_1, shared->dim_ICU_W_R_conf_2, shared->dim_ICU_W_R_conf_3, shared->dim_ICU_W_R_conf_4});
  dim[150] = cpp11::writable::integers({shared->dim_ICU_W_D_unconf_1, shared->dim_ICU_W_D_unconf_2, shared->dim_ICU_W_D_unconf_3, shared->dim_ICU_W_D_unconf_4});
  dim[151] = cpp11::writable::integers({shared->dim_ICU_W_D_conf_1, shared->dim_ICU_W_D_conf_2, shared->dim_ICU_W_D_conf_3, shared->dim_ICU_W_D_conf_4});
  dim[152] = cpp11::writable::integers({shared->dim_ICU_D_unconf_1, shared->dim_ICU_D_unconf_2, shared->dim_ICU_D_unconf_3, shared->dim_ICU_D_unconf_4});
  dim[153] = cpp11::writable::integers({shared->dim_ICU_D_conf_1, shared->dim_ICU_D_conf_2, shared->dim_ICU_D_conf_3, shared->dim_ICU_D_conf_4});
  dim[154] = cpp11::writable::integers({shared->dim_W_R_unconf_1, shared->dim_W_R_unconf_2, shared->dim_W_R_unconf_3, shared->dim_W_R_unconf_4});
  dim[155] = cpp11::writable::integers({shared->dim_W_R_conf_1, shared->dim_W_R_conf_2, shared->dim_W_R_conf_3, shared->dim_W_R_conf_4});
  dim[156] = cpp11::writable::integers({shared->dim_W_D_unconf_1, shared->dim_W_D_unconf_2, shared->dim_W_D_unconf_3, shared->dim_W_D_unconf_4});
  dim[157] = cpp11::writable::integers({shared->dim_W_D_conf_1, shared->dim_W_D_conf_2, shared->dim_W_D_conf_3, shared->dim_W_D_conf_4});
  dim[158] = cpp11::writable::integers({shared->dim_T_sero_pre_1_1, shared->dim_T_sero_pre_1_2, shared->dim_T_sero_pre_1_3, shared->dim_T_sero_pre_1_4});
  dim[159] = cpp11::writable::integers({shared->dim_T_sero_pos_1_1, shared->dim_T_sero_pos_1_2, shared->dim_T_sero_pos_1_3, shared->dim_T_sero_pos_1_4});
  dim[160] = cpp11::writable::integers({shared->dim_T_sero_pre_2_1, shared->dim_T_sero_pre_2_2, shared->dim_T_sero_pre_2_3, shared->dim_T_sero_pre_2_4});
  dim[161] = cpp11::writable::integers({shared->dim_T_sero_pos_2_1, shared->dim_T_sero_pos_2_2, shared->dim_T_sero_pos_2_3, shared->dim_T_sero_pos_2_4});
  dim[162] = cpp11::writable::integers({shared->dim_T_PCR_pre_1, shared->dim_T_PCR_pre_2, shared->dim_T_PCR_pre_3, shared->dim_T_PCR_pre_4});
  dim[163] = cpp11::writable::integers({shared->dim_T_PCR_pos_1, shared->dim_T_PCR_pos_2, shared->dim_T_PCR_pos_3, shared->dim_T_PCR_pos_4});
  dim.names() = nms;
  cpp11::writable::list index(164);
  ...
  index[92] = integer_sequence(93, shared->dim_cum_infections_per_strain);
  index[93] = integer_sequence(shared->offset_variable_infections_inc_per_strain + 1, shared->dim_infections_inc_per_strain);
  index[94] = integer_sequence(shared->offset_variable_infections_inc_by_age + 1, shared->dim_infections_inc_by_age);
  index[95] = integer_sequence(shared->offset_variable_hospitalisations_inc_by_strain + 1, shared->dim_hospitalisations_inc_by_strain);
  index[96] = integer_sequence(shared->offset_variable_hospitalisations_inc_by_age + 1, shared->dim_hospitalisations_inc_by_age);
  index[97] = integer_sequence(shared->offset_variable_D_non_hosp + 1, shared->dim_D_non_hosp);
  index[98] = integer_sequence(shared->offset_variable_cum_admit_by_age + 1, shared->dim_cum_admit_by_age);
  index[99] = integer_sequence(shared->offset_variable_N_tot + 1, shared->dim_N_tot);
  index[100] = integer_sequence(shared->offset_variable_prob_strain + 1, shared->dim_prob_strain);
  index[101] = integer_sequence(shared->offset_variable_effective_susceptible + 1, shared->dim_effective_susceptible);
  index[102] = integer_sequence(shared->offset_variable_ifr_strain + 1, shared->dim_ifr_strain);
  index[103] = integer_sequence(shared->offset_variable_ihr_strain + 1, shared->dim_ihr_strain);
  index[104] = integer_sequence(shared->offset_variable_hfr_strain + 1, shared->dim_hfr_strain);
  index[105] = integer_sequence(shared->offset_variable_ifr_age + 1, shared->dim_ifr_age);
  index[106] = integer_sequence(shared->offset_variable_ihr_age + 1, shared->dim_ihr_age);
  index[107] = integer_sequence(shared->offset_variable_hfr_age + 1, shared->dim_hfr_age);
  index[108] = integer_sequence(shared->offset_variable_cum_n_S_vaccinated + 1, shared->dim_cum_n_S_vaccinated);
  index[109] = integer_sequence(shared->offset_variable_cum_n_E_vaccinated + 1, shared->dim_cum_n_E_vaccinated);
  index[110] = integer_sequence(shared->offset_variable_cum_n_I_A_vaccinated + 1, shared->dim_cum_n_I_A_vaccinated);
  index[111] = integer_sequence(shared->offset_variable_cum_n_I_P_vaccinated + 1, shared->dim_cum_n_I_P_vaccinated);
  index[112] = integer_sequence(shared->offset_variable_cum_n_R_vaccinated + 1, shared->dim_cum_n_R_vaccinated);
  index[113] = integer_sequence(shared->offset_variable_cum_n_vaccinated + 1, shared->dim_cum_n_vaccinated);
  index[114] = integer_sequence(shared->offset_variable_cum_n_S_vacc_skip + 1, shared->dim_cum_n_S_vacc_skip);
  index[115] = integer_sequence(shared->offset_variable_cum_n_E_vacc_skip + 1, shared->dim_cum_n_E_vacc_skip);
  index[116] = integer_sequence(shared->offset_variable_cum_n_I_A_vacc_skip + 1, shared->dim_cum_n_I_A_vacc_skip);
  index[117] = integer_sequence(shared->offset_variable_cum_n_I_P_vacc_skip + 1, shared->dim_cum_n_I_P_vacc_skip);
  index[118] = integer_sequence(shared->offset_variable_cum_n_R_vacc_skip + 1, shared->dim_cum_n_R_vacc_skip);
  index[119] = integer_sequence(shared->offset_variable_cum_n_vacc_skip + 1, shared->dim_cum_n_vacc_skip);
  index[120] = integer_sequence(shared->offset_variable_diagnoses_admitted + 1, shared->dim_diagnoses_admitted);
  index[121] = integer_sequence(shared->offset_variable_cum_infections_disag + 1, shared->dim_cum_infections_disag);
  index[122] = integer_sequence(shared->offset_variable_D + 1, shared->dim_D);
  index[123] = integer_sequence(shared->offset_variable_S + 1, shared->dim_S);
  index[124] = integer_sequence(shared->offset_variable_D_hosp + 1, shared->dim_D_hosp);
  index[125] = integer_sequence(shared->offset_variable_vaccine_missed_doses + 1, shared->dim_vaccine_missed_doses);
  index[126] = integer_sequence(shared->offset_variable_tmp_vaccine_n_candidates + 1, shared->dim_tmp_vaccine_n_candidates);
  index[127] = integer_sequence(shared->offset_variable_tmp_vaccine_probability + 1, shared->dim_tmp_vaccine_probability);
  index[128] = integer_sequence(shared->offset_variable_ifr_disag + 1, shared->dim_ifr_disag);
  index[129] = integer_sequence(shared->offset_variable_ihr_disag + 1, shared->dim_ihr_disag);
  index[130] = integer_sequence(shared->offset_variable_hfr_disag + 1, shared->dim_hfr_disag);
  index[131] = integer_sequence(shared->offset_variable_T_sero_neg_1 + 1, shared->dim_T_sero_neg_1);
  index[132] = integer_sequence(shared->offset_variable_T_sero_neg_2 + 1, shared->dim_T_sero_neg_2);
  index[133] = integer_sequence(shared->offset_variable_R + 1, shared->dim_R);
  index[134] = integer_sequence(shared->offset_variable_T_PCR_neg + 1, shared->dim_T_PCR_neg);
  index[135] = integer_sequence(shared->offset_variable_I_weighted + 1, shared->dim_I_weighted);
  index[136] = integer_sequence(shared->offset_variable_E + 1, shared->dim_E);
  index[137] = integer_sequence(shared->offset_variable_I_A + 1, shared->dim_I_A);
  index[138] = integer_sequence(shared->offset_variable_I_P + 1, shared->dim_I_P);
  index[139] = integer_sequence(shared->offset_variable_I_C_1 + 1, shared->dim_I_C_1);
  index[140] = integer_sequence(shared->offset_variable_I_C_2 + 1, shared->dim_I_C_2);
  index[141] = integer_sequence(shared->offset_variable_G_D + 1, shared->dim_G_D);
  index[142] = integer_sequence(shared->offset_variable_ICU_pre_unconf + 1, shared->dim_ICU_pre_unconf);
  index[143] = integer_sequence(shared->offset_variable_ICU_pre_conf + 1, shared->dim_ICU_pre_conf);
  index[144] = integer_sequence(shared->offset_variable_H_R_unconf + 1, shared->dim_H_R_unconf);
  index[145] = integer_sequence(shared->offset_variable_H_R_conf + 1, shared->dim_H_R_conf);
  index[146] = integer_sequence(shared->offset_variable_H_D_unconf + 1, shared->dim_H_D_unconf);
  index[147] = integer_sequence(shared->offset_variable_H_D_conf + 1, shared->dim_H_D_conf);
  index[148] = integer_sequence(shared->offset_variable_ICU_W_R_unconf + 1, shared->dim_ICU_W_R_unconf);
  index[149] = integer_sequence(shared->offset_variable_ICU_W_R_conf + 1, shared->dim_ICU_W_R_conf);
  index[150] = integer_sequence(shared->offset_variable_ICU_W_D_unconf + 1, shared->dim_ICU_W_D_unconf);
  index[151] = integer_sequence(shared->offset_variable_ICU_W_D_conf + 1, shared->dim_ICU_W_D_conf);
  index[152] = integer_sequence(shared->offset_variable_ICU_D_unconf + 1, shared->dim_ICU_D_unconf);
  index[153] = integer_sequence(shared->offset_variable_ICU_D_conf + 1, shared->dim_ICU_D_conf);
  index[154] = integer_sequence(shared->offset_variable_W_R_unconf + 1, shared->dim_W_R_unconf);
  index[155] = integer_sequence(shared->offset_variable_W_R_conf + 1, shared->dim_W_R_conf);
  index[156] = integer_sequence(shared->offset_variable_W_D_unconf + 1, shared->dim_W_D_unconf);
  index[157] = integer_sequence(shared->offset_variable_W_D_conf + 1, shared->dim_W_D_conf);
  index[158] = integer_sequence(shared->offset_variable_T_sero_pre_1 + 1, shared->dim_T_sero_pre_1);
  index[159] = integer_sequence(shared->offset_variable_T_sero_pos_1 + 1, shared->dim_T_sero_pos_1);
  index[160] = integer_sequence(shared->offset_variable_T_sero_pre_2 + 1, shared->dim_T_sero_pre_2);
  index[161] = integer_sequence(shared->offset_variable_T_sero_pos_2 + 1, shared->dim_T_sero_pos_2);
  index[162] = integer_sequence(shared->offset_variable_T_PCR_pre + 1, shared->dim_T_PCR_pre);
  index[163] = integer_sequence(shared->offset_variable_T_PCR_pos + 1, shared->dim_T_PCR_pos);
  index.names() = nms;
