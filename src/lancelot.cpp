    ...
    for (int i = 1; i <= shared->dim_n_RE_1; ++i) {
      for (int j = 1; j <= shared->dim_n_RE_2; ++j) {
        for (int k = 1; k <= shared->dim_n_RE_3; ++k) {
          internal.n_RE[i - 1 + shared->dim_n_RE_1 * (j - 1) + shared->dim_n_RE_12 * (k - 1)] = (shared->n_strains == 1 || j < 3 ? 0 : (j == 3 ? internal.n_R5_to_E3[shared->dim_n_R5_to_E3_1 * (k - 1) + i - 1] : (internal.n_R_progress[shared->dim_n_R_progress_12 * (k - 1) + shared->dim_n_R_progress_1 * 0 + i - 1] - internal.n_RS[shared->dim_n_RS_12 * (k - 1) + shared->dim_n_RS_1 * 0 + i - 1] + internal.n_R_progress[shared->dim_n_R_progress_12 * (k - 1) + shared->dim_n_R_progress_1 * 1 + i - 1] - internal.n_RS[shared->dim_n_RS_12 * (k - 1) + shared->dim_n_RS_1 * 1 + i - 1] + internal.n_R_progress[shared->dim_n_R_progress_12 * (k - 1) + shared->dim_n_R_progress_1 * 4 + i - 1] - internal.n_RS[shared->dim_n_RS_12 * (k - 1) + shared->dim_n_RS_1 * 4 + i - 1] - internal.n_R5_to_E3[shared->dim_n_R5_to_E3_1 * (k - 1) + i - 1])));
        }
      }
    }
