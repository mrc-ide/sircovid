...
public:
  ...
  void update(size_t step, const real_type * state, rng_state_type& rng_state, real_type * state_next) {
    ...
    for (int i = 1; i <= shared->dim_new_HFR_disag_weighted_inc_1; ++i) {
      for (int j = 1; j <= shared->dim_new_HFR_disag_weighted_inc_2; ++j) {
        for (int k = 1; k <= shared->dim_new_HFR_disag_weighted_inc_3; ++k) {
          internal.new_HFR_disag_weighted_inc[i - 1 + shared->dim_new_HFR_disag_weighted_inc_1 * (j - 1) + shared->dim_new_HFR_disag_weighted_inc_12 * (k - 1)] = (fmodr<real_type>(step, shared->steps_per_day) == 0 ? internal.HFR_disag[shared->dim_HFR_disag_12 * (k - 1) + shared->dim_HFR_disag_1 * (j - 1) + i - 1] * internal.new_inf[shared->dim_new_inf_12 * (k - 1) + shared->dim_new_inf_1 * (j - 1) + i - 1] : HFR_disag_weighted_inc[shared->dim_HFR_disag_weighted_inc_12 * (k - 1) + shared->dim_HFR_disag_weighted_inc_1 * (j - 1) + i - 1] + internal.HFR_disag[shared->dim_HFR_disag_12 * (k - 1) + shared->dim_HFR_disag_1 * (j - 1) + i - 1] * internal.n_I_C_2_to_hosp[shared->dim_n_I_C_2_to_hosp_12 * (k - 1) + shared->dim_n_I_C_2_to_hosp_1 * (j - 1) + i - 1]);
        }
      }
    }
    for (int i = 1; i <= shared->dim_new_IFR_disag_weighted_inc_1; ++i) {
      for (int j = 1; j <= shared->dim_new_IFR_disag_weighted_inc_2; ++j) {
        for (int k = 1; k <= shared->dim_new_IFR_disag_weighted_inc_3; ++k) {
          internal.new_IFR_disag_weighted_inc[i - 1 + shared->dim_new_IFR_disag_weighted_inc_1 * (j - 1) + shared->dim_new_IFR_disag_weighted_inc_12 * (k - 1)] = (fmodr<real_type>(step, shared->steps_per_day) == 0 ? internal.IFR_disag[shared->dim_IFR_disag_12 * (k - 1) + shared->dim_IFR_disag_1 * (j - 1) + i - 1] * internal.new_inf[shared->dim_new_inf_12 * (k - 1) + shared->dim_new_inf_1 * (j - 1) + i - 1] : IFR_disag_weighted_inc[shared->dim_IFR_disag_weighted_inc_12 * (k - 1) + shared->dim_IFR_disag_weighted_inc_1 * (j - 1) + i - 1] + internal.IFR_disag[shared->dim_IFR_disag_12 * (k - 1) + shared->dim_IFR_disag_1 * (j - 1) + i - 1] * internal.new_inf[shared->dim_new_inf_12 * (k - 1) + shared->dim_new_inf_1 * (j - 1) + i - 1]);
        }
      }
    }
    for (int i = 1; i <= shared->dim_new_IHR_disag_weighted_inc_1; ++i) {
      for (int j = 1; j <= shared->dim_new_IHR_disag_weighted_inc_2; ++j) {
        for (int k = 1; k <= shared->dim_new_IHR_disag_weighted_inc_3; ++k) {
          internal.new_IHR_disag_weighted_inc[i - 1 + shared->dim_new_IHR_disag_weighted_inc_1 * (j - 1) + shared->dim_new_IHR_disag_weighted_inc_12 * (k - 1)] = (fmodr<real_type>(step, shared->steps_per_day) == 0 ? internal.IHR_disag[shared->dim_IHR_disag_12 * (k - 1) + shared->dim_IHR_disag_1 * (j - 1) + i - 1] * internal.new_inf[shared->dim_new_inf_12 * (k - 1) + shared->dim_new_inf_1 * (j - 1) + i - 1] : IHR_disag_weighted_inc[shared->dim_IHR_disag_weighted_inc_12 * (k - 1) + shared->dim_IHR_disag_weighted_inc_1 * (j - 1) + i - 1] + internal.IHR_disag[shared->dim_IHR_disag_12 * (k - 1) + shared->dim_IHR_disag_1 * (j - 1) + i - 1] * internal.new_inf[shared->dim_new_inf_12 * (k - 1) + shared->dim_new_inf_1 * (j - 1) + i - 1]);
        }
      }
    }
    ...
    state_next[87] = odin_sum1<real_type>(internal.new_HFR_disag_weighted_inc.data(), 0, shared->dim_new_HFR_disag_weighted_inc) / (real_type) new_hospitalisations_inc;
    for (int i = 1; i <= shared->dim_hfr_age; ++i) {
      state_next[shared->offset_variable_hfr_age + i - 1] = odin_sum3<real_type>(internal.new_HFR_disag_weighted_inc.data(), i - 1, i, 0, shared->dim_new_HFR_disag_weighted_inc_2, 0, shared->dim_new_HFR_disag_weighted_inc_3, shared->dim_new_HFR_disag_weighted_inc_1, shared->dim_new_HFR_disag_weighted_inc_12) / (real_type) internal.new_hospitalisations_inc_by_age[i - 1];
    }
    for (int i = 1; i <= shared->dim_hfr_strain; ++i) {
      state_next[shared->offset_variable_hfr_strain + i - 1] = (shared->n_real_strains == 1 ? odin_sum3<real_type>(internal.new_HFR_disag_weighted_inc.data(), 0, shared->dim_new_HFR_disag_weighted_inc_1, 0, 1, 0, shared->dim_new_HFR_disag_weighted_inc_3, shared->dim_new_HFR_disag_weighted_inc_1, shared->dim_new_HFR_disag_weighted_inc_12) / (real_type) internal.new_hospitalisations_inc_by_strain[0] : (odin_sum3<real_type>(internal.new_HFR_disag_weighted_inc.data(), 0, shared->dim_new_HFR_disag_weighted_inc_1, i - 1, i, 0, shared->dim_new_HFR_disag_weighted_inc_3, shared->dim_new_HFR_disag_weighted_inc_1, shared->dim_new_HFR_disag_weighted_inc_12) + odin_sum3<real_type>(internal.new_HFR_disag_weighted_inc.data(), 0, shared->dim_new_HFR_disag_weighted_inc_1, 5 - i - 1, 5 - i, 0, shared->dim_new_HFR_disag_weighted_inc_3, shared->dim_new_HFR_disag_weighted_inc_1, shared->dim_new_HFR_disag_weighted_inc_12)) / (real_type) (internal.new_hospitalisations_inc_by_strain[i - 1] + internal.new_hospitalisations_inc_by_strain[5 - i - 1]));
    }
    ...
    state_next[88] = odin_sum1<real_type>(internal.new_IFR_disag_weighted_inc.data(), 0, shared->dim_new_IFR_disag_weighted_inc) / (real_type) new_infections_inc;
    for (int i = 1; i <= shared->dim_ifr_age; ++i) {
      state_next[shared->offset_variable_ifr_age + i - 1] = odin_sum3<real_type>(internal.new_IFR_disag_weighted_inc.data(), i - 1, i, 0, shared->dim_new_IFR_disag_weighted_inc_2, 0, shared->dim_new_IFR_disag_weighted_inc_3, shared->dim_new_IFR_disag_weighted_inc_1, shared->dim_new_IFR_disag_weighted_inc_12) / (real_type) internal.new_infections_inc_by_age[i - 1];
    }
    for (int i = 1; i <= shared->dim_ifr_strain; ++i) {
      state_next[shared->offset_variable_ifr_strain + i - 1] = (shared->n_real_strains == 1 ? odin_sum3<real_type>(internal.new_IFR_disag_weighted_inc.data(), 0, shared->dim_new_IFR_disag_weighted_inc_1, 0, 1, 0, shared->dim_new_IFR_disag_weighted_inc_3, shared->dim_new_IFR_disag_weighted_inc_1, shared->dim_new_IFR_disag_weighted_inc_12) / (real_type) internal.new_infections_inc_per_strain[0] : (odin_sum3<real_type>(internal.new_IFR_disag_weighted_inc.data(), 0, shared->dim_new_IFR_disag_weighted_inc_1, i - 1, i, 0, shared->dim_new_IFR_disag_weighted_inc_3, shared->dim_new_IFR_disag_weighted_inc_1, shared->dim_new_IFR_disag_weighted_inc_12) + odin_sum3<real_type>(internal.new_IFR_disag_weighted_inc.data(), 0, shared->dim_new_IFR_disag_weighted_inc_1, 5 - i - 1, 5 - i, 0, shared->dim_new_IFR_disag_weighted_inc_3, shared->dim_new_IFR_disag_weighted_inc_1, shared->dim_new_IFR_disag_weighted_inc_12)) / (real_type) (internal.new_infections_inc_per_strain[i - 1] + internal.new_infections_inc_per_strain[5 - i - 1]));
    }
    state_next[86] = odin_sum1<real_type>(internal.new_IHR_disag_weighted_inc.data(), 0, shared->dim_new_IHR_disag_weighted_inc) / (real_type) new_infections_inc;
    for (int i = 1; i <= shared->dim_ihr_age; ++i) {
      state_next[shared->offset_variable_ihr_age + i - 1] = odin_sum3<real_type>(internal.new_IHR_disag_weighted_inc.data(), i - 1, i, 0, shared->dim_new_IHR_disag_weighted_inc_2, 0, shared->dim_new_IHR_disag_weighted_inc_3, shared->dim_new_IHR_disag_weighted_inc_1, shared->dim_new_IHR_disag_weighted_inc_12) / (real_type) internal.new_infections_inc_by_age[i - 1];
    }
    for (int i = 1; i <= shared->dim_ihr_strain; ++i) {
      state_next[shared->offset_variable_ihr_strain + i - 1] = (shared->n_real_strains == 1 ? odin_sum3<real_type>(internal.new_IHR_disag_weighted_inc.data(), 0, shared->dim_new_IHR_disag_weighted_inc_1, 0, 1, 0, shared->dim_new_IHR_disag_weighted_inc_3, shared->dim_new_IHR_disag_weighted_inc_1, shared->dim_new_IHR_disag_weighted_inc_12) / (real_type) internal.new_infections_inc_per_strain[0] : (odin_sum3<real_type>(internal.new_IHR_disag_weighted_inc.data(), 0, shared->dim_new_IHR_disag_weighted_inc_1, i - 1, i, 0, shared->dim_new_IHR_disag_weighted_inc_3, shared->dim_new_IHR_disag_weighted_inc_1, shared->dim_new_IHR_disag_weighted_inc_12) + odin_sum3<real_type>(internal.new_IHR_disag_weighted_inc.data(), 0, shared->dim_new_IHR_disag_weighted_inc_1, 5 - i - 1, 5 - i, 0, shared->dim_new_IHR_disag_weighted_inc_3, shared->dim_new_IHR_disag_weighted_inc_1, shared->dim_new_IHR_disag_weighted_inc_12)) / (real_type) (internal.new_infections_inc_per_strain[i - 1] + internal.new_infections_inc_per_strain[5 - i - 1]));
    }
...
cpp11::sexp dust_info<lancelot>(const dust::pars_type<lancelot>& pars) {
  ...
  dim[125] = cpp11::writable::integers({shared->dim_T_sero_neg_1_1, shared->dim_T_sero_neg_1_2, shared->dim_T_sero_neg_1_3});
  dim[126] = cpp11::writable::integers({shared->dim_T_sero_neg_2_1, shared->dim_T_sero_neg_2_2, shared->dim_T_sero_neg_2_3});
  dim[127] = cpp11::writable::integers({shared->dim_R_1, shared->dim_R_2, shared->dim_R_3});
  dim[128] = cpp11::writable::integers({shared->dim_T_PCR_neg_1, shared->dim_T_PCR_neg_2, shared->dim_T_PCR_neg_3});
  dim[129] = cpp11::writable::integers({shared->dim_I_weighted_1, shared->dim_I_weighted_2, shared->dim_I_weighted_3});
  dim[130] = cpp11::writable::integers({shared->dim_IHR_disag_weighted_inc_1, shared->dim_IHR_disag_weighted_inc_2, shared->dim_IHR_disag_weighted_inc_3});
  dim[131] = cpp11::writable::integers({shared->dim_HFR_disag_weighted_inc_1, shared->dim_HFR_disag_weighted_inc_2, shared->dim_HFR_disag_weighted_inc_3});
  dim[132] = cpp11::writable::integers({shared->dim_IFR_disag_weighted_inc_1, shared->dim_IFR_disag_weighted_inc_2, shared->dim_IFR_disag_weighted_inc_3});
  ...
  index[125] = integer_sequence(shared->offset_variable_T_sero_neg_1 + 1, shared->dim_T_sero_neg_1);
  index[126] = integer_sequence(shared->offset_variable_T_sero_neg_2 + 1, shared->dim_T_sero_neg_2);
  index[127] = integer_sequence(shared->offset_variable_R + 1, shared->dim_R);
  index[128] = integer_sequence(shared->offset_variable_T_PCR_neg + 1, shared->dim_T_PCR_neg);
  index[129] = integer_sequence(shared->offset_variable_I_weighted + 1, shared->dim_I_weighted);
  index[130] = integer_sequence(shared->offset_variable_IHR_disag_weighted_inc + 1, shared->dim_IHR_disag_weighted_inc);
  index[131] = integer_sequence(shared->offset_variable_HFR_disag_weighted_inc + 1, shared->dim_HFR_disag_weighted_inc);
  index[132] = integer_sequence(shared->offset_variable_IFR_disag_weighted_inc + 1, shared->dim_IFR_disag_weighted_inc);
