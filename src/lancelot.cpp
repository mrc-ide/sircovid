// Generated by dust (version 0.11.25) - do not edit
#include <cpp11.hpp>
...
public:
  ...
  std::vector<real_type> initial(size_t step) {
    ...
    state[77] = shared->initial_react_pos;
    state[78] = shared->initial_react_5_24_pos;
    state[79] = shared->initial_react_25_34_pos;
    state[80] = shared->initial_react_35_44_pos;
    state[81] = shared->initial_react_45_54_pos;
    state[82] = shared->initial_react_55_64_pos;
    state[83] = shared->initial_react_65_plus_pos;
    state[84] = shared->initial_ihr;
    state[85] = shared->initial_hfr;
    state[86] = shared->initial_ifr;
    std::copy(shared->initial_cum_n_S_vacc_skip.begin(), shared->initial_cum_n_S_vacc_skip.end(), state.begin() + 87);
  ...
  void update(size_t step, const real_type * state, rng_state_type& rng_state, real_type * state_next) {
    ...
    const real_type * cum_n_S_vacc_skip = state + 87;
    ...
    state_next[85] = odin_sum1<real_type>(internal.HFR_disag_weighted.data(), 0, shared->dim_HFR_disag_weighted) / (real_type) odin_sum1<real_type>(internal.n_I_C_2_to_hosp.data(), 0, shared->dim_n_I_C_2_to_hosp);
    ...
    for (int i = 1; i <= shared->dim_cum_n_S_vacc_skip; ++i) {
      state_next[87 + i - 1] = cum_n_S_vacc_skip[i - 1] + internal.n_S_vacc_skip[i - 1];
    }
    ...
    state_next[78] = odin_sum4<real_type>(internal.new_T_PCR_pos.data(), 1, 5, 0, shared->dim_new_T_PCR_pos_2, 0, shared->dim_new_T_PCR_pos_3, 0, shared->dim_new_T_PCR_pos_4, shared->dim_new_T_PCR_pos_1, shared->dim_new_T_PCR_pos_12, shared->dim_new_T_PCR_pos_123);
    state_next[83] = odin_sum4<real_type>(internal.new_T_PCR_pos.data(), 13, 17, 0, shared->dim_new_T_PCR_pos_2, 0, shared->dim_new_T_PCR_pos_3, 0, shared->dim_new_T_PCR_pos_4, shared->dim_new_T_PCR_pos_1, shared->dim_new_T_PCR_pos_12, shared->dim_new_T_PCR_pos_123);
    state_next[77] = odin_sum4<real_type>(internal.new_T_PCR_pos.data(), 1, 18, 0, shared->dim_new_T_PCR_pos_2, 0, shared->dim_new_T_PCR_pos_3, 0, shared->dim_new_T_PCR_pos_4, shared->dim_new_T_PCR_pos_1, shared->dim_new_T_PCR_pos_12, shared->dim_new_T_PCR_pos_123);
    ...
    state_next[86] = odin_sum1<real_type>(internal.IFR_disag_weighted.data(), 0, shared->dim_IFR_disag_weighted) / (real_type) odin_sum1<real_type>(internal.new_inf.data(), 0, shared->dim_new_inf);
    state_next[84] = odin_sum1<real_type>(internal.IHR_disag_weighted.data(), 0, shared->dim_IHR_disag_weighted) / (real_type) odin_sum1<real_type>(internal.new_inf.data(), 0, shared->dim_new_inf);
...
dust::pars_type<lancelot> dust_pars<lancelot>(cpp11::list user) {
  ...
  shared->offset_variable_D_hosp = shared->dim_cum_infections_per_strain + shared->dim_cum_n_E_vacc_skip + shared->dim_cum_n_I_A_vacc_skip + shared->dim_cum_n_I_P_vacc_skip + shared->dim_cum_n_R_vacc_skip + shared->dim_cum_n_S_vacc_skip + shared->dim_cum_n_vacc_skip + shared->dim_infections_inc_per_strain + 87;
  shared->offset_variable_D_non_hosp = shared->dim_D_hosp + shared->dim_cum_infections_per_strain + shared->dim_cum_n_E_vacc_skip + shared->dim_cum_n_I_A_vacc_skip + shared->dim_cum_n_I_P_vacc_skip + shared->dim_cum_n_R_vacc_skip + shared->dim_cum_n_S_vacc_skip + shared->dim_cum_n_vacc_skip + shared->dim_infections_inc_per_strain + 87;
  ...
  shared->offset_variable_cum_infections_per_strain = shared->dim_cum_n_E_vacc_skip + shared->dim_cum_n_I_A_vacc_skip + shared->dim_cum_n_I_P_vacc_skip + shared->dim_cum_n_R_vacc_skip + shared->dim_cum_n_S_vacc_skip + shared->dim_cum_n_vacc_skip + 87;
  shared->offset_variable_cum_n_E_vacc_skip = shared->dim_cum_n_S_vacc_skip + 87;
  shared->offset_variable_cum_n_I_A_vacc_skip = shared->dim_cum_n_E_vacc_skip + shared->dim_cum_n_S_vacc_skip + 87;
  shared->offset_variable_cum_n_I_P_vacc_skip = shared->dim_cum_n_E_vacc_skip + shared->dim_cum_n_I_A_vacc_skip + shared->dim_cum_n_S_vacc_skip + 87;
  shared->offset_variable_cum_n_R_vacc_skip = shared->dim_cum_n_E_vacc_skip + shared->dim_cum_n_I_A_vacc_skip + shared->dim_cum_n_I_P_vacc_skip + shared->dim_cum_n_S_vacc_skip + 87;
  shared->offset_variable_cum_n_vacc_skip = shared->dim_cum_n_E_vacc_skip + shared->dim_cum_n_I_A_vacc_skip + shared->dim_cum_n_I_P_vacc_skip + shared->dim_cum_n_R_vacc_skip + shared->dim_cum_n_S_vacc_skip + 87;
  shared->offset_variable_infections_inc_per_strain = shared->dim_cum_infections_per_strain + shared->dim_cum_n_E_vacc_skip + shared->dim_cum_n_I_A_vacc_skip + shared->dim_cum_n_I_P_vacc_skip + shared->dim_cum_n_R_vacc_skip + shared->dim_cum_n_S_vacc_skip + shared->dim_cum_n_vacc_skip + 87;
...
cpp11::sexp dust_info<lancelot>(const dust::pars_type<lancelot>& pars) {
  ...
  dim[87] = cpp11::writable::integers({shared->dim_cum_n_S_vacc_skip});
  dim[88] = cpp11::writable::integers({shared->dim_cum_n_E_vacc_skip});
  dim[89] = cpp11::writable::integers({shared->dim_cum_n_I_A_vacc_skip});
  dim[90] = cpp11::writable::integers({shared->dim_cum_n_I_P_vacc_skip});
  dim[91] = cpp11::writable::integers({shared->dim_cum_n_R_vacc_skip});
  dim[92] = cpp11::writable::integers({shared->dim_cum_n_vacc_skip});
  dim[93] = cpp11::writable::integers({shared->dim_cum_infections_per_strain});
  dim[94] = cpp11::writable::integers({shared->dim_infections_inc_per_strain});
  dim[95] = cpp11::writable::integers({shared->dim_D_hosp});
  dim[96] = cpp11::writable::integers({shared->dim_D_non_hosp});
  dim[97] = cpp11::writable::integers({shared->dim_cum_admit_by_age});
  dim[98] = cpp11::writable::integers({shared->dim_N_tot});
  dim[99] = cpp11::writable::integers({shared->dim_prob_strain});
  dim[100] = cpp11::writable::integers({shared->dim_effective_susceptible});
  dim[101] = cpp11::writable::integers({shared->dim_cum_n_S_vaccinated_1, shared->dim_cum_n_S_vaccinated_2});
  dim[102] = cpp11::writable::integers({shared->dim_cum_n_E_vaccinated_1, shared->dim_cum_n_E_vaccinated_2});
  dim[103] = cpp11::writable::integers({shared->dim_cum_n_I_A_vaccinated_1, shared->dim_cum_n_I_A_vaccinated_2});
  dim[104] = cpp11::writable::integers({shared->dim_cum_n_I_P_vaccinated_1, shared->dim_cum_n_I_P_vaccinated_2});
  dim[105] = cpp11::writable::integers({shared->dim_cum_n_R_vaccinated_1, shared->dim_cum_n_R_vaccinated_2});
  dim[106] = cpp11::writable::integers({shared->dim_cum_n_vaccinated_1, shared->dim_cum_n_vaccinated_2});
  dim[107] = cpp11::writable::integers({shared->dim_diagnoses_admitted_1, shared->dim_diagnoses_admitted_2});
  dim[108] = cpp11::writable::integers({shared->dim_cum_infections_disag_1, shared->dim_cum_infections_disag_2});
  dim[109] = cpp11::writable::integers({shared->dim_D_1, shared->dim_D_2});
  dim[110] = cpp11::writable::integers({shared->dim_S_1, shared->dim_S_2});
  dim[111] = cpp11::writable::integers({shared->dim_vaccine_missed_doses_1, shared->dim_vaccine_missed_doses_2});
  dim[112] = cpp11::writable::integers({shared->dim_tmp_vaccine_n_candidates_1, shared->dim_tmp_vaccine_n_candidates_2});
  dim[113] = cpp11::writable::integers({shared->dim_tmp_vaccine_probability_1, shared->dim_tmp_vaccine_probability_2});
  dim[114] = cpp11::writable::integers({shared->dim_T_sero_neg_1_1, shared->dim_T_sero_neg_1_2, shared->dim_T_sero_neg_1_3});
  dim[115] = cpp11::writable::integers({shared->dim_T_sero_neg_2_1, shared->dim_T_sero_neg_2_2, shared->dim_T_sero_neg_2_3});
  dim[116] = cpp11::writable::integers({shared->dim_R_1, shared->dim_R_2, shared->dim_R_3});
  dim[117] = cpp11::writable::integers({shared->dim_T_PCR_neg_1, shared->dim_T_PCR_neg_2, shared->dim_T_PCR_neg_3});
  dim[118] = cpp11::writable::integers({shared->dim_I_weighted_1, shared->dim_I_weighted_2, shared->dim_I_weighted_3});
  dim[119] = cpp11::writable::integers({shared->dim_E_1, shared->dim_E_2, shared->dim_E_3, shared->dim_E_4});
  dim[120] = cpp11::writable::integers({shared->dim_I_A_1, shared->dim_I_A_2, shared->dim_I_A_3, shared->dim_I_A_4});
  dim[121] = cpp11::writable::integers({shared->dim_I_P_1, shared->dim_I_P_2, shared->dim_I_P_3, shared->dim_I_P_4});
  dim[122] = cpp11::writable::integers({shared->dim_I_C_1_1, shared->dim_I_C_1_2, shared->dim_I_C_1_3, shared->dim_I_C_1_4});
  dim[123] = cpp11::writable::integers({shared->dim_I_C_2_1, shared->dim_I_C_2_2, shared->dim_I_C_2_3, shared->dim_I_C_2_4});
  dim[124] = cpp11::writable::integers({shared->dim_G_D_1, shared->dim_G_D_2, shared->dim_G_D_3, shared->dim_G_D_4});
  dim[125] = cpp11::writable::integers({shared->dim_ICU_pre_unconf_1, shared->dim_ICU_pre_unconf_2, shared->dim_ICU_pre_unconf_3, shared->dim_ICU_pre_unconf_4});
  dim[126] = cpp11::writable::integers({shared->dim_ICU_pre_conf_1, shared->dim_ICU_pre_conf_2, shared->dim_ICU_pre_conf_3, shared->dim_ICU_pre_conf_4});
  dim[127] = cpp11::writable::integers({shared->dim_H_R_unconf_1, shared->dim_H_R_unconf_2, shared->dim_H_R_unconf_3, shared->dim_H_R_unconf_4});
  dim[128] = cpp11::writable::integers({shared->dim_H_R_conf_1, shared->dim_H_R_conf_2, shared->dim_H_R_conf_3, shared->dim_H_R_conf_4});
  dim[129] = cpp11::writable::integers({shared->dim_H_D_unconf_1, shared->dim_H_D_unconf_2, shared->dim_H_D_unconf_3, shared->dim_H_D_unconf_4});
  dim[130] = cpp11::writable::integers({shared->dim_H_D_conf_1, shared->dim_H_D_conf_2, shared->dim_H_D_conf_3, shared->dim_H_D_conf_4});
  dim[131] = cpp11::writable::integers({shared->dim_ICU_W_R_unconf_1, shared->dim_ICU_W_R_unconf_2, shared->dim_ICU_W_R_unconf_3, shared->dim_ICU_W_R_unconf_4});
  dim[132] = cpp11::writable::integers({shared->dim_ICU_W_R_conf_1, shared->dim_ICU_W_R_conf_2, shared->dim_ICU_W_R_conf_3, shared->dim_ICU_W_R_conf_4});
  dim[133] = cpp11::writable::integers({shared->dim_ICU_W_D_unconf_1, shared->dim_ICU_W_D_unconf_2, shared->dim_ICU_W_D_unconf_3, shared->dim_ICU_W_D_unconf_4});
  dim[134] = cpp11::writable::integers({shared->dim_ICU_W_D_conf_1, shared->dim_ICU_W_D_conf_2, shared->dim_ICU_W_D_conf_3, shared->dim_ICU_W_D_conf_4});
  dim[135] = cpp11::writable::integers({shared->dim_ICU_D_unconf_1, shared->dim_ICU_D_unconf_2, shared->dim_ICU_D_unconf_3, shared->dim_ICU_D_unconf_4});
  dim[136] = cpp11::writable::integers({shared->dim_ICU_D_conf_1, shared->dim_ICU_D_conf_2, shared->dim_ICU_D_conf_3, shared->dim_ICU_D_conf_4});
  dim[137] = cpp11::writable::integers({shared->dim_W_R_unconf_1, shared->dim_W_R_unconf_2, shared->dim_W_R_unconf_3, shared->dim_W_R_unconf_4});
  dim[138] = cpp11::writable::integers({shared->dim_W_R_conf_1, shared->dim_W_R_conf_2, shared->dim_W_R_conf_3, shared->dim_W_R_conf_4});
  dim[139] = cpp11::writable::integers({shared->dim_W_D_unconf_1, shared->dim_W_D_unconf_2, shared->dim_W_D_unconf_3, shared->dim_W_D_unconf_4});
  dim[140] = cpp11::writable::integers({shared->dim_W_D_conf_1, shared->dim_W_D_conf_2, shared->dim_W_D_conf_3, shared->dim_W_D_conf_4});
  dim[141] = cpp11::writable::integers({shared->dim_T_sero_pre_1_1, shared->dim_T_sero_pre_1_2, shared->dim_T_sero_pre_1_3, shared->dim_T_sero_pre_1_4});
  dim[142] = cpp11::writable::integers({shared->dim_T_sero_pos_1_1, shared->dim_T_sero_pos_1_2, shared->dim_T_sero_pos_1_3, shared->dim_T_sero_pos_1_4});
  dim[143] = cpp11::writable::integers({shared->dim_T_sero_pre_2_1, shared->dim_T_sero_pre_2_2, shared->dim_T_sero_pre_2_3, shared->dim_T_sero_pre_2_4});
  dim[144] = cpp11::writable::integers({shared->dim_T_sero_pos_2_1, shared->dim_T_sero_pos_2_2, shared->dim_T_sero_pos_2_3, shared->dim_T_sero_pos_2_4});
  dim[145] = cpp11::writable::integers({shared->dim_T_PCR_pre_1, shared->dim_T_PCR_pre_2, shared->dim_T_PCR_pre_3, shared->dim_T_PCR_pre_4});
  dim[146] = cpp11::writable::integers({shared->dim_T_PCR_pos_1, shared->dim_T_PCR_pos_2, shared->dim_T_PCR_pos_3, shared->dim_T_PCR_pos_4});
  dim.names() = nms;
  cpp11::writable::list index(147);
  ...
  index[87] = integer_sequence(88, shared->dim_cum_n_S_vacc_skip);
  index[88] = integer_sequence(shared->offset_variable_cum_n_E_vacc_skip + 1, shared->dim_cum_n_E_vacc_skip);
  index[89] = integer_sequence(shared->offset_variable_cum_n_I_A_vacc_skip + 1, shared->dim_cum_n_I_A_vacc_skip);
  index[90] = integer_sequence(shared->offset_variable_cum_n_I_P_vacc_skip + 1, shared->dim_cum_n_I_P_vacc_skip);
  index[91] = integer_sequence(shared->offset_variable_cum_n_R_vacc_skip + 1, shared->dim_cum_n_R_vacc_skip);
  index[92] = integer_sequence(shared->offset_variable_cum_n_vacc_skip + 1, shared->dim_cum_n_vacc_skip);
  index[93] = integer_sequence(shared->offset_variable_cum_infections_per_strain + 1, shared->dim_cum_infections_per_strain);
  index[94] = integer_sequence(shared->offset_variable_infections_inc_per_strain + 1, shared->dim_infections_inc_per_strain);
  index[95] = integer_sequence(shared->offset_variable_D_hosp + 1, shared->dim_D_hosp);
  index[96] = integer_sequence(shared->offset_variable_D_non_hosp + 1, shared->dim_D_non_hosp);
  index[97] = integer_sequence(shared->offset_variable_cum_admit_by_age + 1, shared->dim_cum_admit_by_age);
  index[98] = integer_sequence(shared->offset_variable_N_tot + 1, shared->dim_N_tot);
  index[99] = integer_sequence(shared->offset_variable_prob_strain + 1, shared->dim_prob_strain);
  index[100] = integer_sequence(shared->offset_variable_effective_susceptible + 1, shared->dim_effective_susceptible);
  index[101] = integer_sequence(shared->offset_variable_cum_n_S_vaccinated + 1, shared->dim_cum_n_S_vaccinated);
  index[102] = integer_sequence(shared->offset_variable_cum_n_E_vaccinated + 1, shared->dim_cum_n_E_vaccinated);
  index[103] = integer_sequence(shared->offset_variable_cum_n_I_A_vaccinated + 1, shared->dim_cum_n_I_A_vaccinated);
  index[104] = integer_sequence(shared->offset_variable_cum_n_I_P_vaccinated + 1, shared->dim_cum_n_I_P_vaccinated);
  index[105] = integer_sequence(shared->offset_variable_cum_n_R_vaccinated + 1, shared->dim_cum_n_R_vaccinated);
  index[106] = integer_sequence(shared->offset_variable_cum_n_vaccinated + 1, shared->dim_cum_n_vaccinated);
  index[107] = integer_sequence(shared->offset_variable_diagnoses_admitted + 1, shared->dim_diagnoses_admitted);
  index[108] = integer_sequence(shared->offset_variable_cum_infections_disag + 1, shared->dim_cum_infections_disag);
  index[109] = integer_sequence(shared->offset_variable_D + 1, shared->dim_D);
  index[110] = integer_sequence(shared->offset_variable_S + 1, shared->dim_S);
  index[111] = integer_sequence(shared->offset_variable_vaccine_missed_doses + 1, shared->dim_vaccine_missed_doses);
  index[112] = integer_sequence(shared->offset_variable_tmp_vaccine_n_candidates + 1, shared->dim_tmp_vaccine_n_candidates);
  index[113] = integer_sequence(shared->offset_variable_tmp_vaccine_probability + 1, shared->dim_tmp_vaccine_probability);
  index[114] = integer_sequence(shared->offset_variable_T_sero_neg_1 + 1, shared->dim_T_sero_neg_1);
  index[115] = integer_sequence(shared->offset_variable_T_sero_neg_2 + 1, shared->dim_T_sero_neg_2);
  index[116] = integer_sequence(shared->offset_variable_R + 1, shared->dim_R);
  index[117] = integer_sequence(shared->offset_variable_T_PCR_neg + 1, shared->dim_T_PCR_neg);
  index[118] = integer_sequence(shared->offset_variable_I_weighted + 1, shared->dim_I_weighted);
  index[119] = integer_sequence(shared->offset_variable_E + 1, shared->dim_E);
  index[120] = integer_sequence(shared->offset_variable_I_A + 1, shared->dim_I_A);
  index[121] = integer_sequence(shared->offset_variable_I_P + 1, shared->dim_I_P);
  index[122] = integer_sequence(shared->offset_variable_I_C_1 + 1, shared->dim_I_C_1);
  index[123] = integer_sequence(shared->offset_variable_I_C_2 + 1, shared->dim_I_C_2);
  index[124] = integer_sequence(shared->offset_variable_G_D + 1, shared->dim_G_D);
  index[125] = integer_sequence(shared->offset_variable_ICU_pre_unconf + 1, shared->dim_ICU_pre_unconf);
  index[126] = integer_sequence(shared->offset_variable_ICU_pre_conf + 1, shared->dim_ICU_pre_conf);
  index[127] = integer_sequence(shared->offset_variable_H_R_unconf + 1, shared->dim_H_R_unconf);
  index[128] = integer_sequence(shared->offset_variable_H_R_conf + 1, shared->dim_H_R_conf);
  index[129] = integer_sequence(shared->offset_variable_H_D_unconf + 1, shared->dim_H_D_unconf);
  index[130] = integer_sequence(shared->offset_variable_H_D_conf + 1, shared->dim_H_D_conf);
  index[131] = integer_sequence(shared->offset_variable_ICU_W_R_unconf + 1, shared->dim_ICU_W_R_unconf);
  index[132] = integer_sequence(shared->offset_variable_ICU_W_R_conf + 1, shared->dim_ICU_W_R_conf);
  index[133] = integer_sequence(shared->offset_variable_ICU_W_D_unconf + 1, shared->dim_ICU_W_D_unconf);
  index[134] = integer_sequence(shared->offset_variable_ICU_W_D_conf + 1, shared->dim_ICU_W_D_conf);
  index[135] = integer_sequence(shared->offset_variable_ICU_D_unconf + 1, shared->dim_ICU_D_unconf);
  index[136] = integer_sequence(shared->offset_variable_ICU_D_conf + 1, shared->dim_ICU_D_conf);
  index[137] = integer_sequence(shared->offset_variable_W_R_unconf + 1, shared->dim_W_R_unconf);
  index[138] = integer_sequence(shared->offset_variable_W_R_conf + 1, shared->dim_W_R_conf);
  index[139] = integer_sequence(shared->offset_variable_W_D_unconf + 1, shared->dim_W_D_unconf);
  index[140] = integer_sequence(shared->offset_variable_W_D_conf + 1, shared->dim_W_D_conf);
  index[141] = integer_sequence(shared->offset_variable_T_sero_pre_1 + 1, shared->dim_T_sero_pre_1);
  index[142] = integer_sequence(shared->offset_variable_T_sero_pos_1 + 1, shared->dim_T_sero_pos_1);
  index[143] = integer_sequence(shared->offset_variable_T_sero_pre_2 + 1, shared->dim_T_sero_pre_2);
  index[144] = integer_sequence(shared->offset_variable_T_sero_pos_2 + 1, shared->dim_T_sero_pos_2);
  index[145] = integer_sequence(shared->offset_variable_T_PCR_pre + 1, shared->dim_T_PCR_pre);
  index[146] = integer_sequence(shared->offset_variable_T_PCR_pos + 1, shared->dim_T_PCR_pos);
  index.names() = nms;
