  ...
  HOST void update(size_t step, const real_t * state, dust::rng_state_t<real_t>& rng_state, real_t * state_next) {
    ...
    for (int i = 1; i <= shared->dim_n_EE_1; ++i) {
      for (int j = 1; j <= shared->dim_n_EE_2; ++j) {
        for (int k = 1; k <= shared->dim_n_EE_3; ++k) {
          internal.n_EE[i - 1 + shared->dim_n_EE_1 * (j - 1) + shared->dim_n_EE_12 * (k - 1)] = dust::distr::rbinom<real_t>(rng_state, E[shared->dim_E_12 * (k - 1) + shared->dim_E_1 * (j - 1) + i - 1], shared->p_EE);
        }
      }
    }
    for (int i = 1; i <= shared->dim_n_II_A_1; ++i) {
      for (int j = 1; j <= shared->dim_n_II_A_2; ++j) {
        for (int k = 1; k <= shared->dim_n_II_A_3; ++k) {
          internal.n_II_A[i - 1 + shared->dim_n_II_A_1 * (j - 1) + shared->dim_n_II_A_12 * (k - 1)] = dust::distr::rbinom<real_t>(rng_state, I_A[shared->dim_I_A_12 * (k - 1) + shared->dim_I_A_1 * (j - 1) + i - 1], shared->p_II_A);
        }
      }
    }
    for (int i = 1; i <= shared->dim_n_II_C_1; ++i) {
      for (int j = 1; j <= shared->dim_n_II_C_2; ++j) {
        for (int k = 1; k <= shared->dim_n_II_C_3; ++k) {
          internal.n_II_C[i - 1 + shared->dim_n_II_C_1 * (j - 1) + shared->dim_n_II_C_12 * (k - 1)] = dust::distr::rbinom<real_t>(rng_state, I_C[shared->dim_I_C_12 * (k - 1) + shared->dim_I_C_1 * (j - 1) + i - 1], shared->p_II_C);
        }
      }
    }
    for (int i = 1; i <= shared->dim_n_II_ICU_1; ++i) {
      for (int j = 1; j <= shared->dim_n_II_ICU_2; ++j) {
        for (int k = 1; k <= shared->dim_n_II_ICU_3; ++k) {
          internal.n_II_ICU[i - 1 + shared->dim_n_II_ICU_1 * (j - 1) + shared->dim_n_II_ICU_12 * (k - 1)] = dust::distr::rbinom<real_t>(rng_state, I_ICU[shared->dim_I_ICU_12 * (k - 1) + shared->dim_I_ICU_1 * (j - 1) + i - 1], shared->p_II_ICU);
        }
      }
    }
    for (int i = 1; i <= shared->dim_n_II_hosp_1; ++i) {
      for (int j = 1; j <= shared->dim_n_II_hosp_2; ++j) {
        for (int k = 1; k <= shared->dim_n_II_hosp_3; ++k) {
          internal.n_II_hosp[i - 1 + shared->dim_n_II_hosp_1 * (j - 1) + shared->dim_n_II_hosp_12 * (k - 1)] = dust::distr::rbinom<real_t>(rng_state, I_hosp[shared->dim_I_hosp_12 * (k - 1) + shared->dim_I_hosp_1 * (j - 1) + i - 1], shared->p_II_hosp);
        }
      }
    }
    for (int i = 1; i <= shared->dim_n_R_hosp_1; ++i) {
      for (int j = 1; j <= shared->dim_n_R_hosp_2; ++j) {
        for (int k = 1; k <= shared->dim_n_R_hosp_3; ++k) {
          internal.n_R_hosp[i - 1 + shared->dim_n_R_hosp_1 * (j - 1) + shared->dim_n_R_hosp_12 * (k - 1)] = dust::distr::rbinom<real_t>(rng_state, R_hosp[shared->dim_R_hosp_12 * (k - 1) + shared->dim_R_hosp_1 * (j - 1) + i - 1], shared->p_R_hosp);
        }
      }
    }
    ...
    for (int i = 1; i <= shared->dim_n_EI_A_1; ++i) {
      for (int j = 1; j <= shared->dim_n_EI_A_2; ++j) {
        internal.n_EI_A[i - 1 + shared->dim_n_EI_A_1 * (j - 1)] = dust::distr::rbinom<real_t>(rng_state, internal.n_EE[shared->dim_n_EE_12 * (j - 1) + shared->dim_n_EE_1 * (shared->k_E - 1) + i - 1], 1 - shared->p_C[i - 1]);
      }
    }
    for (int i = 1; i <= shared->dim_n_ICU_to_R_hosp_1; ++i) {
      for (int j = 1; j <= shared->dim_n_ICU_to_R_hosp_2; ++j) {
        internal.n_ICU_to_R_hosp[i - 1 + shared->dim_n_ICU_to_R_hosp_1 * (j - 1)] = dust::distr::rbinom<real_t>(rng_state, internal.n_II_ICU[shared->dim_n_II_ICU_12 * (j - 1) + shared->dim_n_II_ICU_1 * (shared->k_ICU - 1) + i - 1], shared->p_recov_ICU[i - 1]);
      }
    }
    for (int i = 1; i <= shared->dim_n_death_hosp_1; ++i) {
      for (int j = 1; j <= shared->dim_n_death_hosp_2; ++j) {
        internal.n_death_hosp[i - 1 + shared->dim_n_death_hosp_1 * (j - 1)] = dust::distr::rbinom<real_t>(rng_state, internal.n_II_hosp[shared->dim_n_II_hosp_12 * (j - 1) + shared->dim_n_II_hosp_1 * (shared->k_hosp - 1) + i - 1], shared->p_death_hosp[i - 1]);
      }
    }
    for (int i = 1; i <= shared->dim_n_sympt_to_hosp_1; ++i) {
      for (int j = 1; j <= shared->dim_n_sympt_to_hosp_2; ++j) {
        internal.n_sympt_to_hosp[i - 1 + shared->dim_n_sympt_to_hosp_1 * (j - 1)] = dust::distr::rbinom<real_t>(rng_state, internal.n_II_C[shared->dim_n_II_C_12 * (j - 1) + shared->dim_n_II_C_1 * (shared->k_C - 1) + i - 1], 1 - shared->p_recov_sympt[i - 1]);
      }
    }
    ...
    for (int i = 1; i <= shared->dim_n_hosp_to_ICU_1; ++i) {
      for (int j = 1; j <= shared->dim_n_hosp_to_ICU_2; ++j) {
        internal.n_hosp_to_ICU[i - 1 + shared->dim_n_hosp_to_ICU_1 * (j - 1)] = dust::distr::rbinom<real_t>(rng_state, internal.n_II_hosp[shared->dim_n_II_hosp_12 * (j - 1) + shared->dim_n_II_hosp_1 * (shared->k_hosp - 1) + i - 1] - internal.n_death_hosp[shared->dim_n_death_hosp_1 * (j - 1) + i - 1], 1 - shared->p_recov_hosp[i - 1] - shared->p_death_hosp[i - 1]);
      }
    }
    ...
    for (int i = 1; i <= shared->dim_n_SE; ++i) {
      internal.n_SE[i - 1] = dust::distr::rbinom<real_t>(rng_state, S[i - 1], internal.p_SE[i - 1]);
    }
    ...
    for (int i = 1; i <= shared->dim_aux_EE_1; ++i) {
      int j = 1;
      int k = 1;
      internal.aux_EE[i - 1 + shared->dim_aux_EE_1 * (j - 1) + shared->dim_aux_EE_12 * (k - 1)] = dust::distr::rbinom<real_t>(rng_state, internal.n_SE[i - 1], shared->aux_p_bin[shared->dim_aux_p_bin_1 * 0 + i - 1]);
    }
    for (int i = 1; i <= shared->dim_aux_EE_1; ++i) {
      int j = 1;
      for (int k = 2; k <= (shared->n_trans_classes - 1); ++k) {
        internal.aux_EE[i - 1 + shared->dim_aux_EE_1 * (j - 1) + shared->dim_aux_EE_12 * (k - 1)] = dust::distr::rbinom<real_t>(rng_state, internal.n_SE[i - 1] - odin_sum3<real_t>(internal.aux_EE.data(), i - 1, i, 0, 1, 0, k - 1, shared->dim_aux_EE_1, shared->dim_aux_EE_12), shared->aux_p_bin[shared->dim_aux_p_bin_1 * (k - 1) + i - 1]);
      }
    }
    for (int i = 1; i <= shared->dim_aux_EE_1; ++i) {
      int j = 1;
...
SEXP dust_basic_run(SEXP ptr, size_t step_end, bool device,
                       bool deterministic) {
  return dust::r::dust_run<basic>(ptr, step_end, device, deterministic);
}

SEXP dust_basic_simulate(SEXP ptr, cpp11::sexp step_end, bool device,
                            bool deterministic) {
  return dust::r::dust_simulate<basic>(ptr, step_end, device,
                                           deterministic);
}
...
SEXP dust_basic_set_state(SEXP ptr, SEXP r_state, SEXP r_step,
                             bool deterministic) {
  dust::r::dust_set_state<basic>(ptr, r_state, r_step, deterministic);
  return R_NilValue;
}
...
cpp11::sexp dust_basic_device_info() {
  return dust::cuda::device_info<basic::real_t>();
}
