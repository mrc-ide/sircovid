...
public:
  ...
  basic(const init_t& init): shared(init.shared), internal(init.internal) {
  }
  size_t size() {
    return 5 + shared->dim_S + shared->dim_R + shared->dim_D + shared->dim_E + shared->dim_I_A + shared->dim_I_C + shared->dim_I_hosp + shared->dim_I_ICU + shared->dim_R_hosp;
  }
  std::vector<real_t> initial(size_t step) {
    std::vector<real_t> state(5 + shared->dim_S + shared->dim_R + shared->dim_D + shared->dim_E + shared->dim_I_A + shared->dim_I_C + shared->dim_I_hosp + shared->dim_I_ICU + shared->dim_R_hosp);
  ...
  void update(size_t step, const real_t * state, dust::rng_state_t<real_t>& rng_state, real_t * state_next) {
    ...
    const real_t * I_hosp = state + shared->offset_variable_I_hosp;
    const real_t * I_ICU = state + shared->offset_variable_I_ICU;
    const real_t * R_hosp = state + shared->offset_variable_R_hosp;
    const real_t * D = state + shared->offset_variable_D;
    state_next[1] = odin_sum1(S, 0, shared->dim_S) + odin_sum1(R, 0, shared->dim_R) + odin_sum1(D, 0, shared->dim_D) + odin_sum1(E, 0, shared->dim_E) + odin_sum1(I_A, 0, shared->dim_I_A) + odin_sum1(I_C, 0, shared->dim_I_C) + odin_sum1(I_hosp, 0, shared->dim_I_hosp) + odin_sum1(I_ICU, 0, shared->dim_I_ICU) + odin_sum1(R_hosp, 0, shared->dim_R_hosp);
    ...
    for (int i = 1; i <= shared->dim_n_EE_1; ++i) {
      for (int j = 1; j <= shared->dim_n_EE_2; ++j) {
        for (int k = 1; k <= shared->dim_n_EE_3; ++k) {
          internal.n_EE[i - 1 + shared->dim_n_EE_1 * (j - 1) + shared->dim_n_EE_12 * (k - 1)] = dust::distr::rbinom(rng_state, std::round(E[shared->dim_E_12 * (k - 1) + shared->dim_E_1 * (j - 1) + i - 1]), shared->p_EE);
        }
      }
    }
    for (int i = 1; i <= shared->dim_n_II_A_1; ++i) {
      for (int j = 1; j <= shared->dim_n_II_A_2; ++j) {
        for (int k = 1; k <= shared->dim_n_II_A_3; ++k) {
          internal.n_II_A[i - 1 + shared->dim_n_II_A_1 * (j - 1) + shared->dim_n_II_A_12 * (k - 1)] = dust::distr::rbinom(rng_state, std::round(I_A[shared->dim_I_A_12 * (k - 1) + shared->dim_I_A_1 * (j - 1) + i - 1]), shared->p_II_A);
        }
      }
    }
    for (int i = 1; i <= shared->dim_n_II_C_1; ++i) {
      for (int j = 1; j <= shared->dim_n_II_C_2; ++j) {
        for (int k = 1; k <= shared->dim_n_II_C_3; ++k) {
          internal.n_II_C[i - 1 + shared->dim_n_II_C_1 * (j - 1) + shared->dim_n_II_C_12 * (k - 1)] = dust::distr::rbinom(rng_state, std::round(I_C[shared->dim_I_C_12 * (k - 1) + shared->dim_I_C_1 * (j - 1) + i - 1]), shared->p_II_C);
        }
      }
    }
    for (int i = 1; i <= shared->dim_n_II_ICU_1; ++i) {
      for (int j = 1; j <= shared->dim_n_II_ICU_2; ++j) {
        for (int k = 1; k <= shared->dim_n_II_ICU_3; ++k) {
          internal.n_II_ICU[i - 1 + shared->dim_n_II_ICU_1 * (j - 1) + shared->dim_n_II_ICU_12 * (k - 1)] = dust::distr::rbinom(rng_state, std::round(I_ICU[shared->dim_I_ICU_12 * (k - 1) + shared->dim_I_ICU_1 * (j - 1) + i - 1]), shared->p_II_ICU);
        }
      }
    }
    for (int i = 1; i <= shared->dim_n_II_hosp_1; ++i) {
      for (int j = 1; j <= shared->dim_n_II_hosp_2; ++j) {
        for (int k = 1; k <= shared->dim_n_II_hosp_3; ++k) {
          internal.n_II_hosp[i - 1 + shared->dim_n_II_hosp_1 * (j - 1) + shared->dim_n_II_hosp_12 * (k - 1)] = dust::distr::rbinom(rng_state, std::round(I_hosp[shared->dim_I_hosp_12 * (k - 1) + shared->dim_I_hosp_1 * (j - 1) + i - 1]), shared->p_II_hosp);
        }
      }
    }
    for (int i = 1; i <= shared->dim_n_R_hosp_1; ++i) {
      for (int j = 1; j <= shared->dim_n_R_hosp_2; ++j) {
        for (int k = 1; k <= shared->dim_n_R_hosp_3; ++k) {
          internal.n_R_hosp[i - 1 + shared->dim_n_R_hosp_1 * (j - 1) + shared->dim_n_R_hosp_12 * (k - 1)] = dust::distr::rbinom(rng_state, std::round(R_hosp[shared->dim_R_hosp_12 * (k - 1) + shared->dim_R_hosp_1 * (j - 1) + i - 1]), shared->p_R_hosp);
        }
      }
    }
    for (int i = 1; i <= shared->dim_I_with_diff_trans_1; ++i) {
      for (int j = 1; j <= shared->dim_I_with_diff_trans_2; ++j) {
        internal.I_with_diff_trans[i - 1 + shared->dim_I_with_diff_trans_1 * (j - 1)] = shared->trans_increase[shared->dim_trans_increase_1 * (j - 1) + i - 1] * (odin_sum3(I_A, i - 1, i, 0, shared->dim_I_A_2, j - 1, j, shared->dim_I_A_1, shared->dim_I_A_12) + odin_sum3(I_C, i - 1, i, 0, shared->dim_I_C_2, j - 1, j, shared->dim_I_C_1, shared->dim_I_C_12) + shared->hosp_transmission * odin_sum3(I_hosp, i - 1, i, 0, shared->dim_I_hosp_2, j - 1, j, shared->dim_I_hosp_1, shared->dim_I_hosp_12) + shared->ICU_transmission * odin_sum3(I_ICU, i - 1, i, 0, shared->dim_I_ICU_2, j - 1, j, shared->dim_I_ICU_1, shared->dim_I_ICU_12));
      }
    }
    for (int i = 1; i <= shared->dim_n_EI_A_1; ++i) {
      for (int j = 1; j <= shared->dim_n_EI_A_2; ++j) {
        internal.n_EI_A[i - 1 + shared->dim_n_EI_A_1 * (j - 1)] = dust::distr::rbinom(rng_state, std::round(internal.n_EE[shared->dim_n_EE_12 * (j - 1) + shared->dim_n_EE_1 * (shared->k_E - 1) + i - 1]), 1 - shared->p_C[i - 1]);
      }
    }
    for (int i = 1; i <= shared->dim_n_ICU_to_R_hosp_1; ++i) {
      for (int j = 1; j <= shared->dim_n_ICU_to_R_hosp_2; ++j) {
        internal.n_ICU_to_R_hosp[i - 1 + shared->dim_n_ICU_to_R_hosp_1 * (j - 1)] = dust::distr::rbinom(rng_state, std::round(internal.n_II_ICU[shared->dim_n_II_ICU_12 * (j - 1) + shared->dim_n_II_ICU_1 * (shared->k_ICU - 1) + i - 1]), shared->p_recov_ICU[i - 1]);
      }
    }
    for (int i = 1; i <= shared->dim_n_death_hosp_1; ++i) {
      for (int j = 1; j <= shared->dim_n_death_hosp_2; ++j) {
        internal.n_death_hosp[i - 1 + shared->dim_n_death_hosp_1 * (j - 1)] = dust::distr::rbinom(rng_state, std::round(internal.n_II_hosp[shared->dim_n_II_hosp_12 * (j - 1) + shared->dim_n_II_hosp_1 * (shared->k_hosp - 1) + i - 1]), shared->p_death_hosp[i - 1]);
      }
    }
    for (int i = 1; i <= shared->dim_n_sympt_to_hosp_1; ++i) {
      for (int j = 1; j <= shared->dim_n_sympt_to_hosp_2; ++j) {
        internal.n_sympt_to_hosp[i - 1 + shared->dim_n_sympt_to_hosp_1 * (j - 1)] = dust::distr::rbinom(rng_state, std::round(internal.n_II_C[shared->dim_n_II_C_12 * (j - 1) + shared->dim_n_II_C_1 * (shared->k_C - 1) + i - 1]), 1 - shared->p_recov_sympt[i - 1]);
      }
    }
    ...
    for (int i = 1; i <= shared->dim_delta_D; ++i) {
      internal.delta_D[i - 1] = odin_sum3(internal.n_II_ICU.data(), i - 1, i, shared->k_ICU - 1, shared->k_ICU, 0, shared->dim_n_II_ICU_3, shared->dim_n_II_ICU_1, shared->dim_n_II_ICU_12) - odin_sum2(internal.n_ICU_to_R_hosp.data(), i - 1, i, 0, shared->dim_n_ICU_to_R_hosp_2, shared->dim_n_ICU_to_R_hosp_1) + odin_sum2(internal.n_death_hosp.data(), i - 1, i, 0, shared->dim_n_death_hosp_2, shared->dim_n_death_hosp_1);
    }
    ...
    for (int i = 1; i <= shared->dim_n_hosp_to_ICU_1; ++i) {
      for (int j = 1; j <= shared->dim_n_hosp_to_ICU_2; ++j) {
        internal.n_hosp_to_ICU[i - 1 + shared->dim_n_hosp_to_ICU_1 * (j - 1)] = dust::distr::rbinom(rng_state, std::round(internal.n_II_hosp[shared->dim_n_II_hosp_12 * (j - 1) + shared->dim_n_II_hosp_1 * (shared->k_hosp - 1) + i - 1] - internal.n_death_hosp[shared->dim_n_death_hosp_1 * (j - 1) + i - 1]), 1 - shared->p_recov_hosp[i - 1] - shared->p_death_hosp[i - 1]);
      }
    }
    for (int i = 1; i <= shared->dim_s_ij_1; ++i) {
      for (int j = 1; j <= shared->dim_s_ij_2; ++j) {
        internal.s_ij[i - 1 + shared->dim_s_ij_1 * (j - 1)] = shared->m[shared->dim_m_1 * (j - 1) + i - 1] * odin_sum2(internal.I_with_diff_trans.data(), j - 1, j, 0, shared->dim_I_with_diff_trans_2, shared->dim_I_with_diff_trans_1);
      }
    }
    ...
    for (int i = 1; i <= shared->dim_delta_R; ++i) {
      internal.delta_R[i - 1] = odin_sum3(internal.n_II_A.data(), i - 1, i, shared->k_A - 1, shared->k_A, 0, shared->dim_n_II_A_3, shared->dim_n_II_A_1, shared->dim_n_II_A_12) + odin_sum3(internal.n_II_C.data(), i - 1, i, shared->k_C - 1, shared->k_C, 0, shared->dim_n_II_C_3, shared->dim_n_II_C_1, shared->dim_n_II_C_12) - odin_sum2(internal.n_sympt_to_hosp.data(), i - 1, i, 0, shared->dim_n_sympt_to_hosp_2, shared->dim_n_sympt_to_hosp_1) + odin_sum3(internal.n_II_hosp.data(), i - 1, i, shared->k_hosp - 1, shared->k_hosp, 0, shared->dim_n_II_hosp_3, shared->dim_n_II_hosp_1, shared->dim_n_II_hosp_12) - odin_sum2(internal.n_hosp_to_ICU.data(), i - 1, i, 0, shared->dim_n_hosp_to_ICU_2, shared->dim_n_hosp_to_ICU_1) - odin_sum2(internal.n_death_hosp.data(), i - 1, i, 0, shared->dim_n_death_hosp_2, shared->dim_n_death_hosp_1) + odin_sum3(internal.n_R_hosp.data(), i - 1, i, shared->k_rec - 1, shared->k_rec, 0, shared->dim_n_R_hosp_3, shared->dim_n_R_hosp_1, shared->dim_n_R_hosp_12);
    }
    ...
    for (int i = 1; i <= shared->dim_lambda; ++i) {
      internal.lambda[i - 1] = beta * odin_sum2(internal.s_ij.data(), i - 1, i, 0, shared->dim_s_ij_2, shared->dim_s_ij_1);
    }
    ...
    for (int i = 1; i <= shared->dim_D; ++i) {
      state_next[shared->offset_variable_D + i - 1] = internal.new_D[i - 1];
    }
    state_next[3] = odin_sum1(internal.new_D.data(), 0, shared->dim_new_D);
    ...
    for (int i = 1; i <= shared->dim_aux_EE_1; ++i) {
      int j = 1;
      int k = 1;
      internal.aux_EE[i - 1 + shared->dim_aux_EE_1 * (j - 1) + shared->dim_aux_EE_12 * (k - 1)] = dust::distr::rbinom(rng_state, std::round(internal.n_SE[i - 1]), shared->aux_p_bin[shared->dim_aux_p_bin_1 * 0 + i - 1]);
    }
    for (int i = 1; i <= shared->dim_aux_EE_1; ++i) {
      int j = 1;
      for (int k = 2; k <= (shared->n_trans_classes - 1); ++k) {
        internal.aux_EE[i - 1 + shared->dim_aux_EE_1 * (j - 1) + shared->dim_aux_EE_12 * (k - 1)] = dust::distr::rbinom(rng_state, std::round(internal.n_SE[i - 1] - odin_sum3(internal.aux_EE.data(), i - 1, i, 0, 1, 0, k - 1, shared->dim_aux_EE_1, shared->dim_aux_EE_12)), shared->aux_p_bin[shared->dim_aux_p_bin_1 * (k - 1) + i - 1]);
      }
    }
    for (int i = 1; i <= shared->dim_aux_EE_1; ++i) {
      int j = 1;
      int k = shared->n_trans_classes;
      internal.aux_EE[i - 1 + shared->dim_aux_EE_1 * (j - 1) + shared->dim_aux_EE_12 * (k - 1)] = internal.n_SE[i - 1] - odin_sum3(internal.aux_EE.data(), i - 1, i, 0, 1, 0, shared->n_trans_classes - 1, shared->dim_aux_EE_1, shared->dim_aux_EE_12);
    }
    for (int i = 1; i <= shared->dim_aux_EE_1; ++i) {
    ...
    state_next[2] = odin_sum1(internal.new_I_ICU.data(), 0, shared->dim_new_I_ICU);
...
template<>
basic::init_t dust_pars<basic>(cpp11::list user) {
  typedef typename basic::real_t real_t;
  auto shared = std::make_shared<basic::shared_t>();
  basic::internal_t internal;
  ...
  shared->gamma_ICU = user_get_scalar<real_t>(user, "gamma_ICU", shared->gamma_ICU, NA_REAL, NA_REAL);
  shared->gamma_hosp = user_get_scalar<real_t>(user, "gamma_hosp", shared->gamma_hosp, NA_REAL, NA_REAL);
  shared->gamma_rec = user_get_scalar<real_t>(user, "gamma_rec", shared->gamma_rec, NA_REAL, NA_REAL);
  ...
  internal.delta_D = std::vector<real_t>(shared->dim_delta_D);
  internal.delta_R = std::vector<real_t>(shared->dim_delta_R);
  ...
  internal.new_D = std::vector<real_t>(shared->dim_new_D);
  ...
  shared->offset_variable_D = 5 + shared->dim_S + shared->dim_R;
  shared->offset_variable_E = 5 + shared->dim_S + shared->dim_R + shared->dim_D;
  shared->offset_variable_R = 5 + shared->dim_S;
  shared->p_C = user_get_array_fixed<real_t, 1>(user, "p_C", shared->p_C, {shared->dim_p_C}, NA_REAL, NA_REAL);
  shared->p_death_hosp = user_get_array_fixed<real_t, 1>(user, "p_death_hosp", shared->p_death_hosp, {shared->dim_p_death_hosp}, NA_REAL, NA_REAL);
  shared->p_recov_ICU = user_get_array_fixed<real_t, 1>(user, "p_recov_ICU", shared->p_recov_ICU, {shared->dim_p_recov_ICU}, NA_REAL, NA_REAL);
  shared->p_recov_hosp = user_get_array_fixed<real_t, 1>(user, "p_recov_hosp", shared->p_recov_hosp, {shared->dim_p_recov_hosp}, NA_REAL, NA_REAL);
  shared->p_recov_sympt = user_get_array_fixed<real_t, 1>(user, "p_recov_sympt", shared->p_recov_sympt, {shared->dim_p_recov_sympt}, NA_REAL, NA_REAL);
  internal.I_with_diff_trans = std::vector<real_t>(shared->dim_I_with_diff_trans);
  internal.aux_EE = std::vector<real_t>(shared->dim_aux_EE);
  internal.aux_II_A = std::vector<real_t>(shared->dim_aux_II_A);
  internal.aux_II_C = std::vector<real_t>(shared->dim_aux_II_C);
  internal.aux_II_ICU = std::vector<real_t>(shared->dim_aux_II_ICU);
  internal.aux_II_hosp = std::vector<real_t>(shared->dim_aux_II_hosp);
  internal.aux_R_hosp = std::vector<real_t>(shared->dim_aux_R_hosp);
  shared->aux_p_bin = std::vector<real_t>(shared->dim_aux_p_bin);
  internal.delta_E = std::vector<real_t>(shared->dim_delta_E);
  internal.delta_I_A = std::vector<real_t>(shared->dim_delta_I_A);
  internal.delta_I_C = std::vector<real_t>(shared->dim_delta_I_C);
  internal.delta_I_ICU = std::vector<real_t>(shared->dim_delta_I_ICU);
  internal.delta_I_hosp = std::vector<real_t>(shared->dim_delta_I_hosp);
  internal.delta_R_hosp = std::vector<real_t>(shared->dim_delta_R_hosp);
  ...
  shared->initial_I_ICU = std::vector<real_t>(shared->dim_I_ICU);
  shared->initial_I_hosp = std::vector<real_t>(shared->dim_I_hosp);
  shared->initial_R_hosp = std::vector<real_t>(shared->dim_R_hosp);
  ...
  internal.n_ICU_to_R_hosp = std::vector<real_t>(shared->dim_n_ICU_to_R_hosp);
  internal.n_II_A = std::vector<real_t>(shared->dim_n_II_A);
  internal.n_II_C = std::vector<real_t>(shared->dim_n_II_C);
  internal.n_II_ICU = std::vector<real_t>(shared->dim_n_II_ICU);
  internal.n_II_hosp = std::vector<real_t>(shared->dim_n_II_hosp);
  internal.n_R_hosp = std::vector<real_t>(shared->dim_n_R_hosp);
  internal.n_death_hosp = std::vector<real_t>(shared->dim_n_death_hosp);
  internal.n_hosp_to_ICU = std::vector<real_t>(shared->dim_n_hosp_to_ICU);
  internal.n_sympt_to_hosp = std::vector<real_t>(shared->dim_n_sympt_to_hosp);
  internal.new_I_ICU = std::vector<real_t>(shared->dim_new_I_ICU);
  internal.s_ij = std::vector<real_t>(shared->dim_s_ij);
  shared->trans_increase = std::vector<real_t>(shared->dim_trans_increase);
  shared->trans_profile = std::vector<real_t>(shared->dim_trans_profile);
  for (int i = 1; i <= shared->dim_E_1; ++i) {
    for (int j = 1; j <= shared->dim_E_2; ++j) {
      for (int k = 1; k <= shared->dim_E_3; ++k) {
  ...
  shared->offset_variable_I_A = 5 + shared->dim_S + shared->dim_R + shared->dim_D + shared->dim_E;
  shared->offset_variable_I_C = 5 + shared->dim_S + shared->dim_R + shared->dim_D + shared->dim_E + shared->dim_I_A;
  shared->offset_variable_I_ICU = 5 + shared->dim_S + shared->dim_R + shared->dim_D + shared->dim_E + shared->dim_I_A + shared->dim_I_C + shared->dim_I_hosp;
  shared->offset_variable_I_hosp = 5 + shared->dim_S + shared->dim_R + shared->dim_D + shared->dim_E + shared->dim_I_A + shared->dim_I_C;
  shared->offset_variable_R_hosp = 5 + shared->dim_S + shared->dim_R + shared->dim_D + shared->dim_E + shared->dim_I_A + shared->dim_I_C + shared->dim_I_hosp + shared->dim_I_ICU;
  ...
  for (int i = 1; i <= shared->dim_aux_p_bin_1; ++i) {
    for (int j = 2; j <= (shared->n_trans_classes - 1); ++j) {
      shared->aux_p_bin[i - 1 + shared->dim_aux_p_bin_1 * (j - 1)] = shared->trans_profile[shared->dim_trans_profile_1 * (j - 1) + i - 1] / (real_t) odin_sum2(shared->trans_profile.data(), i - 1, i, j - 1, shared->n_trans_classes, shared->dim_trans_profile_1);
    }
  }
  basic::init_t init = {shared, internal};
  return init;
}
template <>
cpp11::sexp dust_info<basic>(const basic::init_t& init) {
  const basic::internal_t internal = init.internal;
  const std::shared_ptr<const basic::shared_t> shared = init.shared;
  ...
  dim[8] = cpp11::writable::integers({shared->dim_E_1, shared->dim_E_2, shared->dim_E_3});
  dim[9] = cpp11::writable::integers({shared->dim_I_A_1, shared->dim_I_A_2, shared->dim_I_A_3});
  dim[10] = cpp11::writable::integers({shared->dim_I_C_1, shared->dim_I_C_2, shared->dim_I_C_3});
  dim[11] = cpp11::writable::integers({shared->dim_I_hosp_1, shared->dim_I_hosp_2, shared->dim_I_hosp_3});
  dim[12] = cpp11::writable::integers({shared->dim_I_ICU_1, shared->dim_I_ICU_2, shared->dim_I_ICU_3});
  dim[13] = cpp11::writable::integers({shared->dim_R_hosp_1, shared->dim_R_hosp_2, shared->dim_R_hosp_3});
  dim.names() = nms;
  ...
  index[8] = integer_sequence(shared->offset_variable_E + 1, shared->dim_E);
  index[9] = integer_sequence(shared->offset_variable_I_A + 1, shared->dim_I_A);
  index[10] = integer_sequence(shared->offset_variable_I_C + 1, shared->dim_I_C);
  index[11] = integer_sequence(shared->offset_variable_I_hosp + 1, shared->dim_I_hosp);
  index[12] = integer_sequence(shared->offset_variable_I_ICU + 1, shared->dim_I_ICU);
  index[13] = integer_sequence(shared->offset_variable_R_hosp + 1, shared->dim_R_hosp);
  index.names() = nms;
...
[[cpp11::register]]
SEXP dust_basic_alloc(cpp11::list r_pars, bool pars_multi, size_t step,
                         size_t n_particles, size_t n_threads,
                         cpp11::sexp r_seed) {
  return dust_alloc<basic>(r_pars, pars_multi, step, n_particles,
                               n_threads, r_seed);
}
...
SEXP dust_basic_reset(SEXP ptr, cpp11::list r_pars, size_t step) {
  return dust_reset<basic>(ptr, r_pars, step);
}
...
SEXP dust_basic_set_pars(SEXP ptr, cpp11::list r_pars) {
  return dust_set_pars<basic>(ptr, r_pars);
}
...
SEXP dust_basic_set_data(SEXP ptr, cpp11::list data) {
  dust_set_data<basic>(ptr, data);
  return R_NilValue;
}

[[cpp11::register]]
SEXP dust_basic_compare_data(SEXP ptr) {
  return dust_compare_data<basic>(ptr);
}
...
SEXP dust_basic_simulate(cpp11::sexp r_steps,
                            ...
                            cpp11::sexp r_seed,
                            bool return_state) {
  return dust_simulate<basic>(r_steps, r_pars, r_state, r_index,
                                  n_threads, r_seed, return_state);
}

[[cpp11::register]]
cpp11::sexp dust_basic_capabilities() {
  return dust_capabilities<basic>();
}
