...
[[cpp11::register]]
SEXP dust_basic_rng_state(SEXP ptr, bool first_only, bool last_only);
...
[[cpp11::register]]
SEXP dust_basic_filter(SEXP ptr, bool save_trajectories,
...
typename T::real_t compare(const typename T::real_t * state,
  ...
  real_t ll_icu = ll_nbinom(data.icu, shared->phi_ICU * state[2],
                            shared->kappa_ICU, shared->exp_noise,
                            rng_state);
  real_t ll_deaths = ll_nbinom(data.deaths, shared->phi_death * state[4],
...
public:
  ...
  basic(const dust::pars_t<basic>& pars) :
  ...
  std::vector<real_t> initial(size_t step) {
    std::vector<real_t> state(shared->dim_D + shared->dim_E + shared->dim_I_A + shared->dim_I_C + shared->dim_I_ICU + shared->dim_I_hosp + shared->dim_R + shared->dim_R_hosp + shared->dim_S + 6);
  ...
  HOST void update(size_t step, const real_t * state, dust::rng_state_t<real_t>& rng_state, real_t * state_next) {
    ...
    const real_t D_tot = state[3];
    const real_t D_inc = state[4];
    state_next[1] = odin_sum1<real_t>(S, 0, shared->dim_S) + odin_sum1<real_t>(R, 0, shared->dim_R) + odin_sum1<real_t>(D, 0, shared->dim_D) + odin_sum1<real_t>(E, 0, shared->dim_E) + odin_sum1<real_t>(I_A, 0, shared->dim_I_A) + odin_sum1<real_t>(I_C, 0, shared->dim_I_C) + odin_sum1<real_t>(I_hosp, 0, shared->dim_I_hosp) + odin_sum1<real_t>(I_ICU, 0, shared->dim_I_ICU) + odin_sum1<real_t>(R_hosp, 0, shared->dim_R_hosp);
    ...
    for (int i = 1; i <= shared->dim_I_with_diff_trans_1; ++i) {
      for (int j = 1; j <= shared->dim_I_with_diff_trans_2; ++j) {
        internal.I_with_diff_trans[i - 1 + shared->dim_I_with_diff_trans_1 * (j - 1)] = shared->trans_increase[shared->dim_trans_increase_1 * (j - 1) + i - 1] * (odin_sum3<real_t>(I_A, i - 1, i, 0, shared->dim_I_A_2, j - 1, j, shared->dim_I_A_1, shared->dim_I_A_12) + odin_sum3<real_t>(I_C, i - 1, i, 0, shared->dim_I_C_2, j - 1, j, shared->dim_I_C_1, shared->dim_I_C_12) + shared->hosp_transmission * odin_sum3<real_t>(I_hosp, i - 1, i, 0, shared->dim_I_hosp_2, j - 1, j, shared->dim_I_hosp_1, shared->dim_I_hosp_12) + shared->ICU_transmission * odin_sum3<real_t>(I_ICU, i - 1, i, 0, shared->dim_I_ICU_2, j - 1, j, shared->dim_I_ICU_1, shared->dim_I_ICU_12));
      }
    }
    ...
    for (int i = 1; i <= shared->dim_delta_D; ++i) {
      internal.delta_D[i - 1] = odin_sum3<real_t>(internal.n_II_ICU.data(), i - 1, i, shared->k_ICU - 1, shared->k_ICU, 0, shared->dim_n_II_ICU_3, shared->dim_n_II_ICU_1, shared->dim_n_II_ICU_12) - odin_sum2<real_t>(internal.n_ICU_to_R_hosp.data(), i - 1, i, 0, shared->dim_n_ICU_to_R_hosp_2, shared->dim_n_ICU_to_R_hosp_1) + odin_sum2<real_t>(internal.n_death_hosp.data(), i - 1, i, 0, shared->dim_n_death_hosp_2, shared->dim_n_death_hosp_1);
    }
    ...
    for (int i = 1; i <= shared->dim_s_ij_1; ++i) {
      for (int j = 1; j <= shared->dim_s_ij_2; ++j) {
        internal.s_ij[i - 1 + shared->dim_s_ij_1 * (j - 1)] = shared->m[shared->dim_m_1 * (j - 1) + i - 1] * odin_sum2<real_t>(internal.I_with_diff_trans.data(), j - 1, j, 0, shared->dim_I_with_diff_trans_2, shared->dim_I_with_diff_trans_1);
      }
    }
    ...
    for (int i = 1; i <= shared->dim_delta_R; ++i) {
      internal.delta_R[i - 1] = odin_sum3<real_t>(internal.n_II_A.data(), i - 1, i, shared->k_A - 1, shared->k_A, 0, shared->dim_n_II_A_3, shared->dim_n_II_A_1, shared->dim_n_II_A_12) + odin_sum3<real_t>(internal.n_II_C.data(), i - 1, i, shared->k_C - 1, shared->k_C, 0, shared->dim_n_II_C_3, shared->dim_n_II_C_1, shared->dim_n_II_C_12) - odin_sum2<real_t>(internal.n_sympt_to_hosp.data(), i - 1, i, 0, shared->dim_n_sympt_to_hosp_2, shared->dim_n_sympt_to_hosp_1) + odin_sum3<real_t>(internal.n_II_hosp.data(), i - 1, i, shared->k_hosp - 1, shared->k_hosp, 0, shared->dim_n_II_hosp_3, shared->dim_n_II_hosp_1, shared->dim_n_II_hosp_12) - odin_sum2<real_t>(internal.n_hosp_to_ICU.data(), i - 1, i, 0, shared->dim_n_hosp_to_ICU_2, shared->dim_n_hosp_to_ICU_1) - odin_sum2<real_t>(internal.n_death_hosp.data(), i - 1, i, 0, shared->dim_n_death_hosp_2, shared->dim_n_death_hosp_1) + odin_sum3<real_t>(internal.n_R_hosp.data(), i - 1, i, shared->k_rec - 1, shared->k_rec, 0, shared->dim_n_R_hosp_3, shared->dim_n_R_hosp_1, shared->dim_n_R_hosp_12);
    }
    ...
    for (int i = 1; i <= shared->dim_lambda; ++i) {
      internal.lambda[i - 1] = beta * odin_sum2<real_t>(internal.s_ij.data(), i - 1, i, 0, shared->dim_s_ij_2, shared->dim_s_ij_1);
    }
    ...
    real_t tot_new_D = odin_sum1<real_t>(internal.new_D.data(), 0, shared->dim_new_D);
    ...
    state_next[4] = (fmodr<real_t>(step, shared->steps_per_day) == 0 ? tot_new_D : D_inc + tot_new_D);
    ...
    for (int i = 1; i <= shared->dim_aux_EE_1; ++i) {
      ...
      for (int k = 2; k <= (shared->n_trans_classes - 1); ++k) {
        internal.aux_EE[i - 1 + shared->dim_aux_EE_1 * (j - 1) + shared->dim_aux_EE_12 * (k - 1)] = dust::distr::rbinom(rng_state, std::round(internal.n_SE[i - 1] - odin_sum3<real_t>(internal.aux_EE.data(), i - 1, i, 0, 1, 0, k - 1, shared->dim_aux_EE_1, shared->dim_aux_EE_12)), shared->aux_p_bin[shared->dim_aux_p_bin_1 * (k - 1) + i - 1]);
      }
    }
    for (int i = 1; i <= shared->dim_aux_EE_1; ++i) {
      int j = 1;
      int k = shared->n_trans_classes;
      internal.aux_EE[i - 1 + shared->dim_aux_EE_1 * (j - 1) + shared->dim_aux_EE_12 * (k - 1)] = internal.n_SE[i - 1] - odin_sum3<real_t>(internal.aux_EE.data(), i - 1, i, 0, 1, 0, shared->n_trans_classes - 1, shared->dim_aux_EE_1, shared->dim_aux_EE_12);
    }
    for (int i = 1; i <= shared->dim_aux_EE_1; ++i) {
    ...
    state_next[2] = odin_sum1<real_t>(internal.new_I_ICU.data(), 0, shared->dim_new_I_ICU);
...
namespace dust {
template<>
dust::pars_t<basic> dust_pars<basic>(cpp11::list user) {
  ...
  shared->phi_death = user_get_scalar<real_t>(user, "phi_death", shared->phi_death, NA_REAL, NA_REAL);
  ...
  for (int i = 1; i <= shared->dim_aux_p_bin_1; ++i) {
    for (int j = 2; j <= (shared->n_trans_classes - 1); ++j) {
      shared->aux_p_bin[i - 1 + shared->dim_aux_p_bin_1 * (j - 1)] = shared->trans_profile[shared->dim_trans_profile_1 * (j - 1) + i - 1] / (real_t) odin_sum2<real_t>(shared->trans_profile.data(), i - 1, i, j - 1, shared->n_trans_classes, shared->dim_trans_profile_1);
    }
  }
  return dust::pars_t<basic>(shared, internal);
}
template <>
cpp11::sexp dust_info<basic>(const dust::pars_t<basic>& pars) {
  const basic::internal_t internal = pars.internal;
  const std::shared_ptr<const basic::shared_t> shared = pars.shared;
...
template <>
basic::data_t dust_data<basic>(cpp11::list data) {
  typedef basic::real_t real_t;
  return basic::data_t{
...
SEXP dust_basic_alloc(cpp11::list r_pars, bool pars_multi, size_t step,
                         cpp11::sexp r_n_particles, size_t n_threads,
                         cpp11::sexp r_seed, cpp11::sexp device_id) {
  return dust::r::dust_alloc<basic>(r_pars, pars_multi, step, r_n_particles,
                                        n_threads, r_seed, device_id);
}

SEXP dust_basic_run(SEXP ptr, size_t step_end, bool device) {
  return dust::r::dust_run<basic>(ptr, step_end, device);
}

SEXP dust_basic_simulate(SEXP ptr, cpp11::sexp step_end) {
  return dust::r::dust_simulate<basic>(ptr, step_end);
}

SEXP dust_basic_set_index(SEXP ptr, cpp11::sexp r_index) {
  dust::r::dust_set_index<basic>(ptr, r_index);
  return R_NilValue;
}

SEXP dust_basic_set_state(SEXP ptr, SEXP r_state, SEXP r_step) {
  dust::r::dust_set_state<basic>(ptr, r_state, r_step);
  return R_NilValue;
}

SEXP dust_basic_reset(SEXP ptr, cpp11::list r_pars, size_t step) {
  return dust::r::dust_reset<basic>(ptr, r_pars, step);
}

SEXP dust_basic_state(SEXP ptr, SEXP r_index) {
  return dust::r::dust_state<basic>(ptr, r_index);
}

size_t dust_basic_step(SEXP ptr) {
  return dust::r::dust_step<basic>(ptr);
}

void dust_basic_reorder(SEXP ptr, cpp11::sexp r_index) {
  return dust::r::dust_reorder<basic>(ptr, r_index);
}

SEXP dust_basic_resample(SEXP ptr, cpp11::doubles r_weights) {
  return dust::r::dust_resample<basic>(ptr, r_weights);
}

SEXP dust_basic_set_pars(SEXP ptr, cpp11::list r_pars) {
  return dust::r::dust_set_pars<basic>(ptr, r_pars);
}

SEXP dust_basic_rng_state(SEXP ptr, bool first_only, bool last_only) {
  return dust::r::dust_rng_state<basic>(ptr, first_only, last_only);
}

SEXP dust_basic_set_rng_state(SEXP ptr, cpp11::raws rng_state) {
  dust::r::dust_set_rng_state<basic>(ptr, rng_state);
  return R_NilValue;
}

SEXP dust_basic_set_data(SEXP ptr, cpp11::list data) {
  dust::r::dust_set_data<basic>(ptr, data);
  return R_NilValue;
}

SEXP dust_basic_compare_data(SEXP ptr, bool device) {
  return dust::r::dust_compare_data<basic>(ptr, device);
}

SEXP dust_basic_filter(SEXP ptr, bool save_trajectories,
                          cpp11::sexp step_snapshot,
                          bool device) {
  return dust::r::dust_filter<basic>(ptr, save_trajectories, step_snapshot, device);
}
...
void dust_basic_set_n_threads(SEXP ptr, int n_threads) {
  return dust::r::dust_set_n_threads<basic>(ptr, n_threads);
}

int dust_basic_n_state(SEXP ptr) {
  return dust::r::dust_n_state<basic>(ptr);
}

cpp11::sexp dust_basic_device_info() {
  return dust::cuda::device_info<basic>();
}
