// Generated by dust (version 0.9.5) - do not edit
#include <cpp11.hpp>
[[cpp11::register]]
...
public:
  ...
  HOST void update(size_t step, const real_t * state, dust::rng_state_t<real_t>& rng_state, real_t * state_next) {
    ...
    real_t strain_seed = dust::distr::rpois(rng_state, strain_rate);
      ...
      for (int j = 1; j <= shared->dim_n_T_sero_pos_1_progress_2; ++j) {
        for (int k = 1; k <= shared->dim_n_T_sero_pos_1_progress_3; ++k) {
          for (int l = 1; l <= shared->dim_n_T_sero_pos_1_progress_4; ++l) {
            internal.n_T_sero_pos_1_progress[i - 1 + shared->dim_n_T_sero_pos_1_progress_1 * (j - 1) + shared->dim_n_T_sero_pos_1_progress_12 * (k - 1) + shared->dim_n_T_sero_pos_1_progress_123 * (l - 1)] = dust::distr::rbinom(rng_state, std::round(T_sero_pos_1[shared->dim_T_sero_pos_1_123 * (l - 1) + shared->dim_T_sero_pos_1_12 * (k - 1) + shared->dim_T_sero_pos_1_1 * (j - 1) + i - 1]), shared->p_T_sero_pos_1_progress);
          }
        }
      }
    }
      ...
      for (int j = 1; j <= shared->dim_n_T_sero_pos_2_progress_2; ++j) {
        for (int k = 1; k <= shared->dim_n_T_sero_pos_2_progress_3; ++k) {
          for (int l = 1; l <= shared->dim_n_T_sero_pos_2_progress_4; ++l) {
            internal.n_T_sero_pos_2_progress[i - 1 + shared->dim_n_T_sero_pos_2_progress_1 * (j - 1) + shared->dim_n_T_sero_pos_2_progress_12 * (k - 1) + shared->dim_n_T_sero_pos_2_progress_123 * (l - 1)] = dust::distr::rbinom(rng_state, std::round(T_sero_pos_2[shared->dim_T_sero_pos_2_123 * (l - 1) + shared->dim_T_sero_pos_2_12 * (k - 1) + shared->dim_T_sero_pos_2_1 * (j - 1) + i - 1]), shared->p_T_sero_pos_2_progress);
          }
        }
      }
    }
      ...
      for (int j = 1; j <= shared->dim_n_T_sero_pre_2_progress_2; ++j) {
        for (int k = 1; k <= shared->dim_n_T_sero_pre_2_progress_3; ++k) {
          for (int l = 1; l <= shared->dim_n_T_sero_pre_2_progress_4; ++l) {
            internal.n_T_sero_pre_2_progress[i - 1 + shared->dim_n_T_sero_pre_2_progress_1 * (j - 1) + shared->dim_n_T_sero_pre_2_progress_12 * (k - 1) + shared->dim_n_T_sero_pre_2_progress_123 * (l - 1)] = dust::distr::rbinom(rng_state, std::round(T_sero_pre_2[shared->dim_T_sero_pre_2_123 * (l - 1) + shared->dim_T_sero_pre_2_12 * (k - 1) + shared->dim_T_sero_pre_2_1 * (j - 1) + i - 1]), shared->p_T_sero_pre_2_progress);
          }
        }
      }
    }
      ...
      for (int j = 1; j <= shared->dim_n_E_progress_2; ++j) {
        for (int k = 1; k <= shared->dim_n_E_progress_3; ++k) {
          for (int l = 1; l <= shared->dim_n_E_progress_4; ++l) {
            internal.n_E_progress[i - 1 + shared->dim_n_E_progress_1 * (j - 1) + shared->dim_n_E_progress_12 * (k - 1) + shared->dim_n_E_progress_123 * (l - 1)] = dust::distr::rbinom(rng_state, std::round(E[shared->dim_E_123 * (l - 1) + shared->dim_E_12 * (k - 1) + shared->dim_E_1 * (j - 1) + i - 1]), internal.p_E_progress[j - 1]);
          }
        }
      }
    }
      ...
      for (int j = 1; j <= shared->dim_n_G_D_progress_2; ++j) {
        for (int k = 1; k <= shared->dim_n_G_D_progress_3; ++k) {
          for (int l = 1; l <= shared->dim_n_G_D_progress_4; ++l) {
            internal.n_G_D_progress[i - 1 + shared->dim_n_G_D_progress_1 * (j - 1) + shared->dim_n_G_D_progress_12 * (k - 1) + shared->dim_n_G_D_progress_123 * (l - 1)] = dust::distr::rbinom(rng_state, std::round(G_D[shared->dim_G_D_123 * (l - 1) + shared->dim_G_D_12 * (k - 1) + shared->dim_G_D_1 * (j - 1) + i - 1]), internal.p_G_D_progress[j - 1]);
          }
        }
      }
    }
      ...
      for (int j = 1; j <= shared->dim_n_I_A_progress_2; ++j) {
        for (int k = 1; k <= shared->dim_n_I_A_progress_3; ++k) {
          for (int l = 1; l <= shared->dim_n_I_A_progress_4; ++l) {
            internal.n_I_A_progress[i - 1 + shared->dim_n_I_A_progress_1 * (j - 1) + shared->dim_n_I_A_progress_12 * (k - 1) + shared->dim_n_I_A_progress_123 * (l - 1)] = dust::distr::rbinom(rng_state, std::round(I_A[shared->dim_I_A_123 * (l - 1) + shared->dim_I_A_12 * (k - 1) + shared->dim_I_A_1 * (j - 1) + i - 1]), internal.p_I_A_progress[j - 1]);
          }
        }
      }
    }
      ...
      for (int j = 1; j <= shared->dim_n_I_C_1_progress_2; ++j) {
        for (int k = 1; k <= shared->dim_n_I_C_1_progress_3; ++k) {
          for (int l = 1; l <= shared->dim_n_I_C_1_progress_4; ++l) {
            internal.n_I_C_1_progress[i - 1 + shared->dim_n_I_C_1_progress_1 * (j - 1) + shared->dim_n_I_C_1_progress_12 * (k - 1) + shared->dim_n_I_C_1_progress_123 * (l - 1)] = dust::distr::rbinom(rng_state, std::round(I_C_1[shared->dim_I_C_1_123 * (l - 1) + shared->dim_I_C_1_12 * (k - 1) + shared->dim_I_C_1_1 * (j - 1) + i - 1]), internal.p_I_C_1_progress[j - 1]);
          }
        }
      }
    }
      ...
      for (int j = 1; j <= shared->dim_n_I_C_2_progress_2; ++j) {
        for (int k = 1; k <= shared->dim_n_I_C_2_progress_3; ++k) {
          for (int l = 1; l <= shared->dim_n_I_C_2_progress_4; ++l) {
            internal.n_I_C_2_progress[i - 1 + shared->dim_n_I_C_2_progress_1 * (j - 1) + shared->dim_n_I_C_2_progress_12 * (k - 1) + shared->dim_n_I_C_2_progress_123 * (l - 1)] = dust::distr::rbinom(rng_state, std::round(I_C_2[shared->dim_I_C_2_123 * (l - 1) + shared->dim_I_C_2_12 * (k - 1) + shared->dim_I_C_2_1 * (j - 1) + i - 1]), internal.p_I_C_2_progress[j - 1]);
          }
        }
      }
    }
      ...
      for (int j = 1; j <= shared->dim_n_I_P_progress_2; ++j) {
        for (int k = 1; k <= shared->dim_n_I_P_progress_3; ++k) {
          for (int l = 1; l <= shared->dim_n_I_P_progress_4; ++l) {
            internal.n_I_P_progress[i - 1 + shared->dim_n_I_P_progress_1 * (j - 1) + shared->dim_n_I_P_progress_12 * (k - 1) + shared->dim_n_I_P_progress_123 * (l - 1)] = dust::distr::rbinom(rng_state, std::round(I_P[shared->dim_I_P_123 * (l - 1) + shared->dim_I_P_12 * (k - 1) + shared->dim_I_P_1 * (j - 1) + i - 1]), internal.p_I_P_progress[j - 1]);
          }
        }
      }
    }
    ...
    for (int i = 1; i <= shared->dim_n_T_sero_pre_2_to_T_sero_pos_2_1; ++i) {
      for (int j = 1; j <= shared->dim_n_T_sero_pre_2_to_T_sero_pos_2_2; ++j) {
        for (int k = 1; k <= shared->dim_n_T_sero_pre_2_to_T_sero_pos_2_3; ++k) {
          internal.n_T_sero_pre_2_to_T_sero_pos_2[i - 1 + shared->dim_n_T_sero_pre_2_to_T_sero_pos_2_1 * (j - 1) + shared->dim_n_T_sero_pre_2_to_T_sero_pos_2_12 * (k - 1)] = dust::distr::rbinom(rng_state, std::round(internal.n_T_sero_pre_2_progress[shared->dim_n_T_sero_pre_2_progress_123 * (k - 1) + shared->dim_n_T_sero_pre_2_progress_12 * (shared->k_sero_pre_2 - 1) + shared->dim_n_T_sero_pre_2_progress_1 * (j - 1) + i - 1]), shared->p_sero_pos_2[i - 1]);
        }
      }
    }
      ...
      for (int j = 1; j <= shared->dim_n_W_D_conf_progress_2; ++j) {
        for (int k = 1; k <= shared->dim_n_W_D_conf_progress_3; ++k) {
          for (int l = 1; l <= shared->dim_n_W_D_conf_progress_4; ++l) {
            internal.n_W_D_conf_progress[i - 1 + shared->dim_n_W_D_conf_progress_1 * (j - 1) + shared->dim_n_W_D_conf_progress_12 * (k - 1) + shared->dim_n_W_D_conf_progress_123 * (l - 1)] = dust::distr::rbinom(rng_state, std::round(W_D_conf[shared->dim_W_D_conf_123 * (l - 1) + shared->dim_W_D_conf_12 * (k - 1) + shared->dim_W_D_conf_1 * (j - 1) + i - 1]), internal.p_W_D_progress[j - 1]);
          }
        }
      }
    }
    ...
    for (int i = 1; i <= shared->dim_n_EI_A_1; ++i) {
      for (int j = 1; j <= shared->dim_n_EI_A_2; ++j) {
        for (int k = 1; k <= shared->dim_n_EI_A_3; ++k) {
          internal.n_EI_A[i - 1 + shared->dim_n_EI_A_1 * (j - 1) + shared->dim_n_EI_A_12 * (k - 1)] = dust::distr::rbinom(rng_state, std::round(internal.n_E_progress[shared->dim_n_E_progress_123 * (k - 1) + shared->dim_n_E_progress_12 * (shared->k_E - 1) + shared->dim_n_E_progress_1 * (j - 1) + i - 1]), 1 - internal.p_C[shared->dim_p_C_12 * (k - 1) + shared->dim_p_C_1 * (j - 1) + i - 1]);
        }
      }
    }
    for (int i = 1; i <= shared->dim_n_ICU_pre_conf_to_ICU_D_conf_1; ++i) {
      for (int j = 1; j <= shared->dim_n_ICU_pre_conf_to_ICU_D_conf_2; ++j) {
        for (int k = 1; k <= shared->dim_n_ICU_pre_conf_to_ICU_D_conf_3; ++k) {
          internal.n_ICU_pre_conf_to_ICU_D_conf[i - 1 + shared->dim_n_ICU_pre_conf_to_ICU_D_conf_1 * (j - 1) + shared->dim_n_ICU_pre_conf_to_ICU_D_conf_12 * (k - 1)] = dust::distr::rbinom(rng_state, std::round(internal.n_ICU_pre_conf_progress[shared->dim_n_ICU_pre_conf_progress_123 * (k - 1) + shared->dim_n_ICU_pre_conf_progress_12 * (shared->k_ICU_pre - 1) + shared->dim_n_ICU_pre_conf_progress_1 * (j - 1) + i - 1]), internal.p_ICU_D[shared->dim_p_ICU_D_12 * (k - 1) + shared->dim_p_ICU_D_1 * (j - 1) + i - 1]);
        }
      }
    }
    for (int i = 1; i <= shared->dim_n_ICU_pre_unconf_to_ICU_D_unconf_1; ++i) {
      for (int j = 1; j <= shared->dim_n_ICU_pre_unconf_to_ICU_D_unconf_2; ++j) {
        for (int k = 1; k <= shared->dim_n_ICU_pre_unconf_to_ICU_D_unconf_3; ++k) {
          internal.n_ICU_pre_unconf_to_ICU_D_unconf[i - 1 + shared->dim_n_ICU_pre_unconf_to_ICU_D_unconf_1 * (j - 1) + shared->dim_n_ICU_pre_unconf_to_ICU_D_unconf_12 * (k - 1)] = dust::distr::rbinom(rng_state, std::round(internal.n_ICU_pre_unconf_progress[shared->dim_n_ICU_pre_unconf_progress_123 * (k - 1) + shared->dim_n_ICU_pre_unconf_progress_12 * (shared->k_ICU_pre - 1) + shared->dim_n_ICU_pre_unconf_progress_1 * (j - 1) + i - 1]), internal.p_ICU_D[shared->dim_p_ICU_D_12 * (k - 1) + shared->dim_p_ICU_D_1 * (j - 1) + i - 1]);
        }
      }
    }
    ...
    for (int i = 1; i <= shared->dim_n_I_C_2_to_G_D_1; ++i) {
      for (int j = 1; j <= shared->dim_n_I_C_2_to_G_D_2; ++j) {
        for (int k = 1; k <= shared->dim_n_I_C_2_to_G_D_3; ++k) {
          internal.n_I_C_2_to_G_D[i - 1 + shared->dim_n_I_C_2_to_G_D_1 * (j - 1) + shared->dim_n_I_C_2_to_G_D_12 * (k - 1)] = dust::distr::rbinom(rng_state, std::round(internal.n_I_C_2_progress[shared->dim_n_I_C_2_progress_123 * (k - 1) + shared->dim_n_I_C_2_progress_12 * (shared->k_C_2 - 1) + shared->dim_n_I_C_2_progress_1 * (j - 1) + i - 1] - internal.n_I_C_2_to_R[shared->dim_n_I_C_2_to_R_12 * (k - 1) + shared->dim_n_I_C_2_to_R_1 * (j - 1) + i - 1]), internal.p_G_D[shared->dim_p_G_D_12 * (k - 1) + shared->dim_p_G_D_1 * (j - 1) + i - 1]);
        }
      }
    }
    ...
    for (int i = 1; i <= shared->dim_n_I_C_2_to_ICU_pre_1; ++i) {
      for (int j = 1; j <= shared->dim_n_I_C_2_to_ICU_pre_2; ++j) {
        for (int k = 1; k <= shared->dim_n_I_C_2_to_ICU_pre_3; ++k) {
          internal.n_I_C_2_to_ICU_pre[i - 1 + shared->dim_n_I_C_2_to_ICU_pre_1 * (j - 1) + shared->dim_n_I_C_2_to_ICU_pre_12 * (k - 1)] = dust::distr::rbinom(rng_state, std::round(internal.n_I_C_2_to_hosp[shared->dim_n_I_C_2_to_hosp_12 * (k - 1) + shared->dim_n_I_C_2_to_hosp_1 * (j - 1) + i - 1]), internal.p_ICU[shared->dim_p_ICU_12 * (k - 1) + shared->dim_p_ICU_1 * (j - 1) + i - 1]);
        }
      }
    }
    ...
    for (int i = 1; i <= shared->dim_n_I_C_2_to_ICU_pre_conf_1; ++i) {
      for (int j = 1; j <= shared->dim_n_I_C_2_to_ICU_pre_conf_2; ++j) {
        for (int k = 1; k <= shared->dim_n_I_C_2_to_ICU_pre_conf_3; ++k) {
          internal.n_I_C_2_to_ICU_pre_conf[i - 1 + shared->dim_n_I_C_2_to_ICU_pre_conf_1 * (j - 1) + shared->dim_n_I_C_2_to_ICU_pre_conf_12 * (k - 1)] = dust::distr::rbinom(rng_state, std::round(internal.n_I_C_2_to_ICU_pre[shared->dim_n_I_C_2_to_ICU_pre_12 * (k - 1) + shared->dim_n_I_C_2_to_ICU_pre_1 * (j - 1) + i - 1]), internal.p_star[i - 1]);
        }
      }
    }
    ...
    for (int i = 1; i <= shared->dim_n_R_progress_1; ++i) {
      for (int j = 1; j <= shared->dim_n_R_progress_2; ++j) {
        for (int k = 1; k <= shared->dim_n_R_progress_3; ++k) {
          internal.n_R_progress[i - 1 + shared->dim_n_R_progress_1 * (j - 1) + shared->dim_n_R_progress_12 * (k - 1)] = dust::distr::rbinom(rng_state, std::round(R[shared->dim_R_12 * (k - 1) + shared->dim_R_1 * (j - 1) + i - 1]), internal.p_R_progress[shared->dim_p_R_progress_12 * (k - 1) + shared->dim_p_R_progress_1 * (j - 1) + i - 1]);
        }
      }
    }
    ...
    for (int i = 1; i <= shared->dim_n_I_C_2_to_H_D_1; ++i) {
      for (int j = 1; j <= shared->dim_n_I_C_2_to_H_D_2; ++j) {
        for (int k = 1; k <= shared->dim_n_I_C_2_to_H_D_3; ++k) {
          internal.n_I_C_2_to_H_D[i - 1 + shared->dim_n_I_C_2_to_H_D_1 * (j - 1) + shared->dim_n_I_C_2_to_H_D_12 * (k - 1)] = dust::distr::rbinom(rng_state, std::round(internal.n_hosp_non_ICU[shared->dim_n_hosp_non_ICU_12 * (k - 1) + shared->dim_n_hosp_non_ICU_1 * (j - 1) + i - 1]), internal.p_H_D[shared->dim_p_H_D_12 * (k - 1) + shared->dim_p_H_D_1 * (j - 1) + i - 1]);
        }
      }
    }
    for (int i = 1; i <= shared->dim_n_RS_1; ++i) {
      for (int j = 1; j <= shared->dim_n_RS_2; ++j) {
        for (int k = 1; k <= shared->dim_n_RS_3; ++k) {
          internal.n_RS[i - 1 + shared->dim_n_RS_1 * (j - 1) + shared->dim_n_RS_12 * (k - 1)] = dust::distr::rbinom(rng_state, std::round(internal.n_R_progress[shared->dim_n_R_progress_12 * (k - 1) + shared->dim_n_R_progress_1 * (j - 1) + i - 1]), internal.p_RS[shared->dim_p_RS_12 * (k - 1) + shared->dim_p_RS_1 * (j - 1) + i - 1]);
        }
      }
    }
    ...
    for (int i = 1; i <= shared->dim_n_I_C_2_to_H_D_conf_1; ++i) {
      for (int j = 1; j <= shared->dim_n_I_C_2_to_H_D_conf_2; ++j) {
        for (int k = 1; k <= shared->dim_n_I_C_2_to_H_D_conf_3; ++k) {
          internal.n_I_C_2_to_H_D_conf[i - 1 + shared->dim_n_I_C_2_to_H_D_conf_1 * (j - 1) + shared->dim_n_I_C_2_to_H_D_conf_12 * (k - 1)] = dust::distr::rbinom(rng_state, std::round(internal.n_I_C_2_to_H_D[shared->dim_n_I_C_2_to_H_D_12 * (k - 1) + shared->dim_n_I_C_2_to_H_D_1 * (j - 1) + i - 1]), internal.p_star[i - 1]);
        }
      }
    }
    ...
    for (int i = 1; i <= shared->dim_n_R_next_vacc_class_1; ++i) {
      for (int j = 1; j <= shared->dim_n_R_next_vacc_class_2; ++j) {
        for (int k = 1; k <= shared->dim_n_R_next_vacc_class_3; ++k) {
          internal.n_R_next_vacc_class[i - 1 + shared->dim_n_R_next_vacc_class_1 * (j - 1) + shared->dim_n_R_next_vacc_class_12 * (k - 1)] = dust::distr::rbinom(rng_state, std::round(internal.n_R_tmp[shared->dim_n_R_tmp_12 * (k - 1) + shared->dim_n_R_tmp_1 * (j - 1) + i - 1]), internal.p_R_next_vacc_class[shared->dim_p_R_next_vacc_class_12 * (k - 1) + shared->dim_p_R_next_vacc_class_1 * (j - 1) + i - 1]);
        }
      }
    }
    ...
    for (int i = 1; i <= shared->dim_n_I_C_2_to_H_R_conf_1; ++i) {
      for (int j = 1; j <= shared->dim_n_I_C_2_to_H_R_conf_2; ++j) {
        for (int k = 1; k <= shared->dim_n_I_C_2_to_H_R_conf_3; ++k) {
          internal.n_I_C_2_to_H_R_conf[i - 1 + shared->dim_n_I_C_2_to_H_R_conf_1 * (j - 1) + shared->dim_n_I_C_2_to_H_R_conf_12 * (k - 1)] = dust::distr::rbinom(rng_state, std::round(internal.n_I_C_2_to_H_R[shared->dim_n_I_C_2_to_H_R_12 * (k - 1) + shared->dim_n_I_C_2_to_H_R_1 * (j - 1) + i - 1]), internal.p_star[i - 1]);
        }
      }
    }
    ...
    for (int i = 1; i <= shared->dim_new_R_1; ++i) {
      for (int j = 1; j <= shared->dim_new_R_2; ++j) {
        for (int k = 1; k <= shared->dim_new_R_3; ++k) {
          internal.new_R[i - 1 + shared->dim_new_R_1 * (j - 1) + shared->dim_new_R_12 * (k - 1)] = R[shared->dim_R_12 * (k - 1) + shared->dim_R_1 * (j - 1) + i - 1] - internal.n_R_progress[shared->dim_n_R_progress_12 * (k - 1) + shared->dim_n_R_progress_1 * (j - 1) + i - 1] - internal.n_R_next_vacc_class[shared->dim_n_R_next_vacc_class_12 * (k - 1) + shared->dim_n_R_next_vacc_class_1 * (j - 1) + i - 1] + internal.n_I_A_progress[shared->dim_n_I_A_progress_123 * (k - 1) + shared->dim_n_I_A_progress_12 * (shared->k_A - 1) + shared->dim_n_I_A_progress_1 * (j - 1) + i - 1] + internal.n_I_C_2_to_R[shared->dim_n_I_C_2_to_R_12 * (k - 1) + shared->dim_n_I_C_2_to_R_1 * (j - 1) + i - 1] + internal.n_H_R_conf_progress[shared->dim_n_H_R_conf_progress_123 * (k - 1) + shared->dim_n_H_R_conf_progress_12 * (shared->k_H_R - 1) + shared->dim_n_H_R_conf_progress_1 * (j - 1) + i - 1] + internal.n_H_R_unconf_progress[shared->dim_n_H_R_unconf_progress_123 * (k - 1) + shared->dim_n_H_R_unconf_progress_12 * (shared->k_H_R - 1) + shared->dim_n_H_R_unconf_progress_1 * (j - 1) + i - 1] + internal.n_W_R_conf_progress[shared->dim_n_W_R_conf_progress_123 * (k - 1) + shared->dim_n_W_R_conf_progress_12 * (shared->k_W_R - 1) + shared->dim_n_W_R_conf_progress_1 * (j - 1) + i - 1] + internal.n_W_R_unconf_progress[shared->dim_n_W_R_unconf_progress_123 * (k - 1) + shared->dim_n_W_R_unconf_progress_12 * (shared->k_W_R - 1) + shared->dim_n_W_R_unconf_progress_1 * (j - 1) + i - 1] + ((k == 1 ? internal.n_R_next_vacc_class[shared->dim_n_R_next_vacc_class_12 * (shared->n_vacc_classes - 1) + shared->dim_n_R_next_vacc_class_1 * (j - 1) + i - 1] : internal.n_R_next_vacc_class[shared->dim_n_R_next_vacc_class_12 * (k - 1 - 1) + shared->dim_n_R_next_vacc_class_1 * (j - 1) + i - 1]));
        }
      }
    }
...
dust::pars_t<carehomes> dust_pars<carehomes>(cpp11::list user) {
  ...
  shared->offset_variable_I_weighted = shared->dim_D + shared->dim_D_hosp + shared->dim_D_non_hosp + shared->dim_N_tot + shared->dim_S + shared->dim_cum_admit_by_age + shared->dim_cum_infections_disag + shared->dim_cum_infections_per_strain + shared->dim_cum_n_E_vaccinated + shared->dim_cum_n_I_A_vaccinated + shared->dim_cum_n_I_P_vaccinated + shared->dim_cum_n_R_vaccinated + shared->dim_cum_n_S_vaccinated + shared->dim_cum_n_vaccinated + shared->dim_diagnoses_admitted + shared->dim_prob_strain + 70;
  shared->offset_variable_R = shared->dim_D + shared->dim_D_hosp + shared->dim_D_non_hosp + shared->dim_I_weighted + shared->dim_N_tot + shared->dim_S + shared->dim_T_sero_neg_1 + shared->dim_T_sero_neg_2 + shared->dim_cum_admit_by_age + shared->dim_cum_infections_disag + shared->dim_cum_infections_per_strain + shared->dim_cum_n_E_vaccinated + shared->dim_cum_n_I_A_vaccinated + shared->dim_cum_n_I_P_vaccinated + shared->dim_cum_n_R_vaccinated + shared->dim_cum_n_S_vaccinated + shared->dim_cum_n_vaccinated + shared->dim_diagnoses_admitted + shared->dim_prob_strain + shared->dim_tmp_vaccine_n_candidates + shared->dim_tmp_vaccine_probability + shared->dim_vaccine_missed_doses + 70;
  ...
  shared->offset_variable_T_PCR_neg = shared->dim_D + shared->dim_D_hosp + shared->dim_D_non_hosp + shared->dim_I_weighted + shared->dim_N_tot + shared->dim_R + shared->dim_S + shared->dim_T_sero_neg_1 + shared->dim_T_sero_neg_2 + shared->dim_cum_admit_by_age + shared->dim_cum_infections_disag + shared->dim_cum_infections_per_strain + shared->dim_cum_n_E_vaccinated + shared->dim_cum_n_I_A_vaccinated + shared->dim_cum_n_I_P_vaccinated + shared->dim_cum_n_R_vaccinated + shared->dim_cum_n_S_vaccinated + shared->dim_cum_n_vaccinated + shared->dim_diagnoses_admitted + shared->dim_prob_strain + shared->dim_tmp_vaccine_n_candidates + shared->dim_tmp_vaccine_probability + shared->dim_vaccine_missed_doses + 70;
  ...
  shared->offset_variable_T_sero_neg_1 = shared->dim_D + shared->dim_D_hosp + shared->dim_D_non_hosp + shared->dim_I_weighted + shared->dim_N_tot + shared->dim_S + shared->dim_cum_admit_by_age + shared->dim_cum_infections_disag + shared->dim_cum_infections_per_strain + shared->dim_cum_n_E_vaccinated + shared->dim_cum_n_I_A_vaccinated + shared->dim_cum_n_I_P_vaccinated + shared->dim_cum_n_R_vaccinated + shared->dim_cum_n_S_vaccinated + shared->dim_cum_n_vaccinated + shared->dim_diagnoses_admitted + shared->dim_prob_strain + shared->dim_tmp_vaccine_n_candidates + shared->dim_tmp_vaccine_probability + shared->dim_vaccine_missed_doses + 70;
  shared->offset_variable_T_sero_neg_2 = shared->dim_D + shared->dim_D_hosp + shared->dim_D_non_hosp + shared->dim_I_weighted + shared->dim_N_tot + shared->dim_S + shared->dim_T_sero_neg_1 + shared->dim_cum_admit_by_age + shared->dim_cum_infections_disag + shared->dim_cum_infections_per_strain + shared->dim_cum_n_E_vaccinated + shared->dim_cum_n_I_A_vaccinated + shared->dim_cum_n_I_P_vaccinated + shared->dim_cum_n_R_vaccinated + shared->dim_cum_n_S_vaccinated + shared->dim_cum_n_vaccinated + shared->dim_diagnoses_admitted + shared->dim_prob_strain + shared->dim_tmp_vaccine_n_candidates + shared->dim_tmp_vaccine_probability + shared->dim_vaccine_missed_doses + 70;
...
cpp11::sexp dust_info<carehomes>(const dust::pars_t<carehomes>& pars) {
  ...
  dim[86] = cpp11::writable::integers({shared->dim_I_weighted_1, shared->dim_I_weighted_2});
  dim[87] = cpp11::writable::integers({shared->dim_vaccine_missed_doses_1, shared->dim_vaccine_missed_doses_2});
  dim[88] = cpp11::writable::integers({shared->dim_tmp_vaccine_n_candidates_1, shared->dim_tmp_vaccine_n_candidates_2});
  dim[89] = cpp11::writable::integers({shared->dim_tmp_vaccine_probability_1, shared->dim_tmp_vaccine_probability_2});
  dim[90] = cpp11::writable::integers({shared->dim_T_sero_neg_1_1, shared->dim_T_sero_neg_1_2, shared->dim_T_sero_neg_1_3});
  dim[91] = cpp11::writable::integers({shared->dim_T_sero_neg_2_1, shared->dim_T_sero_neg_2_2, shared->dim_T_sero_neg_2_3});
  dim[92] = cpp11::writable::integers({shared->dim_R_1, shared->dim_R_2, shared->dim_R_3});
  dim[93] = cpp11::writable::integers({shared->dim_T_PCR_neg_1, shared->dim_T_PCR_neg_2, shared->dim_T_PCR_neg_3});
  ...
  index[86] = integer_sequence(shared->offset_variable_I_weighted + 1, shared->dim_I_weighted);
  index[87] = integer_sequence(shared->offset_variable_vaccine_missed_doses + 1, shared->dim_vaccine_missed_doses);
  index[88] = integer_sequence(shared->offset_variable_tmp_vaccine_n_candidates + 1, shared->dim_tmp_vaccine_n_candidates);
  index[89] = integer_sequence(shared->offset_variable_tmp_vaccine_probability + 1, shared->dim_tmp_vaccine_probability);
  index[90] = integer_sequence(shared->offset_variable_T_sero_neg_1 + 1, shared->dim_T_sero_neg_1);
  index[91] = integer_sequence(shared->offset_variable_T_sero_neg_2 + 1, shared->dim_T_sero_neg_2);
  index[92] = integer_sequence(shared->offset_variable_R + 1, shared->dim_R);
  index[93] = integer_sequence(shared->offset_variable_T_PCR_neg + 1, shared->dim_T_PCR_neg);
...
SEXP dust_carehomes_run(SEXP ptr, size_t step_end, bool device) {
  return dust::r::dust_run<carehomes>(ptr, step_end, device);
}
...
SEXP dust_carehomes_set_state(SEXP ptr, SEXP r_state, SEXP r_step) {
  dust::r::dust_set_state<carehomes>(ptr, r_state, r_step);
  return R_NilValue;
}
...
cpp11::sexp dust_carehomes_device_info() {
  return dust::cuda::device_info<carehomes>();
}
