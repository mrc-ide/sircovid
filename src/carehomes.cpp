...
// Generated by odin.dust (version 0.2.6) - do not edit
...
typename T::real_t compare(const typename T::real_t * state,
  ...
  const real_t model_icu = state[10];
  const real_t model_general = state[11];
  const real_t model_hosp = model_icu + model_general;
  const real_t model_deaths_carehomes = state[17];
  const real_t model_deaths_comm = state[15];
  const real_t model_deaths_hosp = state[18];
...
public:
  ...
  struct shared_t {
    ...
    int dim_cum_diagonal;
    int dim_cum_diagonal_1;
    int dim_cum_diagonal_2;
    ...
    std::vector<real_t> initial_cum_diagonal;
    ...
    int offset_variable_cum_diagonal;
  ...
  std::vector<real_t> initial(size_t step) {
    ...
    state[13] = shared->initial_D_hosp_tot;
    state[14] = shared->initial_D_comm_tot;
    state[15] = shared->initial_D_comm_inc;
    state[16] = shared->initial_D_carehomes_tot;
    state[17] = shared->initial_D_carehomes_inc;
    state[18] = shared->initial_D_hosp_inc;
    state[19] = shared->initial_D_tot;
    state[20] = shared->initial_sero_pos_1;
    state[21] = shared->initial_sero_pos_2;
    state[22] = shared->initial_cum_sympt_cases;
    state[23] = shared->initial_cum_sympt_cases_over25;
    state[24] = shared->initial_cum_sympt_cases_non_variant_over25;
    state[25] = shared->initial_sympt_cases_inc;
    state[26] = shared->initial_sympt_cases_over25_inc;
    state[27] = shared->initial_sympt_cases_non_variant_over25_inc;
    state[28] = shared->initial_react_pos;
    std::copy(shared->initial_cum_infections_per_strain.begin(), shared->initial_cum_infections_per_strain.end(), state.begin() + 29);
    ...
    std::copy(shared->initial_cum_diagonal.begin(), shared->initial_cum_diagonal.end(), state.begin() + shared->offset_variable_cum_diagonal);
  ...
  HOST void update(size_t step, const real_t * state, dust::rng_state_t<real_t>& rng_state, real_t * state_next) {
    ...
    const real_t * cum_diagonal = state + shared->offset_variable_cum_diagonal;
    ...
    const real_t * cum_infections_per_strain = state + 29;
    const real_t * D = state + shared->offset_variable_D;
    const real_t D_hosp_tot = state[13];
    const real_t D_comm_tot = state[14];
    const real_t D_comm_inc = state[15];
    const real_t D_carehomes_tot = state[16];
    const real_t D_carehomes_inc = state[17];
    const real_t D_hosp_inc = state[18];
    const real_t D_tot = state[19];
    const real_t cum_sympt_cases = state[22];
    const real_t cum_sympt_cases_over25 = state[23];
    const real_t cum_sympt_cases_non_variant_over25 = state[24];
    const real_t sympt_cases_inc = state[25];
    ...
    state_next[20] = odin_sum4<real_t>(internal.new_T_sero_pos_1.data(), 3, 13, 0, shared->dim_new_T_sero_pos_1_2, 0, shared->dim_new_T_sero_pos_1_3, 0, shared->dim_new_T_sero_pos_1_4, shared->dim_new_T_sero_pos_1_1, shared->dim_new_T_sero_pos_1_12, shared->dim_new_T_sero_pos_1_123);
    ...
    state_next[17] = (fmodr<real_t>(step, shared->steps_per_day) == 0 ? delta_D_carehomes_tot : D_carehomes_inc + delta_D_carehomes_tot);
    state_next[16] = D_carehomes_tot + delta_D_carehomes_tot;
    state_next[15] = (fmodr<real_t>(step, shared->steps_per_day) == 0 ? delta_D_comm_tot : D_comm_inc + delta_D_comm_tot);
    state_next[14] = D_comm_tot + delta_D_comm_tot;
    state_next[18] = (fmodr<real_t>(step, shared->steps_per_day) == 0 ? delta_D_hosp_tot : D_hosp_inc + delta_D_hosp_tot);
    state_next[13] = D_hosp_tot + delta_D_hosp_tot;
    state_next[19] = D_tot + delta_D_hosp_tot + delta_D_comm_tot + delta_D_carehomes_tot;
    ...
    for (int i = 1; i <= shared->dim_n_EI_A_next_vacc_class_1; ++i) {
      for (int j = 1; j <= shared->dim_n_EI_A_next_vacc_class_2; ++j) {
        for (int k = 1; k <= shared->dim_n_EI_A_next_vacc_class_3; ++k) {
          internal.n_EI_A_next_vacc_class[i - 1 + shared->dim_n_EI_A_next_vacc_class_1 * (j - 1) + shared->dim_n_EI_A_next_vacc_class_12 * (k - 1)] = dust::distr::rbinom(rng_state, std::round(internal.n_EE_next_vacc_class[shared->dim_n_EE_next_vacc_class_123 * (k - 1) + shared->dim_n_EE_next_vacc_class_12 * (shared->k_E - 1) + shared->dim_n_EE_next_vacc_class_1 * (j - 1) + i - 1]), 1 - internal.p_C[shared->dim_p_C_12 * (k - 1) + shared->dim_p_C_1 * (j - 1) + i - 1]);
        }
      }
    }
    ...
    for (int i = 1; i <= shared->dim_n_EI_A_1; ++i) {
      for (int j = 1; j <= shared->dim_n_EI_A_2; ++j) {
        for (int k = 1; k <= shared->dim_n_EI_A_3; ++k) {
          internal.n_EI_A[i - 1 + shared->dim_n_EI_A_1 * (j - 1) + shared->dim_n_EI_A_12 * (k - 1)] = dust::distr::rbinom(rng_state, std::round(internal.n_EE[shared->dim_n_EE_123 * (k - 1) + shared->dim_n_EE_12 * (shared->k_E - 1) + shared->dim_n_EE_1 * (j - 1) + i - 1]), 1 - internal.p_C[shared->dim_p_C_12 * (k - 1) + shared->dim_p_C_1 * (j - 1) + i - 1]);
        }
      }
    }
    ...
    for (int i = 1; i <= shared->dim_n_RS_tmp_1; ++i) {
      for (int j = 1; j <= shared->dim_n_RS_tmp_2; ++j) {
        for (int k = 1; k <= shared->dim_n_RS_tmp_3; ++k) {
          internal.n_RS_tmp[i - 1 + shared->dim_n_RS_tmp_1 * (j - 1) + shared->dim_n_RS_tmp_12 * (k - 1)] = dust::distr::rbinom(rng_state, std::round(internal.n_R_progress[shared->dim_n_R_progress_12 * (k - 1) + shared->dim_n_R_progress_1 * (j - 1) + i - 1]), internal.p_RS[shared->dim_p_RS_12 * (k - 1) + shared->dim_p_RS_1 * (j - 1) + i - 1]);
        }
      }
    }
    ...
    for (int i = 1; i <= shared->dim_n_com_to_T_sero_pre_1; ++i) {
      for (int j = 1; j <= shared->dim_n_com_to_T_sero_pre_2; ++j) {
        for (int k = 1; k <= shared->dim_n_com_to_T_sero_pre_3; ++k) {
          internal.n_com_to_T_sero_pre[i - 1 + shared->dim_n_com_to_T_sero_pre_1 * (j - 1) + shared->dim_n_com_to_T_sero_pre_12 * (k - 1)] = internal.n_EE[shared->dim_n_EE_123 * (k - 1) + shared->dim_n_EE_12 * (shared->k_E - 1) + shared->dim_n_EE_1 * (j - 1) + i - 1] + ((k == 1 ? internal.n_EE_next_vacc_class[shared->dim_n_EE_next_vacc_class_123 * (shared->n_vacc_classes - 1) + shared->dim_n_EE_next_vacc_class_12 * (shared->k_E - 1) + shared->dim_n_EE_next_vacc_class_1 * (j - 1) + i - 1] : internal.n_EE_next_vacc_class[shared->dim_n_EE_next_vacc_class_123 * (k - 1 - 1) + shared->dim_n_EE_next_vacc_class_12 * (shared->k_E - 1) + shared->dim_n_EE_next_vacc_class_1 * (j - 1) + i - 1]));
        }
      }
    }
    ...
    for (int i = 1; i <= shared->dim_n_RE_tmp_1; ++i) {
      for (int j = 1; j <= shared->dim_n_RE_tmp_2; ++j) {
        for (int k = 1; k <= shared->dim_n_RE_tmp_3; ++k) {
          internal.n_RE_tmp[i - 1 + shared->dim_n_RE_tmp_1 * (j - 1) + shared->dim_n_RE_tmp_12 * (k - 1)] = internal.n_R_progress[shared->dim_n_R_progress_12 * (k - 1) + shared->dim_n_R_progress_1 * (j - 1) + i - 1] - internal.n_RS_tmp[shared->dim_n_RS_tmp_12 * (k - 1) + shared->dim_n_RS_tmp_1 * (j - 1) + i - 1];
        }
      }
    }
    ...
    for (int i = 1; i <= shared->dim_n_RS_1; ++i) {
      for (int j = 1; j <= shared->dim_n_RS_2; ++j) {
        for (int k = 1; k <= shared->dim_n_RS_3; ++k) {
          internal.n_RS[i - 1 + shared->dim_n_RS_1 * (j - 1) + shared->dim_n_RS_12 * (k - 1)] = internal.n_RS_tmp[shared->dim_n_RS_tmp_12 * (k - 1) + shared->dim_n_RS_tmp_1 * (j - 1) + i - 1] - internal.n_RS_next_vacc_class[shared->dim_n_RS_next_vacc_class_12 * (k - 1) + shared->dim_n_RS_next_vacc_class_1 * (j - 1) + i - 1];
        }
      }
    }
    ...
    for (int i = 1; i <= shared->dim_n_RE_1; ++i) {
      for (int j = 1; j <= shared->dim_n_RE_2; ++j) {
        for (int k = 1; k <= shared->dim_n_RE_3; ++k) {
          internal.n_RE[i - 1 + shared->dim_n_RE_1 * (j - 1) + shared->dim_n_RE_12 * (k - 1)] = internal.n_RE_tmp[shared->dim_n_RE_tmp_12 * (k - 1) + shared->dim_n_RE_tmp_1 * (j - 1) + i - 1] - internal.n_RE_next_vacc_class[shared->dim_n_RE_next_vacc_class_12 * (k - 1) + shared->dim_n_RE_next_vacc_class_1 * (j - 1) + i - 1];
        }
      }
    }
    for (int i = 1; i <= shared->dim_n_R_vaccinated_1; ++i) {
      for (int j = 1; j <= shared->dim_n_R_vaccinated_2; ++j) {
        internal.n_R_vaccinated[i - 1 + shared->dim_n_R_vaccinated_1 * (j - 1)] = odin_sum3<real_t>(internal.n_R_next_vacc_class.data(), i - 1, i, 0, shared->dim_n_R_next_vacc_class_2, j - 1, j, shared->dim_n_R_next_vacc_class_1, shared->dim_n_R_next_vacc_class_12) + odin_sum3<real_t>(internal.n_RS_next_vacc_class.data(), i - 1, i, 0, shared->dim_n_RS_next_vacc_class_2, j - 1, j, shared->dim_n_RS_next_vacc_class_1, shared->dim_n_RS_next_vacc_class_12) + odin_sum3<real_t>(internal.n_RE_next_vacc_class.data(), i - 1, i, 0, shared->dim_n_RE_next_vacc_class_2, j - 1, j, shared->dim_n_RE_next_vacc_class_1, shared->dim_n_RE_next_vacc_class_12);
      }
    }
    ...
    for (int i = 1; i <= shared->dim_cum_diagonal_1; ++i) {
      for (int j = 1; j <= shared->dim_cum_diagonal_2; ++j) {
        state_next[shared->offset_variable_cum_diagonal + i - 1 + shared->dim_cum_diagonal_1 * (j - 1)] = 0;
      }
    }
    for (int i = 1; i <= shared->dim_cum_diagonal_1; ++i) {
      for (int j = 1; j <= shared->dim_cum_diagonal_2; ++j) {
        state_next[shared->offset_variable_cum_diagonal + i - 1 + shared->dim_cum_diagonal_1 * (j - 1)] = cum_diagonal[shared->dim_cum_diagonal_1 * (j - 1) + i - 1] + odin_sum3<real_t>(internal.n_SE_next_vacc_class.data(), i - 1, i, 0, shared->dim_n_SE_next_vacc_class_2, j - 1, j, shared->dim_n_SE_next_vacc_class_1, shared->dim_n_SE_next_vacc_class_12) + odin_sum4<real_t>(internal.n_EE_next_vacc_class.data(), i - 1, i, 0, shared->dim_n_EE_next_vacc_class_2, 0, shared->dim_n_EE_next_vacc_class_3, j - 1, j, shared->dim_n_EE_next_vacc_class_1, shared->dim_n_EE_next_vacc_class_12, shared->dim_n_EE_next_vacc_class_123) + odin_sum4<real_t>(internal.n_II_A_next_vacc_class.data(), i - 1, i, 0, shared->dim_n_II_A_next_vacc_class_2, 0, shared->dim_n_II_A_next_vacc_class_3, j - 1, j, shared->dim_n_II_A_next_vacc_class_1, shared->dim_n_II_A_next_vacc_class_12, shared->dim_n_II_A_next_vacc_class_123) + odin_sum4<real_t>(internal.n_II_P_next_vacc_class.data(), i - 1, i, 0, shared->dim_n_II_P_next_vacc_class_2, 0, shared->dim_n_II_P_next_vacc_class_3, j - 1, j, shared->dim_n_II_P_next_vacc_class_1, shared->dim_n_II_P_next_vacc_class_12, shared->dim_n_II_P_next_vacc_class_123) + odin_sum3<real_t>(internal.n_RS_next_vacc_class.data(), i - 1, i, 0, shared->dim_n_RS_next_vacc_class_2, j - 1, j, shared->dim_n_RS_next_vacc_class_1, shared->dim_n_RS_next_vacc_class_12) + odin_sum3<real_t>(internal.n_RE_next_vacc_class.data(), i - 1, i, 0, shared->dim_n_RE_next_vacc_class_2, j - 1, j, shared->dim_n_RE_next_vacc_class_1, shared->dim_n_RE_next_vacc_class_12);
      }
    }
    state_next[22] = cum_sympt_cases + new_sympt_cases;
    ...
    state_next[25] = ((fmodr<real_t>(step, shared->steps_per_day) == 0 ? new_sympt_cases : sympt_cases_inc + new_sympt_cases));
    state_next[27] = ((fmodr<real_t>(step, shared->steps_per_day) == 0 ? new_sympt_cases_non_variant_over25 : sympt_cases_non_variant_over25_inc + new_sympt_cases_non_variant_over25));
    state_next[26] = ((fmodr<real_t>(step, shared->steps_per_day) == 0 ? new_sympt_cases_over25 : sympt_cases_over25_inc + new_sympt_cases_over25));
    ...
    state_next[3] = cum_infections + odin_sum1<real_t>(internal.n_S_progress.data(), 0, shared->dim_n_S_progress) + odin_sum1<real_t>(internal.n_RE.data(), 0, shared->dim_n_RE) + odin_sum1<real_t>(internal.n_RE_next_vacc_class.data(), 0, shared->dim_n_RE_next_vacc_class);
    for (int i = 1; i <= shared->dim_cum_infections_per_strain; ++i) {
      state_next[29 + i - 1] = cum_infections_per_strain[i - 1] + odin_sum3<real_t>(internal.n_S_progress.data(), 0, shared->dim_n_S_progress_1, i - 1, i, 0, shared->dim_n_S_progress_3, shared->dim_n_S_progress_1, shared->dim_n_S_progress_12) + ((i > 2 ? (odin_sum3<real_t>(internal.n_RE.data(), 0, shared->dim_n_RE_1, i - 2 - 1, i - 2, 0, shared->dim_n_RE_3, shared->dim_n_RE_1, shared->dim_n_RE_12) + odin_sum3<real_t>(internal.n_RE_next_vacc_class.data(), 0, shared->dim_n_RE_next_vacc_class_1, i - 2 - 1, i - 2, 0, shared->dim_n_RE_next_vacc_class_3, shared->dim_n_RE_next_vacc_class_1, shared->dim_n_RE_next_vacc_class_12)) : 0));
    }
    ...
    state_next[11] = new_general_tot;
    state_next[12] = new_ICU_tot + new_general_tot;
    state_next[28] = odin_sum4<real_t>(internal.new_T_PCR_pos.data(), 1, 18, 0, shared->dim_new_T_PCR_pos_2, 0, shared->dim_new_T_PCR_pos_3, 0, shared->dim_new_T_PCR_pos_4, shared->dim_new_T_PCR_pos_1, shared->dim_new_T_PCR_pos_12, shared->dim_new_T_PCR_pos_123);
...
dust::pars_t<carehomes> dust_pars<carehomes>(cpp11::list user) {
  ...
  shared->dim_cum_diagonal_1 = shared->n_groups;
  shared->dim_cum_diagonal_2 = shared->n_vacc_classes;
  ...
  shared->dim_n_RE_tmp_1 = shared->n_groups;
  shared->dim_n_RE_tmp_2 = shared->n_strains;
  shared->dim_n_RE_tmp_3 = shared->n_vacc_classes;
  ...
  shared->dim_n_RS_tmp_1 = shared->n_groups;
  shared->dim_n_RS_tmp_2 = shared->n_strains;
  shared->dim_n_RS_tmp_3 = shared->n_vacc_classes;
  ...
  shared->dim_cum_diagonal = shared->dim_cum_diagonal_1 * shared->dim_cum_diagonal_2;
  ...
  shared->dim_n_RE_tmp = shared->dim_n_RE_tmp_1 * shared->dim_n_RE_tmp_2 * shared->dim_n_RE_tmp_3;
  shared->dim_n_RE_tmp_12 = shared->dim_n_RE_tmp_1 * shared->dim_n_RE_tmp_2;
  ...
  shared->dim_n_RS_tmp = shared->dim_n_RS_tmp_1 * shared->dim_n_RS_tmp_2 * shared->dim_n_RS_tmp_3;
  shared->dim_n_RS_tmp_12 = shared->dim_n_RS_tmp_1 * shared->dim_n_RS_tmp_2;
  ...
  shared->offset_variable_D_hosp = shared->dim_cum_infections_per_strain + 29;
  shared->offset_variable_D_non_hosp = shared->dim_D_hosp + shared->dim_cum_infections_per_strain + 29;
  shared->offset_variable_N_tot = shared->dim_D_hosp + shared->dim_D_non_hosp + shared->dim_cum_admit_by_age + shared->dim_cum_infections_per_strain + 29;
  shared->offset_variable_cum_admit_by_age = shared->dim_D_hosp + shared->dim_D_non_hosp + shared->dim_cum_infections_per_strain + 29;
  shared->offset_variable_prob_strain = shared->dim_D_hosp + shared->dim_D_non_hosp + shared->dim_N_tot + shared->dim_cum_admit_by_age + shared->dim_cum_infections_per_strain + 29;
  ...
  shared->initial_cum_diagonal = std::vector<real_t>(shared->dim_cum_diagonal);
  ...
  internal.n_RE_tmp = std::vector<real_t>(shared->dim_n_RE_tmp);
  ...
  internal.n_RS_tmp = std::vector<real_t>(shared->dim_n_RS_tmp);
  ...
  for (int i = 1; i <= shared->dim_cum_diagonal_1; ++i) {
    for (int j = 1; j <= shared->dim_cum_diagonal_2; ++j) {
      shared->initial_cum_diagonal[i - 1 + shared->dim_cum_diagonal_1 * (j - 1)] = 0;
    }
  }
  for (int i = 1; i <= shared->dim_cum_infections_disag_1; ++i) {
    for (int j = 1; j <= shared->dim_cum_infections_disag_2; ++j) {
  ...
  shared->offset_variable_cum_diagonal = shared->dim_D_hosp + shared->dim_D_non_hosp + shared->dim_N_tot + shared->dim_cum_admit_by_age + shared->dim_cum_infections_per_strain + shared->dim_cum_n_E_vaccinated + shared->dim_cum_n_I_A_vaccinated + shared->dim_cum_n_I_P_vaccinated + shared->dim_cum_n_R_vaccinated + shared->dim_cum_n_S_vaccinated + shared->dim_cum_n_vaccinated + shared->dim_prob_strain + 29;
  shared->offset_variable_cum_infections_disag = shared->dim_D_hosp + shared->dim_D_non_hosp + shared->dim_N_tot + shared->dim_cum_admit_by_age + shared->dim_cum_diagonal + shared->dim_cum_infections_per_strain + shared->dim_cum_n_E_vaccinated + shared->dim_cum_n_I_A_vaccinated + shared->dim_cum_n_I_P_vaccinated + shared->dim_cum_n_R_vaccinated + shared->dim_cum_n_S_vaccinated + shared->dim_cum_n_vaccinated + shared->dim_diagnoses_admitted + shared->dim_prob_strain + 29;
  shared->offset_variable_cum_n_E_vaccinated = shared->dim_D_hosp + shared->dim_D_non_hosp + shared->dim_N_tot + shared->dim_cum_admit_by_age + shared->dim_cum_infections_per_strain + shared->dim_cum_n_S_vaccinated + shared->dim_prob_strain + 29;
  shared->offset_variable_cum_n_I_A_vaccinated = shared->dim_D_hosp + shared->dim_D_non_hosp + shared->dim_N_tot + shared->dim_cum_admit_by_age + shared->dim_cum_infections_per_strain + shared->dim_cum_n_E_vaccinated + shared->dim_cum_n_S_vaccinated + shared->dim_prob_strain + 29;
  shared->offset_variable_cum_n_I_P_vaccinated = shared->dim_D_hosp + shared->dim_D_non_hosp + shared->dim_N_tot + shared->dim_cum_admit_by_age + shared->dim_cum_infections_per_strain + shared->dim_cum_n_E_vaccinated + shared->dim_cum_n_I_A_vaccinated + shared->dim_cum_n_S_vaccinated + shared->dim_prob_strain + 29;
  shared->offset_variable_cum_n_R_vaccinated = shared->dim_D_hosp + shared->dim_D_non_hosp + shared->dim_N_tot + shared->dim_cum_admit_by_age + shared->dim_cum_infections_per_strain + shared->dim_cum_n_E_vaccinated + shared->dim_cum_n_I_A_vaccinated + shared->dim_cum_n_I_P_vaccinated + shared->dim_cum_n_S_vaccinated + shared->dim_prob_strain + 29;
  shared->offset_variable_cum_n_S_vaccinated = shared->dim_D_hosp + shared->dim_D_non_hosp + shared->dim_N_tot + shared->dim_cum_admit_by_age + shared->dim_cum_infections_per_strain + shared->dim_prob_strain + 29;
  shared->offset_variable_cum_n_vaccinated = shared->dim_D_hosp + shared->dim_D_non_hosp + shared->dim_N_tot + shared->dim_cum_admit_by_age + shared->dim_cum_infections_per_strain + shared->dim_cum_n_E_vaccinated + shared->dim_cum_n_I_A_vaccinated + shared->dim_cum_n_I_P_vaccinated + shared->dim_cum_n_R_vaccinated + shared->dim_cum_n_S_vaccinated + shared->dim_prob_strain + 29;
...
cpp11::sexp dust_info<carehomes>(const dust::pars_t<carehomes>& pars) {
  ...
  dim[29] = cpp11::writable::integers({shared->dim_cum_infections_per_strain});
  dim[30] = cpp11::writable::integers({shared->dim_D_hosp});
  dim[31] = cpp11::writable::integers({shared->dim_D_non_hosp});
  dim[32] = cpp11::writable::integers({shared->dim_cum_admit_by_age});
  dim[33] = cpp11::writable::integers({shared->dim_N_tot});
  dim[34] = cpp11::writable::integers({shared->dim_prob_strain});
  ...
  dim[40] = cpp11::writable::integers({shared->dim_cum_n_vaccinated_1, shared->dim_cum_n_vaccinated_2});
  dim[41] = cpp11::writable::integers({shared->dim_cum_diagonal_1, shared->dim_cum_diagonal_2});
  dim[42] = cpp11::writable::integers({shared->dim_diagnoses_admitted_1, shared->dim_diagnoses_admitted_2});
  dim[43] = cpp11::writable::integers({shared->dim_cum_infections_disag_1, shared->dim_cum_infections_disag_2});
  dim[44] = cpp11::writable::integers({shared->dim_D_1, shared->dim_D_2});
  dim[45] = cpp11::writable::integers({shared->dim_S_1, shared->dim_S_2});
  dim[46] = cpp11::writable::integers({shared->dim_I_weighted_1, shared->dim_I_weighted_2});
  dim[47] = cpp11::writable::integers({shared->dim_vaccine_missed_doses_1, shared->dim_vaccine_missed_doses_2});
  dim[48] = cpp11::writable::integers({shared->dim_tmp_vaccine_n_candidates_1, shared->dim_tmp_vaccine_n_candidates_2});
  dim[49] = cpp11::writable::integers({shared->dim_tmp_vaccine_probability_1, shared->dim_tmp_vaccine_probability_2});
  dim[50] = cpp11::writable::integers({shared->dim_T_sero_neg_1_1, shared->dim_T_sero_neg_1_2, shared->dim_T_sero_neg_1_3});
  dim[51] = cpp11::writable::integers({shared->dim_T_sero_neg_2_1, shared->dim_T_sero_neg_2_2, shared->dim_T_sero_neg_2_3});
  dim[52] = cpp11::writable::integers({shared->dim_R_1, shared->dim_R_2, shared->dim_R_3});
  dim[53] = cpp11::writable::integers({shared->dim_T_PCR_neg_1, shared->dim_T_PCR_neg_2, shared->dim_T_PCR_neg_3});
  dim[54] = cpp11::writable::integers({shared->dim_E_1, shared->dim_E_2, shared->dim_E_3, shared->dim_E_4});
  dim[55] = cpp11::writable::integers({shared->dim_I_A_1, shared->dim_I_A_2, shared->dim_I_A_3, shared->dim_I_A_4});
  dim[56] = cpp11::writable::integers({shared->dim_I_P_1, shared->dim_I_P_2, shared->dim_I_P_3, shared->dim_I_P_4});
  dim[57] = cpp11::writable::integers({shared->dim_I_C_1_1, shared->dim_I_C_1_2, shared->dim_I_C_1_3, shared->dim_I_C_1_4});
  dim[58] = cpp11::writable::integers({shared->dim_I_C_2_1, shared->dim_I_C_2_2, shared->dim_I_C_2_3, shared->dim_I_C_2_4});
  dim[59] = cpp11::writable::integers({shared->dim_G_D_1, shared->dim_G_D_2, shared->dim_G_D_3, shared->dim_G_D_4});
  dim[60] = cpp11::writable::integers({shared->dim_ICU_pre_unconf_1, shared->dim_ICU_pre_unconf_2, shared->dim_ICU_pre_unconf_3, shared->dim_ICU_pre_unconf_4});
  dim[61] = cpp11::writable::integers({shared->dim_ICU_pre_conf_1, shared->dim_ICU_pre_conf_2, shared->dim_ICU_pre_conf_3, shared->dim_ICU_pre_conf_4});
  dim[62] = cpp11::writable::integers({shared->dim_H_R_unconf_1, shared->dim_H_R_unconf_2, shared->dim_H_R_unconf_3, shared->dim_H_R_unconf_4});
  dim[63] = cpp11::writable::integers({shared->dim_H_R_conf_1, shared->dim_H_R_conf_2, shared->dim_H_R_conf_3, shared->dim_H_R_conf_4});
  dim[64] = cpp11::writable::integers({shared->dim_H_D_unconf_1, shared->dim_H_D_unconf_2, shared->dim_H_D_unconf_3, shared->dim_H_D_unconf_4});
  dim[65] = cpp11::writable::integers({shared->dim_H_D_conf_1, shared->dim_H_D_conf_2, shared->dim_H_D_conf_3, shared->dim_H_D_conf_4});
  dim[66] = cpp11::writable::integers({shared->dim_ICU_W_R_unconf_1, shared->dim_ICU_W_R_unconf_2, shared->dim_ICU_W_R_unconf_3, shared->dim_ICU_W_R_unconf_4});
  dim[67] = cpp11::writable::integers({shared->dim_ICU_W_R_conf_1, shared->dim_ICU_W_R_conf_2, shared->dim_ICU_W_R_conf_3, shared->dim_ICU_W_R_conf_4});
  dim[68] = cpp11::writable::integers({shared->dim_ICU_W_D_unconf_1, shared->dim_ICU_W_D_unconf_2, shared->dim_ICU_W_D_unconf_3, shared->dim_ICU_W_D_unconf_4});
  dim[69] = cpp11::writable::integers({shared->dim_ICU_W_D_conf_1, shared->dim_ICU_W_D_conf_2, shared->dim_ICU_W_D_conf_3, shared->dim_ICU_W_D_conf_4});
  dim[70] = cpp11::writable::integers({shared->dim_ICU_D_unconf_1, shared->dim_ICU_D_unconf_2, shared->dim_ICU_D_unconf_3, shared->dim_ICU_D_unconf_4});
  dim[71] = cpp11::writable::integers({shared->dim_ICU_D_conf_1, shared->dim_ICU_D_conf_2, shared->dim_ICU_D_conf_3, shared->dim_ICU_D_conf_4});
  dim[72] = cpp11::writable::integers({shared->dim_W_R_unconf_1, shared->dim_W_R_unconf_2, shared->dim_W_R_unconf_3, shared->dim_W_R_unconf_4});
  dim[73] = cpp11::writable::integers({shared->dim_W_R_conf_1, shared->dim_W_R_conf_2, shared->dim_W_R_conf_3, shared->dim_W_R_conf_4});
  dim[74] = cpp11::writable::integers({shared->dim_W_D_unconf_1, shared->dim_W_D_unconf_2, shared->dim_W_D_unconf_3, shared->dim_W_D_unconf_4});
  dim[75] = cpp11::writable::integers({shared->dim_W_D_conf_1, shared->dim_W_D_conf_2, shared->dim_W_D_conf_3, shared->dim_W_D_conf_4});
  dim[76] = cpp11::writable::integers({shared->dim_T_sero_pre_1_1, shared->dim_T_sero_pre_1_2, shared->dim_T_sero_pre_1_3, shared->dim_T_sero_pre_1_4});
  dim[77] = cpp11::writable::integers({shared->dim_T_sero_pos_1_1, shared->dim_T_sero_pos_1_2, shared->dim_T_sero_pos_1_3, shared->dim_T_sero_pos_1_4});
  dim[78] = cpp11::writable::integers({shared->dim_T_sero_pre_2_1, shared->dim_T_sero_pre_2_2, shared->dim_T_sero_pre_2_3, shared->dim_T_sero_pre_2_4});
  dim[79] = cpp11::writable::integers({shared->dim_T_sero_pos_2_1, shared->dim_T_sero_pos_2_2, shared->dim_T_sero_pos_2_3, shared->dim_T_sero_pos_2_4});
  dim[80] = cpp11::writable::integers({shared->dim_T_PCR_pre_1, shared->dim_T_PCR_pre_2, shared->dim_T_PCR_pre_3, shared->dim_T_PCR_pre_4});
  dim[81] = cpp11::writable::integers({shared->dim_T_PCR_pos_1, shared->dim_T_PCR_pos_2, shared->dim_T_PCR_pos_3, shared->dim_T_PCR_pos_4});
  dim.names() = nms;
  ...
  index[29] = integer_sequence(30, shared->dim_cum_infections_per_strain);
  ...
  index[32] = integer_sequence(shared->offset_variable_cum_admit_by_age + 1, shared->dim_cum_admit_by_age);
  index[33] = integer_sequence(shared->offset_variable_N_tot + 1, shared->dim_N_tot);
  index[34] = integer_sequence(shared->offset_variable_prob_strain + 1, shared->dim_prob_strain);
  ...
  index[40] = integer_sequence(shared->offset_variable_cum_n_vaccinated + 1, shared->dim_cum_n_vaccinated);
  index[41] = integer_sequence(shared->offset_variable_cum_diagonal + 1, shared->dim_cum_diagonal);
  index[42] = integer_sequence(shared->offset_variable_diagnoses_admitted + 1, shared->dim_diagnoses_admitted);
  index[43] = integer_sequence(shared->offset_variable_cum_infections_disag + 1, shared->dim_cum_infections_disag);
  index[44] = integer_sequence(shared->offset_variable_D + 1, shared->dim_D);
  index[45] = integer_sequence(shared->offset_variable_S + 1, shared->dim_S);
  index[46] = integer_sequence(shared->offset_variable_I_weighted + 1, shared->dim_I_weighted);
  index[47] = integer_sequence(shared->offset_variable_vaccine_missed_doses + 1, shared->dim_vaccine_missed_doses);
  index[48] = integer_sequence(shared->offset_variable_tmp_vaccine_n_candidates + 1, shared->dim_tmp_vaccine_n_candidates);
  index[49] = integer_sequence(shared->offset_variable_tmp_vaccine_probability + 1, shared->dim_tmp_vaccine_probability);
  index[50] = integer_sequence(shared->offset_variable_T_sero_neg_1 + 1, shared->dim_T_sero_neg_1);
  index[51] = integer_sequence(shared->offset_variable_T_sero_neg_2 + 1, shared->dim_T_sero_neg_2);
  index[52] = integer_sequence(shared->offset_variable_R + 1, shared->dim_R);
  index[53] = integer_sequence(shared->offset_variable_T_PCR_neg + 1, shared->dim_T_PCR_neg);
  index[54] = integer_sequence(shared->offset_variable_E + 1, shared->dim_E);
  index[55] = integer_sequence(shared->offset_variable_I_A + 1, shared->dim_I_A);
  index[56] = integer_sequence(shared->offset_variable_I_P + 1, shared->dim_I_P);
  index[57] = integer_sequence(shared->offset_variable_I_C_1 + 1, shared->dim_I_C_1);
  index[58] = integer_sequence(shared->offset_variable_I_C_2 + 1, shared->dim_I_C_2);
  index[59] = integer_sequence(shared->offset_variable_G_D + 1, shared->dim_G_D);
  index[60] = integer_sequence(shared->offset_variable_ICU_pre_unconf + 1, shared->dim_ICU_pre_unconf);
  index[61] = integer_sequence(shared->offset_variable_ICU_pre_conf + 1, shared->dim_ICU_pre_conf);
  index[62] = integer_sequence(shared->offset_variable_H_R_unconf + 1, shared->dim_H_R_unconf);
  index[63] = integer_sequence(shared->offset_variable_H_R_conf + 1, shared->dim_H_R_conf);
  index[64] = integer_sequence(shared->offset_variable_H_D_unconf + 1, shared->dim_H_D_unconf);
  index[65] = integer_sequence(shared->offset_variable_H_D_conf + 1, shared->dim_H_D_conf);
  index[66] = integer_sequence(shared->offset_variable_ICU_W_R_unconf + 1, shared->dim_ICU_W_R_unconf);
  index[67] = integer_sequence(shared->offset_variable_ICU_W_R_conf + 1, shared->dim_ICU_W_R_conf);
  index[68] = integer_sequence(shared->offset_variable_ICU_W_D_unconf + 1, shared->dim_ICU_W_D_unconf);
  index[69] = integer_sequence(shared->offset_variable_ICU_W_D_conf + 1, shared->dim_ICU_W_D_conf);
  index[70] = integer_sequence(shared->offset_variable_ICU_D_unconf + 1, shared->dim_ICU_D_unconf);
  index[71] = integer_sequence(shared->offset_variable_ICU_D_conf + 1, shared->dim_ICU_D_conf);
  index[72] = integer_sequence(shared->offset_variable_W_R_unconf + 1, shared->dim_W_R_unconf);
  index[73] = integer_sequence(shared->offset_variable_W_R_conf + 1, shared->dim_W_R_conf);
  index[74] = integer_sequence(shared->offset_variable_W_D_unconf + 1, shared->dim_W_D_unconf);
  index[75] = integer_sequence(shared->offset_variable_W_D_conf + 1, shared->dim_W_D_conf);
  index[76] = integer_sequence(shared->offset_variable_T_sero_pre_1 + 1, shared->dim_T_sero_pre_1);
  index[77] = integer_sequence(shared->offset_variable_T_sero_pos_1 + 1, shared->dim_T_sero_pos_1);
  index[78] = integer_sequence(shared->offset_variable_T_sero_pre_2 + 1, shared->dim_T_sero_pre_2);
  index[79] = integer_sequence(shared->offset_variable_T_sero_pos_2 + 1, shared->dim_T_sero_pos_2);
  index[80] = integer_sequence(shared->offset_variable_T_PCR_pre + 1, shared->dim_T_PCR_pre);
  index[81] = integer_sequence(shared->offset_variable_T_PCR_pos + 1, shared->dim_T_PCR_pos);
  index.names() = nms;
