...
public:
  ...
  void update(size_t step, const real_t * state, dust::rng_state_t<real_t>& rng_state, real_t * state_next) {
    ...
    for (int i = 1; i <= internal.dim_prob_H_D; ++i) {
      internal.prob_H_D[i - 1] = p_H_D * internal.psi_H_D[i - 1];
    }
    for (int i = 1; i <= internal.dim_prob_ICU; ++i) {
      internal.prob_ICU[i - 1] = p_ICU * internal.psi_ICU[i - 1];
    }
    for (int i = 1; i <= internal.dim_prob_ICU_D; ++i) {
      internal.prob_ICU_D[i - 1] = p_ICU_D * internal.psi_ICU_D[i - 1];
    }
    for (int i = 1; i <= internal.dim_prob_W_D; ++i) {
      internal.prob_W_D[i - 1] = p_W_D * internal.psi_W_D[i - 1];
    }
    state_next[4] = beta;
    ...
    for (int i = 1; i <= internal.dim_n_ICU_pre_conf_to_ICU_D_conf_1; ++i) {
      for (int j = 1; j <= internal.dim_n_ICU_pre_conf_to_ICU_D_conf_2; ++j) {
        for (int k = 1; k <= internal.dim_n_ICU_pre_conf_to_ICU_D_conf_3; ++k) {
          internal.n_ICU_pre_conf_to_ICU_D_conf[i - 1 + internal.dim_n_ICU_pre_conf_to_ICU_D_conf_1 * (j - 1) + internal.dim_n_ICU_pre_conf_to_ICU_D_conf_12 * (k - 1)] = dust::distr::rbinom(rng_state, std::round(internal.n_ICU_pre_conf[internal.dim_n_ICU_pre_conf_123 * (k - 1) + internal.dim_n_ICU_pre_conf_12 * (internal.s_ICU_pre - 1) + internal.dim_n_ICU_pre_conf_1 * (j - 1) + i - 1]), internal.prob_ICU_D[i - 1]);
        }
      }
    }
    for (int i = 1; i <= internal.dim_n_ICU_pre_unconf_to_ICU_D_unconf_1; ++i) {
      for (int j = 1; j <= internal.dim_n_ICU_pre_unconf_to_ICU_D_unconf_2; ++j) {
        for (int k = 1; k <= internal.dim_n_ICU_pre_unconf_to_ICU_D_unconf_3; ++k) {
          internal.n_ICU_pre_unconf_to_ICU_D_unconf[i - 1 + internal.dim_n_ICU_pre_unconf_to_ICU_D_unconf_1 * (j - 1) + internal.dim_n_ICU_pre_unconf_to_ICU_D_unconf_12 * (k - 1)] = dust::distr::rbinom(rng_state, std::round(internal.n_ICU_pre_unconf[internal.dim_n_ICU_pre_unconf_123 * (k - 1) + internal.dim_n_ICU_pre_unconf_12 * (internal.s_ICU_pre - 1) + internal.dim_n_ICU_pre_unconf_1 * (j - 1) + i - 1]), internal.prob_ICU_D[i - 1]);
        }
      }
    }
    ...
    for (int i = 1; i <= internal.dim_n_sympt_to_ICU_pre_1; ++i) {
      for (int j = 1; j <= internal.dim_n_sympt_to_ICU_pre_2; ++j) {
        for (int k = 1; k <= internal.dim_n_sympt_to_ICU_pre_3; ++k) {
          internal.n_sympt_to_ICU_pre[i - 1 + internal.dim_n_sympt_to_ICU_pre_1 * (j - 1) + internal.dim_n_sympt_to_ICU_pre_12 * (k - 1)] = dust::distr::rbinom(rng_state, std::round(internal.n_sympt_to_hosp[internal.dim_n_sympt_to_hosp_12 * (k - 1) + internal.dim_n_sympt_to_hosp_1 * (j - 1) + i - 1]), internal.prob_ICU[i - 1]);
        }
      }
    }
    ...
    for (int i = 1; i <= internal.dim_n_sympt_to_hosp_D_1; ++i) {
      for (int j = 1; j <= internal.dim_n_sympt_to_hosp_D_2; ++j) {
        for (int k = 1; k <= internal.dim_n_sympt_to_hosp_D_3; ++k) {
          internal.n_sympt_to_hosp_D[i - 1 + internal.dim_n_sympt_to_hosp_D_1 * (j - 1) + internal.dim_n_sympt_to_hosp_D_12 * (k - 1)] = dust::distr::rbinom(rng_state, std::round(internal.n_hosp_non_ICU[internal.dim_n_hosp_non_ICU_12 * (k - 1) + internal.dim_n_hosp_non_ICU_1 * (j - 1) + i - 1]), internal.prob_H_D[i - 1]);
        }
      }
    }
    ...
    for (int i = 1; i <= internal.dim_new_R_neg_1; ++i) {
      for (int j = 1; j <= internal.dim_new_R_neg_2; ++j) {
        for (int k = 1; k <= internal.dim_new_R_neg_3; ++k) {
          internal.new_R_neg[i - 1 + internal.dim_new_R_neg_1 * (j - 1) + internal.dim_new_R_neg_12 * (k - 1)] = R_neg[internal.dim_R_neg_12 * (k - 1) + internal.dim_R_neg_1 * (j - 1) + i - 1] + odin_sum4(internal.n_R_pre.data(), i - 1, i, j - 1, j, 0, internal.dim_n_R_pre_3, k - 1, k, internal.dim_n_R_pre_1, internal.dim_n_R_pre_12, internal.dim_n_R_pre_123) - internal.n_R_pre_to_R_pos[internal.dim_n_R_pre_to_R_pos_12 * (k - 1) + internal.dim_n_R_pre_to_R_pos_1 * (j - 1) + i - 1] + internal.n_R_pos[internal.dim_n_R_pos_123 * (k - 1) + internal.dim_n_R_pos_12 * (internal.s_sero_pos - 1) + internal.dim_n_R_pos_1 * (j - 1) + i - 1] - internal.model_pcr_and_serology * internal.n_R_progress[internal.dim_n_R_progress_12 * (k - 1) + internal.dim_n_R_progress_1 * (j - 1) + i - 1] - internal.model_pcr_and_serology * internal.n_R_next_vacc_class[internal.dim_n_R_next_vacc_class_12 * (k - 1) + internal.dim_n_R_next_vacc_class_1 * (j - 1) + i - 1];
        }
      }
    }
    for (int i = 1; i <= internal.dim_new_R_neg_1; ++i) {
...
carehomes::init_t dust_data<carehomes>(cpp11::list user) {
  ...
  internal.dim_prob_H_D = internal.n_groups;
  ...
  internal.prob_H_D = std::vector<real_t>(internal.dim_prob_H_D);
  internal.prob_ICU = std::vector<real_t>(internal.dim_prob_ICU);
  internal.prob_ICU_D = std::vector<real_t>(internal.dim_prob_ICU_D);
  internal.prob_W_D = std::vector<real_t>(internal.dim_prob_W_D);
  ...
  internal.dim_n_R_pos_3 = internal.s_sero_pos;
  ...
  internal.dim_new_R_pos_3 = internal.s_sero_pos;
  ...
  internal.dim_R_pos_3 = internal.s_sero_pos;
