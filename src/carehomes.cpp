...
compare(const typename T::real_type * state,
  ...
  const real_type model_sympt_cases = state[27];
  const real_type model_sympt_cases_non_variant =
    state[28];
  const real_type model_sympt_cases_over25 = state[29];
  const real_type model_sympt_cases_non_variant_over25 =
    state[30];
  const real_type model_react_pos = state[31];
...
public:
  ...
  struct shared_type {
    ...
    real_type phi_hosp;
    real_type phi_pillar2_cases;
    real_type phi_pillar2_cases_weekend;
  ...
  carehomes(const dust::pars_type<carehomes>& pars) :
  ...
  HOST void update(size_t step, const real_type * state, rng_state_type& rng_state, real_type * state_next) {
    ...
    const real_type * cum_infections_per_strain = state + 32;
    ...
    const real_type sympt_cases_inc = state[27];
    const real_type sympt_cases_non_variant_inc = state[28];
    const real_type sympt_cases_over25_inc = state[29];
    const real_type sympt_cases_non_variant_over25_inc = state[30];
    ...
    state_next[27] = ((fmodr<real_type>(step, shared->steps_per_day) == 0 ? new_sympt_cases : sympt_cases_inc + new_sympt_cases));
    state_next[28] = ((fmodr<real_type>(step, shared->steps_per_day) == 0 ? new_sympt_cases_non_variant : sympt_cases_non_variant_inc + new_sympt_cases_non_variant));
    state_next[30] = ((fmodr<real_type>(step, shared->steps_per_day) == 0 ? new_sympt_cases_non_variant_over25 : sympt_cases_non_variant_over25_inc + new_sympt_cases_non_variant_over25));
    state_next[29] = ((fmodr<real_type>(step, shared->steps_per_day) == 0 ? new_sympt_cases_over25 : sympt_cases_over25_inc + new_sympt_cases_over25));
    ...
    state_next[31] = odin_sum4<real_type>(internal.new_T_PCR_pos.data(), 1, 18, 0, shared->dim_new_T_PCR_pos_2, 0, shared->dim_new_T_PCR_pos_3, 0, shared->dim_new_T_PCR_pos_4, shared->dim_new_T_PCR_pos_1, shared->dim_new_T_PCR_pos_12, shared->dim_new_T_PCR_pos_123);
...
namespace dust {
template<>
dust::pars_type<carehomes> dust_pars<carehomes>(cpp11::list user) {
  typedef typename carehomes::real_type real_type;
  auto shared = std::make_shared<carehomes::shared_type>();
  carehomes::internal_type internal;
  ...
  shared->p_NC = user_get_scalar<real_type>(user, "p_NC", shared->p_NC, NA_REAL, NA_REAL);
  shared->p_NC_weekend = user_get_scalar<real_type>(user, "p_NC_weekend", shared->p_NC_weekend, NA_REAL, NA_REAL);
  ...
  shared->phi_pillar2_cases = user_get_scalar<real_type>(user, "phi_pillar2_cases", shared->phi_pillar2_cases, NA_REAL, NA_REAL);
  shared->phi_pillar2_cases_weekend = user_get_scalar<real_type>(user, "phi_pillar2_cases_weekend", shared->phi_pillar2_cases_weekend, NA_REAL, NA_REAL);
  ...
  return dust::pars_type<carehomes>(shared, internal);
}
template <>
cpp11::sexp dust_info<carehomes>(const dust::pars_type<carehomes>& pars) {
  const carehomes::internal_type internal = pars.internal;
  const std::shared_ptr<const carehomes::shared_type> shared = pars.shared;
...
template <>
carehomes::data_type dust_data<carehomes>(cpp11::list data) {
  typedef carehomes::real_type real_type;
  return carehomes::data_type{
...
cpp11::sexp dust_carehomes_device_info() {
  return dust::cuda::device_info<carehomes::real_type>();
}
