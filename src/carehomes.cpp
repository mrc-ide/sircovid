...
public:
  ...
  void update(size_t step, const real_t * state, dust::rng_state_t<real_t>& rng_state, real_t * state_next) {
    ...
    for (int i = 1; i <= shared->dim_vaccine_n_candidates; ++i) {
      internal.vaccine_n_candidates[i - 1] = S[shared->dim_S_1 * 0 + i - 1] + odin_sum4<real_t>(E, i - 1, i, 0, shared->dim_E_2, 0, shared->dim_E_3, 0, 1, shared->dim_E_1, shared->dim_E_12, shared->dim_E_123) + odin_sum4<real_t>(I_A, i - 1, i, 0, shared->dim_I_A_2, 0, shared->dim_I_A_3, 0, 1, shared->dim_I_A_1, shared->dim_I_A_12, shared->dim_I_A_123) + odin_sum3<real_t>(R, i - 1, i, 0, shared->dim_R_2, 0, 1, shared->dim_R_1, shared->dim_R_12);
    }
    ...
    for (int i = 1; i <= shared->dim_delta_D_comm; ++i) {
      internal.delta_D_comm[i - 1] = odin_sum4<real_t>(internal.n_G_D_progress.data(), i - 1, i, 0, shared->dim_n_G_D_progress_2, shared->k_G_D - 1, shared->k_G_D, 0, shared->dim_n_G_D_progress_4, shared->dim_n_G_D_progress_1, shared->dim_n_G_D_progress_12, shared->dim_n_G_D_progress_123);
    }
    ...
    real_t delta_D_comm_tot = odin_sum1<real_t>(internal.delta_D_comm.data(), 0, shared->dim_delta_D_comm);
    ...
    state_next[24] = odin_sum4<real_t>(internal.new_T_PCR_pos.data(), 1, 18, 0, shared->dim_new_T_PCR_pos_2, 0, shared->dim_new_T_PCR_pos_3, 0, shared->dim_new_T_PCR_pos_4, shared->dim_new_T_PCR_pos_1, shared->dim_new_T_PCR_pos_12, shared->dim_new_T_PCR_pos_123);
    ...
    for (int i = 1; i <= shared->dim_cum_admit_by_age; ++i) {
      state_next[shared->offset_variable_cum_admit_by_age + i - 1] = cum_admit_by_age[i - 1] + odin_sum3<real_t>(internal.n_I_C_to_hosp.data(), i - 1, i, 0, shared->dim_n_I_C_to_hosp_2, 0, shared->dim_n_I_C_to_hosp_3, shared->dim_n_I_C_to_hosp_1, shared->dim_n_I_C_to_hosp_12);
    }
    ...
    state_next[17] = odin_sum4<real_t>(internal.new_T_sero_pos.data(), 3, 13, 0, shared->dim_new_T_sero_pos_2, 0, shared->dim_new_T_sero_pos_3, 0, shared->dim_new_T_sero_pos_4, shared->dim_new_T_sero_pos_1, shared->dim_new_T_sero_pos_12, shared->dim_new_T_sero_pos_123);
    ...
    for (int i = 1; i <= shared->dim_cum_infections_per_strain; ++i) {
      state_next[25 + i - 1] = cum_infections_per_strain[i - 1] + odin_sum3<real_t>(internal.n_S_progress.data(), 0, shared->dim_n_S_progress_1, i - 1, i, 0, shared->dim_n_S_progress_3, shared->dim_n_S_progress_1, shared->dim_n_S_progress_12);
    }
    ...
    real_t new_sympt_cases = odin_sum1<real_t>(internal.n_EI_C.data(), 0, shared->dim_n_EI_C) + odin_sum1<real_t>(internal.n_EI_C_next_vacc_class.data(), 0, shared->dim_n_EI_C_next_vacc_class);
    real_t new_sympt_cases_non_variant_over25 = odin_sum3<real_t>(internal.n_EI_C.data(), 5, shared->n_groups, 0, 1, 0, shared->dim_n_EI_C_3, shared->dim_n_EI_C_1, shared->dim_n_EI_C_12) + odin_sum3<real_t>(internal.n_EI_C_next_vacc_class.data(), 5, shared->n_groups, 0, 1, 0, shared->dim_n_EI_C_next_vacc_class_3, shared->dim_n_EI_C_next_vacc_class_1, shared->dim_n_EI_C_next_vacc_class_12);
    real_t new_sympt_cases_over25 = odin_sum3<real_t>(internal.n_EI_C.data(), 5, shared->n_groups, 0, shared->dim_n_EI_C_2, 0, shared->dim_n_EI_C_3, shared->dim_n_EI_C_1, shared->dim_n_EI_C_12) + odin_sum3<real_t>(internal.n_EI_C_next_vacc_class.data(), 5, shared->n_groups, 0, shared->dim_n_EI_C_next_vacc_class_2, 0, shared->dim_n_EI_C_next_vacc_class_3, shared->dim_n_EI_C_next_vacc_class_1, shared->dim_n_EI_C_next_vacc_class_12);
    ...
    real_t delta_admit_conf = odin_sum1<real_t>(internal.n_I_C_to_H_D_conf.data(), 0, shared->dim_n_I_C_to_H_D_conf) + odin_sum1<real_t>(internal.n_I_C_to_H_R_conf.data(), 0, shared->dim_n_I_C_to_H_R_conf) + odin_sum1<real_t>(internal.n_I_C_to_ICU_pre_conf.data(), 0, shared->dim_n_I_C_to_ICU_pre_conf);
