...
public:
  ...
  struct shared_t {
    ...
    std::vector<real_t> initial_n_I_A_vaccinated;
    std::vector<real_t> initial_n_I_P_vaccinated;
    std::vector<real_t> initial_n_R_vaccinated;
    std::vector<real_t> initial_n_S_vaccinated;
    std::vector<real_t> initial_n_vaccinated;
    ...
    int offset_variable_n_I_A_vaccinated;
    int offset_variable_n_I_P_vaccinated;
    int offset_variable_n_R_vaccinated;
    int offset_variable_n_S_vaccinated;
    int offset_variable_n_vaccinated;
  ...
  std::vector<real_t> initial(size_t step) {
    ...
    std::copy(shared->initial_n_S_vaccinated.begin(), shared->initial_n_S_vaccinated.end(), state.begin() + shared->offset_variable_n_S_vaccinated);
    std::copy(shared->initial_n_E_vaccinated.begin(), shared->initial_n_E_vaccinated.end(), state.begin() + shared->offset_variable_n_E_vaccinated);
    std::copy(shared->initial_n_I_A_vaccinated.begin(), shared->initial_n_I_A_vaccinated.end(), state.begin() + shared->offset_variable_n_I_A_vaccinated);
    std::copy(shared->initial_n_I_P_vaccinated.begin(), shared->initial_n_I_P_vaccinated.end(), state.begin() + shared->offset_variable_n_I_P_vaccinated);
    std::copy(shared->initial_n_R_vaccinated.begin(), shared->initial_n_R_vaccinated.end(), state.begin() + shared->offset_variable_n_R_vaccinated);
    ...
    std::copy(shared->initial_n_vaccinated.begin(), shared->initial_n_vaccinated.end(), state.begin() + shared->offset_variable_n_vaccinated);
  ...
  HOST void update(size_t step, const real_t * state, dust::rng_state_t<real_t>& rng_state, real_t * state_next) {
    const real_t * n_S_vaccinated = state + shared->offset_variable_n_S_vaccinated;
    const real_t * n_E_vaccinated = state + shared->offset_variable_n_E_vaccinated;
    const real_t * n_I_A_vaccinated = state + shared->offset_variable_n_I_A_vaccinated;
    const real_t * n_I_P_vaccinated = state + shared->offset_variable_n_I_P_vaccinated;
    const real_t * n_R_vaccinated = state + shared->offset_variable_n_R_vaccinated;
    ...
    const real_t * n_vaccinated = state + shared->offset_variable_n_vaccinated;
    ...
    for (int i = 1; i <= shared->dim_cum_n_E_vaccinated_1; ++i) {
      for (int j = 1; j <= shared->dim_cum_n_E_vaccinated_2; ++j) {
        state_next[shared->offset_variable_cum_n_E_vaccinated + i - 1 + shared->dim_cum_n_E_vaccinated_1 * (j - 1)] = cum_n_E_vaccinated[shared->dim_cum_n_E_vaccinated_1 * (j - 1) + i - 1] + n_E_vaccinated[shared->dim_n_E_vaccinated_1 * (j - 1) + i - 1];
      }
    }
    for (int i = 1; i <= shared->dim_cum_n_I_A_vaccinated_1; ++i) {
      for (int j = 1; j <= shared->dim_cum_n_I_A_vaccinated_2; ++j) {
        state_next[shared->offset_variable_cum_n_I_A_vaccinated + i - 1 + shared->dim_cum_n_I_A_vaccinated_1 * (j - 1)] = cum_n_I_A_vaccinated[shared->dim_cum_n_I_A_vaccinated_1 * (j - 1) + i - 1] + n_I_A_vaccinated[shared->dim_n_I_A_vaccinated_1 * (j - 1) + i - 1];
      }
    }
    for (int i = 1; i <= shared->dim_cum_n_I_P_vaccinated_1; ++i) {
      for (int j = 1; j <= shared->dim_cum_n_I_P_vaccinated_2; ++j) {
        state_next[shared->offset_variable_cum_n_I_P_vaccinated + i - 1 + shared->dim_cum_n_I_P_vaccinated_1 * (j - 1)] = cum_n_I_P_vaccinated[shared->dim_cum_n_I_P_vaccinated_1 * (j - 1) + i - 1] + n_I_P_vaccinated[shared->dim_n_I_P_vaccinated_1 * (j - 1) + i - 1];
      }
    }
    for (int i = 1; i <= shared->dim_cum_n_R_vaccinated_1; ++i) {
      for (int j = 1; j <= shared->dim_cum_n_R_vaccinated_2; ++j) {
        state_next[shared->offset_variable_cum_n_R_vaccinated + i - 1 + shared->dim_cum_n_R_vaccinated_1 * (j - 1)] = cum_n_R_vaccinated[shared->dim_cum_n_R_vaccinated_1 * (j - 1) + i - 1] + n_R_vaccinated[shared->dim_n_R_vaccinated_1 * (j - 1) + i - 1];
      }
    }
    for (int i = 1; i <= shared->dim_cum_n_S_vaccinated_1; ++i) {
      for (int j = 1; j <= shared->dim_cum_n_S_vaccinated_2; ++j) {
        state_next[shared->offset_variable_cum_n_S_vaccinated + i - 1 + shared->dim_cum_n_S_vaccinated_1 * (j - 1)] = cum_n_S_vaccinated[shared->dim_cum_n_S_vaccinated_1 * (j - 1) + i - 1] + n_S_vaccinated[shared->dim_n_S_vaccinated_1 * (j - 1) + i - 1];
      }
    }
    for (int i = 1; i <= shared->dim_cum_n_vaccinated_1; ++i) {
      for (int j = 1; j <= shared->dim_cum_n_vaccinated_2; ++j) {
        state_next[shared->offset_variable_cum_n_vaccinated + i - 1 + shared->dim_cum_n_vaccinated_1 * (j - 1)] = cum_n_vaccinated[shared->dim_cum_n_vaccinated_1 * (j - 1) + i - 1] + n_vaccinated[shared->dim_n_vaccinated_1 * (j - 1) + i - 1];
      }
    }
    for (int i = 1; i <= shared->dim_n_vaccinated_1; ++i) {
      for (int j = 1; j <= shared->dim_n_vaccinated_2; ++j) {
        state_next[shared->offset_variable_n_vaccinated + i - 1 + shared->dim_n_vaccinated_1 * (j - 1)] = n_S_vaccinated[shared->dim_n_S_vaccinated_1 * (j - 1) + i - 1] + n_E_vaccinated[shared->dim_n_E_vaccinated_1 * (j - 1) + i - 1] + n_I_A_vaccinated[shared->dim_n_I_A_vaccinated_1 * (j - 1) + i - 1] + n_I_P_vaccinated[shared->dim_n_I_P_vaccinated_1 * (j - 1) + i - 1] + n_R_vaccinated[shared->dim_n_R_vaccinated_1 * (j - 1) + i - 1];
      }
    }
    ...
    for (int i = 1; i <= shared->dim_vaccine_missed_doses_1; ++i) {
      for (int j = 1; j <= shared->dim_vaccine_missed_doses_2; ++j) {
        state_next[shared->offset_variable_vaccine_missed_doses + i - 1 + shared->dim_vaccine_missed_doses_1 * (j - 1)] = shared->vaccine_catchup_fraction * std::max(internal.vaccine_attempted_doses[shared->dim_vaccine_attempted_doses_1 * (j - 1) + i - 1] - n_vaccinated[shared->dim_n_vaccinated_1 * (j - 1) + i - 1], static_cast<real_t>(0));
      }
    }
...
dust::pars_t<carehomes> dust_pars<carehomes>(cpp11::list user) {
  ...
  shared->offset_variable_n_S_vaccinated = shared->dim_D_hosp + shared->dim_D_non_hosp + shared->dim_N_tot + shared->dim_cum_admit_by_age + shared->dim_cum_infections_per_strain + 27;
  ...
  shared->initial_n_E_vaccinated = std::vector<real_t>(shared->dim_n_E_vaccinated);
  shared->initial_n_I_A_vaccinated = std::vector<real_t>(shared->dim_n_I_A_vaccinated);
  shared->initial_n_I_P_vaccinated = std::vector<real_t>(shared->dim_n_I_P_vaccinated);
  shared->initial_n_R_vaccinated = std::vector<real_t>(shared->dim_n_R_vaccinated);
  shared->initial_n_S_vaccinated = std::vector<real_t>(shared->dim_n_S_vaccinated);
  shared->initial_n_vaccinated = std::vector<real_t>(shared->dim_n_vaccinated);
  ...
  for (int i = 1; i <= shared->dim_n_E_vaccinated_1; ++i) {
    for (int j = 1; j <= shared->dim_n_E_vaccinated_2; ++j) {
      shared->initial_n_E_vaccinated[i - 1 + shared->dim_n_E_vaccinated_1 * (j - 1)] = 0;
    }
  }
  for (int i = 1; i <= shared->dim_n_I_A_vaccinated_1; ++i) {
    for (int j = 1; j <= shared->dim_n_I_A_vaccinated_2; ++j) {
      shared->initial_n_I_A_vaccinated[i - 1 + shared->dim_n_I_A_vaccinated_1 * (j - 1)] = 0;
    }
  }
  for (int i = 1; i <= shared->dim_n_I_P_vaccinated_1; ++i) {
    for (int j = 1; j <= shared->dim_n_I_P_vaccinated_2; ++j) {
      shared->initial_n_I_P_vaccinated[i - 1 + shared->dim_n_I_P_vaccinated_1 * (j - 1)] = 0;
    }
  }
  for (int i = 1; i <= shared->dim_n_R_vaccinated_1; ++i) {
    for (int j = 1; j <= shared->dim_n_R_vaccinated_2; ++j) {
      shared->initial_n_R_vaccinated[i - 1 + shared->dim_n_R_vaccinated_1 * (j - 1)] = 0;
    }
  }
  for (int i = 1; i <= shared->dim_n_S_vaccinated_1; ++i) {
    for (int j = 1; j <= shared->dim_n_S_vaccinated_2; ++j) {
      shared->initial_n_S_vaccinated[i - 1 + shared->dim_n_S_vaccinated_1 * (j - 1)] = 0;
    }
  }
  for (int i = 1; i <= shared->dim_n_vaccinated_1; ++i) {
    for (int j = 1; j <= shared->dim_n_vaccinated_2; ++j) {
      shared->initial_n_vaccinated[i - 1 + shared->dim_n_vaccinated_1 * (j - 1)] = 0;
    }
  }
  for (int i = 1; i <= shared->dim_prob_strain_1; ++i) {
    for (int j = 1; j <= shared->dim_prob_strain_2; ++j) {
  ...
  shared->offset_variable_I_weighted = shared->dim_D_hosp + shared->dim_D_non_hosp + shared->dim_N_tot + shared->dim_S + shared->dim_cum_admit_by_age + shared->dim_cum_infections_per_strain + shared->dim_cum_n_E_vaccinated + shared->dim_cum_n_I_A_vaccinated + shared->dim_cum_n_I_P_vaccinated + shared->dim_cum_n_R_vaccinated + shared->dim_cum_n_S_vaccinated + shared->dim_cum_n_vaccinated + shared->dim_n_E_vaccinated + shared->dim_n_I_A_vaccinated + shared->dim_n_I_P_vaccinated + shared->dim_n_R_vaccinated + shared->dim_n_S_vaccinated + shared->dim_n_vaccinated + shared->dim_prob_strain + 27;
  ...
  shared->offset_variable_S = shared->dim_D_hosp + shared->dim_D_non_hosp + shared->dim_N_tot + shared->dim_cum_admit_by_age + shared->dim_cum_infections_per_strain + shared->dim_cum_n_E_vaccinated + shared->dim_cum_n_I_A_vaccinated + shared->dim_cum_n_I_P_vaccinated + shared->dim_cum_n_R_vaccinated + shared->dim_cum_n_S_vaccinated + shared->dim_cum_n_vaccinated + shared->dim_n_E_vaccinated + shared->dim_n_I_A_vaccinated + shared->dim_n_I_P_vaccinated + shared->dim_n_R_vaccinated + shared->dim_n_S_vaccinated + shared->dim_n_vaccinated + 27;
  ...
  shared->offset_variable_cum_n_E_vaccinated = shared->dim_D_hosp + shared->dim_D_non_hosp + shared->dim_N_tot + shared->dim_cum_admit_by_age + shared->dim_cum_infections_per_strain + shared->dim_cum_n_S_vaccinated + shared->dim_n_E_vaccinated + shared->dim_n_I_A_vaccinated + shared->dim_n_I_P_vaccinated + shared->dim_n_R_vaccinated + shared->dim_n_S_vaccinated + 27;
  shared->offset_variable_cum_n_I_A_vaccinated = shared->dim_D_hosp + shared->dim_D_non_hosp + shared->dim_N_tot + shared->dim_cum_admit_by_age + shared->dim_cum_infections_per_strain + shared->dim_cum_n_E_vaccinated + shared->dim_cum_n_S_vaccinated + shared->dim_n_E_vaccinated + shared->dim_n_I_A_vaccinated + shared->dim_n_I_P_vaccinated + shared->dim_n_R_vaccinated + shared->dim_n_S_vaccinated + 27;
  ...
  shared->offset_variable_cum_n_R_vaccinated = shared->dim_D_hosp + shared->dim_D_non_hosp + shared->dim_N_tot + shared->dim_cum_admit_by_age + shared->dim_cum_infections_per_strain + shared->dim_cum_n_E_vaccinated + shared->dim_cum_n_I_A_vaccinated + shared->dim_cum_n_I_P_vaccinated + shared->dim_cum_n_S_vaccinated + shared->dim_n_E_vaccinated + shared->dim_n_I_A_vaccinated + shared->dim_n_I_P_vaccinated + shared->dim_n_R_vaccinated + shared->dim_n_S_vaccinated + 27;
  shared->offset_variable_cum_n_S_vaccinated = shared->dim_D_hosp + shared->dim_D_non_hosp + shared->dim_N_tot + shared->dim_cum_admit_by_age + shared->dim_cum_infections_per_strain + shared->dim_n_E_vaccinated + shared->dim_n_I_A_vaccinated + shared->dim_n_I_P_vaccinated + shared->dim_n_R_vaccinated + shared->dim_n_S_vaccinated + 27;
  shared->offset_variable_cum_n_vaccinated = shared->dim_D_hosp + shared->dim_D_non_hosp + shared->dim_N_tot + shared->dim_cum_admit_by_age + shared->dim_cum_infections_per_strain + shared->dim_cum_n_E_vaccinated + shared->dim_cum_n_I_A_vaccinated + shared->dim_cum_n_I_P_vaccinated + shared->dim_cum_n_R_vaccinated + shared->dim_cum_n_S_vaccinated + shared->dim_n_E_vaccinated + shared->dim_n_I_A_vaccinated + shared->dim_n_I_P_vaccinated + shared->dim_n_R_vaccinated + shared->dim_n_S_vaccinated + shared->dim_n_vaccinated + 27;
  shared->offset_variable_n_E_vaccinated = shared->dim_D_hosp + shared->dim_D_non_hosp + shared->dim_N_tot + shared->dim_cum_admit_by_age + shared->dim_cum_infections_per_strain + shared->dim_n_S_vaccinated + 27;
  shared->offset_variable_n_I_A_vaccinated = shared->dim_D_hosp + shared->dim_D_non_hosp + shared->dim_N_tot + shared->dim_cum_admit_by_age + shared->dim_cum_infections_per_strain + shared->dim_n_E_vaccinated + shared->dim_n_S_vaccinated + 27;
  shared->offset_variable_n_I_P_vaccinated = shared->dim_D_hosp + shared->dim_D_non_hosp + shared->dim_N_tot + shared->dim_cum_admit_by_age + shared->dim_cum_infections_per_strain + shared->dim_n_E_vaccinated + shared->dim_n_I_A_vaccinated + shared->dim_n_S_vaccinated + 27;
  shared->offset_variable_n_R_vaccinated = shared->dim_D_hosp + shared->dim_D_non_hosp + shared->dim_N_tot + shared->dim_cum_admit_by_age + shared->dim_cum_infections_per_strain + shared->dim_n_E_vaccinated + shared->dim_n_I_A_vaccinated + shared->dim_n_I_P_vaccinated + shared->dim_n_S_vaccinated + 27;
  shared->offset_variable_n_vaccinated = shared->dim_D_hosp + shared->dim_D_non_hosp + shared->dim_N_tot + shared->dim_cum_admit_by_age + shared->dim_cum_infections_per_strain + shared->dim_cum_n_E_vaccinated + shared->dim_cum_n_I_A_vaccinated + shared->dim_cum_n_I_P_vaccinated + shared->dim_cum_n_R_vaccinated + shared->dim_cum_n_S_vaccinated + shared->dim_n_E_vaccinated + shared->dim_n_I_A_vaccinated + shared->dim_n_I_P_vaccinated + shared->dim_n_R_vaccinated + shared->dim_n_S_vaccinated + 27;
  ...
  shared->offset_variable_tmp_vaccine_n_candidates = shared->dim_D_hosp + shared->dim_D_non_hosp + shared->dim_I_weighted + shared->dim_N_tot + shared->dim_S + shared->dim_cum_admit_by_age + shared->dim_cum_infections_per_strain + shared->dim_cum_n_E_vaccinated + shared->dim_cum_n_I_A_vaccinated + shared->dim_cum_n_I_P_vaccinated + shared->dim_cum_n_R_vaccinated + shared->dim_cum_n_S_vaccinated + shared->dim_cum_n_vaccinated + shared->dim_n_E_vaccinated + shared->dim_n_I_A_vaccinated + shared->dim_n_I_P_vaccinated + shared->dim_n_R_vaccinated + shared->dim_n_S_vaccinated + shared->dim_n_vaccinated + shared->dim_prob_strain + shared->dim_vaccine_missed_doses + 27;
...
cpp11::sexp dust_info<carehomes>(const dust::pars_t<carehomes>& pars) {
  ...
  dim[32] = cpp11::writable::integers({shared->dim_n_S_vaccinated_1, shared->dim_n_S_vaccinated_2});
  dim[33] = cpp11::writable::integers({shared->dim_n_E_vaccinated_1, shared->dim_n_E_vaccinated_2});
  dim[34] = cpp11::writable::integers({shared->dim_n_I_A_vaccinated_1, shared->dim_n_I_A_vaccinated_2});
  dim[35] = cpp11::writable::integers({shared->dim_n_I_P_vaccinated_1, shared->dim_n_I_P_vaccinated_2});
  dim[36] = cpp11::writable::integers({shared->dim_n_R_vaccinated_1, shared->dim_n_R_vaccinated_2});
  ...
  dim[42] = cpp11::writable::integers({shared->dim_n_vaccinated_1, shared->dim_n_vaccinated_2});
  ...
  dim[45] = cpp11::writable::integers({shared->dim_prob_strain_1, shared->dim_prob_strain_2});
  ...
  dim[50] = cpp11::writable::integers({shared->dim_T_sero_neg_1, shared->dim_T_sero_neg_2, shared->dim_T_sero_neg_3});
  ...
  dim[75] = cpp11::writable::integers({shared->dim_T_sero_pre_1, shared->dim_T_sero_pre_2, shared->dim_T_sero_pre_3, shared->dim_T_sero_pre_4});
  dim[76] = cpp11::writable::integers({shared->dim_T_sero_pos_1, shared->dim_T_sero_pos_2, shared->dim_T_sero_pos_3, shared->dim_T_sero_pos_4});
  dim[77] = cpp11::writable::integers({shared->dim_T_PCR_pre_1, shared->dim_T_PCR_pre_2, shared->dim_T_PCR_pre_3, shared->dim_T_PCR_pre_4});
  dim[78] = cpp11::writable::integers({shared->dim_T_PCR_pos_1, shared->dim_T_PCR_pos_2, shared->dim_T_PCR_pos_3, shared->dim_T_PCR_pos_4});
  dim.names() = nms;
  cpp11::writable::list index(79);
  ...
  index[32] = integer_sequence(shared->offset_variable_n_S_vaccinated + 1, shared->dim_n_S_vaccinated);
  index[33] = integer_sequence(shared->offset_variable_n_E_vaccinated + 1, shared->dim_n_E_vaccinated);
  index[34] = integer_sequence(shared->offset_variable_n_I_A_vaccinated + 1, shared->dim_n_I_A_vaccinated);
  index[35] = integer_sequence(shared->offset_variable_n_I_P_vaccinated + 1, shared->dim_n_I_P_vaccinated);
  index[36] = integer_sequence(shared->offset_variable_n_R_vaccinated + 1, shared->dim_n_R_vaccinated);
  ...
  index[42] = integer_sequence(shared->offset_variable_n_vaccinated + 1, shared->dim_n_vaccinated);
  ...
  index[50] = integer_sequence(shared->offset_variable_T_sero_neg + 1, shared->dim_T_sero_neg);
  ...
  index[75] = integer_sequence(shared->offset_variable_T_sero_pre + 1, shared->dim_T_sero_pre);
  index[76] = integer_sequence(shared->offset_variable_T_sero_pos + 1, shared->dim_T_sero_pos);
  index[77] = integer_sequence(shared->offset_variable_T_PCR_pre + 1, shared->dim_T_PCR_pre);
  index[78] = integer_sequence(shared->offset_variable_T_PCR_pos + 1, shared->dim_T_PCR_pos);
  index.names() = nms;
