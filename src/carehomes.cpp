...
public:
  ...
  struct shared_t {
    ...
    int dim_infections_inc;
    int dim_infections_inc_1;
    int dim_infections_inc_2;
    int dim_lambda;
    int dim_lambda_1;
    int dim_lambda_2;
    int dim_m;
    ...
    std::vector<real_t> initial_infections_inc;
    ...
    int offset_variable_infections_inc;
  ...
  std::vector<real_t> initial(size_t step) {
    ...
    std::copy(shared->initial_infections_inc.begin(), shared->initial_infections_inc.end(), state.begin() + shared->offset_variable_infections_inc);
  ...
  void update(size_t step, const real_t * state, dust::rng_state_t<real_t>& rng_state, real_t * state_next) {
    ...
    const real_t * infections_inc = state + shared->offset_variable_infections_inc;
    ...
    for (int i = 1; i <= shared->dim_infections_inc_1; ++i) {
      for (int j = 1; j <= shared->dim_infections_inc_2; ++j) {
        state_next[shared->offset_variable_infections_inc + i - 1 + shared->dim_infections_inc_1 * (j - 1)] = ((std::fmod(step, shared->steps_per_day) == 0 ? internal.n_S_progress_tot[shared->dim_n_S_progress_tot_1 * (j - 1) + i - 1] : infections_inc[shared->dim_infections_inc_1 * (j - 1) + i - 1] + internal.n_S_progress_tot[shared->dim_n_S_progress_tot_1 * (j - 1) + i - 1]));
      }
    }
...
carehomes::init_t dust_pars<carehomes>(cpp11::list user) {
  ...
  shared->dim_infections_inc_1 = shared->n_groups;
  shared->dim_infections_inc_2 = shared->n_vacc_classes;
  ...
  shared->dim_infections_inc = shared->dim_infections_inc_1 * shared->dim_infections_inc_2;
  ...
  shared->initial_infections_inc = std::vector<real_t>(shared->dim_infections_inc);
  ...
  for (int i = 1; i <= shared->dim_infections_inc_1; ++i) {
    for (int j = 1; j <= shared->dim_infections_inc_2; ++j) {
      shared->initial_infections_inc[i - 1 + shared->dim_infections_inc_1 * (j - 1)] = 0;
    }
  }
  ...
  shared->offset_variable_R = 25 + shared->dim_cum_infections_per_strain + shared->dim_D_hosp + shared->dim_D_comm + shared->dim_cum_admit_by_age + shared->dim_N_tot + shared->dim_cum_n_S_vaccinated + shared->dim_cum_n_E_vaccinated + shared->dim_cum_n_I_A_vaccinated + shared->dim_cum_n_R_vaccinated + shared->dim_cum_n_vaccinated + shared->dim_S + shared->dim_prob_strain + shared->dim_infections_inc + shared->dim_T_sero_neg;
  ...
  shared->offset_variable_T_sero_neg = 25 + shared->dim_cum_infections_per_strain + shared->dim_D_hosp + shared->dim_D_comm + shared->dim_cum_admit_by_age + shared->dim_N_tot + shared->dim_cum_n_S_vaccinated + shared->dim_cum_n_E_vaccinated + shared->dim_cum_n_I_A_vaccinated + shared->dim_cum_n_R_vaccinated + shared->dim_cum_n_vaccinated + shared->dim_S + shared->dim_prob_strain + shared->dim_infections_inc;
  ...
  shared->offset_variable_infections_inc = 25 + shared->dim_cum_infections_per_strain + shared->dim_D_hosp + shared->dim_D_comm + shared->dim_cum_admit_by_age + shared->dim_N_tot + shared->dim_cum_n_S_vaccinated + shared->dim_cum_n_E_vaccinated + shared->dim_cum_n_I_A_vaccinated + shared->dim_cum_n_R_vaccinated + shared->dim_cum_n_vaccinated + shared->dim_S + shared->dim_prob_strain;
...
cpp11::sexp dust_info<carehomes>(const carehomes::init_t& init) {
  ...
  dim[37] = cpp11::writable::integers({shared->dim_infections_inc_1, shared->dim_infections_inc_2});
  ...
  index[37] = integer_sequence(shared->offset_variable_infections_inc + 1, shared->dim_infections_inc);
