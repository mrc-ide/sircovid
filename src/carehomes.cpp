...
// Generated by odin.dust (version 0.1.15) - do not edit
...
template <typename real_t, typename container>
real_t odin_sum3(const container x, int from_i, int to_i, int from_j, int to_j, int from_k, int to_k, int dim_x_1, int dim_x_12);
template <typename real_t, typename container>
real_t odin_sum4(const container x, int from_i, int to_i, int from_j, int to_j, int from_k, int to_k, int from_l, int to_l, int dim_x_1, int dim_x_12, int dim_x_123);
...
// [[dust::param(vaccine_daily_dose1_time, has_default = FALSE, default_value = NULL, rank = 2, min = -Inf, max = Inf, integer = FALSE)]]
// [[dust::param(vaccine_daily_dose2_time, has_default = FALSE, default_value = NULL, rank = 2, min = -Inf, max = Inf, integer = FALSE)]]
// [[dust::param(vaccine_daily_dose_offset, has_default = FALSE, default_value = NULL, rank = 0, min = -Inf, max = Inf, integer = FALSE)]]
...
// [[dust::param(index_dose1, has_default = TRUE, default_value = 1L, rank = 0, min = -Inf, max = Inf, integer = FALSE)]]
// [[dust::param(index_dose2, has_default = TRUE, default_value = 1L, rank = 0, min = -Inf, max = Inf, integer = FALSE)]]
...
public:
  ...
  struct shared_t {
    ...
    int dim_tmp_vaccine_n_candidates1;
    int dim_tmp_vaccine_probability;
    int dim_tmp_vaccine_probability1;
    int dim_tmp_vaccine_probability2;
    int dim_tmp_vaccine_probability_1;
    int dim_tmp_vaccine_probability_2;
    int dim_vaccine_daily_dose1_time;
    int dim_vaccine_daily_dose1_time_1;
    int dim_vaccine_daily_dose1_time_2;
    int dim_vaccine_daily_dose2_time;
    int dim_vaccine_daily_dose2_time_1;
    int dim_vaccine_daily_dose2_time_2;
    int dim_vaccine_n_candidates1;
    int dim_vaccine_n_candidates2;
    int dim_vaccine_probability;
    int dim_vaccine_probability1;
    int dim_vaccine_probability2;
    ...
    real_t initial_time;
    std::vector<real_t> initial_tmp_vaccine_n_candidates1;
    std::vector<real_t> initial_tmp_vaccine_probability;
    std::vector<real_t> initial_tmp_vaccine_probability1;
    std::vector<real_t> initial_tmp_vaccine_probability2;
    int k_A;
    int k_C_1;
    int k_C_2;
    int k_E;
    ...
    int offset_variable_tmp_vaccine_n_candidates1;
    int offset_variable_tmp_vaccine_probability;
    int offset_variable_tmp_vaccine_probability1;
    int offset_variable_tmp_vaccine_probability2;
    ...
    std::vector<real_t> vaccine_daily_dose1_time;
    std::vector<real_t> vaccine_daily_dose2_time;
    int vaccine_daily_dose_offset;
  ...
  std::vector<real_t> initial(size_t step) {
    ...
    std::copy(shared->initial_tmp_vaccine_n_candidates1.begin(), shared->initial_tmp_vaccine_n_candidates1.end(), state.begin() + shared->offset_variable_tmp_vaccine_n_candidates1);
    std::copy(shared->initial_tmp_vaccine_probability1.begin(), shared->initial_tmp_vaccine_probability1.end(), state.begin() + shared->offset_variable_tmp_vaccine_probability1);
    std::copy(shared->initial_tmp_vaccine_probability2.begin(), shared->initial_tmp_vaccine_probability2.end(), state.begin() + shared->offset_variable_tmp_vaccine_probability2);
  ...
  void update(size_t step, const real_t * state, dust::rng_state_t<real_t>& rng_state, real_t * state_next) {
    ...
    for (int i = 1; i <= shared->dim_vaccine_n_candidates1; ++i) {
      internal.vaccine_n_candidates1[i - 1] = S[shared->dim_S_1 * (shared->index_dose1 - 1) + i - 1] + odin_sum4<real_t>(E, i - 1, i, 0, shared->dim_E_2, 0, shared->dim_E_3, shared->index_dose1 - 1, shared->index_dose1, shared->dim_E_1, shared->dim_E_12, shared->dim_E_123) + odin_sum4<real_t>(I_A, i - 1, i, 0, shared->dim_I_A_2, 0, shared->dim_I_A_3, shared->index_dose1 - 1, shared->index_dose1, shared->dim_I_A_1, shared->dim_I_A_12, shared->dim_I_A_123) + odin_sum4<real_t>(I_P, i - 1, i, 0, shared->dim_I_P_2, 0, shared->dim_I_P_3, shared->index_dose1 - 1, shared->index_dose1, shared->dim_I_P_1, shared->dim_I_P_12, shared->dim_I_P_123) + odin_sum3<real_t>(R, i - 1, i, 0, shared->dim_R_2, shared->index_dose1 - 1, shared->index_dose1, shared->dim_R_1, shared->dim_R_12);
    }
    for (int i = 1; i <= shared->dim_vaccine_n_candidates2; ++i) {
      internal.vaccine_n_candidates2[i - 1] = S[shared->dim_S_1 * (shared->index_dose2 - 1) + i - 1] + odin_sum4<real_t>(E, i - 1, i, 0, shared->dim_E_2, 0, shared->dim_E_3, shared->index_dose2 - 1, shared->index_dose2, shared->dim_E_1, shared->dim_E_12, shared->dim_E_123) + odin_sum4<real_t>(I_A, i - 1, i, 0, shared->dim_I_A_2, 0, shared->dim_I_A_3, shared->index_dose2 - 1, shared->index_dose2, shared->dim_I_A_1, shared->dim_I_A_12, shared->dim_I_A_123) + odin_sum4<real_t>(I_P, i - 1, i, 0, shared->dim_I_P_2, 0, shared->dim_I_P_3, shared->index_dose2 - 1, shared->index_dose2, shared->dim_I_P_1, shared->dim_I_P_12, shared->dim_I_P_123) + odin_sum3<real_t>(R, i - 1, i, 0, shared->dim_R_2, shared->index_dose2 - 1, shared->index_dose2, shared->dim_R_1, shared->dim_R_12);
    }
    ...
    for (int i = 1; i <= shared->dim_tmp_vaccine_n_candidates1; ++i) {
      state_next[shared->offset_variable_tmp_vaccine_n_candidates1 + i - 1] = internal.vaccine_n_candidates1[i - 1];
    }
    for (int i = 1; i <= shared->dim_vaccine_probability1; ++i) {
      internal.vaccine_probability1[i - 1] = ((internal.vaccine_n_candidates1[i - 1] == 0 ? 0 : shared->vaccine_daily_dose1_time[shared->dim_vaccine_daily_dose1_time_1 * (step - 1) + i - 1] / (real_t) internal.vaccine_n_candidates1[i - 1]));
    }
    for (int i = 1; i <= shared->dim_vaccine_probability2; ++i) {
      internal.vaccine_probability2[i - 1] = ((internal.vaccine_n_candidates2[i - 1] == 0 ? 0 : shared->vaccine_daily_dose2_time[shared->dim_vaccine_daily_dose2_time_1 * (step - 1) + i - 1] / (real_t) internal.vaccine_n_candidates2[i - 1]));
    }
    ...
    for (int i = 1; i <= shared->dim_tmp_vaccine_probability1; ++i) {
      state_next[shared->offset_variable_tmp_vaccine_probability1 + i - 1] = internal.vaccine_probability1[i - 1];
    }
    for (int i = 1; i <= shared->dim_tmp_vaccine_probability2; ++i) {
      state_next[shared->offset_variable_tmp_vaccine_probability2 + i - 1] = internal.vaccine_probability2[i - 1];
    }
    ...
    for (int i = 1; i <= shared->dim_vaccine_probability_1; ++i) {
      int j = shared->index_dose1;
      internal.vaccine_probability[i - 1 + shared->dim_vaccine_probability_1 * (j - 1)] = internal.vaccine_probability1[i - 1];
    }
    for (int i = 1; i <= shared->dim_vaccine_probability_1; ++i) {
      int j = shared->index_dose2;
      internal.vaccine_probability[i - 1 + shared->dim_vaccine_probability_1 * (j - 1)] = internal.vaccine_probability2[i - 1];
    }
...
dust::pars_t<carehomes> dust_pars<carehomes>(cpp11::list user) {
  ...
  shared->vaccine_daily_dose_offset = NA_INTEGER;
  ...
  shared->index_dose1 = 1;
  shared->index_dose2 = 1;
  ...
  shared->index_dose1 = user_get_scalar<int>(user, "index_dose1", shared->index_dose1, NA_REAL, NA_REAL);
  shared->index_dose2 = user_get_scalar<int>(user, "index_dose2", shared->index_dose2, NA_REAL, NA_REAL);
  ...
  std::array <int, 2> dim_vaccine_daily_dose1_time;
  shared->vaccine_daily_dose1_time = user_get_array_variable<real_t, 2>(user, "vaccine_daily_dose1_time", shared->vaccine_daily_dose1_time, dim_vaccine_daily_dose1_time, NA_REAL, NA_REAL);
  shared->dim_vaccine_daily_dose1_time = shared->vaccine_daily_dose1_time.size();
  shared->dim_vaccine_daily_dose1_time_1 = dim_vaccine_daily_dose1_time[0];
  shared->dim_vaccine_daily_dose1_time_2 = dim_vaccine_daily_dose1_time[1];
  std::array <int, 2> dim_vaccine_daily_dose2_time;
  shared->vaccine_daily_dose2_time = user_get_array_variable<real_t, 2>(user, "vaccine_daily_dose2_time", shared->vaccine_daily_dose2_time, dim_vaccine_daily_dose2_time, NA_REAL, NA_REAL);
  shared->dim_vaccine_daily_dose2_time = shared->vaccine_daily_dose2_time.size();
  shared->dim_vaccine_daily_dose2_time_1 = dim_vaccine_daily_dose2_time[0];
  shared->dim_vaccine_daily_dose2_time_2 = dim_vaccine_daily_dose2_time[1];
  shared->vaccine_daily_dose_offset = user_get_scalar<int>(user, "vaccine_daily_dose_offset", shared->vaccine_daily_dose_offset, NA_REAL, NA_REAL);
  ...
  shared->dim_tmp_vaccine_n_candidates1 = shared->n_groups;
  shared->dim_tmp_vaccine_probability1 = shared->n_groups;
  shared->dim_tmp_vaccine_probability2 = shared->n_groups;
  shared->dim_vaccine_n_candidates1 = shared->n_groups;
  shared->dim_vaccine_n_candidates2 = shared->n_groups;
  shared->dim_vaccine_probability1 = shared->n_groups;
  shared->dim_vaccine_probability2 = shared->n_groups;
  ...
  shared->initial_tmp_vaccine_n_candidates1 = std::vector<real_t>(shared->dim_tmp_vaccine_n_candidates1);
  shared->initial_tmp_vaccine_probability1 = std::vector<real_t>(shared->dim_tmp_vaccine_probability1);
  shared->initial_tmp_vaccine_probability2 = std::vector<real_t>(shared->dim_tmp_vaccine_probability2);
  ...
  internal.vaccine_n_candidates1 = std::vector<real_t>(shared->dim_vaccine_n_candidates1);
  internal.vaccine_n_candidates2 = std::vector<real_t>(shared->dim_vaccine_n_candidates2);
  internal.vaccine_probability1 = std::vector<real_t>(shared->dim_vaccine_probability1);
  internal.vaccine_probability2 = std::vector<real_t>(shared->dim_vaccine_probability2);
  ...
  for (int i = 1; i <= shared->dim_tmp_vaccine_n_candidates1; ++i) {
    shared->initial_tmp_vaccine_n_candidates1[i - 1] = 0;
  }
  for (int i = 1; i <= shared->dim_tmp_vaccine_probability1; ++i) {
    shared->initial_tmp_vaccine_probability1[i - 1] = 0;
  }
  for (int i = 1; i <= shared->dim_tmp_vaccine_probability2; ++i) {
    shared->initial_tmp_vaccine_probability2[i - 1] = 0;
  }
  ...
  shared->offset_variable_cum_n_S_vaccinated = 27 + shared->dim_cum_infections_per_strain + shared->dim_D_hosp + shared->dim_D_non_hosp + shared->dim_cum_admit_by_age + shared->dim_N_tot + shared->dim_tmp_vaccine_n_candidates1 + shared->dim_tmp_vaccine_probability1 + shared->dim_tmp_vaccine_probability2;
  shared->offset_variable_tmp_vaccine_n_candidates1 = 27 + shared->dim_cum_infections_per_strain + shared->dim_D_hosp + shared->dim_D_non_hosp + shared->dim_cum_admit_by_age + shared->dim_N_tot;
  shared->offset_variable_tmp_vaccine_probability1 = 27 + shared->dim_cum_infections_per_strain + shared->dim_D_hosp + shared->dim_D_non_hosp + shared->dim_cum_admit_by_age + shared->dim_N_tot + shared->dim_tmp_vaccine_n_candidates1;
  shared->offset_variable_tmp_vaccine_probability2 = 27 + shared->dim_cum_infections_per_strain + shared->dim_D_hosp + shared->dim_D_non_hosp + shared->dim_cum_admit_by_age + shared->dim_N_tot + shared->dim_tmp_vaccine_n_candidates1 + shared->dim_tmp_vaccine_probability1;
  ...
  shared->offset_variable_cum_n_E_vaccinated = 27 + shared->dim_cum_infections_per_strain + shared->dim_D_hosp + shared->dim_D_non_hosp + shared->dim_cum_admit_by_age + shared->dim_N_tot + shared->dim_tmp_vaccine_n_candidates1 + shared->dim_tmp_vaccine_probability1 + shared->dim_tmp_vaccine_probability2 + shared->dim_cum_n_S_vaccinated;
  shared->offset_variable_cum_n_I_A_vaccinated = 27 + shared->dim_cum_infections_per_strain + shared->dim_D_hosp + shared->dim_D_non_hosp + shared->dim_cum_admit_by_age + shared->dim_N_tot + shared->dim_tmp_vaccine_n_candidates1 + shared->dim_tmp_vaccine_probability1 + shared->dim_tmp_vaccine_probability2 + shared->dim_cum_n_S_vaccinated + shared->dim_cum_n_E_vaccinated;
  shared->offset_variable_cum_n_I_P_vaccinated = 27 + shared->dim_cum_infections_per_strain + shared->dim_D_hosp + shared->dim_D_non_hosp + shared->dim_cum_admit_by_age + shared->dim_N_tot + shared->dim_tmp_vaccine_n_candidates1 + shared->dim_tmp_vaccine_probability1 + shared->dim_tmp_vaccine_probability2 + shared->dim_cum_n_S_vaccinated + shared->dim_cum_n_E_vaccinated + shared->dim_cum_n_I_A_vaccinated;
  shared->offset_variable_cum_n_R_vaccinated = 27 + shared->dim_cum_infections_per_strain + shared->dim_D_hosp + shared->dim_D_non_hosp + shared->dim_cum_admit_by_age + shared->dim_N_tot + shared->dim_tmp_vaccine_n_candidates1 + shared->dim_tmp_vaccine_probability1 + shared->dim_tmp_vaccine_probability2 + shared->dim_cum_n_S_vaccinated + shared->dim_cum_n_E_vaccinated + shared->dim_cum_n_I_A_vaccinated + shared->dim_cum_n_I_P_vaccinated;
...
cpp11::sexp dust_info<carehomes>(const dust::pars_t<carehomes>& pars) {
  ...
  dim[32] = cpp11::writable::integers({shared->dim_tmp_vaccine_n_candidates1});
  dim[33] = cpp11::writable::integers({shared->dim_tmp_vaccine_probability1});
  dim[34] = cpp11::writable::integers({shared->dim_tmp_vaccine_probability2});
  ...
  dim[42] = cpp11::writable::integers({shared->dim_prob_strain_1, shared->dim_prob_strain_2});
  ...
  dim[45] = cpp11::writable::integers({shared->dim_T_sero_neg_1, shared->dim_T_sero_neg_2, shared->dim_T_sero_neg_3});
  dim[46] = cpp11::writable::integers({shared->dim_R_1, shared->dim_R_2, shared->dim_R_3});
  dim[47] = cpp11::writable::integers({shared->dim_T_PCR_neg_1, shared->dim_T_PCR_neg_2, shared->dim_T_PCR_neg_3});
  dim[48] = cpp11::writable::integers({shared->dim_E_1, shared->dim_E_2, shared->dim_E_3, shared->dim_E_4});
  dim[49] = cpp11::writable::integers({shared->dim_I_A_1, shared->dim_I_A_2, shared->dim_I_A_3, shared->dim_I_A_4});
  dim[50] = cpp11::writable::integers({shared->dim_I_P_1, shared->dim_I_P_2, shared->dim_I_P_3, shared->dim_I_P_4});
  dim[51] = cpp11::writable::integers({shared->dim_I_C_1_1, shared->dim_I_C_1_2, shared->dim_I_C_1_3, shared->dim_I_C_1_4});
  dim[52] = cpp11::writable::integers({shared->dim_I_C_2_1, shared->dim_I_C_2_2, shared->dim_I_C_2_3, shared->dim_I_C_2_4});
  dim[53] = cpp11::writable::integers({shared->dim_G_D_1, shared->dim_G_D_2, shared->dim_G_D_3, shared->dim_G_D_4});
  dim[54] = cpp11::writable::integers({shared->dim_ICU_pre_unconf_1, shared->dim_ICU_pre_unconf_2, shared->dim_ICU_pre_unconf_3, shared->dim_ICU_pre_unconf_4});
  dim[55] = cpp11::writable::integers({shared->dim_ICU_pre_conf_1, shared->dim_ICU_pre_conf_2, shared->dim_ICU_pre_conf_3, shared->dim_ICU_pre_conf_4});
  dim[56] = cpp11::writable::integers({shared->dim_H_R_unconf_1, shared->dim_H_R_unconf_2, shared->dim_H_R_unconf_3, shared->dim_H_R_unconf_4});
  dim[57] = cpp11::writable::integers({shared->dim_H_R_conf_1, shared->dim_H_R_conf_2, shared->dim_H_R_conf_3, shared->dim_H_R_conf_4});
  dim[58] = cpp11::writable::integers({shared->dim_H_D_unconf_1, shared->dim_H_D_unconf_2, shared->dim_H_D_unconf_3, shared->dim_H_D_unconf_4});
  dim[59] = cpp11::writable::integers({shared->dim_H_D_conf_1, shared->dim_H_D_conf_2, shared->dim_H_D_conf_3, shared->dim_H_D_conf_4});
  dim[60] = cpp11::writable::integers({shared->dim_ICU_W_R_unconf_1, shared->dim_ICU_W_R_unconf_2, shared->dim_ICU_W_R_unconf_3, shared->dim_ICU_W_R_unconf_4});
  dim[61] = cpp11::writable::integers({shared->dim_ICU_W_R_conf_1, shared->dim_ICU_W_R_conf_2, shared->dim_ICU_W_R_conf_3, shared->dim_ICU_W_R_conf_4});
  dim[62] = cpp11::writable::integers({shared->dim_ICU_W_D_unconf_1, shared->dim_ICU_W_D_unconf_2, shared->dim_ICU_W_D_unconf_3, shared->dim_ICU_W_D_unconf_4});
  dim[63] = cpp11::writable::integers({shared->dim_ICU_W_D_conf_1, shared->dim_ICU_W_D_conf_2, shared->dim_ICU_W_D_conf_3, shared->dim_ICU_W_D_conf_4});
  dim[64] = cpp11::writable::integers({shared->dim_ICU_D_unconf_1, shared->dim_ICU_D_unconf_2, shared->dim_ICU_D_unconf_3, shared->dim_ICU_D_unconf_4});
  dim[65] = cpp11::writable::integers({shared->dim_ICU_D_conf_1, shared->dim_ICU_D_conf_2, shared->dim_ICU_D_conf_3, shared->dim_ICU_D_conf_4});
  dim[66] = cpp11::writable::integers({shared->dim_W_R_unconf_1, shared->dim_W_R_unconf_2, shared->dim_W_R_unconf_3, shared->dim_W_R_unconf_4});
  dim[67] = cpp11::writable::integers({shared->dim_W_R_conf_1, shared->dim_W_R_conf_2, shared->dim_W_R_conf_3, shared->dim_W_R_conf_4});
  dim[68] = cpp11::writable::integers({shared->dim_W_D_unconf_1, shared->dim_W_D_unconf_2, shared->dim_W_D_unconf_3, shared->dim_W_D_unconf_4});
  dim[69] = cpp11::writable::integers({shared->dim_W_D_conf_1, shared->dim_W_D_conf_2, shared->dim_W_D_conf_3, shared->dim_W_D_conf_4});
  dim[70] = cpp11::writable::integers({shared->dim_T_sero_pre_1, shared->dim_T_sero_pre_2, shared->dim_T_sero_pre_3, shared->dim_T_sero_pre_4});
  dim[71] = cpp11::writable::integers({shared->dim_T_sero_pos_1, shared->dim_T_sero_pos_2, shared->dim_T_sero_pos_3, shared->dim_T_sero_pos_4});
  dim[72] = cpp11::writable::integers({shared->dim_T_PCR_pre_1, shared->dim_T_PCR_pre_2, shared->dim_T_PCR_pre_3, shared->dim_T_PCR_pre_4});
  dim[73] = cpp11::writable::integers({shared->dim_T_PCR_pos_1, shared->dim_T_PCR_pos_2, shared->dim_T_PCR_pos_3, shared->dim_T_PCR_pos_4});
  dim.names() = nms;
  ...
  index[32] = integer_sequence(shared->offset_variable_tmp_vaccine_n_candidates1 + 1, shared->dim_tmp_vaccine_n_candidates1);
  index[33] = integer_sequence(shared->offset_variable_tmp_vaccine_probability1 + 1, shared->dim_tmp_vaccine_probability1);
  index[34] = integer_sequence(shared->offset_variable_tmp_vaccine_probability2 + 1, shared->dim_tmp_vaccine_probability2);
  ...
  index[42] = integer_sequence(shared->offset_variable_prob_strain + 1, shared->dim_prob_strain);
  ...
  index[45] = integer_sequence(shared->offset_variable_T_sero_neg + 1, shared->dim_T_sero_neg);
  index[46] = integer_sequence(shared->offset_variable_R + 1, shared->dim_R);
  index[47] = integer_sequence(shared->offset_variable_T_PCR_neg + 1, shared->dim_T_PCR_neg);
  index[48] = integer_sequence(shared->offset_variable_E + 1, shared->dim_E);
  index[49] = integer_sequence(shared->offset_variable_I_A + 1, shared->dim_I_A);
  index[50] = integer_sequence(shared->offset_variable_I_P + 1, shared->dim_I_P);
  index[51] = integer_sequence(shared->offset_variable_I_C_1 + 1, shared->dim_I_C_1);
  index[52] = integer_sequence(shared->offset_variable_I_C_2 + 1, shared->dim_I_C_2);
  index[53] = integer_sequence(shared->offset_variable_G_D + 1, shared->dim_G_D);
  index[54] = integer_sequence(shared->offset_variable_ICU_pre_unconf + 1, shared->dim_ICU_pre_unconf);
  index[55] = integer_sequence(shared->offset_variable_ICU_pre_conf + 1, shared->dim_ICU_pre_conf);
  index[56] = integer_sequence(shared->offset_variable_H_R_unconf + 1, shared->dim_H_R_unconf);
  index[57] = integer_sequence(shared->offset_variable_H_R_conf + 1, shared->dim_H_R_conf);
  index[58] = integer_sequence(shared->offset_variable_H_D_unconf + 1, shared->dim_H_D_unconf);
  index[59] = integer_sequence(shared->offset_variable_H_D_conf + 1, shared->dim_H_D_conf);
  index[60] = integer_sequence(shared->offset_variable_ICU_W_R_unconf + 1, shared->dim_ICU_W_R_unconf);
  index[61] = integer_sequence(shared->offset_variable_ICU_W_R_conf + 1, shared->dim_ICU_W_R_conf);
  index[62] = integer_sequence(shared->offset_variable_ICU_W_D_unconf + 1, shared->dim_ICU_W_D_unconf);
  index[63] = integer_sequence(shared->offset_variable_ICU_W_D_conf + 1, shared->dim_ICU_W_D_conf);
  index[64] = integer_sequence(shared->offset_variable_ICU_D_unconf + 1, shared->dim_ICU_D_unconf);
  index[65] = integer_sequence(shared->offset_variable_ICU_D_conf + 1, shared->dim_ICU_D_conf);
  index[66] = integer_sequence(shared->offset_variable_W_R_unconf + 1, shared->dim_W_R_unconf);
  index[67] = integer_sequence(shared->offset_variable_W_R_conf + 1, shared->dim_W_R_conf);
  index[68] = integer_sequence(shared->offset_variable_W_D_unconf + 1, shared->dim_W_D_unconf);
  index[69] = integer_sequence(shared->offset_variable_W_D_conf + 1, shared->dim_W_D_conf);
