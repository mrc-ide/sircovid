...
public:
  ...
  HOST void update(size_t step, const real_t * state, dust::rng_state_t<real_t>& rng_state, real_t * state_next) {
    ...
    real_t prob_strain_1 = odin_sum3<real_t>(internal.I_weighted_strain.data(), 0, shared->dim_I_weighted_strain_1, 0, 1, 0, shared->dim_I_weighted_strain_3, shared->dim_I_weighted_strain_1, shared->dim_I_weighted_strain_12) / (real_t) odin_sum1<real_t>(internal.I_weighted_strain.data(), 0, shared->dim_I_weighted_strain);
...
cpp11::sexp dust_info<carehomes>(const dust::pars_t<carehomes>& pars) {
  ...
  dim[48] = cpp11::writable::integers({shared->dim_I_weighted_1, shared->dim_I_weighted_2});
  dim[49] = cpp11::writable::integers({shared->dim_vaccine_missed_doses_1, shared->dim_vaccine_missed_doses_2});
  dim[50] = cpp11::writable::integers({shared->dim_tmp_vaccine_n_candidates_1, shared->dim_tmp_vaccine_n_candidates_2});
  dim[51] = cpp11::writable::integers({shared->dim_tmp_vaccine_probability_1, shared->dim_tmp_vaccine_probability_2});
  dim[52] = cpp11::writable::integers({shared->dim_T_sero_neg_1_1, shared->dim_T_sero_neg_1_2, shared->dim_T_sero_neg_1_3});
  dim[53] = cpp11::writable::integers({shared->dim_T_sero_neg_2_1, shared->dim_T_sero_neg_2_2, shared->dim_T_sero_neg_2_3});
  dim[54] = cpp11::writable::integers({shared->dim_R_1, shared->dim_R_2, shared->dim_R_3});
  dim[55] = cpp11::writable::integers({shared->dim_T_PCR_neg_1, shared->dim_T_PCR_neg_2, shared->dim_T_PCR_neg_3});
  ...
  index[48] = integer_sequence(shared->offset_variable_I_weighted + 1, shared->dim_I_weighted);
  index[49] = integer_sequence(shared->offset_variable_vaccine_missed_doses + 1, shared->dim_vaccine_missed_doses);
  index[50] = integer_sequence(shared->offset_variable_tmp_vaccine_n_candidates + 1, shared->dim_tmp_vaccine_n_candidates);
  index[51] = integer_sequence(shared->offset_variable_tmp_vaccine_probability + 1, shared->dim_tmp_vaccine_probability);
  index[52] = integer_sequence(shared->offset_variable_T_sero_neg_1 + 1, shared->dim_T_sero_neg_1);
  index[53] = integer_sequence(shared->offset_variable_T_sero_neg_2 + 1, shared->dim_T_sero_neg_2);
  index[54] = integer_sequence(shared->offset_variable_R + 1, shared->dim_R);
  index[55] = integer_sequence(shared->offset_variable_T_PCR_neg + 1, shared->dim_T_PCR_neg);
