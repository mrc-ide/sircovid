...
test_that("carehomes check severity works as expected", {
  ...
  expect_error(carehomes_check_severity(list(n_groups = 19)),
               "Parameter 'rel_p_sympt' is missing")
  expect_error(carehomes_check_severity(list(n_groups = 19, rel_p_sympt = 1)),
               "Parameter 'p_C_step' is missing")

  ## check on required parameters
  steps <- c("p_C_step", "p_H_step", "p_ICU_step", "p_ICU_D_step",
