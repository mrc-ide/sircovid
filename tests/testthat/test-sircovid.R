...
test_that("Can be run on real data", {
  ...
  results <- run_particle_filter(case_data_file = "covid_cases_2020_4_3.csv",
  ...
  expect_lt(results$log_likelihood, 0)
  
  # Try some bad files and dates
  expect_error(run_particle_filter(case_data_file = "covid_cases_2020_4_3.csv",
                                   admissions_data_file = "combin_time_series.csv",
                                   itu_colname = "itu",
                                   model_params = model_params,
                                   obs_params = list(phi_ICU = 0.95,
