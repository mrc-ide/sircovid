...
test_that("can combine rt calculations over trajectories", {
  dat <- reference_data_trajectories_multistrain()

  index_S <- grep("^S_", names(dat$predict$index))
  index_R <- grep("^R_", names(dat$predict$index))
  index_ps <- grep("^prob_strain", names(dat$predict$index))
  S <- dat$trajectories$state[index_S, , , drop = FALSE]
  R <- dat$trajectories$state[index_R, , , drop = FALSE]
  ps <- dat$trajectories$state[index_ps, , , drop = FALSE]

  pars <- lapply(seq_len(nrow(dat$pars)), function(i)
    dat$predict$transform(dat$pars[i, ]))
  rt_multi <- carehomes_Rt_trajectories(
    dat$trajectories$step, S, pars,
    initial_step_from_parameters = TRUE,
    shared_parameters = FALSE, R = R, prob_strain = ps,
    weight_Rt = TRUE
  )

  res <- combine_rt(list(rt_multi, rt_multi), list(dat, dat))
...
test_that("can combine averaged rt calculations over multistrain trajectories", {
  ## Run model with 2 variants, but both have same transmissibility
  ## no seeding for second variant so noone infected with that one
  p <- carehomes_parameters(sircovid_date("2020-02-07"), "england",
                            strain_transmission = c(1, 1),
  ...
  rt_multi <- carehomes_Rt_trajectories(steps, S, p, prob_strain, R = R,
                       weight_Rt = TRUE)

  res <- combine_rt(list(rt_multi, rt_multi), list(y, y))
