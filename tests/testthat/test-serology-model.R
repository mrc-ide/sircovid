...
test_that("setting a gamma to Inf results in progress in corresponding compartment in 1 time-step", {    
  ...
  test_gamma_inf <- function(gamma_name,compartment_name){
    ...
    while (!check_cases && iter<= max_iter){
      ...
      results$I_ICU_D <- apply(results$I_ICU_D,c(1,2,3,4),sum)
      results$R_stepdown <- apply(results$R_stepdown,c(1,2,3),sum)
...
test_that("No one is unconfirmed, if p_admit_conf = 1", {
  ...
  while (!check_cases && iter<= max_iter){
    ...
    if (all(c(any(results$I_hosp_R[,,,,2] > 0),
              ...
              any(results$I_ICU_D[,,,,2] > 0),
              any(results$R_stepdown[,,,2] > 0)))){
      check_cases <- TRUE
    }
  }
  ...
  expect_true(all(results$I_triage_R[,,,,1] == 0))
  ...
  expect_true(any(results$I_ICU_D[,,,,2] > 0))
  expect_true(all(results$R_stepdown[,,,1] == 0))
  expect_true(any(results$R_stepdown[,,,2] > 0))
}
)
...
test_that("No one is unconfirmed, if p_admit_conf = 0 and gamma_test = 0", {
  ...
  while (!check_cases && iter<= max_iter){
    ...
    if (all(c(any(results$I_hosp_R[,,,,1] > 0),
              ...
              any(results$I_ICU_D[,,,,1] > 0),
              any(results$R_stepdown[,,,1] > 0)))){
      check_cases <- TRUE
    }
  }
  ...
  expect_true(any(results$I_triage_R[,,,,1] > 0))
  ...
  expect_true(any(results$I_ICU_D[,,,,1] > 0))
  expect_true(all(results$I_ICU_D[,,,,2] == 0))
  expect_true(any(results$R_stepdown[,,,1] > 0))
...
test_that("Confirmation in one time-step, if p_admit_conf = 0 and gamma_test = Inf", {
  ...
  pars_model$I0_ICU_D[,1,,1] <- 50 
  pars_model$R0_stepdown[,1,1] <- 50
  mod <- sircovid_model$odin_model(user = pars_model)
  t_max <- 150
  t <- seq(from = 1, to = t_max)
  check_cases <- FALSE
  ...
  expect_true(all(results$I_hosp_D[,,2,,1] == 0))
  #check triage_R
  expect_true(all(results$I_triage_R[,,1,,2] == 0))
  expect_true(all(results$I_triage_R[2:t_max,,2,,2] == results$I_triage_R[1:(t_max-1),,1,,1]))
  expect_true(all(results$I_triage_R[,,2,,1] == 0))
  #check triage_D
  expect_true(all(results$I_triage_D[,,1,,2] == 0))
  ...
  #check ICU_D
  expect_true(all(results$I_ICU_D[2,,2,,2] == results$I_ICU_D[1,,1,,1]))
  expect_true(all(results$I_ICU_D[2:t_max,,1,,2] == results$I_triage_D[1:(t_max-1),,2,,2]))
  expect_true(all(results$I_ICU_D[,,2,,1] == 0))
  #check stepdown
  expect_true(all(results$R_stepdown[2,,2,2] == results$R_stepdown[1,,1,1]))
  I_ICU_R <- apply(results$I_ICU_R,c(1,2,3,5),sum)
  expect_true(all(results$R_stepdown[2:t_max,,1,2] == I_ICU_R[1:(t_max-1),,2,2]))
  expect_true(all(results$R_stepdown[,,2,1] == 0))
}
)