  ...
  HOST void update(size_t step, const real_t * state, dust::rng_state_t<real_t>& rng_state, real_t * state_next) {
    ...
    for (int i = 1; i <= shared->dim_aux_I_A_1; ++i) {
      for (int j = 1; j <= shared->dim_aux_I_A_2; ++j) {
        for (int k = 1; k <= shared->dim_aux_I_A_3; ++k) {
          int l = 1;
          internal.aux_I_A[i - 1 + shared->dim_aux_I_A_1 * (j - 1) + shared->dim_aux_I_A_12 * (k - 1) + shared->dim_aux_I_A_123 * (l - 1)] = internal.aux_I_A[shared->dim_aux_I_A_123 * (l - 1) + shared->dim_aux_I_A_12 * (k - 1) + shared->dim_aux_I_A_1 * (j - 1) + i - 1] + internal.n_I_A_next_vacc_class[shared->dim_n_I_A_next_vacc_class_123 * (shared->n_vacc_classes - 1) + shared->dim_n_I_A_next_vacc_class_12 * (k - 1) + shared->dim_n_I_A_next_vacc_class_1 * (j - 1) + i - 1];
        }
      }
    }
    for (int i = 1; i <= shared->dim_aux_I_A_1; ++i) {
    ...
    for (int i = 1; i <= shared->dim_aux_I_P_1; ++i) {
      for (int j = 1; j <= shared->dim_aux_I_P_2; ++j) {
        for (int k = 1; k <= shared->dim_aux_I_P_3; ++k) {
          int l = 1;
          internal.aux_I_P[i - 1 + shared->dim_aux_I_P_1 * (j - 1) + shared->dim_aux_I_P_12 * (k - 1) + shared->dim_aux_I_P_123 * (l - 1)] = internal.aux_I_P[shared->dim_aux_I_P_123 * (l - 1) + shared->dim_aux_I_P_12 * (k - 1) + shared->dim_aux_I_P_1 * (j - 1) + i - 1] + internal.n_I_P_next_vacc_class[shared->dim_n_I_P_next_vacc_class_123 * (shared->n_vacc_classes - 1) + shared->dim_n_I_P_next_vacc_class_12 * (k - 1) + shared->dim_n_I_P_next_vacc_class_1 * (j - 1) + i - 1];
        }
      }
    }
    for (int i = 1; i <= shared->dim_aux_I_P_1; ++i) {
