...
delta_admit_conf <-
  sum(n_I_L_to_H_D_conf) +
  sum(n_I_L_to_H_R_conf) +
  sum(n_I_L_to_ICU_pre_conf)
...
update(cum_admit_by_age[]) <- cum_admit_by_age[i] + sum(n_I_L_to_hosp[i, , ])
...
p_I_L_progress <- 1 - exp(-gamma_L * dt)
...
n_I_L_progress[, , , ] <- rbinom(I_L[i, j, k, l], p_I_L_progress)
  ...
  (if (l == 1) n_II_P[i, j, k_P, 1] +
     n_II_P_next_vacc_class[i, j, k_P, n_vacc_classes] else
       n_II_P[i, j, k_P, l] + n_II_P_next_vacc_class[i, j, k_P, l - 1]) else
         n_I_C_progress[i, j, k - 1, l]) - n_I_C_progress[i, j, k, l]

new_I_C[, , , ] <- I_C[i, j, k, l] + aux_I_C[i, j, k, l]

## Work out the I_L->I_L transitions
aux_I_L[, , , ] <- (if (k == 1) n_I_C_progress[i, j, k_C, l] else
  n_I_L_progress[i, j, k - 1, l]) - n_I_L_progress[i, j, k, l]

new_I_L[, , , ] <- I_L[i, j, k, l] + aux_I_L[i, j, k, l]

## Work out the flow from I_L -> R, G_D, hosp
n_I_L_to_R[, , ] <- rbinom(n_I_L_progress[i, j, k_L, k],
                             1 - p_H_by_age[i] * rel_p_hosp_if_sympt[i, k])
n_I_L_to_G_D[, , ] <-
  rbinom(n_I_L_progress[i, j, k_L, k] - n_I_L_to_R[i, j, k],
         p_G_D_by_age[i])
n_I_L_to_hosp[, , ] <- n_I_L_progress[i, j, k_L, k] -
  n_I_L_to_R[i, j, k] - n_I_L_to_G_D[i, j, k]

## Work out the G_D -> G_D transitions
aux_G_D[, , , ] <- (if (k == 1) n_I_L_to_G_D[i, j, l] else
...
n_I_L_to_ICU_pre[, , ] <- rbinom(n_I_L_to_hosp[i, j, k], p_ICU_by_age[i])
n_I_L_to_ICU_pre_conf[, , ] <- rbinom(n_I_L_to_ICU_pre[i, j, k],
                                        p_star_by_age[i])
n_hosp_non_ICU[, , ] <- n_I_L_to_hosp[i, j, k] - n_I_L_to_ICU_pre[i, j, k]
n_I_L_to_H_D[, , ] <- rbinom(n_hosp_non_ICU[i, j, k], p_H_D_by_age[i])
n_I_L_to_H_D_conf[, , ] <- rbinom(n_I_L_to_H_D[i, j, k],
                                    p_star_by_age[i])
n_I_L_to_H_R[, , ] <- n_hosp_non_ICU[i, j, k] - n_I_L_to_H_D[i, j, k]
n_I_L_to_H_R_conf[, , ] <- rbinom(n_I_L_to_H_R[i, j, k],
                                    p_star_by_age[i])
...
new_ICU_pre_unconf[, , , ] <-
  ...
  (if (k == 1) n_I_L_to_ICU_pre[i, j, l] - n_I_L_to_ICU_pre_conf[i, j, l]
   else 0)
new_ICU_pre_conf[, , , ] <-
  aux_ICU_pre_conf[i, j, k, l] + n_ICU_pre_unconf_to_conf[i, j, k, l] +
  (if (k == 1) n_I_L_to_ICU_pre_conf[i, j, l] else 0)
...
new_H_R_unconf[, , , ] <-
  ...
  (if (k == 1) n_I_L_to_H_R[i, j, l] - n_I_L_to_H_R_conf[i, j, l] else 0)
new_H_R_conf[, , , ] <-
  aux_H_R_conf[i, j, k, l] + n_H_R_unconf_to_conf[i, j, k, l] +
  (if (k == 1) n_I_L_to_H_R_conf[i, j, l] else 0)
...
new_H_D_unconf[, , , ] <-
  ...
  (if (k == 1) n_I_L_to_H_D[i, j, l] - n_I_L_to_H_D_conf[i, j, l] else 0)
new_H_D_conf[, , , ] <-
  aux_H_D_conf[i, j, k, l] + n_H_D_unconf_to_conf[i, j, k, l] +
  (if (k == 1) n_I_L_to_H_D_conf[i, j, l] else 0)
...
I_with_diff_trans[, , ] <-
  rel_infectivity[i, k] * strain_transmission[j] * (
    I_A_transmission * sum(I_A[i, j, , k]) +
      ...
      I_C_transmission * sum(I_C[i, j, , k]) +
      I_L_transmission * sum(I_L[i, j, , k]) +
      hosp_transmission * (
...
I_C_transmission <- user()
I_L_transmission <- user()
...
## Vectors handling the I_L class
...
## Vectors handling the I_L class
dim(I_L) <- c(n_groups, n_strains, k_L, n_vacc_classes)
dim(aux_I_L) <- c(n_groups, n_strains, k_L, n_vacc_classes)
dim(new_I_L) <- c(n_groups, n_strains, k_L, n_vacc_classes)
dim(n_I_L_progress) <- c(n_groups, n_strains, k_L, n_vacc_classes)
...
## Vectors handling I_L to R, G_D transition
dim(n_I_L_to_G_D) <- c(n_groups, n_strains, n_vacc_classes)
dim(n_I_L_to_R) <- c(n_groups, n_strains, n_vacc_classes)

## Vectors handling number of new hospitalisations, ICU admissions and
## recoveries in hospital
dim(n_I_L_to_hosp) <- c(n_groups, n_strains, n_vacc_classes)
dim(n_I_L_to_ICU_pre) <- c(n_groups, n_strains, n_vacc_classes)
dim(n_I_L_to_ICU_pre_conf) <- c(n_groups, n_strains, n_vacc_classes)
dim(n_hosp_non_ICU) <- c(n_groups, n_strains, n_vacc_classes)
dim(n_I_L_to_H_D) <- c(n_groups, n_strains, n_vacc_classes)
dim(n_I_L_to_H_D_conf) <- c(n_groups, n_strains, n_vacc_classes)
dim(n_I_L_to_H_R) <- c(n_groups, n_strains, n_vacc_classes)
dim(n_I_L_to_H_R_conf) <- c(n_groups, n_strains, n_vacc_classes)
...
I_weighted_strain[, , ] <-
  strain_transmission[j] * (
    I_A_transmission * sum(new_I_A[i, j, , k]) +
      ...
      I_C_transmission * sum(new_I_C[i, j, , k]) +
      I_L_transmission * sum(new_I_L[i, j, , k]) +
      hosp_transmission * (
