...
p_II_hosp_R <- 1 - exp(-gamma_H_R * dt)
p_II_hosp_D <- 1 - exp(-gamma_H_D * dt)
p_II_ICU_S_R <- 1 - exp(-gamma_ICU_W_R * dt)
p_II_ICU_S_D <- 1 - exp(-gamma_ICU_W_D * dt)
...
n_W_R_unconf[, , , ] <- rbinom(W_R_unconf[i, j, k, l], p_WW_R)
n_W_R_conf[, , , ] <- rbinom(W_R_conf[i, j, k, l], p_WW_R)
n_W_D_unconf[, , , ] <- rbinom(W_D_unconf[i, j, k, l], p_WW_D)
n_W_D_conf[, , , ] <- rbinom(W_D_conf[i, j, k, l], p_WW_D)
...
aux_II_ICU_D_unconf[, , 1, ] <-
  aux_II_ICU_D_unconf[i, j, k, l] + n_ICU_pre_unconf_to_ICU_D_unconf[i, j, l]
...
aux_II_ICU_D_conf[, , 1, ] <-
  aux_II_ICU_D_conf[i, j, k, l] + n_ICU_pre_conf_to_ICU_D_conf[i, j, l]
...
aux_W_R_unconf[, , 1, ] <-
  aux_W_R_unconf[i, j, k, l] + n_II_ICU_S_R_unconf[i, j, s_ICU_W_R, l]
aux_W_R_unconf[, , 2:s_W_R, ] <-
  aux_W_R_unconf[i, j, k, l] + n_W_R_unconf[i, j, k - 1, l]
aux_W_R_unconf[, , 1:s_W_R, ] <-
  aux_W_R_unconf[i, j, k, l] - n_W_R_unconf[i, j, k, l]
aux_W_R_conf[, , , ] <- W_R_conf[i, j, k, l]
aux_W_R_conf[, , 1, ] <-
  aux_W_R_conf[i, j, k, l] + n_II_ICU_S_R_conf[i, j, s_ICU_W_R, l]
aux_W_R_conf[, , 2:s_W_R, ] <-
  aux_W_R_conf[i, j, k, l] + n_W_R_conf[i, j, k - 1, l]
aux_W_R_conf[, , 1:s_W_R, ] <-
  aux_W_R_conf[i, j, k, l] - n_W_R_conf[i, j, k, l]
...
aux_W_D_unconf[, , 1, ] <-
  aux_W_D_unconf[i, j, k, l] + n_II_ICU_S_D_unconf[i, j, s_ICU_W_D, l]
aux_W_D_unconf[, , 2:s_W_D, ] <-
  aux_W_D_unconf[i, j, k, l] + n_W_D_unconf[i, j, k - 1, l]
aux_W_D_unconf[, , 1:s_W_D, ] <-
  aux_W_D_unconf[i, j, k, l] - n_W_D_unconf[i, j, k, l]
aux_W_D_conf[, , , ] <- W_D_conf[i, j, k, l]
aux_W_D_conf[, , 1, ] <-
  aux_W_D_conf[i, j, k, l] + n_II_ICU_S_D_conf[i, j, s_ICU_W_D, l]
aux_W_D_conf[, , 2:s_W_D, ] <-
  aux_W_D_conf[i, j, k, l] + n_W_D_conf[i, j, k - 1, l]
aux_W_D_conf[, , 1:s_W_D, ] <-
  aux_W_D_conf[i, j, k, l] - n_W_D_conf[i, j, k, l]
...
new_D_hosp[] <- D_hosp[i] +
  sum(n_II_hosp_D_unconf[i, , s_H_D, ]) +
  sum(n_II_hosp_D_conf[i, , s_H_D, ]) +
  sum(n_II_ICU_D_unconf[i, , s_ICU_D, ]) +
  sum(n_II_ICU_D_conf[i, , s_ICU_D, ]) +
  sum(n_W_D_unconf[i, , s_W_D, ]) +
  sum(n_W_D_conf[i, , s_W_D, ])
...
new_R[, , ] <- R[i, j, k] +
  n_II_A[i, j, s_A, k] +
  n_sympt_to_R[i, j, k] +
  n_II_hosp_R_conf[i, j, s_H_R, k] +
  n_II_hosp_R_unconf[i, j, s_H_R, k] +
  n_W_R_conf[i, j, s_W_R, k] +
  n_W_R_unconf[i, j, s_W_R, k] -
  n_R_progress[i, j, k] -
...
dim(aux_II_hosp_R_unconf) <- c(n_groups, n_strains, s_H_R, n_vacc_classes)
dim(new_I_hosp_R_unconf) <- c(n_groups, n_strains, s_H_R, n_vacc_classes)
dim(n_II_hosp_R_unconf) <- c(n_groups, n_strains, s_H_R, n_vacc_classes)
dim(I_hosp_R_conf) <- c(n_groups, n_strains, s_H_R, n_vacc_classes)
dim(aux_II_hosp_R_conf) <- c(n_groups, n_strains, s_H_R, n_vacc_classes)
dim(new_I_hosp_R_conf) <- c(n_groups, n_strains, s_H_R, n_vacc_classes)
dim(n_II_hosp_R_conf) <- c(n_groups, n_strains, s_H_R, n_vacc_classes)
dim(n_I_hosp_R_unconf_to_conf) <-
  c(n_groups, n_strains, s_H_R, n_vacc_classes)

## Vectors handling the I_hosp_D class
dim(I_hosp_D_unconf) <- c(n_groups, n_strains, s_H_D, n_vacc_classes)
dim(aux_II_hosp_D_unconf) <- c(n_groups, n_strains, s_H_D, n_vacc_classes)
dim(new_I_hosp_D_unconf) <- c(n_groups, n_strains, s_H_D, n_vacc_classes)
dim(n_II_hosp_D_unconf) <- c(n_groups, n_strains, s_H_D, n_vacc_classes)
dim(I_hosp_D_conf) <- c(n_groups, n_strains, s_H_D, n_vacc_classes)
dim(aux_II_hosp_D_conf) <- c(n_groups, n_strains, s_H_D, n_vacc_classes)
dim(new_I_hosp_D_conf) <- c(n_groups, n_strains, s_H_D, n_vacc_classes)
dim(n_II_hosp_D_conf) <- c(n_groups, n_strains, s_H_D, n_vacc_classes)
dim(n_I_hosp_D_unconf_to_conf) <-
  c(n_groups, n_strains, s_H_D, n_vacc_classes)

## Vectors handling the I_ICU_S_R class
dim(I_ICU_S_R_unconf) <- c(n_groups, n_strains, s_ICU_W_R, n_vacc_classes)
dim(aux_II_ICU_S_R_unconf) <- c(n_groups, n_strains, s_ICU_W_R, n_vacc_classes)
dim(new_I_ICU_S_R_unconf) <- c(n_groups, n_strains, s_ICU_W_R, n_vacc_classes)
dim(n_II_ICU_S_R_unconf) <- c(n_groups, n_strains, s_ICU_W_R, n_vacc_classes)
dim(I_ICU_S_R_conf) <- c(n_groups, n_strains, s_ICU_W_R, n_vacc_classes)
dim(aux_II_ICU_S_R_conf) <- c(n_groups, n_strains, s_ICU_W_R, n_vacc_classes)
dim(new_I_ICU_S_R_conf) <- c(n_groups, n_strains, s_ICU_W_R, n_vacc_classes)
dim(n_II_ICU_S_R_conf) <- c(n_groups, n_strains, s_ICU_W_R, n_vacc_classes)
dim(n_I_ICU_S_R_unconf_to_conf) <-
  c(n_groups, n_strains, s_ICU_W_R, n_vacc_classes)

## Vectors handling the I_ICU_S_D class
dim(I_ICU_S_D_unconf) <- c(n_groups, n_strains, s_ICU_W_D, n_vacc_classes)
dim(aux_II_ICU_S_D_unconf) <- c(n_groups, n_strains, s_ICU_W_D, n_vacc_classes)
dim(new_I_ICU_S_D_unconf) <- c(n_groups, n_strains, s_ICU_W_D, n_vacc_classes)
dim(n_II_ICU_S_D_unconf) <- c(n_groups, n_strains, s_ICU_W_D, n_vacc_classes)
dim(I_ICU_S_D_conf) <- c(n_groups, n_strains, s_ICU_W_D, n_vacc_classes)
dim(aux_II_ICU_S_D_conf) <- c(n_groups, n_strains, s_ICU_W_D, n_vacc_classes)
dim(new_I_ICU_S_D_conf) <- c(n_groups, n_strains, s_ICU_W_D, n_vacc_classes)
dim(n_II_ICU_S_D_conf) <- c(n_groups, n_strains, s_ICU_W_D, n_vacc_classes)
...
dim(new_W_R_conf) <-
  c(n_groups, n_strains, s_W_R, n_vacc_classes)
dim(n_W_R_conf) <- c(n_groups, n_strains, s_W_R, n_vacc_classes)
...
dim(new_W_D_conf) <-
  c(n_groups, n_strains, s_W_D, n_vacc_classes)
dim(n_W_D_conf) <- c(n_groups, n_strains, s_W_D, n_vacc_classes)
...
dim(n_sympt_to_ICU_pre) <- c(n_groups, n_strains, n_vacc_classes)
dim(n_sympt_to_ICU_pre_conf) <- c(n_groups, n_strains, n_vacc_classes)
...
new_general_tot <- sum(new_ICU_pre_conf) + sum(new_I_hosp_R_conf) +
  sum(new_I_hosp_D_conf) + sum(new_W_R_conf) +
  sum(new_W_D_conf)
