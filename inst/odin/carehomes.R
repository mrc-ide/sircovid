...
update(cum_admit_conf) <-
  cum_admit_conf +
  sum(n_I_C_to_hosp_D_conf) +
  sum(n_I_C_to_hosp_R_conf) +
  sum(n_I_C_to_triage_conf)
update(cum_new_conf) <-
  cum_new_conf +
...
n_G_D[, , , ] <- rbinom(G_D[i, j, k, l], p_G_D)
...
aux_G_D[, , 2:s_G_D, ] <- n_G_D[i, j, k - 1, l]
aux_G_D[, , 1:s_G_D, ] <-
  aux_G_D[i, j, k, l] - n_G_D[i, j, k, l]
new_G_D[, , , ] <- G_D[i, j, k, l] + aux_G_D[i, j, k, l]

## Work out the split in hospitals between hosp_D, hosp_R and triage
n_I_C_to_triage[, , ] <- rbinom(n_I_C_to_hosp[i, j, k], prob_ICU_hosp[i])
n_I_C_to_triage_conf[, , ] <- rbinom(n_I_C_to_triage[i, j, k],
                                   prob_admit_conf[i])
n_hosp_non_ICU[, , ] <- n_I_C_to_hosp[i, j, k] - n_I_C_to_triage[i, j, k]
n_I_C_to_hosp_D[, , ] <- rbinom(n_hosp_non_ICU[i, j, k], prob_death_hosp_D[i])
n_I_C_to_hosp_D_conf[, , ] <- rbinom(n_I_C_to_hosp_D[i, j, k],
                                     prob_admit_conf[i])
n_I_C_to_hosp_R[, , ] <- n_hosp_non_ICU[i, j, k] - n_I_C_to_hosp_D[i, j, k]
n_I_C_to_hosp_R_conf[, , ] <- rbinom(n_I_C_to_hosp_R[i, j, k],
                                     prob_admit_conf[i])
...
new_I_triage_unconf[, , 1, ] <-
  new_I_triage_unconf[i, j, 1, l] + n_I_C_to_triage[i, j, l] -
  n_I_C_to_triage_conf[i, j, l]
new_I_triage_conf[, , , ] <-
  aux_II_triage_conf[i, j, k, l] + n_I_triage_unconf_to_conf[i, j, k, l]
new_I_triage_conf[, , 1, ] <-
  new_I_triage_conf[i, j, 1, l] + n_I_C_to_triage_conf[i, j, l]
...
new_I_hosp_R_unconf[, , 1, ] <-
  new_I_hosp_R_unconf[i, j, 1, l] + n_I_C_to_hosp_R[i, j, l] -
  n_I_C_to_hosp_R_conf[i, j, l]
new_I_hosp_R_conf[, , , ] <-
  aux_II_hosp_R_conf[i, j, k, l] + n_I_hosp_R_unconf_to_conf[i, j, k, l]
new_I_hosp_R_conf[, , 1, ] <-
  new_I_hosp_R_conf[i, j, 1, l] + n_I_C_to_hosp_R_conf[i, j, l]
...
new_I_hosp_D_unconf[, , 1, ] <-
  new_I_hosp_D_unconf[i, j, 1, l] + n_I_C_to_hosp_D[i, j, l] -
  n_I_C_to_hosp_D_conf[i, j, l]
new_I_hosp_D_conf[, , , ] <-
  aux_II_hosp_D_conf[i, j, k, l] + n_I_hosp_D_unconf_to_conf[i, j, k, l]
new_I_hosp_D_conf[, , 1, ] <-
  new_I_hosp_D_conf[i, j, 1, l] + n_I_C_to_hosp_D_conf[i, j, l]
...
new_D_comm[] <- D_comm[i] + sum(n_G_D[i, , s_G_D, ])
...
dim(aux_G_D) <- c(n_groups, n_strains, s_G_D, n_vacc_classes)
dim(new_G_D) <- c(n_groups, n_strains, s_G_D, n_vacc_classes)
dim(n_G_D) <- c(n_groups, n_strains, s_G_D, n_vacc_classes)
...
dim(n_I_C_to_triage) <- c(n_groups, n_strains, n_vacc_classes)
dim(n_I_C_to_triage_conf) <- c(n_groups, n_strains, n_vacc_classes)
dim(n_hosp_non_ICU) <- c(n_groups, n_strains, n_vacc_classes)
dim(n_I_C_to_hosp_D) <- c(n_groups, n_strains, n_vacc_classes)
dim(n_I_C_to_hosp_D_conf) <- c(n_groups, n_strains, n_vacc_classes)
dim(n_I_C_to_hosp_R) <- c(n_groups, n_strains, n_vacc_classes)
dim(n_I_C_to_hosp_R_conf) <- c(n_groups, n_strains, n_vacc_classes)
