...
cum_deaths <- samples$trajectories$state[c("deaths_hosp", "deaths_comm"), 2000, ]
...
matplot(dx[-1], deaths, type = "l", lty = 1)
points(data$date, data$deaths)
```
...
cum_deaths <- samples_tuned$trajectories$state[c("deaths_hosp", "deaths_comm"),2000, ]
deaths <- apply(cum_deaths, 1, diff)
deaths <- cbind(deaths = rowSums(deaths), deaths)
dx <- sircovid::sircovid_date_as_date(samples_tuned$trajectories$step / samples_tuned$trajectories$rate)
# plot against the data
par(bty = "n", mar = c(3, 3, 1, 1))
matplot(dx[-1], deaths, type = "l", lty = 1, ylim = c(0, max(data$deaths, deaths)))
points(data$date, data$deaths)
```
...
control <- list(n_sample = 100, burnin = 500, forecast_days = 14)
samples_predict <-
