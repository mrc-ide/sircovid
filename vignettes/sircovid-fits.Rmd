...
In deciding how to operate parameters, the first thing to do is to look at the odin model under the section `## User defined parameters - default in parentheses:` which also has comments about what these parameters mean. You can also use `coef()` to see the names and odin-set defaults of all of the model parameters:

```{r}
coef(sircovid::carehomes)
```

All of these will automatically be set up, but any can be sampled. Run the following to see the defaults sent from R:
...
n_steps <- nrow(samples_tuned$pars)
