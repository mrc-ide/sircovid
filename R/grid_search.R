...
##' 
##' @param start_date Earliest start date as 'yyyy-mm-dd'
##' 
##' @param end_date Latest start date as 'yyyy-mm-dd'
##' 
...
scan_beta_date <- function(
  ...
  date_list <- seq(as.Date(start_date), as.Date(end_date), day_step)
  ...
  # Make sure beta is compatible with a single value in search
  model_params$beta_y <- NULL
  model_params$beta_t <- NULL
  ...
  pf_run_outputs <- furrr::future_pmap(
    ...
    function(beta, start_date) {
      ...
      model_params$beta_t <- start_date
      
      # Objects used by particle filter run
      pf_data <- particle_filter_data(data, start_date, steps_per_day)
      model_run <- sircovid(params = model_params)
      compare_func <- compare_icu(model_run, pars_obs, pf_data)
      
      X <- particle_filter(pf_data, model_params, compare_func, n_particles=100)
    }
  )
  ...
  image(x=beta_1D, y=start_date, z=mat_log_ll,
        xlab="beta", ylab="Start date", main = "Log-likelihood")
  image(x=beta_1D, y=start_date, z=renorm_mat_LL,
