...
##' Run a grid search of the particle filter over beta, start date and
##' beta_reduction. This is parallelised, first run \code{plan(multiprocess)}
##' to set this up.
##' 
##' @title Grid search of beta and start date
##' 
...
scan_beta_date_reduction <- function(
  ...
  class(results) <- "sircovid_scan_3D"
  results
}
...
plot.sircovid_scan_3D <- function(x, ..., integrate_out = "start_date") {
  ...
  graphics::image(x=xx, y=yy, z=z,
                    xlab=xlab, ylab=ylab, main = "Probability")
}
