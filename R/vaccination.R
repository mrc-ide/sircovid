...
##' @title Modify severity and transmission of VOC
...
##'  rel_infectivity, rel_p_death. Columns correspond to vaccine strata and
##'  rows to age groups.
...
##' @return Returns an array with same dimensions and names as `efficacy`
##' @export
modify_severity <- function(efficacy, efficacy_strain_2,
  ...
  stopifnot(
    identical(names(strain1), expected),
    identical(names(strain1), names(strain2)),
    identical(lapply(strain1, dim), lapply(strain2, dim)))
