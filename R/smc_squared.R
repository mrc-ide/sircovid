...
##' 
##' @param beta_params List of parameters used for time-varying beta
##' 
...
##'
##' @param fitted_params list of fitted parameters, with a list of 
##' log-density function for prior ($dprior), random generator for 
##' prior ($rprior) and random generator used for proposal ($rproposal)
##' 
...
##'
##' @export
smc_squared <- function(data, sircovid_model, model_params, beta_params,
                        pars_obs, pars_seeding, earliest_seeding_date, 
                        n_particles, n_param_sets, fitted_params,
                        degeneracy_threshold, covariance_scaling) {
  ...
  for (t in seq_len(nrow(data))[-1L]) {
    ...
    #MCMC update to parameters only if ESS is below a threshold
