...
basic <- R6::R6Class(
  ...
  public = list(
    ...
    run = function(step_end, device = FALSE, deterministic = FALSE) {
      m <- dust_basic_run(private$ptr_, step_end, device, deterministic)
      rownames(m) <- names(private$index_)
      m
    },

    simulate = function(step_end, device = FALSE, deterministic = FALSE) {
      m <- dust_basic_simulate(private$ptr_, step_end, device, deterministic)
      rownames(m) <- names(private$index_)
      m
    },
    ...
    set_state = function(state, step = NULL, deterministic = FALSE) {
      dust_basic_set_state(private$ptr_, state, step, deterministic)
      invisible()
    },
    ...
    compare_data = function(device = FALSE) {
      dust_basic_compare_data(private$ptr_, device)
    },

    filter = function(save_trajectories = FALSE, step_snapshot = NULL,
                      device = FALSE) {
      dust_basic_filter(private$ptr_, save_trajectories,
...
carehomes <- R6::R6Class(
  ...
  private = list(
    ...
    param_ = list(G_D_transmission = list(has_default = FALSE, default_value = NULL, rank = 0, min = -Inf, max = Inf, integer = FALSE),
     ...
     p_NC_65plus = list(has_default = FALSE, default_value = NULL, rank = 0, min = -Inf, max = Inf, integer = FALSE),
     p_NC_under65 = list(has_default = FALSE, default_value = NULL, rank = 0, min = -Inf, max = Inf, integer = FALSE),
     p_NC_weekend_65plus = list(has_default = FALSE, default_value = NULL, rank = 0, min = -Inf, max = Inf, integer = FALSE),
     p_NC_weekend_under65 = list(has_default = FALSE, default_value = NULL, rank = 0, min = -Inf, max = Inf, integer = FALSE),
  ...
  public = list(
    ...
    run = function(step_end, device = FALSE, deterministic = FALSE) {
      m <- dust_carehomes_run(private$ptr_, step_end, device, deterministic)
      rownames(m) <- names(private$index_)
      m
    },

    simulate = function(step_end, device = FALSE, deterministic = FALSE) {
      m <- dust_carehomes_simulate(private$ptr_, step_end, device, deterministic)
      rownames(m) <- names(private$index_)
      m
    },
    ...
    set_state = function(state, step = NULL, deterministic = FALSE) {
      dust_carehomes_set_state(private$ptr_, state, step, deterministic)
      invisible()
    },
    ...
    compare_data = function(device = FALSE) {
      dust_carehomes_compare_data(private$ptr_, device)
    },

    filter = function(save_trajectories = FALSE, step_snapshot = NULL,
                      device = FALSE) {
      dust_carehomes_filter(private$ptr_, save_trajectories,
                           step_snapshot, device)
    },
