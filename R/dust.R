...
demo <- R6::R6Class(
  ...
  private = list(
    ...
    gpu_config_ = NULL,
    methods_ = NULL,
    param_ = list(I0 = list(has_default = FALSE, default_value = NULL, rank = 2, min = -Inf, max = Inf, integer = FALSE),
     ...
     beta = list(has_default = FALSE, default_value = NULL, rank = 1, min = -Inf, max = Inf, integer = FALSE),
     dt = list(has_default = FALSE, default_value = NULL, rank = 0, min = -Inf, max = Inf, integer = FALSE),
     gamma = list(has_default = FALSE, default_value = NULL, rank = 0, min = -Inf, max = Inf, integer = FALSE),
     m = list(has_default = FALSE, default_value = NULL, rank = 2, min = -Inf, max = Inf, integer = FALSE),
     n_age_groups = list(has_default = FALSE, default_value = NULL, rank = 0, min = -Inf, max = Inf, integer = FALSE),
  ...
  public = list(
    initialize = function(pars, step, n_particles, n_threads = 1L,
      ...
      if (is.null(gpu_config)) {
        private$methods_ <- list(
           alloc = dust_cpu_demo_alloc,
           run = dust_cpu_demo_run,
           simulate = dust_cpu_demo_simulate,
           set_index = dust_cpu_demo_set_index,
           n_state = dust_cpu_demo_n_state,
           update_state = dust_cpu_demo_update_state,
           state = dust_cpu_demo_state,
           step = dust_cpu_demo_step,
           reorder = dust_cpu_demo_reorder,
           resample = dust_cpu_demo_resample,
           rng_state = dust_cpu_demo_rng_state,
           set_rng_state = dust_cpu_demo_set_rng_state,
           set_n_threads = dust_cpu_demo_set_n_threads,
           set_data = dust_cpu_demo_set_data,
           compare_data = dust_cpu_demo_compare_data,
           filter = dust_cpu_demo_filter)
      } else {
        private$methods_ <- list(alloc = function(...) {
          stop("GPU support not enabled for this object")
        })
    ...
    has_gpu_support = function(fake_gpu = FALSE) {
      if (fake_gpu) {
        ...
        dust_demo_capabilities()[["gpu"]]
      }
    },

    has_compare = function() {
      dust_demo_capabilities()[["compare"]]
    },

    real_size = function() {
      dust_demo_capabilities()[["real_size"]]
    },

    rng_algorithm = function() {
      dust_demo_capabilities()[["rng_algorithm"]]
    },
...
class(demo) <- c("dust_generator", class(demo))
lancelot <- R6::R6Class(
  "dust",
  cloneable = FALSE,
...
demo$set("public", "transform_variables", function(y) {
    info <- self$info()
    set_dim <- function(x, dimx) {
        if (length(dimx) > 1L) {
            dim(x) <- dimx
