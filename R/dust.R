...
carehomes <- R6::R6Class(
  ...
  public = list(
    initialize = function(pars, step, n_particles, n_threads = 1L,
      ...
      if (is.null(gpu_config)) {
        private$methods_ <- list(
           alloc = dust_cpu_carehomes_alloc,
           run = dust_cpu_carehomes_run,
           simulate = dust_cpu_carehomes_simulate,
           set_index = dust_cpu_carehomes_set_index,
           n_state = dust_cpu_carehomes_n_state,
           update_state = dust_cpu_carehomes_update_state,
           state = dust_cpu_carehomes_state,
           step = dust_cpu_carehomes_step,
           reorder = dust_cpu_carehomes_reorder,
           resample = dust_cpu_carehomes_resample,
           rng_state = dust_cpu_carehomes_rng_state,
           set_rng_state = dust_cpu_carehomes_set_rng_state,
           set_n_threads = dust_cpu_carehomes_set_n_threads,
           set_data = dust_cpu_carehomes_set_data,
           compare_data = dust_cpu_carehomes_compare_data,
           filter = dust_cpu_carehomes_filter)
      } else {
        private$methods_ <- list(alloc = function(...) {
          stop("GPU support not enabled for this object")
        })
    ...
    has_gpu_support = function(fake_gpu = FALSE) {
      if (fake_gpu) {
        ...
        dust_carehomes_capabilities()[["gpu"]]
      }
    },
    ...
    real_size = function() {
      dust_carehomes_capabilities()[["real_size"]]
    },
